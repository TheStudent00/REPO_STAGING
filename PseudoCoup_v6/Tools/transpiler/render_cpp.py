...
# <WORKSPACE_DIR>/PseudoCoup_v5/Research/cpp_ingress/transpile_cpp.py,
...
# achievable. <WORKSPACE_DIR>/PseudoCoup_v6/Tools/transpiler/ingest_llvm_encoder.py
...
(X86MCCodeEmitter.cpp) into Python, using the same discipline as the sibling
Rust transpile applied to its own source crate:
...
WHY THIS FILE EXISTS (see the plan doc, Phase 2): a Rust encoder and
LLVM's C++ encoder are two independent implementations of the SAME x86
ISA facts (ModRM byte packing, REX byte packing). Transpiling LLVM's
version and checking it against our already-verified Rust
transpile is diverse-double-compiling
...
the sibling Rust transpile's own header for this second source language): every
...
# exactly the same kind of judgment call the sibling Rust transpile already
# makes for Gpr.enc() (an integer register encoding is accepted directly,
# without modelling the source compiler's register-allocation machinery). The
...
This is LLVM's independent derivation of the SAME x86 ISA facts the
sibling Rust transpile derives from its own source crate. See
...
def u8(x: int) -> int:
    ...
    matching the sibling Rust transpile's own uniform-wrapping rule."""
    return x & 0xFF
...
def gen_mod_rm_byte(src):
    ...
    return (
        ...
        'rather than\n    masking it the way the sibling Rust '
        'transpile\'s encode_modrm\n    does (`m0d & 3`, etc.). '
