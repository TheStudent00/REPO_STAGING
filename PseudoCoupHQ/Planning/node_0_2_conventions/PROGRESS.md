...
- 2026-08-02 **standing prohibition, the owner**: a Rust code generation
  ...
  `~/Programming/PseudoCoupHQ/CRANELIFT_IS_BANNED.md` is the authority
...
- 2026-08-02 **done** (the owner's instruction): the framework repo is now
  `~/Programming/PlanPlan`, renamed from `PlanningPlan`. Every path in
  this node's CORE, in `~/Programming/PseudoCoupHQ/hq.sh` and in
  `~/Programming/PseudoCoupHQ/plan_and_code.md` was updated with it.
  The full account of the rename, including what was deliberately not
  edited, is in `~/Programming/PlanPlan/Planning/PROGRESS.md`.
...
- 2026-08-01 **done and exercised**: `generate_nodes.py` built in
  `~/Programming/PlanPlan/framework/` — the register realizer the
...
- 2026-07-31: **no arguments now means do everything** (the owner: "i just
  ...
  `bash ~/Programming/PseudoCoupHQ/hq.sh` regenerates dashboards,
...
- 2026-07-31 **done and exercised**: `~/Programming/PseudoCoupHQ/hq.sh`
  ...
  in `~/Programming/PlanPlan/framework/`; `render_plan.py` and
...
- open: warnings stand, all expected. **counts restated 2026-08-01**
  ...
  - four are duplicated prose, three of which are the exchange/cycle
    ...
    silence.
- open, carried from `~/Programming/PseudoCoupHQ/plan_and_code.md` §6:
...
- raised 2026-07-31 as "open, and sharper than first recorded",
  **closed 2026-08-01** (see the closing sub-bullet):
  `~/Programming/DevComms/` was not a git repo, so
  ...
  - `~/Programming/PseudoCoupHQ/DevComms/LLM_communication_protocol.md`
    was briefly a real copy when HQ was founded; made a symlink
    2026-07-31 for consistency with the other three.
  - the ruling asked for was: version control `~/Programming/DevComms/`
  ...
  - 2026-08-01 **done**: the owner ruled for its own repo and pushed it.
    Verified on disk: `~/Programming/DevComms/` is a git repo with
    ...
    `~/Programming/PseudoCoupHQ/plan_and_code.md`.
