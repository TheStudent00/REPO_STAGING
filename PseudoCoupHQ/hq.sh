...
#
#   bash <WORKSPACE_DIR>/PseudoCoupHQ/hq.sh
#
...
#                     open <WORKSPACE_DIR>/PseudoCoupHQ/plan_explorer.html
...
# into <WORKSPACE_DIR>/PlanPlan/framework/ or into a repo's own
...
FRAMEWORK=<WORKSPACE_DIR>/PlanPlan/framework

# Planning roots the checks and dashboards run over.
ROOTS=(
    <WORKSPACE_DIR>/PseudoCoupHQ/Planning
    <WORKSPACE_DIR>/PseudoCoup_v5/Planning
    <WORKSPACE_DIR>/PseudoCoup_v6/Planning
    <WORKSPACE_DIR>/PseudoIR/Planning
)
# NOT here: <WORKSPACE_DIR>/PlanPlan/Planning. It has a planning
...
#       <WORKSPACE_DIR>/PlanPlan/Planning --projections --apply
#   python3 <WORKSPACE_DIR>/PlanPlan/framework/generate_dashboards.py \
#       <WORKSPACE_DIR>/PlanPlan/Planning
...
# git_commit_push.sh.
SOURCES_SCRIPT=<WORKSPACE_DIR>/PseudoCoup_v5/Research/fetch_all_sources.sh
...
# these go wrong).
PLAN_SH=<WORKSPACE_DIR>/PlanPlan/plan.sh
...
do_sources() {
    if [ ! -f "$SOURCES_SCRIPT" ]; then
        ...
        echo "  is <WORKSPACE_DIR>/PseudoCoup_v5 checked out?" >&2
        return 2
    fi
    bash "$SOURCES_SCRIPT" "$@"
    return $?
...
require_tool() {
    if [ ! -f "$1" ]; then
        ...
        echo "  the framework lives in <WORKSPACE_DIR>/PlanPlan/ — is it checked out?" >&2
        exit 2
    fi
}
...
LOCK_REPOS=(
    <WORKSPACE_DIR>/PseudoCoupHQ
    <WORKSPACE_DIR>/PseudoCoup_v5
    <WORKSPACE_DIR>/PseudoCoup_v6
    <WORKSPACE_DIR>/PseudoIR
    <WORKSPACE_DIR>/PlanPlan
    <WORKSPACE_DIR>/DevComms
)
...
case "$CMD" in
    commit)
        ...
        if [ $rc -ne 0 ]; then
            if [ "$FORCE" = "--force" ]; then
                ...
                echo "    bash <WORKSPACE_DIR>/PseudoCoupHQ/hq.sh --force" >&2
                exit 1
            fi
        fi

        bash <WORKSPACE_DIR>/PseudoCoupHQ/git_commit_push_all.sh
        exit $?
        ;;
    ...
    explore)
        ...
        out=<WORKSPACE_DIR>/PseudoCoupHQ/plan_explorer.html
    ...
    list)
        ...
        bash <WORKSPACE_DIR>/PseudoCoupHQ/git_commit_push_all.sh --list | tail -n +2
        echo
