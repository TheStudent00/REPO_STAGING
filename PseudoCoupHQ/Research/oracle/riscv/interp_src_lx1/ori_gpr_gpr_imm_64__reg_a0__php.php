...
// InterpRenderer from the layer-4 term of ori_gpr_gpr_imm_64__reg_a0__php.
// The term's layer-5 text, LITERAL:
//   Concat(Extract(63, 2, v0), 3)
function emu_ori_gpr_gpr_imm_64__reg_a0__php($a) {
    return ex_m(ex_cat(ex_ext($a, 63, 2), 3, 2), 64);
}
...
while (($line = fgets($handle)) !== false) {
    ...
    try {
        $answer = emu_ori_gpr_gpr_imm_64__reg_a0__php(...$ints);
        if ($answer < 0) {
            echo sprintf("%s\n", sprintf("%u", $answer));
        } else {
            echo $answer . "\n";
