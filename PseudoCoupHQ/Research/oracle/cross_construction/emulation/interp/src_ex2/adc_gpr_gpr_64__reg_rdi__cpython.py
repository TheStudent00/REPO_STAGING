...
# InterpRenderer from the layer-4 term of adc_gpr_gpr_64__reg_rdi__cpython.
# The term's layer-5 text, LITERAL:
#   If(Extract(64, 64, Concat(0, v0) + Concat(0, v1)) == 1, 1, 0) + v2 + v3
def emu_adc_gpr_gpr_64__reg_rdi__cpython(a, b, c, d):
    return m(add(add(((1) if (eq(ext(add(cat(0, b, 64), cat(0, a, 64), 65), 64, 64), 1, 1)) else (0)), c, 64), d, 64), 64)
...
def main():
    for line in sys.stdin:
        ...
        try:
            answer = emu_adc_gpr_gpr_64__reg_rdi__cpython(*values)
            sys.stdout.write("%d\n" % answer)
