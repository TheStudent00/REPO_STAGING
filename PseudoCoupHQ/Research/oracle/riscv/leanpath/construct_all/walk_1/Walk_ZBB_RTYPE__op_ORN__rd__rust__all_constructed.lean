...
theorem meaning_ZBB_RTYPE__op_ORN__rd__rust__all_constructed (s : St) (a : BitVec 64) (b : BitVec 64) (h10 : s.regs.get? .x10 = some a) (h11 : s.regs.get? .x11 = some b) (hk0 : ∀ {α : Type} (x0 : α) (s : St), (plat_term_write x0) s = EStateM.Result.ok () s) (hk1 : ∀ (x0 : Arch.pa) (x1 : Nat) (s : St), (load_reservation x0 x1) s = EStateM.Result.ok () s) (hk2 : ∀ (x0 : Unit) (s : St), (cancel_reservation x0) s = EStateM.Result.ok () s) :
    runsTo (walk [LeanIM.instruction.ITYPE (0xfff#12, LeanIM.regidx.Regidx 0x0b#5, LeanIM.regidx.Regidx 0x0b#5, LeanIM.iop.XORI), LeanIM.instruction.C_OR (LeanIM.cregidx.Cregidx 0x2#3, LeanIM.cregidx.Cregidx 0x3#3)]) s
      ((pure_RTYPE (a) ((pure_ITYPE (b) (0xfff#12) (LeanIM.iop.XORI))) (OR))) := by
  simp (config := {decide := true}) [runsTo, walk, step, execute, strip_C_OR, strip_ITYPE, strip_RTYPE, alias_C_OR,
