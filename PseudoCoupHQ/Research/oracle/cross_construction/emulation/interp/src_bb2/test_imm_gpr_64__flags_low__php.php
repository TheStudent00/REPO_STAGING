...
// one named local per gate, over the term of test_imm_gpr_64__flags_low__php.
// The term's layer-5 text, LITERAL:
//   0
function emu_test_imm_gpr_64__flags_low__php() {
    $k0 = 0;
    $w0 = $k0;
    $w1 = ex_cat($w0, $k0, 1);
    $w2 = ex_cat($w1, $k0, 1);
...
while (($line = fgets($handle)) !== false) {
    ...
    try {
        $answer = emu_test_imm_gpr_64__flags_low__php(...$ints);
        if ($answer < 0) {
            echo sprintf("%s\n", sprintf("%u", $answer));
        } else {
            echo $answer . "\n";
