#!/bin/bash
# rv9 lane 5 -- SECTION 2, THE WHOLE POPULATION at setting 'ship':
# the corpus's own ship flags, untouched.
...
mkdir -p "$GOCACHE" "$GOPATH" /work/rv9_set_ship
total=3
i=1
...
echo "[$i/$total] the whole population at setting 'ship'"; i=$((i+1))
timeout 20000 python3 "$G/rv9_settings.py" run ship "$OP" "$OP" \
  "$EMU" "$RV/twins.json" "$RV/model_table_rv.json" \
  "$G/rv9_set_ship" "$G/src_rv9_set_ship" "/work/rv9_set_ship" \
...
python3 "$OP/check_no_spelling_keys.py" "$G/rv9_set_ship.json"
echo "  guard rc=$?"
ls -la "$G"/rv9_set_ship.jsonl "$G"/rv9_set_ship.json
echo "lane rv9_l5 done"
