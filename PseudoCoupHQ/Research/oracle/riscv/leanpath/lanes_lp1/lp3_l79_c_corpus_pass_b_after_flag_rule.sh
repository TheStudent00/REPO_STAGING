#!/bin/bash
# lp3_l79_c_corpus_pass_b_after_flag_rule.sh -- plan node
# hq.research.lean_proof_path_resistant_to_churn.system.pass_b_build over c:
# every arm of every certified pure form is rendered from `operator_for` and
# from nothing else; each emulation is compiled at the corpus's ship flags,
...
# whose operation IS a flag has arms at all (75 arms -> 93). The l74 render
# it replaces had 13 emulations from an 11-key table that held no integer-level
# operation; the gate (l76, l77) rendered 10 from a 10-key table and proved all
# ten, among them the multiply, both divides and both remainders. This is that
# render over the whole c table l78 just wrote.
set -uo pipefail
...
if [ "$n" -gt 0 ]; then
  ...
  grep -E "PROVED|equals:|Traceback" $R/pass_b/equals.log | cut -c1-160 | tail -30
fi
