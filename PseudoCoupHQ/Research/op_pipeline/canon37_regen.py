#!/usr/bin/env python3
"""canon37_regen.py -- TASK 47, the REGENERATED population (the 29,288
accepted probes of log_131) put through the memory-wrapped form.

THE FORM is ledger47.py's and THE GATE is canon37_gate.py's, imported
...
trickle itself used.  `canon37_regen_state.json` records, per chunk,
...
work.  Per-chunk results are written to `canon37_regen_store/<chunk
...
usage:
  canon37_regen.py --run [--chunks N] [--seconds S]
  canon37_regen.py --status
"""
...
import canon37_gate as GATE                                      # noqa: E402
import ledger47 as L47                                           # noqa: E402

STORE = os.path.join(HERE, "trickle_store")
OUT_DIR = os.path.join(HERE, "canon37_regen_store")
STATE = os.path.join(HERE, "canon37_regen_state.json")
...
def process_probe(lang, key, probe):
    ...
    ship_lines = L47.split_lines(body_text)
    home, width = BC10.answer_home_from_real(ship_lines)
    if home is None:
        home = entry_contract.get("a")
        width = 64
    ...
    try:
        fields = L47.wrap_unit(body_text, families, home, width)
    except L47.Refusal as bad:
        record["outcome"] = "REFUSED"
        record["refusal_cause"] = bad.cause
        record["refusal"] = bad.detail
        return record
    wrapped_lines = L47.split_lines(fields["wrapped_text_resolved"])
...
def run_chunk(path):
    ...
    out = {
        "meta": {
            ...
            "generated_by": "canon37_regen.py",
            "form": "ledger47: the memory-wrapped form",
            "population": "regenerated",
