...
- **Whether calls are followed.** Already a settled distinction:
  ...
  `~/Programming/PseudoCoup_v6/AgentMemory/02_decisions.md`). **That
...
- PseudoIR's borrow re-points at PCv5. Its
  ...
  `~/Programming/PseudoCoup_v6` — under this proposal it would come
  to mean version 5. That was already flagged as unsettled in
  `~/Programming/PseudoIR/Agent_Memory.md` §4 and this would settle
  it.
...
`~/Programming/PseudoCoup_v6/Tools/` holds `ledgerer` (with
