...
// one named local per gate, over the term of cmp_gpr_gpr_8__flags__php.
// The term's layer-5 text, LITERAL:
//   Concat(Extract(7, 0, v0), Extract(7, 0, v1))
function emu_cmp_gpr_gpr_8__flags__php($a, $b) {
    $x1_0 = ex_ext($b, 0, 0);
    $x1_1 = ex_ext($b, 1, 1);
    $x1_2 = ex_ext($b, 2, 2);
    $x1_3 = ex_ext($b, 3, 3);
    ...
    $w8 = ex_cat($w7, $x1_7, 1);
    $w9 = ex_cat($w8, $x1_6, 1);
    $w10 = ex_cat($w9, $x1_5, 1);
    $w11 = ex_cat($w10, $x1_4, 1);
    $w12 = ex_cat($w11, $x1_3, 1);
    $w13 = ex_cat($w12, $x1_2, 1);
    $w14 = ex_cat($w13, $x1_1, 1);
    $w15 = ex_cat($w14, $x1_0, 1);
    return ex_m($w15, 16);
}
...
while (($line = fgets($handle)) !== false) {
    ...
    try {
        $answer = emu_cmp_gpr_gpr_8__flags__php(...$ints);
        if ($answer < 0) {
            echo sprintf("%s\n", sprintf("%u", $answer));
        } else {
            echo $answer . "\n";
