...
public static class Emu {
    ...
    // InterpRenderer from the layer-4 term of setae_gpr_one_8__reg_rdi__csharp.
    // The term's layer-5 text, LITERAL:
    //   Concat(0, If(Extract(8, 8, Concat(0, Extract(7, 0, v0)) + Concat(0, Extract(7, 0, v1))) == 1, 0, 1))
    public static long emu_setae_gpr_one_8__reg_rdi__csharp(long a, long b) {
        return m(cat(0L, ((eq(ext(add(cat(0L, a, 8), cat(0L, b, 8), 9), 8, 8), 1L, 1)) ? (0L) : (1L)), 8), 64);
    }
    ...
    public static void Main() {
        ...
        while (line != null) {
            ...
            if (text.Length != 0) {
                ...
                try {
                    answers.Append(((ulong) emu_setae_gpr_one_8__reg_rdi__csharp(values[0], values[1])).ToString());
                    answers.Append("\n");
