#!/usr/bin/env python3
"""dom_ops7.py -- dom_ops6.py's own method, one table later:
dominant_table9.json (dominant_table8 plus the tree-exact evidence
ground -- the spill/reload-fixed, z3-normalized normal-path root,
character-identical, gated on the same class key).

Nothing about dom_ops.py's own construction changes here.  Same
languages (dom_ops6.py's list plus `java`), same staged inputs, same
run, renamed products so dom_ops6.json is untouched.
...
usage:
  dom_ops7.py [--in DIR] [--out DIR] [--every N]
"""
...
RENAME = [
    ("dom_ops.json", "dom_ops7.json"),
    ("dom_ops.md", "dom_ops7.md"),
    ("dom_ops_digest.md", "dom_ops7_digest.md"),
    ("dom_ops_digest_shape.json", "dom_ops7_digest_shape.json"),
]
...
def main():
    ...
    stage = os.path.join(outdir, "stage_dom_ops7")
    ...
    doc = json.load(open(os.path.join(outdir, "dom_ops7.json")))
