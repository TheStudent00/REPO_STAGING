#!/usr/bin/env bash
# ap4_l13_aggregate_and_report.sh -- task ap4: the store -> the
...
python3 autopoly4.py aggregate
echo ""
echo "[2/6] the report"
python3 autopoly4.py report
echo ""
echo "[3/6] the store and the aggregate compared run for run"
python3 autopoly4.py store
echo ""
echo "[4/6] the spelling guard, over every json this task wrote"
cd PseudoCoupHQ/Research/op_pipeline
for F in autopoly4_cells.json autopoly4.json autopoly4_check_L2_rerun.json; do
  python3 check_no_spelling_keys.py "$A/$F"
done
echo ""
echo "[5/6] no exemption anywhere in what this task wrote"
grep -c exempt "$A/autopoly4.py" "$A/lanes_ap4/"*.sh || true
echo ""
echo "[6/6] the tally"
cd "$A"
python3 autopoly4.py tally
echo "done"
