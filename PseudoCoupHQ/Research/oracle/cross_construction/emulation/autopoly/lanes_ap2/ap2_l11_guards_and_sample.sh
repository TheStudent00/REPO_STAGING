#!/usr/bin/env bash
# ap2_l11_guards_and_sample.sh -- task ap2: the four driver guards re-run against the driver as it now stands (lane ap2_l5 ran them before fix 1 was extended to the wide memory symbol), then the SAMPLE the law asks for before a long run: the first twenty of the 1,012 runs, with the peak resident size printed after each one.
