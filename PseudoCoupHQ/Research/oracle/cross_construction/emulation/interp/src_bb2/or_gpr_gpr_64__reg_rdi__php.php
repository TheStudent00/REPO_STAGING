...
// one named local per gate, over the term of or_gpr_gpr_64__reg_rdi__php.
// The term's layer-5 text, LITERAL:
//   v0 | v1
function emu_or_gpr_gpr_64__reg_rdi__php($a, $b) {
    $x1_0 = ex_ext($b, 0, 0);
    $x0_0 = ex_ext($a, 0, 0);
    $x1_1 = ex_ext($b, 1, 1);
    $x0_1 = ex_ext($a, 1, 1);
...
while (($line = fgets($handle)) !== false) {
    ...
    try {
        $answer = emu_or_gpr_gpr_64__reg_rdi__php(...$ints);
        if ($answer < 0) {
            echo sprintf("%s\n", sprintf("%u", $answer));
        } else {
            echo $answer . "\n";
