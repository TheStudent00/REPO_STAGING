/* task o7 emulation -- rendered by emulate.py Renderer from the layer-4 term of E01681__swift_regen_1881.  The term's layer-5 text, LITERAL:
   If(And(v1 == 0, Not(Or(Not(Extract(63, 7, v0) == 0), ULE(64, Extract(6, 0, v0))))), v2 << Concat(0, Extract(5, 0, v0)), 0) */
#include <stdint.h>

uint64_t
emu_E01681__swift_regen_1881(uint64_t a, uint64_t b, uint64_t c)
{
