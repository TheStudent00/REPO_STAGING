#!/usr/bin/env bash
# lp1 lane 1c -- lane 1 again, with the toolchain taken from the host the
...
URL="https://github.com/leanprover/lean4/releases/download/$VER/lean-$VER-linux.tar.zst"
echo "  $URL"
ARCH="$PERSIST/dl/lean-$VER-linux.tar.zst"
if [ ! -s "$ARCH" ]; then timed curl -sSL -o "$ARCH" "$URL"; fi
ls -la "$ARCH"; sha256sum "$ARCH"
command -v zstd || echo "  zstd: ABSENT (tar --zstd needs it)"; command -v unzstd || true
...
if [ ! -x "$DEST/bin/lean" ]; then
  mkdir -p "$DEST"
  if command -v zstd >/dev/null; then tar --zstd -xf "$ARCH" -C "$DEST" --strip-components=1; echo "  tar --zstd rc=$?";
  else python3 - "$ARCH" "$DEST" <<'PY'
import sys, subprocess
print("  no zstd binary; trying python zstandard module")
try:
    import zstandard
except ImportError:
    print("  FLAG: neither zstd nor python zstandard is in the image; cannot unpack .tar.zst")
    sys.exit(3)
PY
  fi
fi
ls "$DEST" "$DEST/bin" | head -20
elan toolchain list
...
df -h /persist | tail -1
echo "lane lp1_l1c done at $(date -u +%FT%TZ)"
