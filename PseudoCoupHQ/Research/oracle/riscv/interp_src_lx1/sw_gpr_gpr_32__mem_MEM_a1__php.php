...
// InterpRenderer from the layer-4 term of sw_gpr_gpr_32__mem_MEM_a1__php.
// The term's layer-5 text, LITERAL:
//   Concat(Extract(63, 32, v0), Extract(31, 0, v1))
function emu_sw_gpr_gpr_32__mem_MEM_a1__php($a, $b) {
...
while (($line = fgets($handle)) !== false) {
    ...
    try {
        $answer = emu_sw_gpr_gpr_32__mem_MEM_a1__php(...$ints);
        if ($answer < 0) {
            echo sprintf("%s\n", sprintf("%u", $answer));
        } else {
            echo $answer . "\n";
