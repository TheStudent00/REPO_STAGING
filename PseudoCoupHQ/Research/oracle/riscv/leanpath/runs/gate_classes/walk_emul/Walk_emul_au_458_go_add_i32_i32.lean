...
theorem meaning_emul_au_458_go_add_i32_i32 (s : St) (a : BitVec 64) (b : BitVec 64) (h10 : s.regs.get? .x10 = some a) (h11 : s.regs.get? .x11 = some b) (hk0 : ∀ {α : Type} (x0 : α) (s : St), (plat_term_write x0) s = EStateM.Result.ok () s) (hk1 : ∀ (x0 : Arch.pa) (x1 : Nat) (s : St), (load_reservation x0 x1) s = EStateM.Result.ok () s) (hk2 : ∀ (x0 : Unit) (s : St), (cancel_reservation x0) s = EStateM.Result.ok () s) :
    runsTo (walk [LeanIM.instruction.C_ADD (LeanIM.regidx.Regidx 0x0a#5, LeanIM.regidx.Regidx 0x0b#5), LeanIM.instruction.C_ZEXT_W (LeanIM.cregidx.Cregidx 0x2#3)]) s
      ((pure_ZBA_RTYPEUW ((pure_RTYPE (a) (b) (ADD))) (zero_reg) (0b00#2))) := by
  simp (config := {decide := true}) [runsTo, walk, step, execute, strip_C_ADD, strip_C_ZEXT_W, strip_RTYPE, strip_ZBA_RTYPEUW, alias_C_ADD, alias_C_ZEXT_W,
