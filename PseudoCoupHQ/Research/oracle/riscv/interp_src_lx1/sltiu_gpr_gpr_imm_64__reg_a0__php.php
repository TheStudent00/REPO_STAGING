...
// InterpRenderer from the layer-4 term of sltiu_gpr_gpr_imm_64__reg_a0__php.
// The term's layer-5 text, LITERAL:
//   If(Or(Extract(1, 0, v0) == 3, Not(Extract(63, 2, v0) == 0)), 0, 1)
function emu_sltiu_gpr_gpr_imm_64__reg_a0__php($a) {
    return ex_m(((((ex_eq(ex_ext($a, 1, 0), 3, 2)) || ((!(ex_eq(ex_ext($a, 63, 2), 0, 62)))))) ? (0) : (1)), 64);
}
...
while (($line = fgets($handle)) !== false) {
    ...
    try {
        $answer = emu_sltiu_gpr_gpr_imm_64__reg_a0__php(...$ints);
        if ($answer < 0) {
            echo sprintf("%s\n", sprintf("%u", $answer));
        } else {
            echo $answer . "\n";
