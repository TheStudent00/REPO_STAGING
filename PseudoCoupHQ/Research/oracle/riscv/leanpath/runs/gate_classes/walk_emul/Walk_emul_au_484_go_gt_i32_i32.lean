...
theorem meaning_emul_au_484_go_gt_i32_i32 (s : St) (a : BitVec 64) (b : BitVec 64) (h10 : s.regs.get? .x10 = some a) (h11 : s.regs.get? .x11 = some b) (hk0 : ∀ {α : Type} (x0 : α) (s : St), (plat_term_write x0) s = EStateM.Result.ok () s) (hk1 : ∀ (x0 : Arch.pa) (x1 : Nat) (s : St), (load_reservation x0 x1) s = EStateM.Result.ok () s) (hk2 : ∀ (x0 : Unit) (s : St), (cancel_reservation x0) s = EStateM.Result.ok () s) :
    runsTo (walk [LeanIM.instruction.C_SLLI (0x20#6, LeanIM.regidx.Regidx 0x0a#5), LeanIM.instruction.C_SLLI (0x20#6, LeanIM.regidx.Regidx 0x0b#5), LeanIM.instruction.RTYPE (LeanIM.regidx.Regidx 0x0a#5, LeanIM.regidx.Regidx 0x0b#5, LeanIM.regidx.Regidx 0x0a#5, LeanIM.rop.SLT)]) s
      ((pure_RTYPE ((pure_SHIFTIOP (b) ((0x20#6)) (SLLI))) ((pure_SHIFTIOP (a) ((0x20#6)) (SLLI))) (LeanIM.rop.SLT))) := by
