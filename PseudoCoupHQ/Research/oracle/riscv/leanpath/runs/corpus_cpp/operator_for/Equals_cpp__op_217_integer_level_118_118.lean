...
theorem cpp__op_217_118_integer_level (a : BitVec 64) (b : BitVec 64) :
    ((pure_DIV (a) (b) (false))) = ((to_bits_truncate (l := 64) (if (((if (((BitVec.toInt b) == 0) : Bool) then (Neg.neg 1) else (Int.tdiv (BitVec.toInt a) (BitVec.toInt b))) ≥b (2 ^i (xlen -i 1))) : Bool) then (Neg.neg (2 ^i (xlen -i 1))) else (if (((BitVec.toInt b) == 0) : Bool) then (Neg.neg 1) else (Int.tdiv (BitVec.toInt a) (BitVec.toInt b)))))) := by
  try simp only [pure_DIV, not, to_bits_truncate, xlen, Sail.get_slice_int]
  all_goals grind
