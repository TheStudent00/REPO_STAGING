...
// InterpRenderer from the layer-4 term of seto_gpr_one_8__reg_rdi__php.
...
function emu_seto_gpr_one_8__reg_rdi__php($a, $b) {
    return ex_m(ex_cat(0, ((ex_eq(ex_ext(ex_add(ex_cat(ex_ext($a, 7, 7), $a, 8), ex_cat(ex_ext($b, 7, 7), $b, 8), 9), 7, 7), ex_ext(ex_add(ex_cat(ex_ext($a, 7, 7), $a, 8), ex_cat(ex_ext($b, 7, 7), $b, 8), 9), 8, 8), 1)) ? (0) : (1)), 8), 64);
}
...
while (($line = fgets($handle)) !== false) {
    ...
    try {
        $answer = emu_seto_gpr_one_8__reg_rdi__php(...$ints);
        if ($answer < 0) {
            echo sprintf("%s\n", sprintf("%u", $answer));
        } else {
            echo $answer . "\n";
