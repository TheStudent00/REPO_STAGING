#!/usr/bin/env bash
# t98 lane 8 -- THE TRANSCRIPT THE LOG PASTES.
#
...
show 'python3 Research/op_pipeline/t98_probe_shapes.py terms | grep -v "walk:"'

echo
echo "======== [2/9] distinct machine code, the two measurements ========"
show 'python3 Research/op_pipeline/t98_settle.py bodies | grep -v "walk:"'

echo
echo "======== [3/9] the definitions of an opcode count, side by side ========"
show 'python3 Research/op_pipeline/t98_settle.py opcodes | grep -v "walk:"'

echo
echo "======== [4/9] every computed figure the pane draws ========"
show 'python3 Research/op_pipeline/t98_render_check.py figures | grep -v "figures:"'

echo
echo "======== [5/9] the pane's rows, controls and unexplained rows ========"
show 'python3 Research/op_pipeline/t98_render_check.py now | grep -v "wall clock" | grep -v "peak resident"'

echo
echo "======== [6/9] no row key goes unexplained at any moment walked ========"
show 'python3 Research/op_pipeline/t98_render_check.py keys 14 | tail -1'
