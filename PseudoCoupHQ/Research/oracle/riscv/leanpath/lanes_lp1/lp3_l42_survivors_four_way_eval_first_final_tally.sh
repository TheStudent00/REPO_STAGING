#!/bin/bash
# lp3_l42_survivors_four_way_eval_first_final_tally.sh -- the units whose
...
# rows replacing lane l40's for the same units. Fetches nothing.
set -uo pipefail
...
for q in range(4): json.dump({"summary":{}, "rows":rows[q::4]}, open("%s/walk_corpus_survivors_%d.json" % (A,q),"w"))
