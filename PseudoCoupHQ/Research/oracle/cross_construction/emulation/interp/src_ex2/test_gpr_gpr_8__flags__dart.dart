...
// InterpRenderer from the layer-4 term of test_gpr_gpr_8__flags__dart.
// The term's layer-5 text, LITERAL:
//   Concat(~(~Extract(7, 0, v0) | ~Extract(7, 0, v1)), 0)
int emu_test_gpr_gpr_8__flags__dart(int a, int b) {
  return m(cat(bnot(bor(bnot(a, 8), bnot(b, 8), 8), 8), 0, 8), 16);
}
  ...
  while (line != null) {
    ...
    if (text.isNotEmpty) {
      final List<int> values = text
          ...
          .toList();
      try {
        final int answer = emu_test_gpr_gpr_8__flags__dart(values[0], values[1]);
