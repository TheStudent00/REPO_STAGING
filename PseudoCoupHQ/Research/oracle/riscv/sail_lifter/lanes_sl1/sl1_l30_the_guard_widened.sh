#!/bin/bash
# sl1 lane 30 -- THE GUARD again, the population widened (structure-typed and multi-line-enum fields, abbreviations), the float points evaluated through a solver model, roundToInt given its shape. The whole generated population: every
...
# model's own C simulator; the store guard_all2.json, then the spelling
# guard over it.
set -u
export HOME=/work
...
timeout 9000 python3 $SL/sail_guard.py $SL/guard_all2 /work/sl1h --points 1000 2>&1 | cut -c1-300
echo "  exit: ${PIPESTATUS[0]}  finished $(date -u +%FT%TZ)"
echo "[2/3] the census, by constructor and outcome"
python3 - <<'PYEOF'
import json
d = json.load(open("PseudoCoupHQ/Research/oracle/riscv/sail_lifter/guard_all2.json"))
rows = d["rows"]
...
python3 $OP/check_no_spelling_keys.py $SL/guard_all2.json $SL/guard_all.json; echo "  guard rc=$?"
