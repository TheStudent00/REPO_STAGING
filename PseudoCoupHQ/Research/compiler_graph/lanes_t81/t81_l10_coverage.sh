#!/usr/bin/env bash
# t81 lane 10 — `Graph.coverage`: the c and cpp diaries joined to
# graph_cpp.json.
#
...
if not found:
    print('   MEMORY GATE: NO PEAK WAS MEASURED. A missing measurement is '
          'not a small one, so the full join is REFUSED BY NAME.')
    sys.exit(9)
peak_mb = float(found.group(1))
if 'exit 0' not in text:
    print('   MEMORY GATE: the sample join did not exit 0, so its peak is '
          'not a measurement of the pass. REFUSED BY NAME.')
    sys.exit(9)
