...
// InterpRenderer from the layer-4 term of auipc_gpr_imm_32__reg_a0__php.
// The term's layer-5 text, LITERAL:
//   v0 + 12288
function emu_auipc_gpr_imm_32__reg_a0__php($a) {
    return ex_m(ex_add($a, 12288, 64), 64);
}
...
while (($line = fgets($handle)) !== false) {
    ...
    try {
        $answer = emu_auipc_gpr_imm_32__reg_a0__php(...$ints);
        if ($answer < 0) {
            echo sprintf("%s\n", sprintf("%u", $answer));
        } else {
            echo $answer . "\n";
