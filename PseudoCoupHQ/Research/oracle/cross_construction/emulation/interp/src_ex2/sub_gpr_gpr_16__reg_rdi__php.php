...
// InterpRenderer from the layer-4 term of sub_gpr_gpr_16__reg_rdi__php.
// The term's layer-5 text, LITERAL:
//   Concat(0, Extract(15, 0, v0)*65535 + Extract(15, 0, v1))
function emu_sub_gpr_gpr_16__reg_rdi__php($a, $b) {
    return ex_m(ex_cat(0, ex_add(ex_mul($b, 65535, 16), $a, 16), 16), 64);
}
...
while (($line = fgets($handle)) !== false) {
    ...
    try {
        $answer = emu_sub_gpr_gpr_16__reg_rdi__php(...$ints);
        if ($answer < 0) {
            echo sprintf("%s\n", sprintf("%u", $answer));
        } else {
            echo $answer . "\n";
