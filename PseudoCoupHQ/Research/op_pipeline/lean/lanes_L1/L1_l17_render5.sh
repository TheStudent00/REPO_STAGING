#!/bin/bash
# L1 lane 17 — the renderer preservation theorem, build 5. Three gaps build 4
...
mkdir -p "$HOME"
echo '[1/1] Render.lean, build 5'
cd "$LEANDIR/archproof" || exit 1
time lean Archproof/Render.lean > /work/render6_out.txt 2>&1
echo "--- lean exit $?"
grep -v 'linter.unusedSimpArgs\|Hint: Omit it\|^$\|Note: This linter\|̵' /work/render6_out.txt | head -120
echo "--- declarations using sorry: $(grep -c "declaration uses 'sorry'" /work/render6_out.txt)"
echo "--- error lines: $(grep -c 'error:' /work/render6_out.txt)"
