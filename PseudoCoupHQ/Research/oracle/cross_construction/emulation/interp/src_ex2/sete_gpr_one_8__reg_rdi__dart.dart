...
// InterpRenderer from the layer-4 term of sete_gpr_one_8__reg_rdi__dart.
// The term's layer-5 text, LITERAL:
//   Concat(0, If(Extract(7, 0, v0) == Extract(7, 0, v1), 1, 0))
int emu_sete_gpr_one_8__reg_rdi__dart(int a, int b) {
  return m(cat(0, ((eq(a, b, 8)) ? (1) : (0)), 8), 64);
}
  ...
  while (line != null) {
    ...
    if (text.isNotEmpty) {
      final List<int> values = text
          ...
          .toList();
      try {
        final int answer = emu_sete_gpr_one_8__reg_rdi__dart(values[0], values[1]);
