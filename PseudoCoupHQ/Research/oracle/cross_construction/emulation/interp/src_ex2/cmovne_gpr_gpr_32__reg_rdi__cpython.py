...
# InterpRenderer from the layer-4 term of cmovne_gpr_gpr_32__reg_rdi__cpython.
# The term's layer-5 text, LITERAL:
#   Concat(0, If(~(~Extract(31, 0, v0) | ~Extract(31, 0, v1)) == 0, Extract(31, 0, v2), Extract(31, 0, v3)))
def emu_cmovne_gpr_gpr_32__reg_rdi__cpython(a, b, c, d):
    return m(cat(0, ((d) if (eq(bnot(bor(bnot(b, 32), bnot(a, 32), 32), 32), 0, 32)) else (c)), 32), 64)
...
def main():
    for line in sys.stdin:
        ...
        try:
            answer = emu_cmovne_gpr_gpr_32__reg_rdi__cpython(*values)
            sys.stdout.write("%d\n" % answer)
