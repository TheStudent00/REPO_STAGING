...
import LeanIM.VmemUtils
import LeanIM.ZicfilpInsts
import LeanIM.BaseInsts
import LeanIM.MextInsts
...
def execute_SRET (_ : Unit) : SailM ExecutionResult := do
  let sret_illegal ← (( do
    ...
    | .VirtualSupervisor => (pure false) ) : SailM Bool )
  let sret_virtual ← (( do
    match (← readReg cur_privilege) with
    | .VirtualUser => (pure true)
    | .VirtualSupervisor => (pure ((_get_Hstatus_VTSR (← readReg hstatus)) == 1#1))
    | _ => (pure false) ) : SailM Bool )
  if (sret_illegal : Bool)
  then (pure (Illegal_Instruction ()))
  else
    (do
      if (sret_virtual : Bool)
      then (pure (Virtual_Instruction ()))
      else
        (do
            ...
            (do
              ...
              let ret ← do
                ...
                then
                  (do
                    let new_priv ← do
                      if (((_get_Mstatus_SPP (← readReg vsstatus)) == 1#1) : Bool)
                      then (pure VirtualSupervisor)
                      else (pure VirtualUser)
                    writeReg vsstatus (Sail.BitVec.updateSubrange (← readReg vsstatus) 8 8 0#1)
                    writeReg vsstatus (Sail.BitVec.updateSubrange (← readReg vsstatus) 1 1
                      (_get_Mstatus_SPIE (← readReg vsstatus)))
                    writeReg vsstatus (Sail.BitVec.updateSubrange (← readReg vsstatus) 5 5 1#1)
                    writeReg cur_privilege new_priv
                    (pure VirtualSupervisor))
                else
                  (do
                    let new_priv ← do
                      (privLevel_bits_forwards
                        ((0#1 +++ (_get_Mstatus_SPP (← readReg mstatus))), ((_get_Hstatus_SPV
                            (← readReg hstatus)) &&& (bool_to_bit (← (currentlyEnabled Ext_H))))))
                    writeReg hstatus (Sail.BitVec.updateSubrange (← readReg hstatus) 7 7 0#1)
              ...
              then (zicfilp_restore_elp_on_xret sRET prev_priv (← readReg cur_privilege))
              else (pure ())
              (long_csr_write_callback "mstatus" "mstatush" (← readReg mstatus))
              if ((privLevel_is_virtual prev_priv) : Bool)
              then
              ...
              (set_next_pc (← (prepare_xret_target ret)))
...
def execute_SFENCE_VMA (rs1 : regidx) (rs2 : regidx) : SailM ExecutionResult := do
  ...
  let asid ← (( do
    ...
    then
      (pure (some
          (zero_extend (m := 16) (Sail.BitVec.extractLsb (← (rX_bits rs2)) (asidlen -i 1) 0))))
    else (pure none) ) : SailM (Option asidbits) )
  let vmid ← do
    if (((← readReg cur_privilege) == VirtualSupervisor) : Bool)
    then (pure (some (← (get_vmid VS_Stage))))
    else (pure none)
  match (← readReg cur_privilege) with
  ...
  | .Supervisor =>
    (do
      ...
      | _ =>
        (do
          (flush_TLB asid vmid addr S_Stage)
          (pure RETIRE_SUCCESS)))
  | .Machine =>
    (do
      (flush_TLB asid vmid addr S_Stage)
  ...
  | .VirtualSupervisor =>
    (do
      match (_get_Hstatus_VTVM (← readReg hstatus)) with
      | 1 => (pure (Virtual_Instruction ()))
      | _ =>
        (do
...
/-- Type quantifiers: k_ex589473_ : Bool -/
...
/-- Type quantifiers: k_ex589482_ : Bool -/
      ...
      then (pure (Ext_XRET_Priv_Failure ()))
        ...
        (do
          ...
          writeReg cur_privilege (← (privLevel_bits_forwards
              ((_get_Mstatus_MPP (← readReg mstatus)), (← do
                if (((_get_Mstatus_MPP (← readReg mstatus)) == 0b11#2) : Bool)
                then (pure 0#1)
                else (pure (_get_Mstatus_MPV (← readReg mstatus)))))))
          ...
          writeReg mstatus (Sail.BitVec.updateSubrange (← readReg mstatus) 39 39 0#1)
          if ((bne (← readReg cur_privilege) Machine) : Bool)
          then writeReg mstatus (Sail.BitVec.updateSubrange (← readReg mstatus) 17 17 0#1)
          else (pure ())
          if ((hartSupports Ext_Zicfilp) : Bool)
          then (zicfilp_restore_elp_on_xret mRET prev_priv (← readReg cur_privilege))
          else (pure ())
          (long_csr_write_callback "mstatus" "mstatush" (← readReg mstatus))
          if ((get_config_print_exception ()) : Bool)
          then
...
/-- Type quantifiers: width : Nat, k_ex589517_ : Bool, width ∈ {1, 2, 4, 8} -/
...
def execute_ECALL (_ : Unit) : SailM ExecutionResult := do
  let exc_type ← (( do
    ...
    | .VirtualUser => (pure (E_U_EnvCall ()))
    | .VirtualSupervisor => (pure (E_VS_EnvCall ())) ) : SailM ExceptionType )
  let c : ExceptionContext :=
    { trap := exc_type
...
def execute_EBREAK (_ : Unit) : SailM ExecutionResult := do
  (trap (← (exception_context (E_Breakpoint Brk_Software) (← readReg PC))))

/-- Type quantifiers: k_ex589524_ : Bool -/
...
/-- Type quantifiers: k_ex589533_ : Bool -/
