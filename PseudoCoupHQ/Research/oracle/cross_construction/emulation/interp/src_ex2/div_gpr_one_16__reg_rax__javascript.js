...
// InterpRenderer from the layer-4 term of div_gpr_one_16__reg_rax__javascript.
// The term's layer-5 text, LITERAL:
//   Concat(Extract(63, 16, v1), Extract(15, 0, bvudiv_i(Concat(Extract(15, 0, v0), Extract(15, 0, v1)), Concat(0, Extract(15, 0, v2)))))
function emu_div_gpr_one_16__reg_rax__javascript(a, b, c) {
    return m(cat(ext(b, 63, 16), ext(udiv(cat(a, ext(b, 15, 0), 16), cat(0n, c, 16), 32), 15, 0), 16), 64);
}
...
for (const line of lines) {
    ...
    try {
        out.push(emu_div_gpr_one_16__reg_rax__javascript(...values).toString());
    } catch (problem) {
