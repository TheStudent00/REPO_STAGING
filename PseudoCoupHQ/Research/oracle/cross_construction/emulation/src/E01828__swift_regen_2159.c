/* task o7 emulation -- rendered by emulate.py Renderer from the layer-4 term of E01828__swift_regen_2159.  The term's layer-5 text, LITERAL:
   If(Or(Not(Extract(31, 4, v0) == 0), ULE(8, Extract(3, 0, v0))), 0, Concat(0, LShR(Extract(7, 0, v1), Concat(0, Extract(2, 0, v0))))) */
#include <stdint.h>

uint32_t
emu_E01828__swift_regen_2159(uint8_t a, uint32_t b)
{
