...
// one named local per gate, over the term of and_imm_gpr_8__reg_rdi__dart.
// The term's layer-5 text, LITERAL:
//   Concat(Extract(63, 8, v1), ~(~Extract(7, 0, v0) | ~Extract(7, 0, v1)))
int emu_and_imm_gpr_8__reg_rdi__dart(int a, int b) {
  int x1_0 = ext(a, 0, 0);
  int x0_0 = ext(b, 0, 0);
  int x1_1 = ext(a, 1, 1);
  int x0_1 = ext(b, 1, 1);
  ...
  int w48 = cat(w47, x1_15, 1);
  int w49 = cat(w48, x1_14, 1);
  int w50 = cat(w49, x1_13, 1);
  int w51 = cat(w50, x1_12, 1);
  int w52 = cat(w51, x1_11, 1);
  int w53 = cat(w52, x1_10, 1);
  int w54 = cat(w53, x1_9, 1);
  int w55 = cat(w54, x1_8, 1);
  ...
  while (line != null) {
    ...
    if (text.isNotEmpty) {
      final List<int> values = text
          ...
          .toList();
      try {
        final int answer = emu_and_imm_gpr_8__reg_rdi__dart(values[0], values[1]);
