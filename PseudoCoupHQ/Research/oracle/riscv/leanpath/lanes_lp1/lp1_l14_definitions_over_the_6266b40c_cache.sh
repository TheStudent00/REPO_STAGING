#!/usr/bin/env bash
# lp1 lane 14 -- brief §2 lane 2 over the THIRD cache (model 6266b40c1c,
...
OUT=$LP/lp1_definitions_6266b40c_sail_8eb1fb6b.json
total=7
...
echo "[3/$total] every key the module read (LITERAL, all of them), the counts by width and by operand form, and the clauses with no key"
python3 - "$OUT" <<'PY'
import json, sys
...
echo "[4/$total] the modules of the emit inside and outside the import closure of LeanIM.lean (the 79 built of 115 files), LITERAL"
python3 - "$CACHE" <<'PY'
import os, sys
...
for f in $LP/leanpath/*.py $LP/leanpath/lean/* $OUT $LP/lanes_lp1/lp1_l1[0-4]*.sh; do
  echo "    $(grep -c exempt "$f") $(basename "$f")"
done
echo "lane lp1_l14 done at $(date -u +%FT%TZ)"
