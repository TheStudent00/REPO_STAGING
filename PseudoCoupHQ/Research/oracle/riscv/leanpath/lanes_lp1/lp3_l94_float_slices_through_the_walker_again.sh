#!/bin/bash
# lp3_l94_float_slices_through_the_walker_again.sh -- l93 again. l93 refused all four:
# "reads x15, outside the ABI arguments". Cause was MY word extraction, which tested the
# first character of the address with isdigit, so every instruction at an address starting
# with a hex LETTER (0xa, 0xc, 0xe...) was dropped -- including the one that initialised the
# scratch register. Words re-extracted with a proper pattern; 9 units now, every flattened
# float operation that has zero branches AND zero memory.
