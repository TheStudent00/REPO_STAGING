...
theorem meaning_emul_au_407_c_ne_i32_i32 (s : St) (a : BitVec 64) (b : BitVec 64) (d : BitVec 64) (h10 : s.regs.get? .x10 = some a) (h11 : s.regs.get? .x11 = some b) (h13 : s.regs.get? .x13 = some d) (hk0 : ∀ {α : Type} (x0 : α) (s : St), (plat_term_write x0) s = EStateM.Result.ok () s) (hk1 : ∀ (x0 : Arch.pa) (x1 : Nat) (s : St), (load_reservation x0 x1) s = EStateM.Result.ok () s) (hk2 : ∀ (x0 : Unit) (s : St), (cancel_reservation x0) s = EStateM.Result.ok () s) :
    runsTo (walk [LeanIM.instruction.C_XOR (LeanIM.cregidx.Cregidx 0x2#3, LeanIM.cregidx.Cregidx 0x3#3), LeanIM.instruction.C_ADDIW (0x00#6, LeanIM.regidx.Regidx 0x0a#5), LeanIM.instruction.RTYPE (LeanIM.regidx.Regidx 0x0a#5, LeanIM.regidx.Regidx 0x00#5, LeanIM.regidx.Regidx 0x0a#5, LeanIM.rop.SLTU)]) s
      ((pure_RTYPE (zero_reg) ((pure_ADDIW ((pure_RTYPE (a) (b) (XOR))) ((sign_extend (m := 12) (0x00#6))))) (LeanIM.rop.SLTU))) := by
  simp (config := {decide := true}) [runsTo, walk, step, execute, strip_C_XOR, strip_C_ADDIW, strip_RTYPE, strip_ADDIW, alias_C_XOR, alias_C_ADDIW,
