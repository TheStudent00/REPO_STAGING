...
// InterpRenderer from the layer-4 term of cmovne_gpr_gpr_64__reg_rdi__dart.
// The term's layer-5 text, LITERAL:
//   If(~(~v0 | ~v1) == 0, v2, v3)
int emu_cmovne_gpr_gpr_64__reg_rdi__dart(int a, int b, int c, int d) {
  return m(((eq(bnot(bor(bnot(b, 64), bnot(a, 64), 64), 64), 0, 64)) ? (d) : (c)), 64);
}
  ...
  while (line != null) {
    ...
    if (text.isNotEmpty) {
      final List<int> values = text
          ...
          .toList();
      try {
        final int answer = emu_cmovne_gpr_gpr_64__reg_rdi__dart(values[0], values[1], values[2], values[3]);
