...
// one named local per gate, over the term of cmp_gpr_gpr_8__flags__dart.
// The term's layer-5 text, LITERAL:
//   Concat(Extract(7, 0, v0), Extract(7, 0, v1))
int emu_cmp_gpr_gpr_8__flags__dart(int a, int b) {
  int x1_0 = ext(b, 0, 0);
  int x1_1 = ext(b, 1, 1);
  int x1_2 = ext(b, 2, 2);
  int x1_3 = ext(b, 3, 3);
  ...
  int x0_7 = ext(a, 7, 7);
  int w0 = x0_7;
  int w1 = cat(w0, x0_6, 1);
  int w2 = cat(w1, x0_5, 1);
  int w3 = cat(w2, x0_4, 1);
  int w4 = cat(w3, x0_3, 1);
  int w5 = cat(w4, x0_2, 1);
  int w6 = cat(w5, x0_1, 1);
  int w7 = cat(w6, x0_0, 1);
  int w8 = cat(w7, x1_7, 1);
  int w9 = cat(w8, x1_6, 1);
  int w10 = cat(w9, x1_5, 1);
  int w11 = cat(w10, x1_4, 1);
  int w12 = cat(w11, x1_3, 1);
  int w13 = cat(w12, x1_2, 1);
  int w14 = cat(w13, x1_1, 1);
  int w15 = cat(w14, x1_0, 1);
  return m(w15, 16);
}
  ...
  while (line != null) {
    ...
    if (text.isNotEmpty) {
      final List<int> values = text
          ...
          .toList();
      try {
        final int answer = emu_cmp_gpr_gpr_8__flags__dart(values[0], values[1]);
