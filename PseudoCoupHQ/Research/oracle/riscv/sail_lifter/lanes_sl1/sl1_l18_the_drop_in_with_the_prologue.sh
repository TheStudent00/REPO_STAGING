#!/bin/bash
# sl1 lane 18 -- lane 17 again: the lifter now runs the harness's own float-unit prologue through the model after reset, and the comparison table's verdict lookup is fixed. The drop-in riscv_reference.py (its walk now the model's
