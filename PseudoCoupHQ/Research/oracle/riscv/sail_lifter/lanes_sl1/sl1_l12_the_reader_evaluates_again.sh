#!/bin/bash
# sl1 lane 12 -- lane 11 again after five reader fixes (the +++ operator, = as equality, unary minus, some as a constructor, an iterative match, rvc on in the assembler). The reader over the real emit: parse (counts, unread
...
for name in ("reset", "reset_sys", "reset_misa"):
    if name not in defs:
