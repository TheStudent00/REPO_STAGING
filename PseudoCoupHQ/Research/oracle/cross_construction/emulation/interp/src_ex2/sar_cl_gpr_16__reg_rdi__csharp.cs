...
public static class Emu {
    ...
    // InterpRenderer from the layer-4 term of sar_cl_gpr_16__reg_rdi__csharp.
    // The term's layer-5 text, LITERAL:
    //   Concat(0, Extract(15, 0, v0) >> Concat(0, Extract(4, 0, v1)))
    public static long emu_sar_cl_gpr_16__reg_rdi__csharp(long a, long b) {
        return m(cat(0L, ashr(a, cat(0L, ext(b, 4, 0), 5), 16), 16), 64);
    }
    ...
    public static void Main() {
        ...
        while (line != null) {
            ...
            if (text.Length != 0) {
                ...
                try {
                    answers.Append(((ulong) emu_sar_cl_gpr_16__reg_rdi__csharp(values[0], values[1])).ToString());
                    answers.Append("\n");
