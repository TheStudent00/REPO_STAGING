...
now on; nothing writes back into `<WORKSPACE_DIR>/PseudoCoup_v5`.
...
- Verdict data: `<WORKSPACE_DIR>/PseudoCoup_v5/Designing/intention_tables_gen.py`
  and `<WORKSPACE_DIR>/PseudoCoup_v5/Designing/build_verdicts.py`,
  copied forward 2026-07-28; the R1-verified reference artifact is
  `<WORKSPACE_DIR>/PseudoCoup_v5/Designing/pc_verdicts.json`.
- row_satisfiers: `<WORKSPACE_DIR>/PseudoCoup_v5/Designing/intention_row_satisfiers.md`
  and `<WORKSPACE_DIR>/PseudoCoup_v5/Designing/BEJ_expansion.md`.
- canon: the `pc_verdict` strings themselves; every entry cites the
  exact string it came from and the builder refuses on drift.
- minimum_set: `<WORKSPACE_DIR>/PseudoCoup_v5/Designing/minimum_intention_set.md`
  (the 11 objects, post-audit).
- policy_refs: the "policy N" mentions in verdict strings, linked to
  `<WORKSPACE_DIR>/PseudoCoup_v5/Designing/PCv7_policy_decisions.md`.
## Run

Rebuild the artifact (refuses on incomplete data):

```
python3 <WORKSPACE_DIR>/PseudoIR/Tools/intentions/build_intentions.py
```

Validate a form:

```
python3 <WORKSPACE_DIR>/PseudoIR/Tools/intentions/validate_form.py \
    FORM.json --root <WORKSPACE_DIR>/PseudoCoup_v5
```

Acceptance:

```
python3 -m pytest <WORKSPACE_DIR>/PseudoIR/Tools/intentions/ -q
```
