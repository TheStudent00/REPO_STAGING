/* task o7 emulation -- rendered by emulate.py Renderer from the layer-4 term of div_gpr_one_64__reg_rdx__c.  The term's layer-5 text, LITERAL:
   Extract(63, 0, bvurem_i(Concat(v0, v1), Concat(0, v2))) */
#include <stdint.h>

uint64_t
emu_div_gpr_one_64__reg_rdx__c(uint64_t a, uint64_t b, uint64_t c)
{
