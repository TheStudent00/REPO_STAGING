#!/bin/bash
# lp3_l84_c_corpus_pass_b_width_rule_by_use.sh -- plan node
...
# ALL of c, after the gate (l82) was read. `render` takes an arch-unit for a key
# when the arch-unit's holder widths equal the widths the key's own text reads
# its operands at -- not 64 for every key. The l79 render it replaces had 18
# emulations and refused the W forms (proved into the table, held at 32-bit
# holders); this render is from the l83 table, which carries each key's widths.
set -uo pipefail
