#!/usr/bin/env bash
# ap5_l9_guard_check_and_the_handful.sh -- task ap5: two of the four
...
json.dump(index, open("/tmp/ap5_check_index.json", "w"))
...
index = json.load(open("/tmp/ap5_check_index.json"))
same = 0
moved = []
