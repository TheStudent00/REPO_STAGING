...
// InterpRenderer from the layer-4 term of lwu_gpr_gpr_same_32__reg_a0__php.
// The term's layer-5 text, LITERAL:
//   Concat(0, Extract(31, 0, v0))
function emu_lwu_gpr_gpr_same_32__reg_a0__php($a) {
    return ex_m(ex_cat(0, $a, 32), 64);
}
...
while (($line = fgets($handle)) !== false) {
    ...
    try {
        $answer = emu_lwu_gpr_gpr_same_32__reg_a0__php(...$ints);
        if ($answer < 0) {
            echo sprintf("%s\n", sprintf("%u", $answer));
        } else {
            echo $answer . "\n";
