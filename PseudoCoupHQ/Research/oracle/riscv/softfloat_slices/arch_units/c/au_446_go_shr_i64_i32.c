...
uint64_t au_446_go_shr_i64_i32(uint64_t p0, uint64_t p1)
{
    ...
    const uint64_t v6 = au_sltu(v5, UINT64_C(0x40));
    const uint64_t v7 = au_add(v6, UINT64_C(0xffffffffffffffff));
    const uint64_t v8 = au_or(v2, v7);
    const uint64_t v9 = au_sra(v1, v8);
    /* the answer is int64, 64 bits */
    return v9;
}

uint64_t au_446_go_shr_i64_i32_trap(uint64_t p0, uint64_t p1)
{
    /* a0: operand `a` (int64) arrives in a0 */
    const uint64_t v1 = p0;
    /* a1: operand `b` (int32) sign-extended to XLEN, as the ABI presents it */
