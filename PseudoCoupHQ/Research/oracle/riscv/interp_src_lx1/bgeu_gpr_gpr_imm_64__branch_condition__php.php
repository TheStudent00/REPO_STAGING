...
// InterpRenderer from the layer-4 term of bgeu_gpr_gpr_imm_64__branch_condition__php.
// The term's layer-5 text, LITERAL:
//   If(ULE(v0, v1), 1, 0)
function emu_bgeu_gpr_gpr_imm_64__branch_condition__php($a, $b) {
...
while (($line = fgets($handle)) !== false) {
    ...
    try {
        $answer = emu_bgeu_gpr_gpr_imm_64__branch_condition__php(...$ints);
        if ($answer < 0) {
            echo sprintf("%s\n", sprintf("%u", $answer));
        } else {
            echo $answer . "\n";
