...
theorem meaning_flw_fpr_fpr_fpr_32__freg_fa0__rust__all_constructed (s : St) (a : BitVec 64) (d : BitVec 64) (h10 : s.regs.get? .x10 = some a) (h13 : s.regs.get? .x13 = some d) (hk0 : ∀ {α : Type} (x0 : α) (s : St), (plat_term_write x0) s = EStateM.Result.ok () s) (hk1 : ∀ (x0 : Arch.pa) (x1 : Nat) (s : St), (load_reservation x0 x1) s = EStateM.Result.ok () s) (hk2 : ∀ (x0 : Unit) (s : St), (cancel_reservation x0) s = EStateM.Result.ok () s) :
    runsTo (walk [LeanIM.instruction.C_LI (0x3f#6, LeanIM.regidx.Regidx 0x0b#5), LeanIM.instruction.C_SLLI (0x20#6, LeanIM.regidx.Regidx 0x0b#5), LeanIM.instruction.C_OR (LeanIM.cregidx.Cregidx 0x2#3, LeanIM.cregidx.Cregidx 0x3#3)]) s
      ((pure_RTYPE (a) ((pure_SHIFTIOP ((pure_ITYPE (zero_reg) (((sign_extend (m := 12) (0x3f#6)))) (ADDI))) ((0x20#6)) (SLLI))) (OR))) := by
  simp (config := {decide := true}) [runsTo, walk, step, execute, strip_C_LI, strip_C_SLLI, strip_C_OR, strip_ITYPE, strip_SHIFTIOP, strip_RTYPE, alias_C_LI, alias_C_SLLI, alias_C_OR,
