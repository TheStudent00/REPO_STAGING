/* task t4 emulation -- rendered by render_general.py, one named
   intermediate per node of the term of xor_gpr_same_32__reg_rdi__c__bit_blast.  The term's text, LITERAL:
   0 */
#include <stdint.h>

uint64_t
emu_xor_gpr_same_32__reg_rdi__c__bit_blast(uint64_t a)
{
    uint64_t k0 = 0;
    uint64_t w0 = (k0 << 0);
    uint64_t w1 = w0 | (k0 << 1);
