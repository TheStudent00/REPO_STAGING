...
// InterpRenderer from the layer-4 term of mul_gpr_gpr_same_64__reg_a0__php.
// The term's layer-5 text, LITERAL:
//   v0*v0
function emu_mul_gpr_gpr_same_64__reg_a0__php($a) {
    return ex_m(ex_mul($a, $a, 64), 64);
}
...
while (($line = fgets($handle)) !== false) {
    ...
    try {
        $answer = emu_mul_gpr_gpr_same_64__reg_a0__php(...$ints);
        if ($answer < 0) {
            echo sprintf("%s\n", sprintf("%u", $answer));
        } else {
            echo $answer . "\n";
