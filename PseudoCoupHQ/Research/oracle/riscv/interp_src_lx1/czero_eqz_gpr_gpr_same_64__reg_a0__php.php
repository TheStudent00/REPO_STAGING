...
// InterpRenderer from the layer-4 term of czero_eqz_gpr_gpr_same_64__reg_a0__php.
// The term's layer-5 text, LITERAL:
//   If(v0 == 0, 0, v0)
function emu_czero_eqz_gpr_gpr_same_64__reg_a0__php($a) {
    return ex_m(((ex_eq($a, 0, 64)) ? (0) : ($a)), 64);
}
...
while (($line = fgets($handle)) !== false) {
    ...
    try {
        $answer = emu_czero_eqz_gpr_gpr_same_64__reg_a0__php(...$ints);
        if ($answer < 0) {
            echo sprintf("%s\n", sprintf("%u", $answer));
        } else {
            echo $answer . "\n";
