...
public static class Emu {
    ...
    // InterpRenderer from the layer-4 term of mul_gpr_one_8__reg_rax__csharp.
    // The term's layer-5 text, LITERAL:
    //   Concat(Extract(63, 16, v0), Concat(0, Extract(7, 0, v0))*Concat(0, Extract(7, 0, v1)))
    public static long emu_mul_gpr_one_8__reg_rax__csharp(long a, long b) {
        return m(cat(ext(a, 63, 16), mul(cat(0L, ext(a, 7, 0), 8), cat(0L, b, 8), 16), 16), 64);
    }
    ...
    public static void Main() {
        ...
        while (line != null) {
            ...
            if (text.Length != 0) {
                ...
                try {
                    answers.Append(((ulong) emu_mul_gpr_one_8__reg_rax__csharp(values[0], values[1])).ToString());
                    answers.Append("\n");
