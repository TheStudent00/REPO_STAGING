...
// InterpRenderer from the layer-4 term of setae_gpr_one_8__reg_rdi__php.
// The term's layer-5 text, LITERAL:
//   Concat(0, If(Extract(8, 8, Concat(0, Extract(7, 0, v0)) + Concat(0, Extract(7, 0, v1))) == 1, 0, 1))
function emu_setae_gpr_one_8__reg_rdi__php($a, $b) {
    return ex_m(ex_cat(0, ((ex_eq(ex_ext(ex_add(ex_cat(0, $a, 8), ex_cat(0, $b, 8), 9), 8, 8), 1, 1)) ? (0) : (1)), 8), 64);
}
...
while (($line = fgets($handle)) !== false) {
    ...
    try {
        $answer = emu_setae_gpr_one_8__reg_rdi__php(...$ints);
        if ($answer < 0) {
            echo sprintf("%s\n", sprintf("%u", $answer));
        } else {
            echo $answer . "\n";
