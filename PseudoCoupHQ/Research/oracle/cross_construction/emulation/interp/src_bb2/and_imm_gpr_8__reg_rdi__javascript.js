...
// one named local per gate, over the term of and_imm_gpr_8__reg_rdi__javascript.
// The term's layer-5 text, LITERAL:
//   Concat(Extract(63, 8, v1), ~(~Extract(7, 0, v0) | ~Extract(7, 0, v1)))
function emu_and_imm_gpr_8__reg_rdi__javascript(a, b) {
    ...
    const w48 = cat(w47, x1_15, 1);
    const w49 = cat(w48, x1_14, 1);
    const w50 = cat(w49, x1_13, 1);
    const w51 = cat(w50, x1_12, 1);
    const w52 = cat(w51, x1_11, 1);
    const w53 = cat(w52, x1_10, 1);
    const w54 = cat(w53, x1_9, 1);
    const w55 = cat(w54, x1_8, 1);
...
for (const line of lines) {
    ...
    try {
        out.push(emu_and_imm_gpr_8__reg_rdi__javascript(...values).toString());
    } catch (problem) {
