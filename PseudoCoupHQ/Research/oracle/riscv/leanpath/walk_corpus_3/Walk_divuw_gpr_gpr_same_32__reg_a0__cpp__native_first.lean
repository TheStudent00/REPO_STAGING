...
theorem meaning_divuw_gpr_gpr_same_32__reg_a0__cpp__native_first (s : St) (a : BitVec 64) (d : BitVec 64) (h10 : s.regs.get? .x10 = some a) (h13 : s.regs.get? .x13 = some d) (hk0 : ∀ {α : Type} (x0 : α) (s : St), (plat_term_write x0) s = EStateM.Result.ok () s) (hk1 : ∀ (x0 : Arch.pa) (x1 : Nat) (s : St), (load_reservation x0 x1) s = EStateM.Result.ok () s) (hk2 : ∀ (x0 : Unit) (s : St), (cancel_reservation x0) s = EStateM.Result.ok () s) :
    runsTo (walk [LeanIM.instruction.RTYPE (LeanIM.regidx.Regidx 0x0a#5, LeanIM.regidx.Regidx 0x00#5, LeanIM.regidx.Regidx 0x0a#5, LeanIM.rop.SLTU), LeanIM.instruction.C_ADDI (0x3f#6, LeanIM.regidx.Regidx 0x0a#5), LeanIM.instruction.ITYPE (0x001#12, LeanIM.regidx.Regidx 0x0a#5, LeanIM.regidx.Regidx 0x0a#5, LeanIM.iop.ORI)]) s
      ((pure_ITYPE ((pure_ITYPE ((pure_RTYPE (zero_reg) (a) (LeanIM.rop.SLTU))) (((sign_extend (m := 12) (0x3f#6)))) (ADDI))) (0x001#12) (LeanIM.iop.ORI))) := by
  simp (config := {decide := true}) [runsTo, walk, step, execute, strip_C_ADDI, strip_RTYPE, strip_ITYPE, alias_C_ADDI,
