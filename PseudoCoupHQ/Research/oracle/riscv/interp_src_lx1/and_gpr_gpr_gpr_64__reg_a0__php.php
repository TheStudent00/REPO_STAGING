...
// InterpRenderer from the layer-4 term of and_gpr_gpr_gpr_64__reg_a0__php.
// The term's layer-5 text, LITERAL:
//   ~(~v0 | ~v1)
function emu_and_gpr_gpr_gpr_64__reg_a0__php($a, $b) {
...
while (($line = fgets($handle)) !== false) {
    ...
    try {
        $answer = emu_and_gpr_gpr_gpr_64__reg_a0__php(...$ints);
        if ($answer < 0) {
            echo sprintf("%s\n", sprintf("%u", $answer));
        } else {
            echo $answer . "\n";
