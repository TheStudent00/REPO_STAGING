...
// InterpRenderer from the layer-4 term of seto_gpr_one_8__reg_rdi__dart.
...
int emu_seto_gpr_one_8__reg_rdi__dart(int a, int b) {
  return m(cat(0, ((eq(ext(add(cat(ext(a, 7, 7), a, 8), cat(ext(b, 7, 7), b, 8), 9), 7, 7), ext(add(cat(ext(a, 7, 7), a, 8), cat(ext(b, 7, 7), b, 8), 9), 8, 8), 1)) ? (0) : (1)), 8), 64);
}
  ...
  while (line != null) {
    ...
    if (text.isNotEmpty) {
      final List<int> values = text
          ...
          .toList();
      try {
        final int answer = emu_seto_gpr_one_8__reg_rdi__dart(values[0], values[1]);
