#!/usr/bin/env python3
"""census48.py -- THE DRIVER for task 48.
...
REGEN_STRIDE = 1
STATE_PATH = os.path.join(HERE, "layer4_state.json")
REGEN_STORE = os.path.join(HERE, "layer4_regen_store")
...
def run_original(state):
    for lang in LANGS:
        ...
        json.dump(out, open(os.path.join(
            HERE, "layer4_terms_%s.json" % lang), "w"),
...
def run_interp(state):
    ...
    json.dump(out, open(os.path.join(HERE, "layer4_interp.json"), "w"),
              indent=1, sort_keys=True)
