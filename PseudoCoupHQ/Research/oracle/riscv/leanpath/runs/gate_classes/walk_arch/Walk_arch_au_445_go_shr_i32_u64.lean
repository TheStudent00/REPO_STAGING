...
theorem meaning_arch_au_445_go_shr_i32_u64 (s : St) (a : BitVec 64) (b : BitVec 64) (d : BitVec 64) (h10 : s.regs.get? .x10 = some a) (h11 : s.regs.get? .x11 = some b) (h13 : s.regs.get? .x13 = some d) (hk0 : ∀ {α : Type} (x0 : α) (s : St), (plat_term_write x0) s = EStateM.Result.ok () s) (hk1 : ∀ (x0 : Arch.pa) (x1 : Nat) (s : St), (load_reservation x0 x1) s = EStateM.Result.ok () s) (hk2 : ∀ (x0 : Unit) (s : St), (cancel_reservation x0) s = EStateM.Result.ok () s) :
    runsTo (walk [LeanIM.instruction.ITYPE (0x020#12, LeanIM.regidx.Regidx 0x0b#5, LeanIM.regidx.Regidx 0x05#5, LeanIM.iop.SLTIU), LeanIM.instruction.C_ADDI (0x3f#6, LeanIM.regidx.Regidx 0x05#5), LeanIM.instruction.RTYPE (LeanIM.regidx.Regidx 0x05#5, LeanIM.regidx.Regidx 0x0b#5, LeanIM.regidx.Regidx 0x05#5, LeanIM.rop.OR), LeanIM.instruction.RTYPEW (LeanIM.regidx.Regidx 0x05#5, LeanIM.regidx.Regidx 0x0a#5, LeanIM.regidx.Regidx 0x0a#5, LeanIM.ropw.SRAW)]) s
      ((pure_RTYPEW (a) ((pure_RTYPE (b) ((pure_ITYPE ((pure_ITYPE (b) (0x020#12) (LeanIM.iop.SLTIU))) (((sign_extend (m := 12) (0x3f#6)))) (ADDI))) (LeanIM.rop.OR))) (LeanIM.ropw.SRAW))) := by
  simp (config := {decide := true}) [runsTo, walk, step, execute, strip_C_ADDI, strip_ITYPE, strip_RTYPE, strip_RTYPEW, alias_C_ADDI,
