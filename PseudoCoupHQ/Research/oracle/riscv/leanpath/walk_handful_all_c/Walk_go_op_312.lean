...
theorem meaning_go_op_312 (s : St) (a : BitVec 64) (b : BitVec 64) (h10 : s.regs.get? .x10 = some a) (h11 : s.regs.get? .x11 = some b) :
    runsTo (walk [LeanIM.instruction.C_ADD (LeanIM.regidx.Regidx 0x0a#5, LeanIM.regidx.Regidx 0x0b#5)]) s
      ((pure_RTYPE (a) (b) (ADD))) := by
  simp (config := {decide := true}) only [runsTo, walk, step, execute, strip_C_ADD strip_RTYPE,
