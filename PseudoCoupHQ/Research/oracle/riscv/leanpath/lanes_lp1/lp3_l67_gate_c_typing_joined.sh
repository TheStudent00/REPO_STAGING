#!/bin/bash
# lp3_l67_gate_c_typing_joined.sh -- THE GATE before any
...
# l62: the gate again WITH THE WIDTH RULE (log 283 §3): every operator_for entry carries
...
# l67: the gate again after the typing fix (Lean prints a long #check result over several
# lines; the one-line regex dropped every such candidate: the shifts, the multiplies, the
# 32-bit forms). Expected: shift keys appear; more than 3 keys from the dozen.
set -uo pipefail
