...
theorem meaning_emul_au_282_c_div_i32_bool (s : St) (a : BitVec 64) (h10 : s.regs.get? .x10 = some a) (hk0 : ∀ {α : Type} (x0 : α) (s : St), (plat_term_write x0) s = EStateM.Result.ok () s) (hk1 : ∀ (x0 : Arch.pa) (x1 : Nat) (s : St), (load_reservation x0 x1) s = EStateM.Result.ok () s) (hk2 : ∀ (x0 : Unit) (s : St), (cancel_reservation x0) s = EStateM.Result.ok () s) :
    runsTo (walk [LeanIM.instruction.C_ZEXT_W (LeanIM.cregidx.Cregidx 0x2#3)]) s
      ((pure_ZBA_RTYPEUW (a) (zero_reg) (0b00#2))) := by
  simp (config := {decide := true}) [runsTo, walk, step, execute, strip_C_ZEXT_W, strip_ZBA_RTYPEUW, alias_C_ZEXT_W,
