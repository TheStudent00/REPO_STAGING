...
theorem meaning_add_gpr_gpr_same_64__reg_a0__cpp__native_first (s : St) (a : BitVec 64) (h10 : s.regs.get? .x10 = some a) (hk0 : ∀ {α : Type} (x0 : α) (s : St), (plat_term_write x0) s = EStateM.Result.ok () s) (hk1 : ∀ (x0 : Arch.pa) (x1 : Nat) (s : St), (load_reservation x0 x1) s = EStateM.Result.ok () s) (hk2 : ∀ (x0 : Unit) (s : St), (cancel_reservation x0) s = EStateM.Result.ok () s) :
    runsTo (walk [LeanIM.instruction.C_SLLI (0x01#6, LeanIM.regidx.Regidx 0x0a#5)]) s
      ((pure_SHIFTIOP (a) ((0x01#6)) (SLLI))) := by
  simp (config := {decide := true}) [runsTo, walk, step, execute, strip_C_SLLI, strip_SHIFTIOP, alias_C_SLLI,
