...
// InterpRenderer from the layer-4 term of cmovne_gpr_gpr_64__reg_rdi__javascript.
// The term's layer-5 text, LITERAL:
//   If(~(~v0 | ~v1) == 0, v2, v3)
function emu_cmovne_gpr_gpr_64__reg_rdi__javascript(a, b, c, d) {
    return m(((eq(bnot(bor(bnot(b, 64), bnot(a, 64), 64), 64), 0n, 64)) ? (d) : (c)), 64);
}
...
for (const line of lines) {
    ...
    try {
        out.push(emu_cmovne_gpr_gpr_64__reg_rdi__javascript(...values).toString());
    } catch (problem) {
