...
// InterpRenderer from the layer-4 term of setne_gpr_one_8__reg_rdi__php.
// The term's layer-5 text, LITERAL:
//   Concat(0, If(~(~Extract(7, 0, v0) | ~Extract(7, 0, v1)) == 0, 0, 1))
function emu_setne_gpr_one_8__reg_rdi__php($a, $b) {
    return ex_m(ex_cat(0, ((ex_eq(ex_bnot(ex_bor(ex_bnot($a, 8), ex_bnot($b, 8), 8), 8), 0, 8)) ? (0) : (1)), 8), 64);
}
...
while (($line = fgets($handle)) !== false) {
    ...
    try {
        $answer = emu_setne_gpr_one_8__reg_rdi__php(...$ints);
        if ($answer < 0) {
            echo sprintf("%s\n", sprintf("%u", $answer));
        } else {
            echo $answer . "\n";
