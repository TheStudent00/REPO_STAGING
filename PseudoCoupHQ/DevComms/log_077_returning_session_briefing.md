...
moved from `SandboxDesign` to `PUBLIC/Airlock`. This
...
2026-08-22 | "if you can migrate from SandboxDesign to Airlock for PCHQ — and the Claude skill if needed — yes please." The sandbox of record for this line is `PUBLIC/Airlock` as of 2026-08-22 | log_060
...
`PUBLIC/Airlock/DevComms/log_001_airlock_derivation.md`; the
move-across instructions are `PUBLIC/Airlock/MIGRATING.md`.

What changed is small and can be said in four lines:

- the repository directory: `SandboxDesign` → `PUBLIC/Airlock`
...
|---|---|---|---|
| `PUBLIC/Airlock/agent/drop/` | `/drop` | in | the session writes `x.sh` |
| `PUBLIC/Airlock/agent/status/` | `/status` | out | the daemon writes `x.sh.status` |
| `PUBLIC/Airlock/agent/logs/` | `/logs` | out | the daemon writes `<stamp>__x.sh.log` |
| `PUBLIC/Airlock/agent/out/` | `/out` | out | the lane script writes products |

Two more paths a session touches:
`PUBLIC/Airlock/agent/batch.json` (the manifest) and
`PUBLIC/Airlock/agent/drop/.done/` (where a run's script is archived
once finished).

The recipe, five steps:

1. Write `PUBLIC/Airlock/agent/drop/<lane>.sh`. A direct write is
   ...
   `python3 PUBLIC/Airlock/airlock submit <lane>.sh --batch <label> --weight <n>`,
   a validating wrapper that refuses ten named ways.
2. Poll `PUBLIC/Airlock/agent/status/<lane>.sh.status`. This path
...
4. Read the log named in the status file's `log=` field, under
   `PUBLIC/Airlock/agent/logs/`.
5. Collect anything the lane wrote to `/out`, which is
   `PUBLIC/Airlock/agent/out/` on the host. The SESSION places
...
```
python3 PUBLIC/Airlock/airlock status            # all lanes
python3 PUBLIC/Airlock/airlock status <lane>.sh  # one, renders ABORT for timeout
python3 PUBLIC/Airlock/airlock watch
python3 PUBLIC/Airlock/airlock doctor
bash    PUBLIC/Airlock/progress.sh   (-w to refresh)
bash    PUBLIC/Airlock/batch.sh <label> <lane>.sh:<weight>
bash    PUBLIC/Airlock/down.sh && bash PUBLIC/Airlock/up.sh
bash    PUBLIC/Airlock/allow.sh sync
...
|---|---|---|---|
| Airlock | `PUBLIC/Airlock/agent/drop/d77_inventory.sh` | YES, in 20.9 s | status file written, log at `agent/logs/20260829T203216Z__d77_inventory.sh.log`, exit 0 |
| Airlock | `PUBLIC/Airlock/agent/drop/d77_ts_and_pins.sh` | YES, in 11.2 s | status file written, exit 0 |
...
- **Live file:** `PUBLIC/Airlock/proxy/allowlist.txt`
- **Template:** `PUBLIC/Airlock/proxy/allowlist.txt.example`
...
1. Append the hostname to `PUBLIC/Airlock/proxy/allowlist.txt`, with
...
|---|---|---|---|
| rust, in the image | `PUBLIC/Airlock/Containerfile` | 82 | `--default-toolchain 1.96.1` |
| rust, in the old repo (not live) | `SandboxDesign/Containerfile` | 79 | `--default-toolchain 1.96.1` |
| go, by apt package and PATH | `PUBLIC/Airlock/Containerfile` | 32 | `/usr/lib/go-1.26/bin` |
| gcc / g++ / clang / lld / llvm | `PUBLIC/Airlock/Containerfile` | 21 | `gcc-15 g++-15 clang-21 lld-21 llvm-21` |
| openjdk | `PUBLIC/Airlock/Containerfile` | 26 | `openjdk-25-jdk-headless` |
| npm major | `PUBLIC/Airlock/Containerfile` | ~76 | `npm install -g npm@11`, never fatal |
| tree-sitter, grammar pin check | `PRIVATE/PseudoCoup_v6/Tools/ledgerer/tree_sitter/test_tree_sitter_base.py` | — | tree-sitter 0.26.0, tree-sitter-rust 0.24.2, tree-sitter-python 0.25.0, tree-sitter-cpp 0.23.4 (12 checks, pass) |
| tree-sitter, compiler graph | `PRIVATE/PseudoCoupHQ/Research/compiler_graph/build_graph.py` | 12 | tree-sitter == 0.26.0; also tree-sitter-go 0.25.0, Go tree commit 9f1012d9 (1.28-dev), provenance UNVERIFIED |
| tree-sitter, operator arity | `PRIVATE/PseudoCoupHQ/Research/kind_fuzz_clustering/operator_arity.py` | 1127 | cross-check note: `tree-sitter==0.26.0` |
| rustc token alphabet, transcribed | `PRIVATE/PseudoCoup_v5/Research/differential_alphabet.py` | — | 53 rustc `TokenKind` punctuation names, fetch-dated 2026-08-12, **no refresh path** |
...
`PRIVATE/PseudoCoupHQ/Research/` had two folders and now has eight.
...
- `PRIVATE/PseudoCoupHQ/hq.sh` — the planning-tree check;
  `bash PRIVATE/PseudoCoupHQ/hq.sh check` reports 0 errors and is the
  gate every housekeeping pass runs before and after.
- `PRIVATE/PseudoCoupHQ/Research/kind_fuzz_clustering/run_all_languages.sh`
...
- `PUBLIC/Airlock/DevComms/toolchain_skill_airlock_patch.md` — the
...
- `PUBLIC/Airlock/DevComms/toolchain_SKILL_updated.md` — a complete
...
The same sitting's probe into `PUBLIC/Airlock/agent/drop/` was
...
- `PUBLIC/Airlock/agent/drop/.done/…d77_inventory.sh` and
  `…d77_ts_and_pins.sh` — the two probes, archived by the daemon.
- `PUBLIC/Airlock/agent/logs/20260829T203216Z__d77_inventory.sh.log`
  — the full language inventory, verbatim.
- `PUBLIC/Airlock/agent/logs/20260829T203340Z__d77_ts_and_pins.sh.log`
