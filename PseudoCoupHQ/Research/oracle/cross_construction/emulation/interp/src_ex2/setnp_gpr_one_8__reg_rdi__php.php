...
// InterpRenderer from the layer-4 term of setnp_gpr_one_8__reg_rdi__php.
...
function emu_setnp_gpr_one_8__reg_rdi__php($a, $b) {
    return ex_m(ex_cat(0, ((ex_eq(ex_ext(ex_add(ex_mul($b, 255, 8), $a, 8), 1, 1), ((ex_eq(ex_ext(ex_add(ex_mul($b, 255, 8), $a, 8), 2, 2), ((ex_eq(ex_ext(ex_add(ex_mul($b, 255, 8), $a, 8), 3, 3), ((ex_eq(ex_ext(ex_add(ex_mul($b, 255, 8), $a, 8), 4, 4), ((ex_eq(ex_ext(ex_add(ex_mul($b, 255, 8), $a, 8), 5, 5), ((ex_eq(ex_ext(ex_add(ex_mul($b, 255, 8), $a, 8), 6, 6), ((ex_eq(ex_ext(ex_add(ex_mul($b, 255, 8), $a, 8), 7, 7), ((ex_eq(ex_add(ex_ext($a, 0, 0), 1, 1), ex_ext($b, 0, 0), 1)) ? (1) : (0)), 1)) ? (1) : (0)), 1)) ? (1) : (0)), 1)) ? (1) : (0)), 1)) ? (1) : (0)), 1)) ? (1) : (0)), 1)) ? (1) : (0)), 1)) ? (0) : (1)), 8), 64);
}
...
while (($line = fgets($handle)) !== false) {
    ...
    try {
        $answer = emu_setnp_gpr_one_8__reg_rdi__php(...$ints);
        if ($answer < 0) {
            echo sprintf("%s\n", sprintf("%u", $answer));
        } else {
            echo $answer . "\n";
