...
- 2026-08-12: basis-report proposals P1–P9 ALL RULED by the owner (walk in
  `<WORKSPACE_DIR>/PseudoCoupHQ/DevComms/log_016_ingress_egress_asymmetry.md`
  ...
  `<WORKSPACE_DIR>/PseudoCoup_v5/Tools/ledgerer/ur.py` (KINDS) and the
  ur CORE the same day.
- 2026-08-13: P-a BUILT and run on rust —
  `<WORKSPACE_DIR>/PseudoCoupHQ/Research/kind_signature_clustering/pack_generator.py`
...
- 2026-08-15: ARTIFACT DIRECTORY RENAMED to match — the `Research/`
  ...
  `<WORKSPACE_DIR>/PlanPlan/framework/rename_node.py` (JOB 1 of the same
  ...
  --scan-root <WORKSPACE_DIR>/PseudoCoupHQ --scan-root
  <WORKSPACE_DIR>/PseudoCoup_v5`). The tool found the node already at its
