...
# InterpRenderer from the layer-4 term of cmove_gpr_gpr_64__reg_rdi__ruby.
# The term's layer-5 text, LITERAL:
#   If(~(~v0 | ~v1) == 0, v2, v3)
def emu_cmove_gpr_gpr_64__reg_rdi__ruby(a, b, c, d)
  m(((eq(bnot(bor(bnot(b, 64), bnot(a, 64), 64), 64), 0, 64)) ? (c) : (d)), 64)
end
...
STDIN.each_line do |line|
  ...
  begin
    puts emu_cmove_gpr_gpr_64__reg_rdi__ruby(*values)
