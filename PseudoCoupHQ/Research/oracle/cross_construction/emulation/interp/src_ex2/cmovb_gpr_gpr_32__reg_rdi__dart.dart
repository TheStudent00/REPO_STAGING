...
// InterpRenderer from the layer-4 term of cmovb_gpr_gpr_32__reg_rdi__dart.
// The term's layer-5 text, LITERAL:
//   Concat(0, If(ULE(Extract(31, 0, v0), Extract(31, 0, v1)), Extract(31, 0, v2), Extract(31, 0, v3)))
int emu_cmovb_gpr_gpr_32__reg_rdi__dart(int a, int b, int c, int d) {
  return m(cat(0, ((ule(b, a, 32)) ? (d) : (c)), 32), 64);
}
  ...
  while (line != null) {
    ...
    if (text.isNotEmpty) {
      final List<int> values = text
          ...
          .toList();
      try {
        final int answer = emu_cmovb_gpr_gpr_32__reg_rdi__dart(values[0], values[1], values[2], values[3]);
