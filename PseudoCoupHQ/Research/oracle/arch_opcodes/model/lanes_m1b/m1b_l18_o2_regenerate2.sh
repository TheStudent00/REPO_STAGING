#!/usr/bin/env bash
# m1b_l18_o2_regenerate2.sh -- task m1b, deliverable 7, first half,
# re-run: lane m1b_l11_o2_regenerate.sh's own before/after count
# assumed every language carries a `narrow` group list and one does
# not, so it stopped before the regeneration. The count now reads the
# key when it is there. Nothing else differs.
# deliverable 7, first half:
...
for lang in sorted(d['single_opcode_groups']):
    n = len(d['single_opcode_groups'][lang].get('narrow') or [])
    total = total + n
...
total = 0
for lang in sorted(d['single_opcode_groups']):
    total = total + len(d['single_opcode_groups'][lang].get('narrow') or [])
print('   narrow groups in all: %d' % total)
n = 0
