...
# InterpRenderer from the layer-4 term of sbb_imm_gpr_8__reg_rdi__cpython.
...
def emu_sbb_imm_gpr_8__reg_rdi__cpython(a, b, c, d):
    return m(cat(0, add(add(mul(d, 255, 8), mul(((1) if (eq(ext(add(cat(0, b, 8), cat(0, a, 8), 9), 8, 8), 1, 1)) else (0)), 255, 8), 8), c, 8), 8), 64)
...
def main():
    for line in sys.stdin:
        ...
        try:
            answer = emu_sbb_imm_gpr_8__reg_rdi__cpython(*values)
            sys.stdout.write("%d\n" % answer)
