/* task o7 emulation -- rendered by emulate.py Renderer from the layer-4 term of shld_cl_gpr_gpr_64__reg_rdi__c.  The term's layer-5 text, LITERAL:
   Extract(127, 64, Concat(v0, v1) << Concat(0, Extract(5, 0, v2))) */
#include <stdint.h>

uint64_t
emu_shld_cl_gpr_gpr_64__reg_rdi__c(uint64_t a, uint64_t b, uint8_t c)
{
