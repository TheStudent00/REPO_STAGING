...
// one named local per gate, over the term of cmp_imm_gpr_32__flags__dart.
// The term's layer-5 text, LITERAL:
//   Concat(Extract(31, 0, v0), Extract(31, 0, v1))
int emu_cmp_imm_gpr_32__flags__dart(int a, int b) {
  int x1_0 = ext(b, 0, 0);
  int x1_1 = ext(b, 1, 1);
  int x1_2 = ext(b, 2, 2);
  int x1_3 = ext(b, 3, 3);
  ...
  while (line != null) {
    ...
    if (text.isNotEmpty) {
      final List<int> values = text
          ...
          .toList();
      try {
        final int answer = emu_cmp_imm_gpr_32__flags__dart(values[0], values[1]);
