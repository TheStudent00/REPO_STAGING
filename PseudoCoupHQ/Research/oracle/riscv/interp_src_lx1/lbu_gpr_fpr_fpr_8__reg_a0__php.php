...
// InterpRenderer from the layer-4 term of lbu_gpr_fpr_fpr_8__reg_a0__php.
// The term's layer-5 text, LITERAL:
//   Concat(0, Extract(7, 0, v0))
function emu_lbu_gpr_fpr_fpr_8__reg_a0__php($a) {
...
while (($line = fgets($handle)) !== false) {
    ...
    try {
        $answer = emu_lbu_gpr_fpr_fpr_8__reg_a0__php(...$ints);
        if ($answer < 0) {
            echo sprintf("%s\n", sprintf("%u", $answer));
        } else {
            echo $answer . "\n";
