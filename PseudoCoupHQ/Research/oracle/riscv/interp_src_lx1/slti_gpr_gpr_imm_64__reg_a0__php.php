...
// InterpRenderer from the layer-4 term of slti_gpr_gpr_imm_64__reg_a0__php.
// The term's layer-5 text, LITERAL:
//   If(3 <= v0, 0, 1)
function emu_slti_gpr_gpr_imm_64__reg_a0__php($a) {
    return ex_m(((ex_sle(3, $a, 64)) ? (0) : (1)), 64);
}
...
while (($line = fgets($handle)) !== false) {
    ...
    try {
        $answer = emu_slti_gpr_gpr_imm_64__reg_a0__php(...$ints);
        if ($answer < 0) {
            echo sprintf("%s\n", sprintf("%u", $answer));
        } else {
            echo $answer . "\n";
