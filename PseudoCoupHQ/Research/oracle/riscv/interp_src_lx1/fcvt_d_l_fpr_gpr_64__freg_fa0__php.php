...
// InterpRenderer from the layer-4 term of fcvt_d_l_fpr_gpr_64__freg_fa0__php.
// The term's layer-5 text, LITERAL:
//   fp.to_ieee_bv(fpToFP(RNE(), v0))
function emu_fcvt_d_l_fpr_gpr_64__freg_fa0__php($a) {
...
while (($line = fgets($handle)) !== false) {
    ...
    try {
        $answer = emu_fcvt_d_l_fpr_gpr_64__freg_fa0__php(...$ints);
        if ($answer < 0) {
            echo sprintf("%s\n", sprintf("%u", $answer));
        } else {
            echo $answer . "\n";
