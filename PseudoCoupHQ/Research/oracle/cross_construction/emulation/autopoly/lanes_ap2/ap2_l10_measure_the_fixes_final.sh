#!/usr/bin/env bash
# ap2_l10_measure_the_fixes_final.sh -- task ap2: the six fixes measured on the pairs they target, the run of record. Same as ap2_l8, after fix 1 was extended to the WIDE memory symbol lane ap2_l9 found: the reference gives a memory operand one 128-bit symbol per operand text, and 104 of the 134 runs read only its low 64 bits.
