/* task o7 emulation -- rendered by emulate.py Renderer from the layer-4 term of E01552__go_regen_341.  The term's layer-5 text, LITERAL:
   ~(~LShR(v0, Concat(0, Extract(5, 0, v1))) | ~(18446744073709551615*If(Or(Not(Extract(15, 7, v1) == 0), ULE(64, Extract(6, 0, v1))), 0, 1))) */
#include <stdint.h>

uint64_t
emu_E01552__go_regen_341(uint64_t a, uint16_t b)
{
