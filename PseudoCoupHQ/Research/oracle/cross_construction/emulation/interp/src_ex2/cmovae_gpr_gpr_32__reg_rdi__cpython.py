...
# InterpRenderer from the layer-4 term of cmovae_gpr_gpr_32__reg_rdi__cpython.
# The term's layer-5 text, LITERAL:
#   Concat(0, If(ULE(Extract(31, 0, v0), Extract(31, 0, v1)), Extract(31, 0, v2), Extract(31, 0, v3)))
def emu_cmovae_gpr_gpr_32__reg_rdi__cpython(a, b, c, d):
    return m(cat(0, ((c) if (ule(b, a, 32)) else (d)), 32), 64)
...
def main():
    for line in sys.stdin:
        ...
        try:
            answer = emu_cmovae_gpr_gpr_32__reg_rdi__cpython(*values)
            sys.stdout.write("%d\n" % answer)
