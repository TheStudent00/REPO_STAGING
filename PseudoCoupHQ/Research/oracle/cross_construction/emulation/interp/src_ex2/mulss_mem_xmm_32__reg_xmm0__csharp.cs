...
public static class Emu {
    ...
    // InterpRenderer from the layer-4 term of mulss_mem_xmm_32__reg_xmm0__csharp.
    // The term's layer-5 text, LITERAL:
    //   fp.to_ieee_bv(fpToFP(Extract(31, 0, v0)) * fpToFP(Extract(31, 0, v1)))
    public static long emu_mulss_mem_xmm_32__reg_xmm0__csharp(long a, long b) {
        return m(fmul(b, a, 32), 32);
    }
    ...
    public static void Main() {
        ...
        while (line != null) {
            ...
            if (text.Length != 0) {
                ...
                try {
                    answers.Append(((ulong) emu_mulss_mem_xmm_32__reg_xmm0__csharp(values[0], values[1])).ToString());
                    answers.Append("\n");
