...
// one named local per gate, over the term of sub_gpr_gpr_8__reg_rdi__dart.
// The term's layer-5 text, LITERAL:
//   Concat(Extract(63, 8, v0), Extract(7, 0, v1)*255 + Extract(7, 0, v0))
int emu_sub_gpr_gpr_8__reg_rdi__dart(int a, int b) {
  int x0_0 = ext(a, 0, 0);
  int x1_0 = ext(b, 0, 0);
  int x0_1 = ext(a, 1, 1);
  int x1_1 = ext(b, 1, 1);
  ...
  while (line != null) {
    ...
    if (text.isNotEmpty) {
      final List<int> values = text
          ...
          .toList();
      try {
        final int answer = emu_sub_gpr_gpr_8__reg_rdi__dart(values[0], values[1]);
