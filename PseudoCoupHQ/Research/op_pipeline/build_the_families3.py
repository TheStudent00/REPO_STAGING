#!/usr/bin/env python3
"""build_the_families3.py -- TASK 54, the families over THE POOL.
...
What changes here is only WHAT THE CLASSES ARE.  TASK 49 ran the rule
over `the_pool2.json` -- the 30,436 units TASK 47 proved over canon37.
It now runs over `the_pool3.json` -- the same 30,436 units as TASK 52
proved them over canon38, merged on ruling 1's three grounds: the
layer-5 normalized text of a PROVED term, the layer-3 wrapped text,
and proved edges.  `the_families2.json` stays on disk, byte for byte,
as the superseded record.

This file was derived from `build_the_families2.py` by name
substitution only; `build_the_families2.py` was not edited.
...
import build_the_pool3 as POOL                          # noqa: E402
...
def main():
    ...
    pool = json.load(open(os.path.join(HERE, "the_pool3.json")))
    records = []
    ...
    meta = {
        ...
        "source_table": "the_pool3.json -- one population, every "
                        "language in the pool",
        "supersedes_as_an_object": "the_families2.json, which ran the "
                                   "same rule over the_pool2.json.  It "
    ...
    path = os.path.join(HERE, "the_families3.json")
    with open(path, "w") as fh:
        json.dump(out, fh, indent=1, sort_keys=True)
    log("-- wrote the_families3.json")
