...
// InterpRenderer from the layer-4 term of sub_gpr_gpr_gpr_64__reg_a0__php.
// The term's layer-5 text, LITERAL:
//   v0*18446744073709551615 + v1
function emu_sub_gpr_gpr_gpr_64__reg_a0__php($a, $b) {
    return ex_m(ex_add(ex_mul($b, -1, 64), $a, 64), 64);
}
...
while (($line = fgets($handle)) !== false) {
    ...
    try {
        $answer = emu_sub_gpr_gpr_gpr_64__reg_a0__php(...$ints);
        if ($answer < 0) {
            echo sprintf("%s\n", sprintf("%u", $answer));
        } else {
            echo $answer . "\n";
