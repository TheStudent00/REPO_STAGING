#!/usr/bin/env python3
"""autopoly2.py -- task ap2: AutoPoly's loop, second pass, after the
measured outer set.
...
CELLS = os.path.join(HERE, "autopoly2_cells.json")
RUNS = os.path.join(HERE, "autopoly2_runs.jsonl")
AGGREGATE = os.path.join(HERE, "autopoly2.json")
REPORT = os.path.join(HERE, "autopoly2.md")
SRC_DIR = os.path.join(HERE, "src2")
PRIMITIVE = os.path.join(HERE, "autopoly2_primitive.json")
SPELLINGS = os.path.join(HERE, "autopoly2_spellings.json")
...
def use_task_ap2():
    ...
    memory bound, and at task g1c's route WITH TASK ap2'S SIX FIXES.
    ...
    lookup).  `handful.use_task_ap2` answers all three the way `g1c`
    answers them, so the ROUTE is task g1b's closer's, unchanged; what
    differs is the six mechanical causes task ap1 counted, each fixed
    in the layer that owns it and none of them gated on a task name
    (`handful.use_task_ap2`'s own docstring says why).

    Every path `handful.py` writes through is repointed into this
    folder, and every one of them is an `autopoly2_*` name, so no lane
    of this task can write task ap1's products."""
    H.use_task_ap2()
    H.RESULTS = AGGREGATE
    H.REPORT = REPORT
    H.SRC_DIR = SRC_DIR
...
def change_command():
    """THE CHANGE TABLE: task ap1's loop beside this one, cause by cause
    and cell by cell.

    Read off the two stores, run for run, joined on the (cell, target)
    pair.  A pair task ap1 never ran -- the six cells whose `key_width`
    was null crashed its driver, and it recorded them by cause, so
    every pair is on both stores -- would show as `not run in ap1`."""
    ...
    theirs = {}
    for run in read_runs(AP1_RUNS):
        theirs[key_of(the_same_cell(cells, run), run["lang"])] = run
    say("runs on task ap1's store: %d" % len(theirs))
    say("runs on task ap2's store: %d" % len(mine))
    ...
    say("Table C1 -- every cause of task ap1's loop, what it counted "
        "then, and what those same runs say now.")
    say("")
    say("| task ap1's cause | ap1 runs | ap2 runs, same cause | where "
        "the rest went |")
    say("|---|---|---|---|")
    ...
    # few there are.
    say("Table C2 -- runs task ap1 proved that task ap2 does not.")
    say("")
    say("| cell | lang | ap2's cause |")
    say("|---|---|---|")
    regressed = 0
    for key in sorted(theirs):
    ...
    say("")
    say("runs task ap1 proved and task ap2 does not: %d" % regressed)
    say("")
    ...
    say("")
    say("| cell | lang | ap1's cause | the region the counterexample "
        "names |")
    say("|---|---|---|---|")
    to_sat = 0
...
def the_same_cell(cells, run):
    """the triple THIS loop's outer set spells for the cell task ap1's
    run names.
...
def measure_command(wanted):
    ...
    `wanted` is a substring of the cause task ap1 recorded.  This
    command reads task ap1's own store, takes EXACTLY the (cell,
    target) pairs whose cause contains it, runs each one again through
    the driver as task ap2 leaves it, and prints the cause before and
    the cause after, pair by pair and summed.

    NOTHING IS WRITTEN.  The runs go nowhere near
    `autopoly2_runs.jsonl`; this is a measurement of a fix and not a
    ...
    for run in read_runs(AP1_RUNS):
        ...
        if cause is None or wanted not in cause:
            continue
        before.append(run)
    say("task ap1's own runs carrying a cause holding %r: %d"
        % (wanted, len(before)))
    say("")
    counted = {}
    say("| cell | lang | ap1's cause | ap2's cause |")
    say("|---|---|---|---|")
    index = 0
    for run in before:
        index = index + 1
...
def write_head(lines, document):
    meta = document["meta"]
    lines.append("# autopoly2.md -- task ap2: AutoPoly's loop, second pass")
    lines.append("")
