/* task o7 emulation -- rendered by emulate.py Renderer from the layer-4 term of E01631__swift_regen_475.  The term's layer-5 text, LITERAL:
   ~(If(Extract(7, 0, v0) <= 0, 255, 254) | If(ULE(Extract(15, 0, v0), Extract(15, 0, v1)), 255, 254)) */
#include <stdint.h>

uint8_t
emu_E01631__swift_regen_475(uint16_t a, uint16_t b, uint64_t c)
{
