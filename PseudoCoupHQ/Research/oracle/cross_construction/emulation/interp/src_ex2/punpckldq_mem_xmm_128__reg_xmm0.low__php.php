...
// InterpRenderer from the layer-4 term of punpckldq_mem_xmm_128__reg_xmm0.low__php.
// The term's layer-5 text, LITERAL:
//   Concat(Extract(31, 0, v0), Extract(31, 0, v1))
function emu_punpckldq_mem_xmm_128__reg_xmm0.low__php($a, $b) {
...
while (($line = fgets($handle)) !== false) {
    ...
    try {
        $answer = emu_punpckldq_mem_xmm_128__reg_xmm0.low__php(...$ints);
        if ($answer < 0) {
            echo sprintf("%s\n", sprintf("%u", $answer));
        } else {
            echo $answer . "\n";
