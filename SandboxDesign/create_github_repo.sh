#!/usr/bin/env bash
# One-time setup: create the PRIVATE GitHub repo for SandboxDesign
...
if ! git rev-parse HEAD >/dev/null 2>&1; then
    git add -A
    git commit -m "SandboxDesign founding: host scripts, toolchain probe, DevComms"
...
if git remote get-url origin >/dev/null 2>&1; then
    ...
    gh repo create SandboxDesign --private --source . --remote origin --push
