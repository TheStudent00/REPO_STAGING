...
// one named local per gate, over the term of mov_imm_gpr_16__reg_rdi__php.
// The term's layer-5 text, LITERAL:
//   Concat(Extract(63, 16, v0), Extract(15, 0, v1))
function emu_mov_imm_gpr_16__reg_rdi__php($a, $b) {
    $x1_0 = ex_ext($b, 0, 0);
    $x1_1 = ex_ext($b, 1, 1);
    $x1_2 = ex_ext($b, 2, 2);
    $x1_3 = ex_ext($b, 3, 3);
...
while (($line = fgets($handle)) !== false) {
    ...
    try {
        $answer = emu_mov_imm_gpr_16__reg_rdi__php(...$ints);
        if ($answer < 0) {
            echo sprintf("%s\n", sprintf("%u", $answer));
        } else {
            echo $answer . "\n";
