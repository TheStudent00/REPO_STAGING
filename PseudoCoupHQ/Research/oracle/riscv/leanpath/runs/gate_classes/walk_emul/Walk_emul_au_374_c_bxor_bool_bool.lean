...
theorem meaning_emul_au_374_c_bxor_bool_bool (s : St) (a : BitVec 64) (b : BitVec 64) (h10 : s.regs.get? .x10 = some a) (h11 : s.regs.get? .x11 = some b) (hk0 : ∀ {α : Type} (x0 : α) (s : St), (plat_term_write x0) s = EStateM.Result.ok () s) (hk1 : ∀ (x0 : Arch.pa) (x1 : Nat) (s : St), (load_reservation x0 x1) s = EStateM.Result.ok () s) (hk2 : ∀ (x0 : Unit) (s : St), (cancel_reservation x0) s = EStateM.Result.ok () s) :
    runsTo (walk [LeanIM.instruction.C_XOR (LeanIM.cregidx.Cregidx 0x2#3, LeanIM.cregidx.Cregidx 0x3#3), LeanIM.instruction.C_ANDI (0x01#6, LeanIM.cregidx.Cregidx 0x2#3)]) s
      ((pure_ITYPE ((pure_RTYPE (a) (b) (XOR))) ((sign_extend (m := 12) (0x01#6))) (ANDI))) := by
  simp (config := {decide := true}) [runsTo, walk, step, execute, strip_C_XOR, strip_C_ANDI, strip_RTYPE, strip_ITYPE, alias_C_XOR, alias_C_ANDI,
