...
def cond_from_flags(cond, bit, z3mod):
    """THE CONDITION AS A READING OF THE FLAGS, which is what the
    machine does.

    Added 2026-09-10 (task ref2, correction 3, on the owner's word).  Every
    condition below is Intel SDM Vol. 1 Appendix B's own rule for that
    condition, over `CF`, `ZF`, `SF`, `OF` and `PF` and nothing else.
    `bit` is a callable from a flag's name to that flag as a z3 Bool;
    `reference.FlagReading.bit` is the one this pipeline passes, and it
    computes each flag from the SETTER'S OWN arithmetic.  A condition
    that reads a flag its setter left undefined refuses through `bit`,
    by that flag's own cause.

    WHY IT SITS BESIDE `cond_to_z3` RATHER THAN REPLACING IT.
    `cond_to_z3` is this same table for one setter family: the one
    whose result is `L - R`.  That is what a `cmp` leaves, and
    `term.RenderBack` emits a `cmp` when it renders a predicate back to
    arch text, so the operand reading is exactly right there and is
    kept.  What was wrong until today was using it for every OTHER
    setter as well -- `add` leaves `L + R` and `neg` leaves `-L`, and
    reading their zero flag as `L == R` is a different function.  The
    two agree on the subtract family, and this task proves that at all
    four widths for all fourteen conditions rather than asserting it.

      condition   suffix     the flags it reads
      CondEQ      e   z      ZF
      CondNE      ne  nz     not ZF
      CondSLT     l   nge    SF differs from OF
      CondSGE     ge  nl     SF equals OF
      CondSLE     le  ng     ZF, or SF differs from OF
      CondSGT     g   nle    not ZF, and SF equals OF
      CondULT     b   c nae  CF
      CondUGE     ae  nb nc  not CF
      CondUGT     a   nbe    not CF and not ZF
      CondULE     be  na     CF or ZF
      CondSGN     s          SF
      CondNSGN    ns         not SF
      CondPAR     p   pe     PF
      CondNPAR    np  po     not PF
    """
    z3 = z3mod
    if cond == "CondEQ":
        return bit("ZF")
    if cond == "CondNE":
        return z3.Not(bit("ZF"))
    if cond == "CondSLT":
        return bit("SF") != bit("OF")
    if cond == "CondSGE":
        return bit("SF") == bit("OF")
    if cond == "CondSLE":
        return z3.Or(bit("ZF"), bit("SF") != bit("OF"))
    if cond == "CondSGT":
        return z3.And(z3.Not(bit("ZF")), bit("SF") == bit("OF"))
    if cond == "CondULT":
        return bit("CF")
    if cond == "CondUGE":
        return z3.Not(bit("CF"))
    if cond == "CondUGT":
        return z3.And(z3.Not(bit("CF")), z3.Not(bit("ZF")))
    if cond == "CondULE":
        return z3.Or(bit("CF"), bit("ZF"))
    if cond == "CondSGN":
        return bit("SF")
    if cond == "CondNSGN":
        return z3.Not(bit("SF"))
    if cond == "CondPAR":
        return bit("PF")
    if cond == "CondNPAR":
        return z3.Not(bit("PF"))
    raise ValueError("unknown synthetic condition %r" % cond)


def cond_to_z3(cond, L, R, z3mod):
    """THE SUBTRACT FAMILY'S reading of a condition: the condition of a
    setter whose own result is `L - R`.

    NARROWED 2026-09-10 (task ref2, correction 3).  The body is
    unchanged and the 112-row proof against angr's own ccall in this
    file's header still holds; what changed is WHO CALLS IT.  Until
    today `reference.predicate_of` called it for every setter, so a
    condition read after an `add` was computed on `L - R` where the
    machine computed it on `L + R`.  The general reading is
    `cond_from_flags` above; this function stays for the setter family
    it is right for, and for `term.RenderBack`, which renders a
    predicate back to arch text AS a `cmp` and so is always in that
    family."""
    z3 = z3mod
    diff = L - R
    if cond == "CondEQ":
