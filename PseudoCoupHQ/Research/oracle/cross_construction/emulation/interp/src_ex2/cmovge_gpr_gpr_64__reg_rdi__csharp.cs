...
public static class Emu {
    ...
    // InterpRenderer from the layer-4 term of cmovge_gpr_gpr_64__reg_rdi__csharp.
    // The term's layer-5 text, LITERAL:
    //   If(v0 <= v1, v2, v3)
    public static long emu_cmovge_gpr_gpr_64__reg_rdi__csharp(long a, long b, long c, long d) {
        return m(((sle(b, a, 64)) ? (c) : (d)), 64);
    }
    ...
    public static void Main() {
        ...
        while (line != null) {
            ...
            if (text.Length != 0) {
                ...
                try {
                    answers.Append(((ulong) emu_cmovge_gpr_gpr_64__reg_rdi__csharp(values[0], values[1], values[2], values[3])).ToString());
                    answers.Append("\n");
