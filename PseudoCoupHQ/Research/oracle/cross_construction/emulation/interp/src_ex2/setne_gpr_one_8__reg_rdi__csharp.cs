...
public static class Emu {
    ...
    // InterpRenderer from the layer-4 term of setne_gpr_one_8__reg_rdi__csharp.
    // The term's layer-5 text, LITERAL:
    //   Concat(0, If(~(~Extract(7, 0, v0) | ~Extract(7, 0, v1)) == 0, 0, 1))
    public static long emu_setne_gpr_one_8__reg_rdi__csharp(long a, long b) {
        return m(cat(0L, ((eq(bnot(bor(bnot(a, 8), bnot(b, 8), 8), 8), 0L, 8)) ? (0L) : (1L)), 8), 64);
    }
    ...
    public static void Main() {
        ...
        while (line != null) {
            ...
            if (text.Length != 0) {
                ...
                try {
                    answers.Append(((ulong) emu_setne_gpr_one_8__reg_rdi__csharp(values[0], values[1])).ToString());
                    answers.Append("\n");
