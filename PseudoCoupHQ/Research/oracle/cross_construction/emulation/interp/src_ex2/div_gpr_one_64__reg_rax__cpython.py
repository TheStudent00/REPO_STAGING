...
# InterpRenderer from the layer-4 term of div_gpr_one_64__reg_rax__cpython.
# The term's layer-5 text, LITERAL:
#   Extract(63, 0, bvudiv_i(Concat(v0, v1), Concat(0, v2)))
def emu_div_gpr_one_64__reg_rax__cpython(a, b, c):
    return m(ext(udiv(cat(a, b, 64), cat(0, c, 64), 128), 63, 0), 64)
...
def main():
    for line in sys.stdin:
        ...
        try:
            answer = emu_div_gpr_one_64__reg_rax__cpython(*values)
            sys.stdout.write("%d\n" % answer)
