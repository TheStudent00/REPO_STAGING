...
theorem meaning_probe_mulhu (s : St) (a : BitVec 64) (b : BitVec 64) (h10 : s.regs.get? .x10 = some a) (h11 : s.regs.get? .x11 = some b) (hk0 : ∀ {α : Type} (x0 : α) (s : St), (plat_term_write x0) s = EStateM.Result.ok () s) (hk1 : ∀ (x0 : Arch.pa) (x1 : Nat) (s : St), (load_reservation x0 x1) s = EStateM.Result.ok () s) (hk2 : ∀ (x0 : Unit) (s : St), (cancel_reservation x0) s = EStateM.Result.ok () s) :
    runsTo (walk [LeanIM.instruction.MUL (LeanIM.regidx.Regidx 0x0a#5, LeanIM.regidx.Regidx 0x0b#5, LeanIM.regidx.Regidx 0x0a#5, { result_part := LeanIM.VectorHalf.High, signed_rs1 := LeanIM.Signedness.Unsigned, signed_rs2 := LeanIM.Signedness.Unsigned })]) s
      ((pure_MUL (b) (a) ({ result_part := LeanIM.VectorHalf.High, signed_rs1 := LeanIM.Signedness.Unsigned, signed_rs2 := LeanIM.Signedness.Unsigned }))) := by
  simp (config := {decide := true}) [runsTo, walk, step, execute, strip_MUL,
