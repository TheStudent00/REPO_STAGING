...
theorem meaning_arch_au_316_c_lor_i64_i64 (s : St) (a : BitVec 64) (b : BitVec 64) (h10 : s.regs.get? .x10 = some a) (h11 : s.regs.get? .x11 = some b) (hk0 : ∀ {α : Type} (x0 : α) (s : St), (plat_term_write x0) s = EStateM.Result.ok () s) (hk1 : ∀ (x0 : Arch.pa) (x1 : Nat) (s : St), (load_reservation x0 x1) s = EStateM.Result.ok () s) (hk2 : ∀ (x0 : Unit) (s : St), (cancel_reservation x0) s = EStateM.Result.ok () s) :
    runsTo (walk [LeanIM.instruction.C_OR (LeanIM.cregidx.Cregidx 0x2#3, LeanIM.cregidx.Cregidx 0x3#3), LeanIM.instruction.RTYPE (LeanIM.regidx.Regidx 0x0a#5, LeanIM.regidx.Regidx 0x00#5, LeanIM.regidx.Regidx 0x0a#5, LeanIM.rop.SLTU)]) s
      ((pure_RTYPE (zero_reg) ((pure_RTYPE (a) (b) (OR))) (LeanIM.rop.SLTU))) := by
