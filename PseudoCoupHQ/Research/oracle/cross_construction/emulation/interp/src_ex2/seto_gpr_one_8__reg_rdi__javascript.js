...
// InterpRenderer from the layer-4 term of seto_gpr_one_8__reg_rdi__javascript.
...
function emu_seto_gpr_one_8__reg_rdi__javascript(a, b) {
    return m(cat(0n, ((eq(ext(add(cat(ext(a, 7, 7), a, 8), cat(ext(b, 7, 7), b, 8), 9), 7, 7), ext(add(cat(ext(a, 7, 7), a, 8), cat(ext(b, 7, 7), b, 8), 9), 8, 8), 1)) ? (0n) : (1n)), 8), 64);
}
...
for (const line of lines) {
    ...
    try {
        out.push(emu_seto_gpr_one_8__reg_rdi__javascript(...values).toString());
    } catch (problem) {
