...
// InterpRenderer from the layer-4 term of rem_gpr_gpr_same_64__reg_a0__php.
// The term's layer-5 text, LITERAL:
//   If(v0 == 0, v0, If(And(v0 == 18446744073709551615, v0 == 9223372036854775808), 0, bvsrem_i(v0, v0)))
function emu_rem_gpr_gpr_same_64__reg_a0__php($a) {
    return ex_m(((ex_eq($a, 0, 64)) ? ($a) : (((((ex_eq($a, -1, 64)) && (ex_eq($a, -9223372036854775808, 64)))) ? (0) : (ex_srem($a, $a, 64))))), 64);
}
...
while (($line = fgets($handle)) !== false) {
    ...
    try {
        $answer = emu_rem_gpr_gpr_same_64__reg_a0__php(...$ints);
        if ($answer < 0) {
            echo sprintf("%s\n", sprintf("%u", $answer));
        } else {
            echo $answer . "\n";
