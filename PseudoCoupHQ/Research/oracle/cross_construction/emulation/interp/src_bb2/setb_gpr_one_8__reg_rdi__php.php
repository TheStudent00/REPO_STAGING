...
// one named local per gate, over the term of setb_gpr_one_8__reg_rdi__php.
// The term's layer-5 text, LITERAL:
//   Concat(Extract(63, 8, v0), If(ULE(Extract(7, 0, v1), Extract(7, 0, v2)), 0, 1))
function emu_setb_gpr_one_8__reg_rdi__php($a, $b, $c) {
    $x2_0 = ex_ext($b, 0, 0);
    $x1_0 = ex_ext($c, 0, 0);
    $x2_1 = ex_ext($b, 1, 1);
    $x1_1 = ex_ext($c, 1, 1);
...
while (($line = fgets($handle)) !== false) {
    ...
    try {
        $answer = emu_setb_gpr_one_8__reg_rdi__php(...$ints);
        if ($answer < 0) {
            echo sprintf("%s\n", sprintf("%u", $answer));
        } else {
            echo $answer . "\n";
