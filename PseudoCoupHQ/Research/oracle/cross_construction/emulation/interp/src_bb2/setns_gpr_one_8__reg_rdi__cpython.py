...
# one named local per gate, over the term of setns_gpr_one_8__reg_rdi__cpython.
# The term's layer-5 text, LITERAL:
#   Concat(Extract(63, 8, v2), If(Or(Extract(7, 7, v0) == 0, Extract(7, 7, v1) == 0), 1, 0))
def emu_setns_gpr_one_8__reg_rdi__cpython(a, b, c):
    x1_7 = ext(a, 7, 7)
    x0_7 = ext(b, 7, 7)
    x2_8 = ext(c, 8, 8)
    x2_9 = ext(c, 9, 9)
...
def main():
    for line in sys.stdin:
        ...
        try:
            answer = emu_setns_gpr_one_8__reg_rdi__cpython(*values)
            sys.stdout.write("%d\n" % answer)
