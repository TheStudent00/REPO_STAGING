...
// InterpRenderer from the layer-4 term of setbe_gpr_one_8__reg_rdi__php.
// The term's layer-5 text, LITERAL:
//   Concat(0, If(ULE(Extract(7, 0, v0), Extract(7, 0, v1)), 1, 0))
function emu_setbe_gpr_one_8__reg_rdi__php($a, $b) {
    return ex_m(ex_cat(0, ((ex_ule($a, $b, 8)) ? (1) : (0)), 8), 64);
}
...
while (($line = fgets($handle)) !== false) {
    ...
    try {
        $answer = emu_setbe_gpr_one_8__reg_rdi__php(...$ints);
        if ($answer < 0) {
            echo sprintf("%s\n", sprintf("%u", $answer));
        } else {
            echo $answer . "\n";
