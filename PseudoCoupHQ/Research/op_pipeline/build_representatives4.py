...
def main():
    ...
    # ---- ground (c): REAL, from proved_edges.json ----
    proved_path = os.path.join(HERE, "proved_edges.json")
    ...
    doc = {
        "meta": {
            ...
            "generator": "build_representatives4.py",
            "note": "ground (b) unchanged from build_representatives3.py "
                   "(JOB 1's constant-substituted normal_path_raw3). "
                   "ground (c) is REAL in this file for the first time: "
                   "%d PROVED edges read from proved_edges.json's own "
                   "`verdict` field (never re-derived)." % c_unions,
            ...
            "grounds": {
                ...
                "c": "cross_unit_prover.py's proved_edges.json, pairs "
                    "with verdict==PROVED only -- %d pairwise unions"
                    % c_unions,
            },
    ...
    out_path = os.path.join(HERE, "representatives4.json")
    ...
    print("")
    print("---- JOB 2: groups formed/changed by ground (c) ----")
    rep3 = json.load(open(os.path.join(HERE, "representatives3.json")))
    r3_group_of = {}
    for g in rep3["groups"]:
        for m in g["members"]:
            r3_group_of[m["unit"]] = frozenset(
                mm["unit"] for mm in g["members"])
    for a, b in c_pairs_used:
        before_a = r3_group_of.get(a, frozenset([a]))
        before_b = r3_group_of.get(b, frozenset([b]))
        after = frozenset(groups[uf.find(a)])
        print("  proved edge %s <-> %s" % (a, b))
        print("    representatives3 group of %s: %d member(s)"
              % (a, len(before_a)))
        print("    representatives3 group of %s: %d member(s)"
              % (b, len(before_b)))
        print("    representatives4 combined group: %d member(s): %s"
