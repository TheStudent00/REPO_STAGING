...
// InterpRenderer from the layer-4 term of cvtsi2sd_gpr_xmm_64__reg_xmm0__php.
// The term's layer-5 text, LITERAL:
//   fp.to_ieee_bv(fpToFP(RNE(), v0))
function emu_cvtsi2sd_gpr_xmm_64__reg_xmm0__php($a) {
    return ex_m(ex_i2f($a, 64, 64), 64);
}
...
while (($line = fgets($handle)) !== false) {
    ...
    try {
        $answer = emu_cvtsi2sd_gpr_xmm_64__reg_xmm0__php(...$ints);
        if ($answer < 0) {
            echo sprintf("%s\n", sprintf("%u", $answer));
        } else {
            echo $answer . "\n";
