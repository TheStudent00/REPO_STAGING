/* task t4 emulation -- rendered by render_general.py, one named
   intermediate per node of the term of setne_gpr_one_8__reg_rdi__c__native_first.  The term's text, LITERAL:
   Concat(Extract(63, 8, v2), If(Extract(7, 0, v0) | Extract(7, 0, v1) == 0, 0, 1)) */
#include <stdint.h>

uint64_t
emu_setne_gpr_one_8__reg_rdi__c__native_first(uint8_t a, uint8_t b, uint64_t c)
{
