...
// InterpRenderer from the layer-4 term of shrd_cl_gpr_gpr_64__reg_rdi__javascript.
// The term's layer-5 text, LITERAL:
//   Extract(63, 0, LShR(Concat(v0, v1), Concat(0, Extract(5, 0, v2))))
function emu_shrd_cl_gpr_gpr_64__reg_rdi__javascript(a, b, c) {
    return m(ext(lshr(cat(a, b, 64), cat(0n, ext(c, 5, 0), 6), 128), 63, 0), 64);
}
...
for (const line of lines) {
    ...
    try {
        out.push(emu_shrd_cl_gpr_gpr_64__reg_rdi__javascript(...values).toString());
    } catch (problem) {
