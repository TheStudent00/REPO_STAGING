#!/usr/bin/env bash
# t93 lane 2 -- THE COMPACT FORM, PROVED ON THE TWO SMALLER GRAPHS.
#
...
# the original BYTE FOR BYTE by md5 and by `cmp`.  Only a lane that
# passes this earns the right to replace anything.
#
...
for LANG in go rust; do
  ...
  say "[$LANG] expand, and the byte-for-byte diff"
  ...
  echo "   diff -q:"; diff -q graph_$LANG.json /work/t93/rebuilt_$LANG.json \
      && echo "   diff: the files are identical"
  printf '   SIZES  before %14d   after %14d   %.1f%% of the original\n' \
    "$(stat -c%s graph_$LANG.json)" \
    "$(stat -c%s /work/t93/graph_$LANG.compact.json)" \
    "$(python3 -c "print(100*$(stat -c%s /work/t93/graph_$LANG.compact.json)/$(stat -c%s graph_$LANG.json))")"
  say "[$LANG] the head read still works -- pins and counts in the first 256 KB"
  python3 - <<PY
import sys
...
print("   counts carved from the head:", counts is not None,
      (counts or {}).get("nodes"), (counts or {}).get("edges"),
      (counts or {}).get("frontier"))
print("   pins carved from the head:  ", pins is not None,
      len((pins or {}).get("directories") or []), "directories")
PY
  say "[$LANG] streaming the compact form back, a record at a time"
  python3 - <<PY
...
for section in ("nodes", "edges", "frontier"):
    ...
    same = old == new
    print("   %-9s old %7d  new %7d  every record equal: %s"
...
say "the frontier, complete, counted BOTH WAYS"
python3 - <<'PY'
...
for lang in ("go", "rust"):
    ...
    print("   %-5s identical: %s  total %d" % (lang, a == b, sum(b.values())))
PY

say "the guard, UNMODIFIED, over the two compact graphs, ONE process"
