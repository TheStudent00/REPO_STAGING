...
import Sl1Lean.Common0
import Sl1Lean.RvfiDii
...
import Sl1Lean.InterruptImplementation
import Sl1Lean.Callbacks0
import Sl1Lean.InstsBegin
import Sl1Lean.ZicfilpInsts
import Sl1Lean.InstsEnd
import Sl1Lean.StepCommon
import Sl1Lean.StepExt
import Sl1Lean.DecodeExt
import Sl1Lean.Fetch
...
/-- Type quantifiers: k_ex714132_ : Bool, _step_no : Nat, 0 ≤ _step_no -/
...
/-- Type quantifiers: k_ex714183_ : Bool, step_no : Nat, 0 ≤ step_no -/
def try_step (step_no : Nat) (exit_wait : Bool) : SailM Bool := do
  ...
  | .Step_Execute (.Retire_Success (), _) =>
    assert (hart_is_active (← readReg hart_state)) "postlude/step.sail:238.74-238.75"
  | .Step_Execute (.ExecuteAs _, _) =>
    (internal_error "postlude/step.sail" 242
  ...
  | .Step_Execute (.Enter_Wait wr, instbits) =>
    (do
      ...
      then assert (hart_is_active (← readReg hart_state)) "postlude/step.sail:252.41-252.42"
      else
        (do
          if ((get_config_print_instr ()) : Bool)
          then
