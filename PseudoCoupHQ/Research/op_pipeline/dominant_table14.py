#!/usr/bin/env python3
"""dominant_table14.py -- re-run the class table and dom_op construction
on THIS LAP'S newest converged text per unit (JOB 1/2/3 -- condition_
table4.py + canon17_float.py + canon17_render.py + canon17_behaviour_
check.py's own accepted candidates, layered on top of dominant_
table13.py's own generations and everything before it).

Byte-for-byte the SAME construction dominant_table13.py already used
(dom_ops7.py's rule via dom_ops_0branch.py/dom_ops_0branch3.py,
imported not restated; result_type_norm.py's DWARF-machine-fact result
type) -- the ONLY change is GENERATIONS, extended with this lap's ONE
new generation (canon17) ahead of the ones dominant_table13.py already
knew about. `final_text_of` already reads "newest generation whose
<gen>_text field is present", so a unit none of this lap's own work
touched keeps falling through to its canon16_text (or further) exactly
as before -- no other logic changes.

SCOPE: 0-BRANCH POPULATION ONLY, same population-integrity assumption
dominant_table13.py already asserts -- canon17.py's own driver
explicitly skips any unit whose `branch_kind` isn't "straight_line",
so this lap cannot have touched a branching unit; the SAME "refuse
silently-wrong scope" check is kept, now widened to cover canon17 too.

BASELINE (dominant_table13.json/dom_ops11.json, already on disk,
...
THE SPELLING BAN: unchanged, same as dominant_table13.py -- the class
key never includes the operator token, only type reps, result type,
and canon-text; the token is read exactly once per node as
`display_label`. check_no_spelling_keys.py is run on both outputs
before this program declares success (see the driver invocation after
main() runs).

usage:
  dominant_table14.py [--out DIR]
"""
...
GENERATIONS = ["canon17", "canon16", "canon15", "canon14", "canon13",
               "canon12", "canon11", "canon10", "canon9", "canon8",
               "canon7"]

BRANCH_CHECK_GENERATIONS = ("canon17", "canon16", "canon15", "canon14",
                             "canon13", "canon12", "canon11", "canon10",
                             "canon9", "canon8")
    ...
    for lang in LANGS:
        ...
        for n, u in canon4_doc["units"].items():
            ...
            if branching:
                ...
                # new generation -- refuse silently-wrong scope if
                # JOB 1-3 (or any earlier job) DID touch a branching
                # unit's text.
...
def main(argv):
    ...
    class_doc = {
        "meta": {
            ...
            "role_note": "this file is a GROUPING/matching artifact "
                        ...
                        "exemption (unlike dom_ops12.json beside it), "
                        "matching dominant_table13.json's own "
            ...
            "population": "canon4_units_*.json, 0-branch (erasure == "
                         ...
                         "from canon17_text/canon16_text/canon15_text/"
            ...
            "baseline": "924 classes / 135 nodes / 26 families / 23 "
                       "edgeless (dominant_table13.json/dom_ops11.json, "
                       "already on disk, produced before this lap's "
                       "JOB 1-3 float-comparison work -- the task's "
                       "own cited figures)",
        },
        "classes": len(table["rows"]),
        "rows": table["rows"],
    }
    class_path = os.path.join(outdir, "dominant_table14.json")
    ...
    dom_ops_doc = {
        "meta": {
            ...
            "note": "the dom_op (family) construction over the SAME "
                   "population/class table as dominant_table14.json "
                   "-- written to dom_ops12.json, the next number "
                   "after dom_ops11.json",
        },
    ...
    dom_ops_path = os.path.join(outdir, "dom_ops12.json")
