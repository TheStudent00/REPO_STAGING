#!/usr/bin/env bash
# t91_l10_guards.sh -- TASK 91, lane 10: THE MECHANICAL GUARD.
#
...
python3 check_no_spelling_keys.py $files $stores \
    > t91_guard_printed.txt 2>&1
rc=$?

echo "check_no_spelling_keys.py exit code: $rc"
echo ""
echo "PASS lines:  $(grep -c '^PASS' t91_guard_printed.txt)"
echo "FAIL lines:  $(grep -c '^FAIL' t91_guard_printed.txt)"
echo "exempt count (must be 0): $(grep -c exempt t91_guard_printed.txt)"
echo ""
echo "--- the first and last lines of the check's own output ---"
head -3 t91_guard_printed.txt
echo "..."
tail -3 t91_guard_printed.txt
echo ""
if grep -q '^FAIL' t91_guard_printed.txt; then
  echo "T91_SPELLING_GUARD_REFUSAL: the check failed on an artifact "\
"this task wrote; the output is refused."
  grep '^FAIL' -A 4 t91_guard_printed.txt | head -40
  exit 6
fi
if [ "$(grep -c exempt t91_guard_printed.txt)" -ne 0 ]; then
