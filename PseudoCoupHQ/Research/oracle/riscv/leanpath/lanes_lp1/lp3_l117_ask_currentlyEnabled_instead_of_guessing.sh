#!/bin/bash
# lp3_l117_ask_currentlyEnabled_instead_of_guessing.sh
#
...
#
# Writes $A/runs/float_on3 only. Budget: fifteen minutes.
set -uo pipefail
...
D = "PseudoCoupHQ/Research/oracle/riscv/leanpath/runs/float_on3"
...
for state in ("s0", "s1"):
    for ext in ("Ext_F", "Ext_D", "Ext_Zicsr"):
        body.append('#eval IO.println ("en_%s_%s\\t" ++ (match ((currentlyEnabled %s) : SailM Bool) %s with '
