...
The sandbox is `PUBLIC/Airlock`. It was derived from
...
`PUBLIC/Airlock/MIGRATING.md`. The migration is recorded in
...
- The file protocol underneath is unchanged and is still the truth:
  lanes land in `PUBLIC/Airlock/agent/drop`, run serially by the
...
- Progress across ALL runs: `python3 PUBLIC/Airlock/airlock status`
  ...
  recent finished. `bash PUBLIC/Airlock/progress.sh` still exists
...
- `python3 PUBLIC/Airlock/airlock doctor` reports podman and
...
- **Lane scripts are kept in the repo** (the owner, 2026-09-07): every lane
  ...
  archive is not the record. Never delete under `PUBLIC/Airlock/`
  or `<runs>/`.
...
- **Not yet migrated, deliberately:** the probe generators and readers
  under `PRIVATE/PseudoCoupHQ/Research/kind_fuzz_clustering/` and
