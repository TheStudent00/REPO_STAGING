...
Inputs: `~/Programming/PseudoCoupHQ/Research/kind_signature_clustering/` — `features_all.json` (counts-fixed v2 vectors for all 31,212 clusterable kinds of 411 grammars, vectors-once format), `archetypes.json` (8,329 archetypes with member lists), `grammar_inventory.json` (category tags), the v2 distance function of `cluster.py` reused exactly.
