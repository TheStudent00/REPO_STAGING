...
// InterpRenderer from the layer-4 term of test_gpr_gpr_8__flags__javascript.
// The term's layer-5 text, LITERAL:
//   Concat(~(~Extract(7, 0, v0) | ~Extract(7, 0, v1)), 0)
function emu_test_gpr_gpr_8__flags__javascript(a, b) {
    return m(cat(bnot(bor(bnot(a, 8), bnot(b, 8), 8), 8), 0n, 8), 16);
}
...
for (const line of lines) {
    ...
    try {
        out.push(emu_test_gpr_gpr_8__flags__javascript(...values).toString());
    } catch (problem) {
