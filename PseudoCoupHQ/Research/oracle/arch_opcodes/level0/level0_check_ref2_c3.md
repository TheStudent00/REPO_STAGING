...
| places that AGREE (`unsat`) | 1119 |
| places that DISAGREE (`sat`) | 24 |
| places `unknown` at the ceiling | 0 |
| places their reading leaves UNDEFINED | 197 |
| places `refused` | 439 |
| variants agreeing on every place compared | 343 |
| variants with at least one disagreement | 6 |
...
|---|---|---|---|---|---|---|---|---|
| `sbb` | 19 | 133 | 98 | 12 | 0 | 0 | 23 | 2510 |
| `sub` | 19 | 133 | 98 | 12 | 0 | 0 | 23 | 938 |
...
|---|---|---|---|
| the flag CF | 6 | `gpr_mem` | `sbb` `sub` |
| the flag ZF | 6 | `gpr_mem` | `sbb` `sub` |
| the flag SF | 6 | `gpr_mem` | `sbb` `sub` |
| the flag OF | 6 | `gpr_mem` | `sbb` `sub` |

### the flag CF -- 6 places

`sbbl_m32_r32` -- our line `sbb %esi,(%rax)`, shape `gpr_mem` at width 32, the place `flags:CF`:
...
|---|---|---|
| ours | `Extract(8, 8, Concat(0, Extract(7, 0, seed_MEM__rax_)) + 511*Concat(0, Extract(7, 0, seed_rsi)) + 511* Concat(0, If(ULE(Extract(31, 0, seed_r10), Extract(31, 0, seed_r11)), 0, 1)))` | 0x0 |
| theirs | `~Extract(32, 32, Concat(0, Extract(31, 0, seed_MEM__rax_)) + If(ULE(Extract(31, 0, seed_r10), Extract(31, 0, seed_r11)), 1 + Concat(0, ~Extract(31, 0, seed_rsi)), Concat(0, ~Extract(31, 0, seed_rsi))))` | 0x1 |

the counterexample, LITERAL: `seed_MEM__rax_` = `0x7ffbffef`, `seed_r10` = `0x0`, `seed_r11` = `0x0`, `seed_rsi` = `0x87fc0013`
...
the counterexample, LITERAL: `seed_MEM__rax_` = `0x3ffffe`, `seed_rsi` = `0x400000`

### the flag ZF -- 6 places

`sbbl_m32_r32` -- our line `sbb %esi,(%rax)`, shape `gpr_mem` at width 32, the place `flags:ZF`:

| reading | term, LITERAL (z3's printing of the simplified term, the register symbols keeping our reference's names) | its value at the counterexample |
|---|---|---|
| ours | `If(Extract(7, 0, seed_MEM__rax_) == Extract(7, 0, seed_rsi) + If(ULE(Extract(31, 0, seed_r10), Extract(31, 0, seed_r11)), 0, 1), 1, 0)` | 0x1 |
| theirs | `If(Extract(31, 0, seed_MEM__rax_) == 4294967295*If(ULE(Extract(31, 0, seed_r10), Extract(31, 0, seed_r11)), 1 + ~Extract(31, 0, seed_rsi), ~Extract(31, 0, seed_rsi)), 1, 0)` | 0x0 |

the counterexample, LITERAL: `seed_MEM__rax_` = `0x6a80c000`, `seed_r10` = `0x1`, `seed_r11` = `0x0`, `seed_rsi` = `0xe1fff`

`subl_m32_r32` -- our line `sub %esi,(%rax)`, shape `gpr_mem` at width 32, the place `flags:ZF`:

| reading | term, LITERAL (z3's printing of the simplified term, the register symbols keeping our reference's names) | its value at the counterexample |
|---|---|---|
| ours | `If(Extract(7, 0, seed_MEM__rax_) == Extract(7, 0, seed_rsi), 1, 0)` | 0x1 |
| theirs | `If(Extract(31, 0, seed_MEM__rax_) == 4294967295 + 4294967295*~Extract(31, 0, seed_rsi), 1, 0)` | 0x0 |

the counterexample, LITERAL: `seed_MEM__rax_` = `0xfdfffe53`, `seed_rsi` = `0x2000053`

### the flag SF -- 6 places

`sbbl_m32_r32` -- our line `sbb %esi,(%rax)`, shape `gpr_mem` at width 32, the place `flags:SF`:

| reading | term, LITERAL (z3's printing of the simplified term, the register symbols keeping our reference's names) | its value at the counterexample |
|---|---|---|
| ours | `Extract(7, 7, Extract(7, 0, seed_MEM__rax_) + 255*Extract(7, 0, seed_rsi) + 255*If(ULE(Extract(31, 0, seed_r10), Extract(31, 0, seed_r11)), 0, 1))` | 0x0 |
| theirs | `Extract(31, 31, Concat(0, Extract(31, 0, seed_MEM__rax_)) + If(ULE(Extract(31, 0, seed_r10), Extract(31, 0, seed_r11)), 1 + Concat(0, ~Extract(31, 0, seed_rsi)), Concat(0, ~Extract(31, 0, seed_rsi))))` | 0x1 |

the counterexample, LITERAL: `seed_MEM__rax_` = `0xbffefff7`, `seed_r10` = `0x0`, `seed_r11` = `0x0`, `seed_rsi` = `0xc01f007b`

`subl_m32_r32` -- our line `sub %esi,(%rax)`, shape `gpr_mem` at width 32, the place `flags:SF`:

| reading | term, LITERAL (z3's printing of the simplified term, the register symbols keeping our reference's names) | its value at the counterexample |
|---|---|---|
| ours | `Extract(7, 7, Extract(7, 0, seed_MEM__rax_) + 255*Extract(7, 0, seed_rsi))` | 0x0 |
| theirs | `Extract(31, 31, 1 + Concat(0, Extract(31, 0, seed_MEM__rax_)) + Concat(0, ~Extract(31, 0, seed_rsi)))` | 0x1 |

the counterexample, LITERAL: `seed_MEM__rax_` = `0x80001fbf`, `seed_rsi` = `0x80002082`

### the flag OF -- 6 places

`sbbl_m32_r32` -- our line `sbb %esi,(%rax)`, shape `gpr_mem` at width 32, the place `flags:OF`:

| reading | term, LITERAL (z3's printing of the simplified term, the register symbols keeping our reference's names) | its value at the counterexample |
|---|---|---|
| ours | `If(Extract(8, 8, Concat(Extract(7, 7, seed_MEM__rax_), Extract(7, 0, seed_MEM__rax_)) + 511*Concat(Extract(7, 7, seed_rsi), Extract(7, 0, seed_rsi)) + 511*Concat(0, If(ULE(Extract(31, 0, seed_r10), Extract(31, 0, seed_r11)), 0, 1))) == Extract(7, 7, Concat(Extract(7, 7, seed_MEM__rax_), Extract(7, 0, seed_MEM__rax_)) + 511*Concat(Extract(7, 7, seed_rsi), Extract(7, 0, seed_rsi)) + 511*Concat(0, If(ULE(Extract(31, 0, seed_r10), Extract(31, 0, seed_r11)), 0, 1))), 0, 1)` | 0x1 |
| theirs | `If(And(Extract(31, 31, seed_rsi) == ~Extract(31, 31, seed_MEM__rax_), Not(Extract(31, 31, seed_rsi) == ~Extract(31, 31, Concat(0, Extract(31, 0, seed_MEM__rax_)) + If(ULE(Extract(31, 0, seed_r10), Extract(31, 0, seed_r11)), 1 + Concat(0, ~Extract(31, 0, seed_rsi)), Concat(0, ~Extract(31, 0, seed_rsi)))))), 1, 0)` | 0x0 |

the counterexample, LITERAL: `seed_MEM__rax_` = `0xbffdfe80`, `seed_r10` = `0x1`, `seed_r11` = `0x0`, `seed_rsi` = `0xc0000360`
...
the counterexample, LITERAL: `seed_MEM__rax_` = `0x575b13ee`, `seed_rsi` = `0xa11d8305`
...
Wall clock 2.7 s; peak resident memory 63.6 MB against the stated 6144 MB ceiling and the named abort `ABORT_MEMORY_REF1`.
