#!/bin/bash
# t2_l21_the_pass_with_the_lemmas.sh -- task t2, lane 21: THE PASS,
...
if [ -f "$C/t2_construct_runs.jsonl" ]; then
    mv "$C/t2_construct_runs.jsonl" \
       "$C/t2_construct_runs.jsonl.before_the_lemmas_were_written"
    echo "  moved: $(wc -l < "$C/t2_construct_runs.jsonl.before_the_lemmas_were_written") line(s)"
else
