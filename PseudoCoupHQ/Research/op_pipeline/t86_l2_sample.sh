#!/usr/bin/env bash
# t86_l2_sample.sh -- TASK 86, lane 2: THE SAMPLE, before any full pass.
#
...
for at, one in enumerate(moments):
    D.STATE["moment_key"] = one.key
    D.STATE["window"] = D.window_start(at)
...
with open("/out/t86_sample.json", "w") as fh:
    json.dump(out, fh, indent=1, sort_keys=True)
print("wrote /out/t86_sample.json")
