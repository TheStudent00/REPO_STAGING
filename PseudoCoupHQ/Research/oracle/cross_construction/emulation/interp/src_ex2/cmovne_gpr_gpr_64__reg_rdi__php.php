...
// InterpRenderer from the layer-4 term of cmovne_gpr_gpr_64__reg_rdi__php.
// The term's layer-5 text, LITERAL:
//   If(~(~v0 | ~v1) == 0, v2, v3)
function emu_cmovne_gpr_gpr_64__reg_rdi__php($a, $b, $c, $d) {
    return ex_m(((ex_eq(ex_bnot(ex_bor(ex_bnot($b, 64), ex_bnot($a, 64), 64), 64), 0, 64)) ? ($d) : ($c)), 64);
}
...
while (($line = fgets($handle)) !== false) {
    ...
    try {
        $answer = emu_cmovne_gpr_gpr_64__reg_rdi__php(...$ints);
        if ($answer < 0) {
            echo sprintf("%s\n", sprintf("%u", $answer));
        } else {
            echo $answer . "\n";
