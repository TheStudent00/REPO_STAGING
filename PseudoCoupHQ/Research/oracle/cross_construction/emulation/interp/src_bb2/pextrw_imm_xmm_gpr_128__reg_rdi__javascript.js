...
// one named local per gate, over the term of pextrw_imm_xmm_gpr_128__reg_rdi__javascript.
// The term's layer-5 text, LITERAL:
//   Concat(0, Extract(31, 16, v0))
function emu_pextrw_imm_xmm_gpr_128__reg_rdi__javascript(a) {
    ...
    const w49 = cat(w48, x0_30, 1);
    const w50 = cat(w49, x0_29, 1);
    const w51 = cat(w50, x0_28, 1);
    const w52 = cat(w51, x0_27, 1);
    const w53 = cat(w52, x0_26, 1);
    const w54 = cat(w53, x0_25, 1);
    const w55 = cat(w54, x0_24, 1);
    const w56 = cat(w55, x0_23, 1);
    const w57 = cat(w56, x0_22, 1);
    const w58 = cat(w57, x0_21, 1);
    const w59 = cat(w58, x0_20, 1);
    const w60 = cat(w59, x0_19, 1);
    const w61 = cat(w60, x0_18, 1);
    const w62 = cat(w61, x0_17, 1);
    const w63 = cat(w62, x0_16, 1);
    return m(w63, 64);
}
...
for (const line of lines) {
    ...
    try {
        out.push(emu_pextrw_imm_xmm_gpr_128__reg_rdi__javascript(...values).toString());
    } catch (problem) {
