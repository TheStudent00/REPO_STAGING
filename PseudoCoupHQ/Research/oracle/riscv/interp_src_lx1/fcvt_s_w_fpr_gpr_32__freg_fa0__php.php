...
// InterpRenderer from the layer-4 term of fcvt_s_w_fpr_gpr_32__freg_fa0__php.
// The term's layer-5 text, LITERAL:
//   Concat(4294967295, fp.to_ieee_bv(fpToFP(RNE(), Extract(31, 0, v0))))
function emu_fcvt_s_w_fpr_gpr_32__freg_fa0__php($a) {
    return ex_m(ex_cat(4294967295, ex_i2f($a, 32, 32), 32), 64);
}
...
while (($line = fgets($handle)) !== false) {
    ...
    try {
        $answer = emu_fcvt_s_w_fpr_gpr_32__freg_fa0__php(...$ints);
        if ($answer < 0) {
            echo sprintf("%s\n", sprintf("%u", $answer));
        } else {
            echo $answer . "\n";
