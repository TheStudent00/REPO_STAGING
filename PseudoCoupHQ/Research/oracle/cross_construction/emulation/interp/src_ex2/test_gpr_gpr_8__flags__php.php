...
// InterpRenderer from the layer-4 term of test_gpr_gpr_8__flags__php.
// The term's layer-5 text, LITERAL:
//   Concat(~(~Extract(7, 0, v0) | ~Extract(7, 0, v1)), 0)
function emu_test_gpr_gpr_8__flags__php($a, $b) {
    return ex_m(ex_cat(ex_bnot(ex_bor(ex_bnot($a, 8), ex_bnot($b, 8), 8), 8), 0, 8), 16);
}
...
while (($line = fgets($handle)) !== false) {
    ...
    try {
        $answer = emu_test_gpr_gpr_8__flags__php(...$ints);
        if ($answer < 0) {
            echo sprintf("%s\n", sprintf("%u", $answer));
        } else {
            echo $answer . "\n";
