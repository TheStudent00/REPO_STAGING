/* task t4 emulation -- rendered by render_general.py, one named
   intermediate per node of the term of setg_gpr_one_8__reg_rdi__c__all_constructed.  The term's text, LITERAL:
   Concat(Extract(63, 8, v2), If(And(Not(~(~Extract(7, 0, v0) | ~Extract(7, 0, v1)) == 0), Or(Extract(7, 7, v0) == 0, Extract(7, 7, v1) == 0)), 1, 0)) */
#include <stdint.h>

uint64_t
emu_setg_gpr_one_8__reg_rdi__c__all_constructed(uint8_t a, uint8_t b, uint64_t c)
{
