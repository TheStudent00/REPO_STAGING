...
// InterpRenderer from the layer-4 term of addss_xmm_same_32__reg_xmm0__php.
// The term's layer-5 text, LITERAL:
//   fp.to_ieee_bv(fpToFP(Extract(31, 0, v0)) + fpToFP(Extract(31, 0, v0)))
function emu_addss_xmm_same_32__reg_xmm0__php($a) {
    return ex_m(ex_fadd($a, $a, 32), 32);
}
...
while (($line = fgets($handle)) !== false) {
    ...
    try {
        $answer = emu_addss_xmm_same_32__reg_xmm0__php(...$ints);
        if ($answer < 0) {
            echo sprintf("%s\n", sprintf("%u", $answer));
        } else {
            echo $answer . "\n";
