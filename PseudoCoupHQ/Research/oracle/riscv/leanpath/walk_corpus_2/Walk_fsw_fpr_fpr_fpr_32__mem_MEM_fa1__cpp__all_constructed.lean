...
theorem meaning_fsw_fpr_fpr_fpr_32__mem_MEM_fa1__cpp__all_constructed (s : St) (a : BitVec 64) (b : BitVec 64) (h10 : s.regs.get? .x10 = some a) (h11 : s.regs.get? .x11 = some b) (hk0 : ∀ {α : Type} (x0 : α) (s : St), (plat_term_write x0) s = EStateM.Result.ok () s) (hk1 : ∀ (x0 : Arch.pa) (x1 : Nat) (s : St), (load_reservation x0 x1) s = EStateM.Result.ok () s) (hk2 : ∀ (x0 : Unit) (s : St), (cancel_reservation x0) s = EStateM.Result.ok () s) :
    runsTo (walk [LeanIM.instruction.C_SRLI (0x20#6, LeanIM.cregidx.Cregidx 0x2#3), LeanIM.instruction.C_SLLI (0x20#6, LeanIM.regidx.Regidx 0x0a#5), LeanIM.instruction.ZBA_RTYPEUW (LeanIM.regidx.Regidx 0x0a#5, LeanIM.regidx.Regidx 0x0b#5, LeanIM.regidx.Regidx 0x0a#5, 0x0#2)]) s
      ((pure_ZBA_RTYPEUW (b) ((pure_SHIFTIOP ((pure_SHIFTIOP (a) ((0x20#6)) (SRLI))) ((0x20#6)) (SLLI))) (0x0#2))) := by
  simp (config := {decide := true}) [runsTo, walk, step, execute, strip_C_SRLI, strip_C_SLLI, strip_SHIFTIOP, strip_ZBA_RTYPEUW, alias_C_SRLI, alias_C_SLLI,
