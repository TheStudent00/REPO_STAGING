#!/usr/bin/env python3
"""guard48.py -- THE GUARDS for task 48, run WITHOUT EXEMPTION.
...
def artifacts():
    ...
    for lang in ("c", "cpp", "go", "rust", "swift"):
        out.append(os.path.join(HERE, "layer4_terms_%s.json" % lang))
    out.append(os.path.join(HERE, "layer4_interp.json"))
    out.append(os.path.join(HERE, "name_census2.json"))
    out.append(os.path.join(HERE, "layer4_state.json"))
...
def check_record_producers(spelled):
    ...
    for path in artifacts():
        if os.path.basename(path) in ("name_census2.json",
                                      "layer4_state.json"):
            continue
        document = json.load(open(path))
        for label, record in document.get("units", {}).items():
...
def check_census_producers(census, spelled):
    ...
    for entry in census["entries"]:
        ...
        if entry["producer_shape"] == "pair":
            members = [one.strip() for one in producer.split("+")]
...
def main():
    ...
    print("THE GENERATOR-PROVENANCE EXEMPTION IS REMOVED for every "
          "file below.")
    print("")
    census_path = os.path.join(HERE, "name_census2.json")
    ...
    print("CHECK TWO -- the generic spelling-key guard, exemption "
          "removed, with `producer` read as MACHINE FORM on the "
          "strength of CHECK ONE and CHECK ONE-B")
    CHECK.PROSE_FIELDS.add("producer")
    CHECK.PROSE_FIELDS.add("blocked_by")
