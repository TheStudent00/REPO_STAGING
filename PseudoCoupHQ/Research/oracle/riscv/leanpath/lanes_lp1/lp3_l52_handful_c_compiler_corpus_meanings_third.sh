#!/bin/bash
# lp3_l52_handful_c_compiler_corpus_meanings_third.sh -- the l49 run again, with the
# library index keyed by whole dotted names (`Vector.length`, not `Vector`), so
# the phantom `Sail.Vector` no longer enters the unfold list (l49, l50 failed every
# certificate on it).
# Plan node
