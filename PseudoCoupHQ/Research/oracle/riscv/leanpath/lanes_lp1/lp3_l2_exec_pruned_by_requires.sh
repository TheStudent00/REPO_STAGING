#!/bin/bash
# lp3_l2_exec_pruned_by_requires.sh -- the evaluable variant over
...
drop={"V_instructions","FD_instructions","Zvabd_insts","Zvabd_types","Zvbb","Zvbc","Zvkg","Zvkned","Zvksed","Zvknhab","Zvksh",
      "bfloat16_core","Zfbfmin","Zvfbfmin","Zvfbfwma"}   # the two instruction modules and everything whose `requires` pulls V or FD back in
print(" ".join(n for n in names if n not in drop))
PY
)
sail --config $CFG --list-files $MODS riscv.sail_project 2>/dev/null | tr ' ' '\n' | grep . > /work/files_pr.txt
echo "[1/$total] modules ($(echo $MODS | wc -w)); files: $(wc -l < /work/files_pr.txt); vector or float instruction files still present: $(grep -cE 'vext_.*insts|fext_insts|dext_insts|zfh_insts|zfa_insts|zvk|zvb|zvabd|bfloat|zfbf' /work/files_pr.txt)"
grep -E 'vext_.*insts|fext_insts|dext_insts|zvk|zvb|zvabd|bfloat' /work/files_pr.txt | head -5 | sed 's/^/    still: /'
EMIT() { sail --strict-var --strict-bitvector --strict-exponentials --memo-z3-path /work/smtcache8 \
  ...
  $2 --lean-import-file ../handwritten_support/RiscvExtrasExecutable.lean -o Lean_IM_pr_executable \
  $MODS riscv.sail_project; }
echo "[2/$total] a plain emit, for the closure"
OUT1=/work/emit_pr1; rm -rf $OUT1; mkdir -p $OUT1; start=$(date +%s); EMIT $OUT1 "" > /work/emit_pr1.log 2>&1; rc=$?; echo "  emit rc=$rc seconds=$(( $(date +%s) - start ))"; [ $rc -eq 0 ] || { tail -8 /work/emit_pr1.log; exit 3; }
D1=$(ls -d $OUT1/Lean_IM_pr_executable/Lean*/ | head -1)
python3 - "$D1" > /work/nc_pr.txt 2> /work/nc_pr.err <<'PY'
...
PY
cat /work/nc_pr.err; grep -qE '^(encdec_backwards|encdec_compressed_backwards|sail_model_init|init_model)$' /work/nc_pr.txt && { echo "FLAG: closure caught a decoder or init"; exit 3; }
echo "[3/$total] the emit with $(wc -l < /work/nc_pr.txt) names marked, then the build (timed)"
NC=""; while read -r n; do NC="$NC --lean-noncomputable-function $n"; done < /work/nc_pr.txt
OUT=/work/emit_pr; rm -rf $OUT; mkdir -p $OUT; start=$(date +%s); EMIT $OUT "$NC" > /work/emit_pr.log 2>&1; rc=$?; echo "  emit rc=$rc seconds=$(( $(date +%s) - start ))"; [ $rc -eq 0 ] || { tail -8 /work/emit_pr.log; exit 3; }
D=$(ls -d $OUT/Lean_IM_pr_executable/Lean*/ | head -1); echo "  InstsEnd lines: $(wc -l < $D/InstsEnd.lean); decoder lines: $(awk '/^def encdec_backwards /{s=NR} s&&/^$/{print NR-s; exit}' $D/InstsEnd.lean)"
X=/work/Lean_IM_pr_exec; rm -rf $X; cp -a $OUT/Lean_IM_pr_executable $X; mkdir -p $X/.lake; cp -a $P/.lake/packages $X/.lake/; cp $P/lake-manifest.json $X/
cd $X; start=$(date +%s); lake build > /work/lake_pr.log 2>&1; rc=$?
echo "  lake rc=$rc seconds=$(( $(date +%s) - start ))"; grep -cE '^error' /work/lake_pr.log | sed 's/^/  error lines: /'; grep -E 'Build completed|error:' /work/lake_pl.log | head -5 | cut -c1-200
...
L
timeout 900 lake env lean /work/Probe8.lean 2>&1 | cut -c1-240 | head -10
echo "[5/$total] done"
