...
// InterpRenderer from the layer-4 term of mul_gpr_one_8__reg_rax__javascript.
// The term's layer-5 text, LITERAL:
//   Concat(Extract(63, 16, v0), Concat(0, Extract(7, 0, v0))*Concat(0, Extract(7, 0, v1)))
function emu_mul_gpr_one_8__reg_rax__javascript(a, b) {
    return m(cat(ext(a, 63, 16), mul(cat(0n, ext(a, 7, 0), 8), cat(0n, b, 8), 16), 16), 64);
}
...
for (const line of lines) {
    ...
    try {
        out.push(emu_mul_gpr_one_8__reg_rax__javascript(...values).toString());
    } catch (problem) {
