#!/bin/bash
# lp3_l110_the_bridge_with_the_bool_to_bit_family.sh
#
# l109 dropped the bridged definitions from the simp set and MOVED TWO -- the
# two 64-bit equality classes. Its residuals show the bridge FIRING: `.ult` is
# in the term where Int.blt used to be. What `bv_decide` still abstracts is the
# WRAPPER around the comparison, Sail's Bool-to-bit conversion, which unfolds
# to a match it cannot see through:
#
...
#       false => 0#1
#
# `BitVec.ofBool` IS that function, natively and inside the fragment, and the
# identity holds by `rfl` -- checked on this toolchain before the generator was
# extended, not after. So the generator now reads a SECOND family out of the
# Prelude, and the same rule applies to it: a definition leaves the simp set
# with its lemma, because a set cannot both unfold and rewrite.
#
...
G1=$A/runs/gate_bridge3; mkdir -p $G1
t0=$(date +%s)
...
G0, G1 = A + "/runs/gate_classes", A + "/runs/gate_bridge3"
