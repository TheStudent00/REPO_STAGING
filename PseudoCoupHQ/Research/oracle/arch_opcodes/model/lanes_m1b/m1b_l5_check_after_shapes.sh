#!/usr/bin/env bash
# m1b_l5_check_after_shapes.sh -- task m1b: the same check as lane
# m1b_l4_check_baseline2.sh, re-run with the three x87 operand shapes
# added to `model_translate.shapes_for` and NOTHING else changed --
# `reference.py` still holds its own unmodified one-operand branch at
# this point. The tally must be what lane 4 read: 259 rows, 172
# STATED, 87 REFUSED.
#
...
mkdir -p "$HOME" /work/m1b_check_shapes/archproof/Archproof
