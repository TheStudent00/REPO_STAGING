#!/usr/bin/env bash
# ref1 lane 20 -- every transcript the report quotes, produced in one place so
...
print('R1 = %rsi = 0x%016x   R2 = %rdi = 0x%016x' % (rsi.as_long(), rdi.as_long()))
...
grep -c exempt $L0/k_to_z3.py
grep -c exempt $L0/key_map.py
grep -c exempt $L0/level0_check.py
grep -c exempt $L0/level0_check.md
python3 -c "
import json
d = json.load(open('$L0/level0_check.json'))
