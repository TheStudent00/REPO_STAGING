...
// one named local per gate, over the term of movswl_widen_gpr_gpr_32__reg_rdi__dart.
...
int emu_movswl_widen_gpr_gpr_32__reg_rdi__dart(int a) {
  int x0_0 = ext(a, 0, 0);
  int x0_1 = ext(a, 1, 1);
  int x0_2 = ext(a, 2, 2);
  int x0_3 = ext(a, 3, 3);
  ...
  int w32 = cat(w31, x0_15, 1);
  int w33 = cat(w32, x0_15, 1);
  int w34 = cat(w33, x0_15, 1);
  int w35 = cat(w34, x0_15, 1);
  int w36 = cat(w35, x0_15, 1);
  int w37 = cat(w36, x0_15, 1);
  int w38 = cat(w37, x0_15, 1);
  int w39 = cat(w38, x0_15, 1);
  int w40 = cat(w39, x0_15, 1);
  int w41 = cat(w40, x0_15, 1);
  int w42 = cat(w41, x0_15, 1);
  int w43 = cat(w42, x0_15, 1);
  int w44 = cat(w43, x0_15, 1);
  int w45 = cat(w44, x0_15, 1);
  int w46 = cat(w45, x0_15, 1);
  int w47 = cat(w46, x0_15, 1);
  ...
  while (line != null) {
    ...
    if (text.isNotEmpty) {
      final List<int> values = text
          ...
          .toList();
      try {
        final int answer = emu_movswl_widen_gpr_gpr_32__reg_rdi__dart(values[0]);
