#!/usr/bin/env python3
"""build_the_families2.py -- TASK 49, the families over THE POOL.
...
What changes here is only WHAT THE CLASSES ARE.  TASK 44 ran the rule
over `the_pool1.json` -- the 28,984 units merged on the region36
universal text.  It now runs over `the_pool2.json` -- the 30,436 units
TASK 47 proved, merged on the layer-5 normalized text, the layer-3
wrapped text and proved edges.  `the_families1.json` stays on disk,
byte for byte, as the superseded record.
...
import build_the_pool2 as POOL                          # noqa: E402
...
def main():
    ...
    pool = json.load(open(os.path.join(HERE, "the_pool2.json")))
    records = []
    ...
    meta = {
        "generator": "build_the_families2.py",
        ...
        "task": "TASK 49 -- the dom_op construction rule over THE POOL",
        ...
        "source_table": "the_pool2.json -- one population, every "
                        "language in the pool",
        "supersedes_as_an_object": "the_families1.json, which ran the "
                                   "same rule over the_pool1.json.  It "
    ...
    path = os.path.join(HERE, "the_families2.json")
    with open(path, "w") as fh:
        json.dump(out, fh, indent=1, sort_keys=True)
    log("-- wrote the_families2.json")
