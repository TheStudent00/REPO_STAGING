...
// InterpRenderer from the layer-4 term of setae_gpr_one_8__reg_rdi__dart.
// The term's layer-5 text, LITERAL:
//   Concat(0, If(Extract(8, 8, Concat(0, Extract(7, 0, v0)) + Concat(0, Extract(7, 0, v1))) == 1, 0, 1))
int emu_setae_gpr_one_8__reg_rdi__dart(int a, int b) {
  return m(cat(0, ((eq(ext(add(cat(0, a, 8), cat(0, b, 8), 9), 8, 8), 1, 1)) ? (0) : (1)), 8), 64);
}
  ...
  while (line != null) {
    ...
    if (text.isNotEmpty) {
      final List<int> values = text
          ...
          .toList();
      try {
        final int answer = emu_setae_gpr_one_8__reg_rdi__dart(values[0], values[1]);
