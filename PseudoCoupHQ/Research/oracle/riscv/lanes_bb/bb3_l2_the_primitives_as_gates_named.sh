...
for name, f in ops.items():
    for w in (8, 16, 32, 64):
        # the inputs as NAMED bits a0..a{w-1}, b0..b{w-1}, so the netlist reads
        # over them rather than over z3's internal k!N atoms
        def named(prefix):
            return z3.Concat(*[z3.If(z3.Bool("%s%d" % (prefix, i)), z3.BitVecVal(1, 1), z3.BitVecVal(0, 1)) for i in reversed(range(w))])
        a, b = named("a"), named("b")
