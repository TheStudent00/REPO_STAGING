...
theorem meaning_sltiu_gpr_gpr_imm_64__reg_a0__c__native_first (s : St) (a : BitVec 64) (h10 : s.regs.get? .x10 = some a) (hk0 : ∀ {α : Type} (x0 : α) (s : St), (plat_term_write x0) s = EStateM.Result.ok () s) (hk1 : ∀ (x0 : Arch.pa) (x1 : Nat) (s : St), (load_reservation x0 x1) s = EStateM.Result.ok () s) (hk2 : ∀ (x0 : Unit) (s : St), (cancel_reservation x0) s = EStateM.Result.ok () s) :
    runsTo (walk [LeanIM.instruction.ITYPE (0x003#12, LeanIM.regidx.Regidx 0x0a#5, LeanIM.regidx.Regidx 0x0a#5, LeanIM.iop.SLTIU)]) s
      ((pure_ITYPE (a) (0x003#12) (LeanIM.iop.SLTIU))) := by
