...
// one named local per gate, over the term of cmp_imm_gpr_8__flags__javascript.
// The term's layer-5 text, LITERAL:
//   Concat(Extract(7, 0, v0), Extract(7, 0, v1))
function emu_cmp_imm_gpr_8__flags__javascript(a, b) {
    ...
    const w0 = x0_7;
    const w1 = cat(w0, x0_6, 1);
    const w2 = cat(w1, x0_5, 1);
    const w3 = cat(w2, x0_4, 1);
    const w4 = cat(w3, x0_3, 1);
    const w5 = cat(w4, x0_2, 1);
    const w6 = cat(w5, x0_1, 1);
    const w7 = cat(w6, x0_0, 1);
    const w8 = cat(w7, x1_7, 1);
    const w9 = cat(w8, x1_6, 1);
    const w10 = cat(w9, x1_5, 1);
    const w11 = cat(w10, x1_4, 1);
    const w12 = cat(w11, x1_3, 1);
    const w13 = cat(w12, x1_2, 1);
    const w14 = cat(w13, x1_1, 1);
    const w15 = cat(w14, x1_0, 1);
    return m(w15, 16);
}
...
for (const line of lines) {
    ...
    try {
        out.push(emu_cmp_imm_gpr_8__flags__javascript(...values).toString());
    } catch (problem) {
