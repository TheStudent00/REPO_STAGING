import LeanIMZ.HexBits
import LeanIMZ.Prelude
import LeanIMZ.Errors
import LeanIMZ.Xlen
import LeanIMZ.PlatformConfig
import LeanIMZ.SysRegs
import LeanIMZ.InterruptRegs
import LeanIMZ.SysExceptions
import LeanIMZ.PmpRegs
import LeanIMZ.StateenRegs
import LeanIMZ.FdextRegs
import LeanIMZ.VextRegs
import LeanIMZ.Smcntrpmf
import LeanIMZ.Vmem
  ...
  | 0x310 =>
    (do
      ...
      then (pure (Sail.BitVec.extractLsb (← readReg mstatus) 63 32))
        ...
        (do
          let v__1134 := 0x310#12
          if ((((Sail.BitVec.extractLsb v__1134 11 4) == (0x3A#8 : (BitVec 8))) && (let idx : (BitVec 4) :=
                 (Sail.BitVec.extractLsb v__1134 3 0)
               (((BitVec.access idx 0) == 0#1) || (xlen == 32)))) : Bool)
          then
            (do
              let idx : (BitVec 4) := (Sail.BitVec.extractLsb v__1134 3 0)
              (pmpReadCfgReg (BitVec.toNatInt idx)))
          else
            (do
              if (((Sail.BitVec.extractLsb v__1134 11 4) == (0x3B#8 : (BitVec 8))) : Bool)
              then
                (do
                  let idx : (BitVec 4) := (Sail.BitVec.extractLsb v__1134 3 0)
                ...
                (do
                  if (((Sail.BitVec.extractLsb v__1134 11 4) == (0x3C#8 : (BitVec 8))) : Bool)
                  then
                    (do
                      let idx : (BitVec 4) := (Sail.BitVec.extractLsb v__1134 3 0)
                    ...
                    (do
                      if (((Sail.BitVec.extractLsb v__1134 11 4) == (0x3D#8 : (BitVec 8))) : Bool)
                      then
                        (do
                          let idx : (BitVec 4) := (Sail.BitVec.extractLsb v__1134 3 0)
                        ...
                        (do
                          if (((Sail.BitVec.extractLsb v__1134 11 4) == (0x3E#8 : (BitVec 8))) : Bool)
                          then
                            (do
                              let idx : (BitVec 4) := (Sail.BitVec.extractLsb v__1134 3 0)
    ...
    (do
      ...
      then
        ...
        (do
          ...
          | (v__1144, value) =>
            (do
              if ((((Sail.BitVec.extractLsb v__1144 11 4) == (0x3A#8 : (BitVec 8))) && (let idx : (BitVec 4) :=
                     (Sail.BitVec.extractLsb v__1144 3 0)
                   (((BitVec.access idx 0) == 0#1) || (xlen == 32)))) : Bool)
              then
                (do
                  let idx : (BitVec 4) := (Sail.BitVec.extractLsb v__1144 3 0)
                  let idx := (BitVec.toNatInt idx)
                  (pmpWriteCfgReg idx value)
                  (pure (Ok (← (pmpReadCfgReg idx)))))
              else
                (do
                  if (((Sail.BitVec.extractLsb v__1144 11 4) == (0x3B#8 : (BitVec 8))) : Bool)
                  then
                    (do
                      let idx : (BitVec 4) := (Sail.BitVec.extractLsb v__1144 3 0)
                    ...
                    (do
                      if (((Sail.BitVec.extractLsb v__1144 11 4) == (0x3C#8 : (BitVec 8))) : Bool)
                      then
                        (do
                          let idx : (BitVec 4) := (Sail.BitVec.extractLsb v__1144 3 0)
                        ...
                        (do
                          if (((Sail.BitVec.extractLsb v__1144 11 4) == (0x3D#8 : (BitVec 8))) : Bool)
                          then
                            (do
                              let idx : (BitVec 4) := (Sail.BitVec.extractLsb v__1144 3 0)
                            ...
                            (do
                              if (((Sail.BitVec.extractLsb v__1144 11 4) == (0x3E#8 : (BitVec 8))) : Bool)
                              then
                                (do
                                  let idx : (BitVec 4) := (Sail.BitVec.extractLsb v__1144 3 0)
