#!/usr/bin/env bash
# t81 lane 14 — the extension past the ORIGINAL corpus, as far as the
...
#
# WHY THIS IS LANE 14. Lane 13 ran and found TWO defects of its own,
# both recorded rather than quietly patched over:
#   1. it read `probe_manifest_<lang>.json`, which carries the ORIGINAL
#      corpus only -- 750 c rows and 1,002 cpp rows. A regenerated unit
#      is `c/regen_<n>` and its source lives in the SECOND generator's
#      manifest, `probe_manifest2_<lang>.json` (51,829 c rows). So 2,557
#      of the 2,600 selected units had no probe source and the lane
#      correctly refused to invent one.
#   2. the 43 that did join were written to a diary path built from the
#      unit id, which carries a `/`: `c__c/regen_1.txt` names a
#      subdirectory that does not exist, so `fopen` failed inside the
#      hook and every one of the 43 was recorded as "compiled, but the
#      hook produced no diary". The path now uses the stem.
# Lane 13 stays on disk as the record of both.
#
...
for language in ('c', 'cpp'):
    payload = json.load(open(os.path.join(
        PIPELINE, 'probe_manifest2_%s.json' % language)))
    ...
    print('   probe_manifest2_%s.json rows keyed by n : %d'
          % (language, len(table)))
