...
// InterpRenderer from the layer-4 term of sbb_gpr_gpr_64__reg_rdi__javascript.
// The term's layer-5 text, LITERAL:
//   If(Extract(64, 64, Concat(0, v0) + Concat(0, v1)) == 1, 1, 0)*18446744073709551615 + v2*18446744073709551615 + v3
function emu_sbb_gpr_gpr_64__reg_rdi__javascript(a, b, c, d) {
    return m(add(add(mul(((eq(ext(add(cat(0n, b, 64), cat(0n, a, 64), 65), 64, 64), 1n, 1)) ? (1n) : (0n)), 18446744073709551615n, 64), mul(d, 18446744073709551615n, 64), 64), c, 64), 64);
}
...
for (const line of lines) {
    ...
    try {
        out.push(emu_sbb_gpr_gpr_64__reg_rdi__javascript(...values).toString());
    } catch (problem) {
