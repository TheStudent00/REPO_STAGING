...
2026-09-02. Airlock is `~/Programming/Airlock`, an application several
...
(`~/Programming/Airlock/README.md`, Instances section) — LITERAL:
...
- Two rows carry more than spelling. Row 8 is call three of this page (a
  ...
  as temporary — `~/Programming/Airlock/README.md`, Configuration table,
...
LITERAL — `~/Programming/Airlock/daemon/watcher.py`, lines 30-32:
...
`~/Programming/Airlock/instances/sandbox.conf.example`).

## 2.2 What a caller gives up, in values in motion

LITERAL — `~/Programming/PseudoCoupHQ/Research/op_pipeline/trickle2.py`,
lines 51-52 and 333:
...
the `airlock` CLI in `~/Programming/Airlock` returns nothing (§4).
...
LITERAL — `~/Programming/Airlock/instance.sh`, lines 146-156:
...
LITERAL — `~/Programming/Airlock/.gitignore`, lines 28-34:
...
LITERAL — `~/Programming/Airlock/instances/sandbox.conf.example`, the
...
|---|---|
| an instance named `trickle` gets `~/Programming/Airlock/instances/trickle/agent/{drop,status,logs,out}` | the earlier fork put its tree at `~/Programming/AirlockTrickle/agent`, deliberately |
...
$ grep -n "one at a time\|serially" ~/Programming/PseudoCoupHQ/Research/op_pipeline/trickle2.py
52:time and waits.
333:    print("%d chunk(s) to run, one at a time (Airlock runs lanes serially)"
```

Sources: `~/Programming/PseudoCoupHQ/DevComms/log_138_airlock_instances_feature.md`
§6.4.1, §7, §8;
`~/Programming/PseudoCoupHQ/DevComms/log_140_open_calls_explained.md` §5-§7;
`~/Programming/Airlock/README.md` Instances and Configuration sections;
`~/Programming/Airlock/instances/sandbox.conf.example`.
