...
step "the emit: sail --lean, the invocation of log 274 §3 verbatim, timed (the sail process's peak resident is the largest-descendant figure)"
mkdir -p /work/smtcache /work/leanout
cd $SRC/model
...
peak
echo "lane lp1_l7 done at $(date -u +%FT%TZ)"
}
