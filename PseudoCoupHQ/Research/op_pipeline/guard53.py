#!/usr/bin/env python3
"""guard53.py -- THE GUARD for task 53: the UNMODIFIED
...
WHAT IS WALKED: `layer4c_terms_{c,cpp,go,rust,swift}.json`,
`layer4c_interp.json`, `layer4c_state.json`, `name_census4.json`, and
every chunk in `layer4c_regen_store/`.  No artifact this task writes
...
TRANSCRIPT = os.path.join(HERE, "guard53_transcript.txt")
SUMMARY = os.path.join(HERE, "guard53.json")
...
def task53_paths():
    ...
    out.append(os.path.join(HERE, "layer4c_state.json"))
    out.append(os.path.join(HERE, "name_census4.json"))
...
def main(argv):
    ...
    counts = run_guard(
        paths, TRANSCRIPT,
        "guard53.py -- every artifact task 53 writes, unmodified "
        "guard, ONE process.  No file declares the provenance role "
        "and nothing was added to any field set.")
    print("TASK 53: %d paths  PASS %d  FAIL %d  exempt %d  exit %d"
