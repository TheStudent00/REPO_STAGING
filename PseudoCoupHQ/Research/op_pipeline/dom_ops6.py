#!/usr/bin/env python3
"""dom_ops6.py -- the dom_op construction over dominant_table8,
the table with the asg units re-keyed on the DWARF machine-fact
result type plus the erased-form matching ground added.
...
products afterwards, so the artifact that ships is `dom_ops5.json` and
...
usage:
  dom_ops5.py [--in DIR] [--out DIR] [--every N]
"""
...
RENAME = [
    ("dom_ops.json", "dom_ops6.json"),
    ("dom_ops.md", "dom_ops6.md"),
    ("dom_ops_digest.md", "dom_ops6_digest.md"),
    ("dom_ops_digest_shape.json", "dom_ops6_digest_shape.json"),
]
...
# the combined-universe manifests, so the assignment probes are nodes too
...
def main():
    ...
    stage = os.path.join(outdir, "stage_dom_ops6")
    ...
    for lang in DO.LANGS:
        for pattern in ("probe_manifest_%s.json", "core_modes_%s.json"):
            ...
            if pattern.startswith("probe_manifest") and \
                ...
                # the combined-universe manifest, so the assignment
                # probes appear as nodes beside the plain ones
                path = staged
    ...
    doc = json.load(open(os.path.join(outdir, "dom_ops6.json")))
