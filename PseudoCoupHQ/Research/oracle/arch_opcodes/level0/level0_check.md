...
`andw_r16_imm16` -- our line `and $0x3,%di`, shape `imm_gpr` at width 16, the place `reg_rdi`:
...
|---|---|---|
| ours | `If(Extract(7, 0, seed_rdi) == 3, 1, 0)` | 0x0 |
| theirs | `If(Extract(7, 0, seed_rdi) == 255*If(ULE(Extract(31, 0, seed_r10), Extract(31, 0, seed_r11)), 253, 252), 1, 0)` | 0x1 |

the counterexample, LITERAL: `seed_r10` = `0x1`, `seed_r11` = `0x0`, `seed_rdi` = `0x4`
...
the counterexample, LITERAL: `seed_MEM__rax_` = `0x1fffffff`, `seed_rsi` = `0x2000002d`
...
the counterexample, LITERAL: `seed_MEM__rax_` = `0x7fffffff`, `seed_rsi` = `0x80000000`
...
Wall clock 4.9 s; peak resident memory 64.3 MB against the stated 6144 MB ceiling and the named abort `ABORT_MEMORY_REF1`.
