#!/bin/bash
# lp2_l3_exec_decoder_only_closure_refined.sh -- lane lp2_l2 again: the seed is the ten
# giants only (no `step`/`loop`/`fetch`/`main`, ordinary words that occur in comments and
# strings), names are matched in CODE only (comments and string literals stripped), and
# a flagged name reports what it mentions. The set of
...
python3 - "$PREV" > /work/nc_closure.txt 2> /work/nc_closure.err <<'PY'
...
for f in glob.glob(os.path.join(d,"*.lean"))+glob.glob(os.path.join(d,"..","*.lean")):
    ...
    for m in re.finditer(r"^(?:noncomputable )?def (\w+)\b(.*?)(?=^(?:noncomputable )?def |\Z)", t, re.S|re.M):
        ...
        defs[m.group(1)] += code_only(m.group(2))
marked=set(seed)
changed=True
while changed:
    changed=False
...
for b in bad:
    pat=re.compile(r"\b(" + "|".join(sorted(map(re.escape, marked - {b}))) + r")\b")
    sys.stderr.write("  %s mentions: %s\n" % (b, sorted(set(pat.findall(defs[b])))[:12]))
for n in sorted(marked): print(n)
PY
cat /work/nc_closure.err; echo "  names marked: $(wc -l < /work/nc_closure.txt)"; head -c 600 /work/nc_closure.txt | tr '\n' ' '; echo
