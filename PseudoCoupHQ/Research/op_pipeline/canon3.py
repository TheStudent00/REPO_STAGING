...
def render_control_line(rec, contract, walker):
    ...
    if canon.JUMP.match(mnem):
        ...
        target_label = labels[0] if labels else "L?"
...
def canon3_branching(lang, n, sem_rec, out, anchor, lines, walker):
    ...
    derived_flat = []
    for rec in driver.records:
        text_lines = rec["body"] + rec["ctrl_lines"]
...
def run_language(lang, indir, outdir, workdir):
    ...
    out["meta"] = {
        ...
        "form": "canon3: total canonical coverage (branching per-block "\
               "runnable derivation, temps-exhausted stack spill, "\
               "honest per-path join blocks)",
    ...
    out["units"] = rows
    name = os.path.join(outdir, "canon3_units_%s.json" % lang)
    fh = open(name, "w")
