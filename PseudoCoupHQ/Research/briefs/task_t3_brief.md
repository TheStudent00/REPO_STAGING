...
| the tier's source is not in `code_version`, so a tier change forces a full pass | `construct/*.py` and the schema lemmas' sources join `handful.code_version` (an existing field's definition, not a new field) | driver | the next pass is a DELTA: runs and seconds beside t2's 669 / 2,301 s |
