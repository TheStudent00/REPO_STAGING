...
// InterpRenderer from the layer-4 term of imul_gpr_one_8__reg_rax__php.
...
function emu_imul_gpr_one_8__reg_rax__php($a, $b) {
    return ex_m(ex_cat(ex_ext($a, 63, 16), ex_mul(ex_cat(ex_cat(ex_cat(ex_cat(ex_cat(ex_cat(ex_cat(ex_cat(ex_ext($a, 7, 7), ex_ext($a, 7, 7), 1), ex_ext($a, 7, 7), 1), ex_ext($a, 7, 7), 1), ex_ext($a, 7, 7), 1), ex_ext($a, 7, 7), 1), ex_ext($a, 7, 7), 1), ex_ext($a, 7, 7), 1), ex_ext($a, 7, 0), 8), ex_cat(ex_cat(ex_cat(ex_cat(ex_cat(ex_cat(ex_cat(ex_cat(ex_ext($b, 7, 7), ex_ext($b, 7, 7), 1), ex_ext($b, 7, 7), 1), ex_ext($b, 7, 7), 1), ex_ext($b, 7, 7), 1), ex_ext($b, 7, 7), 1), ex_ext($b, 7, 7), 1), ex_ext($b, 7, 7), 1), $b, 8), 16), 16), 64);
}
...
while (($line = fgets($handle)) !== false) {
    ...
    try {
        $answer = emu_imul_gpr_one_8__reg_rax__php(...$ints);
        if ($answer < 0) {
            echo sprintf("%s\n", sprintf("%u", $answer));
        } else {
            echo $answer . "\n";
