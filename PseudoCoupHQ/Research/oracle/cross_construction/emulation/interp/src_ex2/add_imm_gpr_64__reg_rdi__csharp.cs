...
public static class Emu {
    ...
    // InterpRenderer from the layer-4 term of add_imm_gpr_64__reg_rdi__csharp.
    // The term's layer-5 text, LITERAL:
    //   v0 + v1
    public static long emu_add_imm_gpr_64__reg_rdi__csharp(long a, long b) {
        return m(add(b, a, 64), 64);
    }
    ...
    public static void Main() {
        ...
        while (line != null) {
            ...
            if (text.Length != 0) {
                ...
                try {
                    answers.Append(((ulong) emu_add_imm_gpr_64__reg_rdi__csharp(values[0], values[1])).ToString());
                    answers.Append("\n");
