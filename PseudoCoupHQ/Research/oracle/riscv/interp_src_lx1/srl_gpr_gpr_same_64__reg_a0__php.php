...
// InterpRenderer from the layer-4 term of srl_gpr_gpr_same_64__reg_a0__php.
// The term's layer-5 text, LITERAL:
//   LShR(v0, Concat(0, Extract(5, 0, v0)))
function emu_srl_gpr_gpr_same_64__reg_a0__php($a) {
    return ex_m(ex_lshr($a, ex_cat(0, ex_ext($a, 5, 0), 6), 64), 64);
}
...
while (($line = fgets($handle)) !== false) {
    ...
    try {
        $answer = emu_srl_gpr_gpr_same_64__reg_a0__php(...$ints);
        if ($answer < 0) {
            echo sprintf("%s\n", sprintf("%u", $answer));
        } else {
            echo $answer . "\n";
