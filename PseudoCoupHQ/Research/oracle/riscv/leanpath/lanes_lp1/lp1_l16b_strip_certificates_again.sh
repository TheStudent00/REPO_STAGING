#!/bin/bash
# lp1_l16b_strip_certificates_again.sh -- lane 16 again after the fix: the certificate file mirrors the emitted file's own preamble (its twenty-odd `open` lines and namespaces), so names resolve as they do in the clause; and the pure compressed clauses are aliases.
...
OUT=PseudoCoupHQ/Research/oracle/riscv/leanpath/strip_6266b40c_8eb1fb6b_IMZ_b
...
python3 - <<'PY'
import json
d=json.load(open("PseudoCoupHQ/Research/oracle/riscv/leanpath/strip_6266b40c_8eb1fb6b_IMZ_b/strip.json"))
for r in d["rows"]:
    if r["verdict"]=="FAILED":
