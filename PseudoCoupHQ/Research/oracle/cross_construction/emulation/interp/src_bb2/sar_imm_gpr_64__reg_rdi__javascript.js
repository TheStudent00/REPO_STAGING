...
// one named local per gate, over the term of sar_imm_gpr_64__reg_rdi__javascript.
// The term's layer-5 text, LITERAL:
//   v0 >> 3
function emu_sar_imm_gpr_64__reg_rdi__javascript(a) {
    ...
    const w4 = cat(w3, x0_62, 1);
    const w5 = cat(w4, x0_61, 1);
    const w6 = cat(w5, x0_60, 1);
    const w7 = cat(w6, x0_59, 1);
    const w8 = cat(w7, x0_58, 1);
    const w9 = cat(w8, x0_57, 1);
    const w10 = cat(w9, x0_56, 1);
    const w11 = cat(w10, x0_55, 1);
    const w12 = cat(w11, x0_54, 1);
    const w13 = cat(w12, x0_53, 1);
    const w14 = cat(w13, x0_52, 1);
    const w15 = cat(w14, x0_51, 1);
    const w16 = cat(w15, x0_50, 1);
    const w17 = cat(w16, x0_49, 1);
    const w18 = cat(w17, x0_48, 1);
    const w19 = cat(w18, x0_47, 1);
    const w20 = cat(w19, x0_46, 1);
    const w21 = cat(w20, x0_45, 1);
    const w22 = cat(w21, x0_44, 1);
    const w23 = cat(w22, x0_43, 1);
    const w24 = cat(w23, x0_42, 1);
    const w25 = cat(w24, x0_41, 1);
    const w26 = cat(w25, x0_40, 1);
    const w27 = cat(w26, x0_39, 1);
    const w28 = cat(w27, x0_38, 1);
    const w29 = cat(w28, x0_37, 1);
    const w30 = cat(w29, x0_36, 1);
    const w31 = cat(w30, x0_35, 1);
    const w32 = cat(w31, x0_34, 1);
    const w33 = cat(w32, x0_33, 1);
    const w34 = cat(w33, x0_32, 1);
    const w35 = cat(w34, x0_31, 1);
    const w36 = cat(w35, x0_30, 1);
    const w37 = cat(w36, x0_29, 1);
    const w38 = cat(w37, x0_28, 1);
    const w39 = cat(w38, x0_27, 1);
    const w40 = cat(w39, x0_26, 1);
    const w41 = cat(w40, x0_25, 1);
    const w42 = cat(w41, x0_24, 1);
    const w43 = cat(w42, x0_23, 1);
    const w44 = cat(w43, x0_22, 1);
    const w45 = cat(w44, x0_21, 1);
    const w46 = cat(w45, x0_20, 1);
    const w47 = cat(w46, x0_19, 1);
    const w48 = cat(w47, x0_18, 1);
    const w49 = cat(w48, x0_17, 1);
    const w50 = cat(w49, x0_16, 1);
    const w51 = cat(w50, x0_15, 1);
    const w52 = cat(w51, x0_14, 1);
    const w53 = cat(w52, x0_13, 1);
    const w54 = cat(w53, x0_12, 1);
    const w55 = cat(w54, x0_11, 1);
    const w56 = cat(w55, x0_10, 1);
    const w57 = cat(w56, x0_9, 1);
    const w58 = cat(w57, x0_8, 1);
    const w59 = cat(w58, x0_7, 1);
    const w60 = cat(w59, x0_6, 1);
    const w61 = cat(w60, x0_5, 1);
    const w62 = cat(w61, x0_4, 1);
    const w63 = cat(w62, x0_3, 1);
    return m(w63, 64);
}
...
for (const line of lines) {
    ...
    try {
        out.push(emu_sar_imm_gpr_64__reg_rdi__javascript(...values).toString());
    } catch (problem) {
