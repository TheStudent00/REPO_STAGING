...
# InterpRenderer from the layer-4 term of sltiu_gpr_gpr_imm_64__reg_a0__cpython.
# The term's layer-5 text, LITERAL:
#   If(Or(Extract(1, 0, v0) == 3, Not(Extract(63, 2, v0) == 0)), 0, 1)
def emu_sltiu_gpr_gpr_imm_64__reg_a0__cpython(a):
    return m(((0) if (((eq(ext(a, 1, 0), 3, 2)) or ((not (eq(ext(a, 63, 2), 0, 62)))))) else (1)), 64)
...
def main():
    for line in sys.stdin:
        ...
        try:
            answer = emu_sltiu_gpr_gpr_imm_64__reg_a0__cpython(*values)
            sys.stdout.write("%d\n" % answer)
