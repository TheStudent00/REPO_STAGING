...
// intermediate per node of the term of ZBB_RTYPE__op_MAXU__rd__go__all_constructed.
//   
package main
...
//go:noinline
func emu_ZBB_RTYPE__op_MAXU__rd__go__all_constructed(a uint64, b uint64) uint64 {
	var v0 uint64 = (uint64(uint32(0x1)))
	var v1 uint64 = (uint64(^(uint64(uint64(a)))))
	var v2 uint64 = (uint64((uint64(uint64(b))) ^ (uint64(v1))))
	var v3 uint64 = (uint64((uint64(v2)) & (uint64(v0))))
	var v4 uint64 = (uint64((uint64(uint64(b))) & (uint64(v1))))
	...
	var v36 bool = (!(v35))
	var v37 uint64 = sel64(v36, uint64(uint64(b)), uint64(uint64(a)))
	return uint64(v37)
}
...
func main() {
	sink = emu_ZBB_RTYPE__op_MAXU__rd__go__all_constructed(g0, g1)
	_ = sink
}
