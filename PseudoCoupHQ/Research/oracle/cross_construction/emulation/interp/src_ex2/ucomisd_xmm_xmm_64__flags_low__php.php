...
// InterpRenderer from the layer-4 term of ucomisd_xmm_xmm_64__flags_low__php.
// The term's layer-5 text, LITERAL:
//   fp.to_ieee_bv(fpToFP(Extract(63, 0, v0)))
function emu_ucomisd_xmm_xmm_64__flags_low__php($a) {
    return ex_m($a, 64);
}
...
while (($line = fgets($handle)) !== false) {
    ...
    try {
        $answer = emu_ucomisd_xmm_xmm_64__flags_low__php(...$ints);
        if ($answer < 0) {
            echo sprintf("%s\n", sprintf("%u", $answer));
        } else {
            echo $answer . "\n";
