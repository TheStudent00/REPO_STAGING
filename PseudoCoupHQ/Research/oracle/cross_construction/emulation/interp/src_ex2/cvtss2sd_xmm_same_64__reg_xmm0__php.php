...
// InterpRenderer from the layer-4 term of cvtss2sd_xmm_same_64__reg_xmm0__php.
// The term's layer-5 text, LITERAL:
//   fp.to_ieee_bv(fpToFP(RNE(), fpToFP(Extract(31, 0, v0))))
function emu_cvtss2sd_xmm_same_64__reg_xmm0__php($a) {
...
while (($line = fgets($handle)) !== false) {
    ...
    try {
        $answer = emu_cvtss2sd_xmm_same_64__reg_xmm0__php(...$ints);
        if ($answer < 0) {
            echo sprintf("%s\n", sprintf("%u", $answer));
        } else {
            echo $answer . "\n";
