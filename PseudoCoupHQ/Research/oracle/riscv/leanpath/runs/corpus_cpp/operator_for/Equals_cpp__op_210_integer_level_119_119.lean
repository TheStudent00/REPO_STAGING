...
theorem cpp__op_210_119_integer_level (a : BitVec 64) (b : BitVec 64) :
    ((pure_DIVW (a) (b) (false))) = ((sign_extend (m := 64) (to_bits_truncate (l := 32) (if (((if (((BitVec.toInt (Sail.BitVec.extractLsb b 31 0)) == 0) : Bool) then (Neg.neg 1) else (Int.tdiv (BitVec.toInt (Sail.BitVec.extractLsb a 31 0)) (BitVec.toInt (Sail.BitVec.extractLsb b 31 0)))) ≥b (2 ^i 31)) : Bool) then (Neg.neg (2 ^i 31)) else (if (((BitVec.toInt (Sail.BitVec.extractLsb b 31 0)) == 0) : Bool) then (Neg.neg 1) else (Int.tdiv (BitVec.toInt (Sail.BitVec.extractLsb a 31 0)) (BitVec.toInt (Sail.BitVec.extractLsb b 31 0)))))))) := by
  try simp only [pure_DIVW, not, sign_extend, to_bits_truncate, Sail.BitVec.signExtend, Sail.get_slice_int, Sail.BitVec.extractLsb]
  all_goals grind
