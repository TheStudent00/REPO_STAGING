...
// one named local per gate, over the term of punpckldq_mem_xmm_128__reg_xmm0_low__php.
// The term's layer-5 text, LITERAL:
//   Concat(Extract(31, 0, v0), Extract(31, 0, v1))
function emu_punpckldq_mem_xmm_128__reg_xmm0_low__php($a, $b) {
    $x1_0 = ex_ext($b, 0, 0);
    $x1_1 = ex_ext($b, 1, 1);
    $x1_2 = ex_ext($b, 2, 2);
    $x1_3 = ex_ext($b, 3, 3);
    ...
    $w56 = ex_cat($w55, $x1_7, 1);
    $w57 = ex_cat($w56, $x1_6, 1);
    $w58 = ex_cat($w57, $x1_5, 1);
    $w59 = ex_cat($w58, $x1_4, 1);
    $w60 = ex_cat($w59, $x1_3, 1);
    $w61 = ex_cat($w60, $x1_2, 1);
    $w62 = ex_cat($w61, $x1_1, 1);
    $w63 = ex_cat($w62, $x1_0, 1);
    return ex_m($w63, 64);
}
...
while (($line = fgets($handle)) !== false) {
    ...
    try {
        $answer = emu_punpckldq_mem_xmm_128__reg_xmm0_low__php(...$ints);
        if ($answer < 0) {
            echo sprintf("%s\n", sprintf("%u", $answer));
        } else {
            echo $answer . "\n";
