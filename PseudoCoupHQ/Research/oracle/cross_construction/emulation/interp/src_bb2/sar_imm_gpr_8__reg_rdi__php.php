...
// one named local per gate, over the term of sar_imm_gpr_8__reg_rdi__php.
// The term's layer-5 text, LITERAL:
//   Concat(Extract(63, 8, v0), Extract(7, 0, v0) >> 3)
function emu_sar_imm_gpr_8__reg_rdi__php($a) {
    $x0_3 = ex_ext($a, 3, 3);
    $x0_4 = ex_ext($a, 4, 4);
    $x0_5 = ex_ext($a, 5, 5);
    $x0_6 = ex_ext($a, 6, 6);
    ...
    $w57 = ex_cat($w56, $x0_7, 1);
    $w58 = ex_cat($w57, $x0_7, 1);
...
while (($line = fgets($handle)) !== false) {
    ...
    try {
        $answer = emu_sar_imm_gpr_8__reg_rdi__php(...$ints);
        if ($answer < 0) {
            echo sprintf("%s\n", sprintf("%u", $answer));
        } else {
            echo $answer . "\n";
