...
// InterpRenderer from the layer-4 term of setne_gpr_one_8__reg_rdi__javascript.
// The term's layer-5 text, LITERAL:
//   Concat(0, If(~(~Extract(7, 0, v0) | ~Extract(7, 0, v1)) == 0, 0, 1))
function emu_setne_gpr_one_8__reg_rdi__javascript(a, b) {
    return m(cat(0n, ((eq(bnot(bor(bnot(a, 8), bnot(b, 8), 8), 8), 0n, 8)) ? (0n) : (1n)), 8), 64);
}
...
for (const line of lines) {
    ...
    try {
        out.push(emu_setne_gpr_one_8__reg_rdi__javascript(...values).toString());
    } catch (problem) {
