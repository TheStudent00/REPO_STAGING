#!/usr/bin/env bash
# lp1 lane 13c -- lane 13b AGAIN (the simp-set probe had dropped the four simp ATTRIBUTE names, which are not constants, so nothing of the model unfolded in the base probe; the probe is now stated over a named `initState`; LeanpathBase is a noncomputable section). Before that: lane 13b -- lane 13 AGAIN after the harness build failed (the attribute block was doubled by the template's own comment carrying the placeholder; names outside the import closure of LeanIM.lean were listed; my defs needed `noncomputable`; three match-form run lemmas Lean refused are dropped, EStateM.bind unfolds to the same match; lake prefixes `error:` so the pruning regex missed every line; the simp set's own names are now probed by #check). Otherwise:
...
peak
echo "lane lp1_l13c done at $(date -u +%FT%TZ)"
