...
# InterpRenderer from the layer-4 term of setae_gpr_one_8__reg_rdi__cpython.
# The term's layer-5 text, LITERAL:
#   Concat(0, If(Extract(8, 8, Concat(0, Extract(7, 0, v0)) + Concat(0, Extract(7, 0, v1))) == 1, 0, 1))
def emu_setae_gpr_one_8__reg_rdi__cpython(a, b):
    return m(cat(0, ((0) if (eq(ext(add(cat(0, a, 8), cat(0, b, 8), 9), 8, 8), 1, 1)) else (1)), 8), 64)
...
def main():
    for line in sys.stdin:
        ...
        try:
            answer = emu_setae_gpr_one_8__reg_rdi__cpython(*values)
            sys.stdout.write("%d\n" % answer)
