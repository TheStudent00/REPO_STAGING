#!/bin/bash
# sl1 lane 19 -- lane 18 again after a tuple-value check that met z3 (a Python tuple whose first element is a term was compared to a string). the lifter now runs the harness's own float-unit prologue through the model after reset, and the comparison table's verdict lookup is fixed. The drop-in riscv_reference.py (its walk now the model's
...
for line in lifter.reset_report + lifter.prologue_report:
    print("  reset:", line)
...
for row in rows:
    ...
    if old_term is not None and new_term is not None:
        if old_term.size() != new_term.size():
            ...
            r = s.check()
            verdict = "EQUAL" if r == z3.unsat else ("DIFFER" if r == z3.sat else "UNDECIDED")
            if r == z3.sat:
