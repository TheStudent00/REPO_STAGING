...
// InterpRenderer from the layer-4 term of ucomisd_mem_xmm_64__flags.low__php.
// The term's layer-5 text, LITERAL:
//   fp.to_ieee_bv(fpToFP(v0))
function emu_ucomisd_mem_xmm_64__flags.low__php($a) {
    return ex_m($a, 64);
}
...
while (($line = fgets($handle)) !== false) {
    ...
    try {
        $answer = emu_ucomisd_mem_xmm_64__flags.low__php(...$ints);
        if ($answer < 0) {
            echo sprintf("%s\n", sprintf("%u", $answer));
        } else {
            echo $answer . "\n";
