#!/bin/bash
# lp3_l23_pass_a_four_languages.sh -- pass A over the rendered corpus in
# all four languages (1,960 sources, both routes). The handful first as
# the gate (the alias unfold, the register-index rule, the job pool);
...
export WALK_JOBS=4; W=$A/walk_handful_all_v7; start=$(date +%s)
...
start=$(date +%s); E=$A/equals_handful_all_v7
