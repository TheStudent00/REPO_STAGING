#!/usr/bin/env python3
"""dominant_table15.py -- re-run the class table and dom_op construction
on THIS LAP'S newest converged text per unit (JOB 1/2/3, THIS lap's own
sense: JOB 1 = condition_table5.py/canon19_float.py/canon19_render.py/
canon19_behaviour_check.py's packed-mask float compare family, JOB 2 =
canon12_normalize.py's `_signed_operand` sign-extension fix (canon18.py's
driver), JOB 3 = condition_table6.py/canon20_arith.py/canon20_behaviour_
check.py's z3-FPA packed float arithmetic family), layered on dominant_
table14.py's own generations and everything before it.

Byte-for-byte the SAME construction dominant_table14.py already used
...
generations (canon20, canon19, canon18) ahead of the ones dominant_
table14.py already knew about. `final_text_of` already reads "newest
generation whose <gen>_text field is present", so a unit none of this
lap's own work touched keeps falling through to its canon17_text (or
further) exactly as before -- no other logic changes.

SCOPE: 0-BRANCH POPULATION ONLY, same population-integrity assumption
dominant_table14.py already asserts -- canon18.py/canon19.py/canon20.py's
own drivers all explicitly skip any unit whose `branch_kind` isn't
"straight_line" (canon18.py, canon19.py: `if old_rec.get("branch_kind")
!= "straight_line": return None`; canon20.py: same check in `convert_
one`), so this lap cannot have touched a branching unit -- the SAME
"refuse silently-wrong scope" check is kept, now widened to cover
canon18/19/20 too.

BASELINE (dominant_table14.json/dom_ops12.json, already on disk,
produced before THIS lap's JOB 1-3 work, matching the task's own cited
figures): 924 classes / 135 nodes / 26 families / 23 edgeless.

THE SPELLING BAN: unchanged, same as dominant_table13.py/dominant_
table14.py -- the class key never includes the operator token, only
type reps, result type, and canon-text; the token is read exactly once
per node as `display_label`. check_no_spelling_keys.py is run on both
outputs before this program declares success (see the driver
invocation after main() runs).

usage:
  dominant_table15.py [--out DIR]
"""
...
GENERATIONS = ["canon20", "canon19", "canon18", "canon17", "canon16",
               "canon15", "canon14", "canon13", "canon12", "canon11",
               "canon10", "canon9", "canon8", "canon7"]

BRANCH_CHECK_GENERATIONS = ("canon20", "canon19", "canon18", "canon17",
                             "canon16", "canon15", "canon14", "canon13",
...
def final_text_of(lang, n, gen_docs):
    ...
    text field is present in, this lap. Falls back to canon7_text
...
def main(argv):
    ...
    class_doc = {
        "meta": {
            ...
            "role_note": "this file is a GROUPING/matching artifact "
                        ...
                        "exemption (unlike dom_ops13.json beside it), "
                        "matching dominant_table14.json's own "
            ...
            "population": "canon4_units_*.json, 0-branch (erasure == "
                         ...
                         "from canon20_text/canon19_text/canon18_text/"
                         "canon17_text/canon16_text/canon15_text/"
                         "canon14_text/canon13_text/canon12_text/"
                         "canon11_text/canon10_text/canon9_text/"
                         "canon8_text/canon7_text, newest generation "
                         "that touched each unit -- see this file's "
                         "own header",
            "baseline": "924 classes / 135 nodes / 26 families / 23 "
                       "edgeless (dominant_table14.json/dom_ops12.json, "
    ...
    class_path = os.path.join(outdir, "dominant_table15.json")
    ...
    dom_ops_doc = {
        "meta": {
            ...
            "note": "the dom_op (family) construction over the SAME "
                   "population/class table as dominant_table15.json "
                   "-- written to dom_ops13.json, the next number "
                   "after dom_ops12.json",
        },
    ...
    dom_ops_path = os.path.join(outdir, "dom_ops13.json")
    ...
    print("classes    %d  (baseline: 924)" % result["class_count"])
