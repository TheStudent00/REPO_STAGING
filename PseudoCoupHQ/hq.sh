...
#
#   bash ~/Programming/PseudoCoupHQ/hq.sh
#
...
#                     open ~/Programming/PseudoCoupHQ/plan_explorer.html
...
# into ~/Programming/PlanPlan/framework/ or into a repo's own
...
FRAMEWORK=~/Programming/PlanPlan/framework

# Planning roots the checks and dashboards run over.
ROOTS=(
    ~/Programming/PseudoCoupHQ/Planning
    ~/Programming/PseudoCoup_v5/Planning
    ~/Programming/PseudoCoup_v6/Planning
    ~/Programming/PseudoIR/Planning
)
# NOT here: ~/Programming/PlanPlan/Planning. It has a planning
...
#       ~/Programming/PlanPlan/Planning --projections --apply
#   python3 ~/Programming/PlanPlan/framework/generate_dashboards.py \
#       ~/Programming/PlanPlan/Planning
...
# git_commit_push.sh.
SOURCES_SCRIPT=~/Programming/PseudoCoup_v5/Research/fetch_all_sources.sh
...
# these go wrong).
PLAN_SH=~/Programming/PlanPlan/plan.sh
...
do_sources() {
    if [ ! -f "$SOURCES_SCRIPT" ]; then
        ...
        echo "  is ~/Programming/PseudoCoup_v5 checked out?" >&2
        return 2
    fi
    bash "$SOURCES_SCRIPT" "$@"
    return $?
...
require_tool() {
    if [ ! -f "$1" ]; then
        ...
        echo "  the framework lives in ~/Programming/PlanPlan/ — is it checked out?" >&2
        exit 2
    fi
}
...
LOCK_REPOS=(
    ...
    ~/Programming/PlanPlan
    ~/Programming/DevComms
)
...
case "$CMD" in
    commit)
        ...
        if [ $rc -ne 0 ]; then
            if [ "$FORCE" = "--force" ]; then
                ...
                echo "    bash ~/Programming/PseudoCoupHQ/hq.sh --force" >&2
                exit 1
            fi
        fi
    ...
    explore)
        ...
        out=~/Programming/PseudoCoupHQ/plan_explorer.html
    ...
    list)
        ...
        bash ~/Programming/PseudoCoupHQ/git_commit_push_all.sh --list | tail -n +2
        echo
