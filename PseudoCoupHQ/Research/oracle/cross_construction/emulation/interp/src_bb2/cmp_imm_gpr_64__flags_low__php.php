...
// one named local per gate, over the term of cmp_imm_gpr_64__flags_low__php.
// The term's layer-5 text, LITERAL:
//   v0
function emu_cmp_imm_gpr_64__flags_low__php($a) {
    $x0_0 = ex_ext($a, 0, 0);
    $x0_1 = ex_ext($a, 1, 1);
    $x0_2 = ex_ext($a, 2, 2);
    $x0_3 = ex_ext($a, 3, 3);
...
while (($line = fgets($handle)) !== false) {
    ...
    try {
        $answer = emu_cmp_imm_gpr_64__flags_low__php(...$ints);
        if ($answer < 0) {
            echo sprintf("%s\n", sprintf("%u", $answer));
        } else {
            echo $answer . "\n";
