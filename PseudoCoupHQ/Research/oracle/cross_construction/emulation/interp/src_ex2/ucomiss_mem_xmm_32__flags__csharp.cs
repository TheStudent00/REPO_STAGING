...
public static class Emu {
    ...
    // InterpRenderer from the layer-4 term of ucomiss_mem_xmm_32__flags__csharp.
    // The term's layer-5 text, LITERAL:
    //   Concat(fp.to_ieee_bv(fpToFP(Extract(31, 0, v0))), fp.to_ieee_bv(fpToFP(Extract(31, 0, v1))))
    public static long emu_ucomiss_mem_xmm_32__flags__csharp(long a, long b) {
        return m(cat(b, a, 32), 64);
    }
    ...
    public static void Main() {
        ...
        while (line != null) {
            ...
            if (text.Length != 0) {
                ...
                try {
                    answers.Append(((ulong) emu_ucomiss_mem_xmm_32__flags__csharp(values[0], values[1])).ToString());
                    answers.Append("\n");
