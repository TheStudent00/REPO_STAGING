#!/usr/bin/env python3
"""guard60.py -- THE GUARD for task 60: the UNMODIFIED
...
WHAT IS WALKED: every JSON artifact task 60 writes -- the five
`canon39_wrapped_<lang>.json`, `canon39_interp.json`, all 326 shards of
`canon39_regen_store/`, `canon39_regen_state.json`,
`canon39_assemble.json` and `canon39_zero_regression.json`.  None is
skipped.  No artifact declares `role: generator provenance`, nothing is
added to any field set, and no exemption is declared, so
...
TRANSCRIPT = os.path.join(HERE, "guard60_transcript.txt")
SUMMARY = os.path.join(HERE, "guard60.json")
...
def task60_paths():
    ...
    out.append(os.path.join(HERE, "canon39_regen_state.json"))
    out.append(os.path.join(HERE, "canon39_assemble.json"))
    out.append(os.path.join(HERE, "canon39_zero_regression.json"))
...
def main(argv):
    ...
    counts = run_guard(
        paths, TRANSCRIPT,
        "guard60.py -- every artifact task 60 writes, unmodified "
        "guard, ONE process.  Nothing was added to any field set, "
        "and no artifact was declared out of the walk.")
    print("TASK 60: %d paths  PASS %d  FAIL %d  exempt %d  exit %d"
