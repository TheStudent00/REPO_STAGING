...
// one named local per gate, over the term of cmp_gpr_gpr_16__flags__dart.
// The term's layer-5 text, LITERAL:
//   Concat(Extract(15, 0, v0), Extract(15, 0, v1))
int emu_cmp_gpr_gpr_16__flags__dart(int a, int b) {
  int x1_0 = ext(b, 0, 0);
  int x1_1 = ext(b, 1, 1);
  int x1_2 = ext(b, 2, 2);
  int x1_3 = ext(b, 3, 3);
  ...
  int w0 = x0_15;
  int w1 = cat(w0, x0_14, 1);
  int w2 = cat(w1, x0_13, 1);
  int w3 = cat(w2, x0_12, 1);
  int w4 = cat(w3, x0_11, 1);
  int w5 = cat(w4, x0_10, 1);
  int w6 = cat(w5, x0_9, 1);
  int w7 = cat(w6, x0_8, 1);
  int w8 = cat(w7, x0_7, 1);
  int w9 = cat(w8, x0_6, 1);
  int w10 = cat(w9, x0_5, 1);
  int w11 = cat(w10, x0_4, 1);
  int w12 = cat(w11, x0_3, 1);
  int w13 = cat(w12, x0_2, 1);
  int w14 = cat(w13, x0_1, 1);
  int w15 = cat(w14, x0_0, 1);
  int w16 = cat(w15, x1_15, 1);
  int w17 = cat(w16, x1_14, 1);
  int w18 = cat(w17, x1_13, 1);
  int w19 = cat(w18, x1_12, 1);
  int w20 = cat(w19, x1_11, 1);
  int w21 = cat(w20, x1_10, 1);
  int w22 = cat(w21, x1_9, 1);
  int w23 = cat(w22, x1_8, 1);
  int w24 = cat(w23, x1_7, 1);
  int w25 = cat(w24, x1_6, 1);
  int w26 = cat(w25, x1_5, 1);
  int w27 = cat(w26, x1_4, 1);
  int w28 = cat(w27, x1_3, 1);
  int w29 = cat(w28, x1_2, 1);
  int w30 = cat(w29, x1_1, 1);
  int w31 = cat(w30, x1_0, 1);
  return m(w31, 32);
}
  ...
  while (line != null) {
    ...
    if (text.isNotEmpty) {
      final List<int> values = text
          ...
          .toList();
      try {
        final int answer = emu_cmp_gpr_gpr_16__flags__dart(values[0], values[1]);
