...
- **The one exception the owner stated himself**, and the round's second
  headline: `~/Programming/Ourobrowser` — "the browser allows python
...
CONTEXT: `~/Programming/Ourobrowser` — `browser_engine.py` (the
...
2. Implement in `~/Programming/Ourobrowser` (the owner's project, and this
...
3. Build the dashboard for Ourobrowser in
   `~/Programming/PseudoCoupHQ/Research/op_pipeline`: a page plus a
...
WORK: (a) in `~/Programming/Airlock` — a one-line header on
