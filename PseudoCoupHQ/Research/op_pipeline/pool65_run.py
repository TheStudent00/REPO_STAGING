#!/usr/bin/env python3
"""pool65_run.py -- the driver that runs `pool.py` (node 0_3_5_7) over
canon39 and `term65_store/`, and writes the pool, the families and the
exception families.

ROUND 13.  `pool61_run.py` is round 12's record and is not edited.
The one difference is the layer-4 / layer-5 store it reads:
`term65_store`, whose verdicts are TASK 64's -- the gate of record for
this round -- so the 3,134 units task 64 DISPROVED are carried into
the pool as members that cannot merge on layer 5, each with its
recorded cause.  A disproved term is withdrawn, not kept as a weaker
key; that is the pool CORE's own rule and it is what makes the split
against pool4 a finding rather than a regression.

WRITES:
  the_pool5.json           the pool, three grounds, transitive closure
  the_pool5_bytes.json     the measured assembled bytes
  the_families5.json       the dom_op rule over the pool's entries
  exception_families5.json the guard families REBUILT over this pool
  pool4_pool5_delta.json   every split and every merge, cause computed

WHY `exception_families5`.  Numbered artifacts accumulate and a
superseded record is never edited, so each rebuild takes the next free
number.  `exception_families4.json` is task 61's rebuild over
`the_pool4.json`; this round's rebuild over `the_pool5.json` is the
next one after it.
...
def main():
    ...
    terms = P.read_terms("term65_store")
    ...
    write("the_pool5_bytes.json", sizes)
    ...
    meta = {
        "generator": "pool65_run.py, running pool.py",
        "node": "hq.research.compiler_graph.pool",
        "task": "TASK 65 -- the pool over canon39 on the round-13 gate, three grounds",
        ...
        "layer4_source": "term65_store/*.json -- task 65's own "
                         "artifacts, written by term.py against "
                         "reference.opcode_table, carrying task 64's "
                         "verdicts as the gate of record",
        ...
        "brief_strict_count_note": "entries_under_the_brief_strict_"
                                   ...
                                   "(1).",
        "supersedes_as_an_object": "the_pool4.json, which ran the same "
                                   "rule over canon39 and term61_store "
                                   "-- the round-12 gate, under which "
                                   "no term was disproved.  It stays "
                                   "on disk, byte for byte, as the "
                                   "superseded record.",
        "operator_note": "the operator field is a display label on the "
                         "member and is read by nothing",
    }
    write("the_pool5.json", {"meta": meta, "summary": summary,
                             "entries": entries})
    ...
    write("the_families5.json", {
        "meta": {
            "generator": "pool65_run.py, running pool.Pool.families",
            ...
            "construction": "THE DOM_OP CONSTRUCTION RULE, unchanged, "
                            ...
                            "components)",
            "source_table": "the_pool5.json -- one population, every "
            ...
            "what_a_node_is": "(language, grammar-operator, arity) -- "
                              ...
                              "read by nothing.",
            "supersedes_as_an_object": "the_families4.json, which ran "
                                       "the same rule over "
                                       "the_pool4.json.  It stays on "
    ...
    write("exception_families5.json", {
        "meta": {
            "generator": "pool65_run.py, running "
            ...
            "population": "the guard rows of guards5.json whose unit "
                          "is a member of the_pool5.json",
            "source": "guards5.json, the_pool5.json",
            "role_note": "GROUPING artifact -- checked by "
                         "check_no_spelling_keys.py IN FULL.  No "
                         "provenance carve-out is claimed and no "
                         "`role` field is declared anywhere.",
            "why_this_number": "numbered artifacts accumulate and a "
                               ...
                               "the round-13 rebuild takes the next "
                               "free number after "
                               "exception_families4.json (task 61).",
            "supersedes_as_an_object": "exception_families4.json, "
                                       "rebuilt over the_pool4.json "
                                       "in task 61.  It stays on disk "
    ...
    log("-- the delta against the_pool4.json")
    before = json.load(open(os.path.join(HERE, "the_pool4.json")))
    ...
    delta["meta"] = {
        "generator": "pool65_run.py, running pool.Pool.compare",
        "joined_on": "MEMBER SETS, never entry numbers -- a "
                     "renumbering is not a change",
        "before": "the_pool4.json (canon39, term61_store -- the "
                  "round-12 gate)",
        "after": "the_pool5.json (canon39, term65_store -- the "
                 "round-13 gate of record, task 64)",
    }
    write("pool4_pool5_delta.json", delta)
