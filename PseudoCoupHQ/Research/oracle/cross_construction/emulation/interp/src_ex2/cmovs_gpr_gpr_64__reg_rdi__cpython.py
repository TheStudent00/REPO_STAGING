...
# InterpRenderer from the layer-4 term of cmovs_gpr_gpr_64__reg_rdi__cpython.
# The term's layer-5 text, LITERAL:
#   If(0 <= ~(~v0 | ~v1), v2, v3)
def emu_cmovs_gpr_gpr_64__reg_rdi__cpython(a, b, c, d):
    return m(((d) if (sle(0, bnot(bor(bnot(b, 64), bnot(a, 64), 64), 64), 64)) else (c)), 64)
...
def main():
    for line in sys.stdin:
        ...
        try:
            answer = emu_cmovs_gpr_gpr_64__reg_rdi__cpython(*values)
            sys.stdout.write("%d\n" % answer)
