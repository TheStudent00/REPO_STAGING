...
// InterpRenderer from the layer-4 term of sh_gpr_gpr_same_16__mem_MEM_a1__php.
// The term's layer-5 text, LITERAL:
//   Concat(Extract(63, 16, v0), Extract(15, 0, v1))
function emu_sh_gpr_gpr_same_16__mem_MEM_a1__php($a, $b) {
...
while (($line = fgets($handle)) !== false) {
    ...
    try {
        $answer = emu_sh_gpr_gpr_same_16__mem_MEM_a1__php(...$ints);
        if ($answer < 0) {
            echo sprintf("%s\n", sprintf("%u", $answer));
        } else {
            echo $answer . "\n";
