...
public static class Emu {
    ...
    // InterpRenderer from the layer-4 term of fadd_d_fpr_fpr_fpr_64__freg_fa0__csharp.
    // The term's layer-5 text, LITERAL:
    //   fp.to_ieee_bv(fpToFP(v0) + fpToFP(v1))
    public static long emu_fadd_d_fpr_fpr_fpr_64__freg_fa0__csharp(long a, long b) {
        return m(fadd(a, b, 64), 64);
    }
    ...
    public static void Main() {
        ...
        while (line != null) {
            ...
            if (text.Length != 0) {
                ...
                try {
                    answers.Append(((ulong) emu_fadd_d_fpr_fpr_fpr_64__freg_fa0__csharp(values[0], values[1])).ToString());
                    answers.Append("\n");
