; ModuleID = '<scratch>/fl/run/i32_to_f16.rm1.flags/i32_to_f16.rm1.flags.rv.flat.ll'
source_filename = "llvm-link"
target datalayout = "e-m:e-p:64:64-i64:64-i128:128-n32:64-S128"
target triple = "riscv64-unknown-unknown-elf"

define dso_local i64 @i32_to_f16_rm1_flags_flat(i32 noundef signext %arg) {
  %i = icmp slt i32 %arg, 0
  %i1 = call i32 @llvm.abs.i32(i32 %arg, i1 false)
  %i2 = icmp eq i32 %arg, 0
  %.n15 = xor i1 %i2, true
  %.k1 = call range(i32 0, 33) i32 @llvm.ctlz.i32(i32 %i1, i1 true)
  %i4 = freeze i32 %.k1
  %i5 = icmp ult i32 %i1, 2048
  %i18 = add i32 %i4, -17
  %i19 = icmp ugt i32 %i1, 32767
  %i7 = add i32 %i4, 235
  %i10 = and i32 %i7, 255
  %.sh2 = lshr i32 %arg, 16
  %i9 = and i32 %.sh2, 32768
  %i12 = or i32 %i9, 24576
  %.sh3 = shl i32 %i1, %i10
  %i11 = freeze i32 %.sh3
  %.sh4 = shl i32 %i10, 10
  %i14 = sub i32 %i12, %.sh4
  %i15 = add i32 %i14, %i11
  %i16 = zext i32 %i15 to i64
  %i21 = sub i32 17, %i4
  %.sh5 = lshr i32 %i1, %i21
  %i22 = freeze i32 %.sh5
  %i23 = and i32 %i18, 31
  %.sh6 = shl i32 %i1, %i23
  %i24 = freeze i32 %.sh6
  %i25 = icmp ne i32 %i24, 0
  %i26 = zext i1 %i25 to i32
  %i27 = or i32 %i22, %i26
  %.sh7 = shl i32 %i1, %i18
  %i29 = freeze i32 %.sh7
  %.m8 = sext i1 %i19 to i32
  %.a9 = and i32 %i27, %.m8
  %.n10 = xor i1 %i19, true
  %.m11 = sext i1 %.n10 to i32
  %.a12 = and i32 %i29, %.m11
  %.o13 = or i32 %.a9, %.a12
  %.n14 = xor i1 %i5, true
  %.c53 = and i1 %i5, %.n15
  %.m54 = sext i1 %.c53 to i64
  %.a55 = and i64 %i16, %.m54
  %.c16 = and i1 %.n14, %.n15
  %.m17 = sext i1 %.c16 to i32
  %.a18 = and i32 %.o13, %.m17
  %i32 = sub i32 45, %i4
  %i36 = icmp eq i32 %i4, 16
  %i35 = and i32 %.a18, 32768
  %.not = icmp eq i32 %i35, 0
  %or.cond = and i1 %i36, %.not
  %.m19 = sext i1 %i to i64
  %.m34 = sext i1 %i to i32
  %i46 = and i32 32768, %.m34
  %.a20 = and i64 64511, %.m19
  %.n21 = xor i64 %.m19, -1
  %.a22 = and i64 31743, %.n21
  %i38 = or i64 %.a20, %.a22
  %.a25 = and i32 %i32, %.m11
  %.c38 = and i1 %.n10, %.n14
  %.c39 = and i1 %.c38, %.n15
  %.c26 = and i1 %i19, %or.cond
  %.m27 = sext i1 %.c26 to i32
  %.a28 = and i32 29, %.m27
  %.o29 = or i32 %.a25, %.a28
  %.a30 = and i32 %.o29, %.m17
  %.sh35 = shl i32 %.a30, 10
  %i47 = freeze i32 %.sh35
  %.sh31 = lshr i32 %.a18, 4
  %i41 = freeze i32 %.sh31
  %i42 = and i32 %i41, 4095
  %i43 = and i32 %.a18, 15
  %i44 = icmp eq i32 %i43, 0
  %.m32 = sext i1 %i44 to i64
  %.n33 = xor i64 %.m32, -1
  %spec.select = and i64 1, %.n33
  %i49 = or i32 %i42, %i46
  %i45 = icmp eq i32 %i42, 0
  %.m36 = sext i1 %i45 to i32
  %.n37 = xor i32 %.m36, -1
  %i48 = and i32 %i47, %.n37
  %i50 = add i32 %i49, %i48
  %i51 = and i32 %i50, 65535
  %i52 = zext i32 %i51 to i64
  %.c42 = and i1 %.c26, %.n15
  %.c43 = or i1 %.c39, %.c42
  %.m44 = sext i1 %.c43 to i64
  %.a45 = and i64 %spec.select, %.m44
  %.a56 = and i64 %i52, %.m44
  %.o57 = or i64 %.a55, %.a56
  %.n46 = xor i1 %or.cond, true
  %.c47 = and i1 %i19, %.n46
  %.c49 = and i1 %.c47, %.n15
  %.m50 = sext i1 %.c49 to i64
  %.a58 = and i64 %i38, %.m50
  %.o59 = or i64 %.o57, %.a58
  %.a51 = and i64 5, %.m50
  %.o52 = or i64 %.a45, %.a51
  %i54 = shl i64 %.o59, 16
  %i55 = and i64 %i54, 4294901760
  %i56 = or i64 %i55, %.o52
  ret i64 %i56
}

; Function Attrs: nocallback nofree nosync nounwind speculatable willreturn memory(none)
declare i32 @llvm.abs.i32(i32, i1 immarg) #0

; Function Attrs: nocallback nofree nosync nounwind speculatable willreturn memory(none)
declare i32 @llvm.ctlz.i32(i32, i1 immarg) #0

attributes #0 = { nocallback nofree nosync nounwind speculatable willreturn memory(none) }

!llvm.ident = !{!0, !0, !0, !0, !0, !0, !0, !0}
!llvm.module.flags = !{!1, !2, !3, !5}

!0 = !{!"Ubuntu clang version 21.1.8 (6ubuntu1)"}
!1 = !{i32 1, !"wchar_size", i32 4}
!2 = !{i32 1, !"target-abi", !"lp64"}
!3 = distinct !{i32 6, !"riscv-isa", !4}
!4 = distinct !{!"rv64i2p1_m2p0_zmmul1p0"}
!5 = !{i32 8, !"SmallDataLimit", i32 0}
