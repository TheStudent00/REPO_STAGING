#!/usr/bin/env bash
# ap5_l11_guard_t100_edges.sh -- task ap5: the FOURTH guard the brief
...
for edge in edges:
    for name in sorted(edge):
        if name in ("outcome", "verdict"):
            counted.setdefault(name, {})
            counted[name][edge[name]] = \
                counted[name].get(edge[name], 0) + 1
for name in sorted(counted):
    print("   by `%s`:" % name)
    for value in sorted(counted[name], key=lambda v: "%s" % v):
        print("      %-28s %d" % (value, counted[name][value]))
proved = 0
applied = 0
for edge in edges:
    if edge.get("proved"):
        proved = proved + 1
    if edge.get("edge_applies"):
        applied = applied + 1
print("   edges whose `proved` is true: %d" % proved)
print("   edges whose `edge_applies` is true: %d" % applied)
