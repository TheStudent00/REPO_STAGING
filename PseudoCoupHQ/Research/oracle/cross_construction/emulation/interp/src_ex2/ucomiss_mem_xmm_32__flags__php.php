...
// InterpRenderer from the layer-4 term of ucomiss_mem_xmm_32__flags__php.
// The term's layer-5 text, LITERAL:
//   Concat(fp.to_ieee_bv(fpToFP(Extract(31, 0, v0))), fp.to_ieee_bv(fpToFP(Extract(31, 0, v1))))
function emu_ucomiss_mem_xmm_32__flags__php($a, $b) {
    return ex_m(ex_cat($b, $a, 32), 64);
}
...
while (($line = fgets($handle)) !== false) {
    ...
    try {
        $answer = emu_ucomiss_mem_xmm_32__flags__php(...$ints);
        if ($answer < 0) {
            echo sprintf("%s\n", sprintf("%u", $answer));
        } else {
            echo $answer . "\n";
