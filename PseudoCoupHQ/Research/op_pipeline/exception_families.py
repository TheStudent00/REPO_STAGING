#!/usr/bin/env python3
"""exception_families.py -- STEP 2/3 of THE JOB (EXCEPTION-FAMILY TABLE).

GUARD IDENTITY (AgentMemory, SEEDED GROUPING UNDER CONDITIONS): two
guard rows from guards2.json are the SAME exception-family component
when
...
is not clustered into an exception family. It stays in guards2.json
...
def main():
    ...
    out = dict(
        meta=dict(
            generator="exception_families.py",
    ...
    json.dump(out, open(os.path.join(HERE, "exception_families.json"), "w"),
               indent=1)
