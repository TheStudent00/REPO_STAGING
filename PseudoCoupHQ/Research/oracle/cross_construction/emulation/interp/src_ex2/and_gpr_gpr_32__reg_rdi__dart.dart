...
// InterpRenderer from the layer-4 term of and_gpr_gpr_32__reg_rdi__dart.
// The term's layer-5 text, LITERAL:
//   Concat(0, ~(~Extract(31, 0, v0) | ~Extract(31, 0, v1)))
int emu_and_gpr_gpr_32__reg_rdi__dart(int a, int b) {
  return m(cat(0, bnot(bor(bnot(a, 32), bnot(b, 32), 32), 32), 32), 64);
}
  ...
  while (line != null) {
    ...
    if (text.isNotEmpty) {
      final List<int> values = text
          ...
          .toList();
      try {
        final int answer = emu_and_gpr_gpr_32__reg_rdi__dart(values[0], values[1]);
