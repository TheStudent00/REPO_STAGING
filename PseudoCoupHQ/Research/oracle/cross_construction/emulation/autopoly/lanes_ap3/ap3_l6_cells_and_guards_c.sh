#!/usr/bin/env bash
# ap3_l6_cells_and_guards_c.sh -- task ap3, the guards a THIRD time
# (a lane name is used ONCE, so each pass takes a new one and
# nothing is removed).  Lane `ap3_l5` is what caught fix 1
# reaching the handful's two vector cells -- a place task h2's fix
# 2 projects a lane out of does read its arrival above bit 63, and
# the projection is what makes that not matter -- so
# `vector_arrivals_in_halves` now leaves such a place to task h2's
# fix and this pass is the guard against the driver as corrected: the outer set for this pass,
