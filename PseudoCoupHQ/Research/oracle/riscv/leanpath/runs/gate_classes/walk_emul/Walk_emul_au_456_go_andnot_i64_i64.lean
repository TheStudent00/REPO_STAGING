...
theorem meaning_emul_au_456_go_andnot_i64_i64 (s : St) (a : BitVec 64) (b : BitVec 64) (h10 : s.regs.get? .x10 = some a) (h11 : s.regs.get? .x11 = some b) (hk0 : ∀ {α : Type} (x0 : α) (s : St), (plat_term_write x0) s = EStateM.Result.ok () s) (hk1 : ∀ (x0 : Arch.pa) (x1 : Nat) (s : St), (load_reservation x0 x1) s = EStateM.Result.ok () s) (hk2 : ∀ (x0 : Unit) (s : St), (cancel_reservation x0) s = EStateM.Result.ok () s) :
    runsTo (walk [LeanIM.instruction.ZBB_RTYPE (LeanIM.regidx.Regidx 0x0b#5, LeanIM.regidx.Regidx 0x0a#5, LeanIM.regidx.Regidx 0x0a#5, LeanIM.brop_zbb.ANDN)]) s
      ((pure_ZBB_RTYPE (a) (b) (LeanIM.brop_zbb.ANDN))) := by
  simp (config := {decide := true}) [runsTo, walk, step, execute, strip_ZBB_RTYPE,
