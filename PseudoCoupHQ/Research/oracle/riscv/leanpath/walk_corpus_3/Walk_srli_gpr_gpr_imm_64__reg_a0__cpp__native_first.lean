...
theorem meaning_srli_gpr_gpr_imm_64__reg_a0__cpp__native_first (s : St) (a : BitVec 64) (h10 : s.regs.get? .x10 = some a) (hk0 : ∀ {α : Type} (x0 : α) (s : St), (plat_term_write x0) s = EStateM.Result.ok () s) (hk1 : ∀ (x0 : Arch.pa) (x1 : Nat) (s : St), (load_reservation x0 x1) s = EStateM.Result.ok () s) (hk2 : ∀ (x0 : Unit) (s : St), (cancel_reservation x0) s = EStateM.Result.ok () s) :
    runsTo (walk [LeanIM.instruction.C_SRLI (0x03#6, LeanIM.cregidx.Cregidx 0x2#3)]) s
      ((pure_SHIFTIOP (a) ((0x03#6)) (SRLI))) := by
  simp (config := {decide := true}) [runsTo, walk, step, execute, strip_C_SRLI, strip_SHIFTIOP, alias_C_SRLI,
