...
// InterpRenderer from the layer-4 term of adc_gpr_gpr_64__reg_rdi__javascript.
// The term's layer-5 text, LITERAL:
//   If(Extract(64, 64, Concat(0, v0) + Concat(0, v1)) == 1, 1, 0) + v2 + v3
function emu_adc_gpr_gpr_64__reg_rdi__javascript(a, b, c, d) {
    return m(add(add(((eq(ext(add(cat(0n, b, 64), cat(0n, a, 64), 65), 64, 64), 1n, 1)) ? (1n) : (0n)), c, 64), d, 64), 64);
}
...
for (const line of lines) {
    ...
    try {
        out.push(emu_adc_gpr_gpr_64__reg_rdi__javascript(...values).toString());
    } catch (problem) {
