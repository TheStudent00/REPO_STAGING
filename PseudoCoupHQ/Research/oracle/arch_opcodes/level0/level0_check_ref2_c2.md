...
| places that AGREE (`unsat`) | 946 |
| places that DISAGREE (`sat`) | 197 |
| places `unknown` at the ceiling | 0 |
| places their reading leaves UNDEFINED | 197 |
| places `refused` | 439 |
| variants agreeing on every place compared | 285 |
| variants with at least one disagreement | 64 |
...
|---|---|---|---|---|---|---|---|---|
| `sbb` | 19 | 133 | 47 | 63 | 0 | 0 | 23 | 2510 |
| `adc` | 19 | 133 | 53 | 57 | 0 | 0 | 23 | 520 |
| `add` | 19 | 133 | 53 | 57 | 0 | 0 | 23 | 795 |
| `sub` | 19 | 133 | 98 | 12 | 0 | 0 | 23 | 938 |
| `neg` | 4 | 28 | 16 | 8 | 0 | 0 | 4 | 211 |
...
the counterexample, LITERAL: `seed_MEM__rax_` = `0x7fffffff`, `seed_r10` = `0x0`, `seed_r11` = `0x0`, `seed_rsi` = `0x80000007`
...
the counterexample, LITERAL: `seed_MEM__rax_` = `0xffffff`, `seed_rsi` = `0x10002dd`
...
| reading | term, LITERAL (z3's printing of the simplified term, the register symbols keeping our reference's names) | its value at the counterexample |
|---|---|---|
| ours | `If(Extract(8, 8, Concat(Extract(7, 7, seed_MEM__rax_), Extract(7, 0, seed_MEM__rax_)) + 511*Concat(Extract(7, 7, seed_rsi), Extract(7, 0, seed_rsi)) + 511*Concat(0, If(ULE(Extract(31, 0, seed_r10), Extract(31, 0, seed_r11)), 0, 1))) == Extract(7, 7, Concat(Extract(7, 7, seed_MEM__rax_), Extract(7, 0, seed_MEM__rax_)) + 511*Concat(Extract(7, 7, seed_rsi), Extract(7, 0, seed_rsi)) + 511*Concat(0, If(ULE(Extract(31, 0, seed_r10), Extract(31, 0, seed_r11)), 0, 1))), 0, 1)` | 0x0 |
| theirs | `If(And(Extract(31, 31, seed_rsi) == ~Extract(31, 31, seed_MEM__rax_), Not(Extract(31, 31, seed_rsi) == ~Extract(31, 31, Concat(0, Extract(31, 0, seed_MEM__rax_)) + If(ULE(Extract(31, 0, seed_r10), Extract(31, 0, seed_r11)), 1 + Concat(0, ~Extract(31, 0, seed_rsi)), Concat(0, ~Extract(31, 0, seed_rsi)))))), 1, 0)` | 0x1 |

the counterexample, LITERAL: `seed_MEM__rax_` = `0x7fffffff`, `seed_r10` = `0x1`, `seed_r11` = `0x0`, `seed_rsi` = `0x80000002`

`subl_m32_r32` -- our line `sub %esi,(%rax)`, shape `gpr_mem` at width 32, the place `flags:OF`:

| reading | term, LITERAL (z3's printing of the simplified term, the register symbols keeping our reference's names) | its value at the counterexample |
|---|---|---|
| ours | `If(Extract(8, 8, Concat(Extract(7, 7, seed_MEM__rax_), Extract(7, 0, seed_MEM__rax_)) + 511*Concat(Extract(7, 7, seed_rsi), Extract(7, 0, seed_rsi))) == Extract(7, 7, Concat(Extract(7, 7, seed_MEM__rax_), Extract(7, 0, seed_MEM__rax_)) + 511*Concat(Extract(7, 7, seed_rsi), Extract(7, 0, seed_rsi))), 0, 1)` | 0x1 |
| theirs | `If(And(Extract(31, 31, seed_rsi) == ~Extract(31, 31, seed_MEM__rax_), Not(Extract(31, 31, seed_rsi) == ~Extract(31, 31, 1 + Concat(0, Extract(31, 0, seed_MEM__rax_)) + Concat(0, ~Extract(31, 0, seed_rsi))))), 1, 0)` | 0x0 |

the counterexample, LITERAL: `seed_MEM__rax_` = `0x15c`, `seed_rsi` = `0x80000280`
...
Wall clock 4.1 s; peak resident memory 64.1 MB against the stated 6144 MB ceiling and the named abort `ABORT_MEMORY_REF1`.
