#!/bin/bash
# lp3_l39_equals_same_text_then_evaluate_final_tally.sh -- equals over every
...
for k in 0 1 2 3; do for h in 0 1; do
  ( python3 -u -m leanpath equals $P $PLIB $S $A/walk_corpus_${k}_half$h.json $A/equals_corpus_i_${k}_$h 120 64 > $A/equals_corpus_i_${k}_$h.log 2>&1 ) &
done; done
wait
echo "  wall seconds=$(( $(date +%s) - start ))"
for k in 0 1 2 3; do for h in 0 1; do grep -E "equals:|Traceback" $A/equals_corpus_i_${k}_$h.log | tail -2; done; done
echo "[2/3] equals on the identity units, the same way"
export EQUALS_JOBS=2; start=$(date +%s)
python3 -u -m leanpath equals $P $PLIB $S $A/walk_corpus_fix2/walk.json $A/equals_corpus_i_fix2 120 64 2>&1 | grep -E "equals:|Traceback" | tail -2
echo "  wall seconds=$(( $(date +%s) - start ))"
echo "[3/3] the final tallies over everything"
python3 -m leanpath tally $A/tally_corpus_final.json $A/walk_corpus_0 $A/walk_corpus_1 $A/walk_corpus_2 $A/walk_corpus_3 $A/walk_corpus_fix2 -- $A/equals_corpus_i_0_0 $A/equals_corpus_i_0_1 $A/equals_corpus_i_1_0 $A/equals_corpus_i_1_1 $A/equals_corpus_i_2_0 $A/equals_corpus_i_2_1 $A/equals_corpus_i_3_0 $A/equals_corpus_i_3_1 $A/equals_corpus_i_fix2 2>&1 | cut -c1-200
echo done
