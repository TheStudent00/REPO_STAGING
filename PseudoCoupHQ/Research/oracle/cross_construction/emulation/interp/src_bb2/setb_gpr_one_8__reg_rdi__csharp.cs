...
public static class Emu {
    ...
    // one named local per gate, over the term of setb_gpr_one_8__reg_rdi__csharp.
    // The term's layer-5 text, LITERAL:
    //   Concat(Extract(63, 8, v0), If(ULE(Extract(7, 0, v1), Extract(7, 0, v2)), 0, 1))
    public static long emu_setb_gpr_one_8__reg_rdi__csharp(long a, long b, long c) {
        long x2_0 = ext(b, 0, 0);
        long x1_0 = ext(c, 0, 0);
        long x2_1 = ext(b, 1, 1);
        long x1_1 = ext(c, 1, 1);
    ...
    public static void Main() {
        ...
        while (line != null) {
            ...
            if (text.Length != 0) {
                ...
                try {
                    answers.Append(((ulong) emu_setb_gpr_one_8__reg_rdi__csharp(values[0], values[1], values[2])).ToString());
                    answers.Append("\n");
