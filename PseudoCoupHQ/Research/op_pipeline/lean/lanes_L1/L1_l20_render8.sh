#!/bin/bash
# L1 lane 17 — the renderer preservation theorem, build 8. Three gaps build 4
...
time lean Archproof/Render.lean > /work/render9_out.txt 2>&1
echo "--- lean exit $?"
grep -v 'linter.unusedSimpArgs\|Hint: Omit it\|^$\|Note: This linter\|̵' /work/render9_out.txt | head -120
echo "--- declarations using sorry: $(grep -c "declaration uses 'sorry'" /work/render9_out.txt)"
echo "--- error lines: $(grep -c 'error:' /work/render9_out.txt)"
