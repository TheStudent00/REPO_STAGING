...
// one named local per gate, over the term of mov_imm_gpr_16__reg_rdi__dart.
// The term's layer-5 text, LITERAL:
//   Concat(Extract(63, 16, v0), Extract(15, 0, v1))
int emu_mov_imm_gpr_16__reg_rdi__dart(int a, int b) {
  int x1_0 = ext(b, 0, 0);
  int x1_1 = ext(b, 1, 1);
  int x1_2 = ext(b, 2, 2);
  int x1_3 = ext(b, 3, 3);
  ...
  while (line != null) {
    ...
    if (text.isNotEmpty) {
      final List<int> values = text
          ...
          .toList();
      try {
        final int answer = emu_mov_imm_gpr_16__reg_rdi__dart(values[0], values[1]);
