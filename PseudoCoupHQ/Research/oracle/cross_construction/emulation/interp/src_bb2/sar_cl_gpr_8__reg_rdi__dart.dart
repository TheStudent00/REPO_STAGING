...
// one named local per gate, over the term of sar_cl_gpr_8__reg_rdi__dart.
// The term's layer-5 text, LITERAL:
//   Concat(Extract(63, 8, v0), Extract(7, 0, v0) >> Concat(0, Extract(4, 0, v1)))
int emu_sar_cl_gpr_8__reg_rdi__dart(int a, int b) {
  int x0_0 = ext(a, 0, 0);
  int x0_1 = ext(a, 1, 1);
  int x1_0 = ext(b, 0, 0);
  int x0_2 = ext(a, 2, 2);
  ...
  int w57 = cat(w56, g102, 1);
  int w58 = cat(w57, g96, 1);
  int w59 = cat(w58, g90, 1);
  int w60 = cat(w59, g86, 1);
  int w61 = cat(w60, g77, 1);
  int w62 = cat(w61, g60, 1);
  int w63 = cat(w62, g32, 1);
  return m(w63, 64);
}
  ...
  while (line != null) {
    ...
    if (text.isNotEmpty) {
      final List<int> values = text
          ...
          .toList();
      try {
        final int answer = emu_sar_cl_gpr_8__reg_rdi__dart(values[0], values[1]);
