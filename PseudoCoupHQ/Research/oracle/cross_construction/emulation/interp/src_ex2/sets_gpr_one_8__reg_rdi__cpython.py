...
# InterpRenderer from the layer-4 term of sets_gpr_one_8__reg_rdi__cpython.
# The term's layer-5 text, LITERAL:
#   Concat(0, If(0 <= ~(~Extract(7, 0, v0) | ~Extract(7, 0, v1)), 0, 1))
def emu_sets_gpr_one_8__reg_rdi__cpython(a, b):
    return m(cat(0, ((0) if (sle(0, bnot(bor(bnot(a, 8), bnot(b, 8), 8), 8), 8)) else (1)), 8), 64)
...
def main():
    for line in sys.stdin:
        ...
        try:
            answer = emu_sets_gpr_one_8__reg_rdi__cpython(*values)
            sys.stdout.write("%d\n" % answer)
