...
// InterpRenderer from the layer-4 term of sub_gpr_gpr_same_64__reg_a0__php.
// The term's layer-5 text, LITERAL:
//   0
function emu_sub_gpr_gpr_same_64__reg_a0__php($a) {
    return ex_m(0, 64);
}
...
while (($line = fgets($handle)) !== false) {
    ...
    try {
        $answer = emu_sub_gpr_gpr_same_64__reg_a0__php(...$ints);
        if ($answer < 0) {
            echo sprintf("%s\n", sprintf("%u", $answer));
        } else {
            echo $answer . "\n";
