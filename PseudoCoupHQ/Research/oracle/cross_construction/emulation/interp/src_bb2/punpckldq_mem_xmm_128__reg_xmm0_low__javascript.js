...
// one named local per gate, over the term of punpckldq_mem_xmm_128__reg_xmm0_low__javascript.
// The term's layer-5 text, LITERAL:
//   Concat(Extract(31, 0, v0), Extract(31, 0, v1))
function emu_punpckldq_mem_xmm_128__reg_xmm0_low__javascript(a, b) {
    ...
    const w0 = x0_31;
    const w1 = cat(w0, x0_30, 1);
    const w2 = cat(w1, x0_29, 1);
    const w3 = cat(w2, x0_28, 1);
    const w4 = cat(w3, x0_27, 1);
    const w5 = cat(w4, x0_26, 1);
    const w6 = cat(w5, x0_25, 1);
    const w7 = cat(w6, x0_24, 1);
    const w8 = cat(w7, x0_23, 1);
    const w9 = cat(w8, x0_22, 1);
    const w10 = cat(w9, x0_21, 1);
    const w11 = cat(w10, x0_20, 1);
    const w12 = cat(w11, x0_19, 1);
    const w13 = cat(w12, x0_18, 1);
    const w14 = cat(w13, x0_17, 1);
    const w15 = cat(w14, x0_16, 1);
    const w16 = cat(w15, x0_15, 1);
    const w17 = cat(w16, x0_14, 1);
    const w18 = cat(w17, x0_13, 1);
    const w19 = cat(w18, x0_12, 1);
    const w20 = cat(w19, x0_11, 1);
    const w21 = cat(w20, x0_10, 1);
    const w22 = cat(w21, x0_9, 1);
    const w23 = cat(w22, x0_8, 1);
    const w24 = cat(w23, x0_7, 1);
    const w25 = cat(w24, x0_6, 1);
    const w26 = cat(w25, x0_5, 1);
    const w27 = cat(w26, x0_4, 1);
    const w28 = cat(w27, x0_3, 1);
    const w29 = cat(w28, x0_2, 1);
    const w30 = cat(w29, x0_1, 1);
    const w31 = cat(w30, x0_0, 1);
    ...
    const w56 = cat(w55, x1_7, 1);
    const w57 = cat(w56, x1_6, 1);
    const w58 = cat(w57, x1_5, 1);
    const w59 = cat(w58, x1_4, 1);
    const w60 = cat(w59, x1_3, 1);
    const w61 = cat(w60, x1_2, 1);
    const w62 = cat(w61, x1_1, 1);
    const w63 = cat(w62, x1_0, 1);
    return m(w63, 64);
}
...
for (const line of lines) {
    ...
    try {
        out.push(emu_punpckldq_mem_xmm_128__reg_xmm0_low__javascript(...values).toString());
    } catch (problem) {
