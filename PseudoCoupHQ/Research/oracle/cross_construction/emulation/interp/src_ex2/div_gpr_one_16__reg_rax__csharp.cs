...
public static class Emu {
    ...
    // InterpRenderer from the layer-4 term of div_gpr_one_16__reg_rax__csharp.
    // The term's layer-5 text, LITERAL:
    //   Concat(Extract(63, 16, v1), Extract(15, 0, bvudiv_i(Concat(Extract(15, 0, v0), Extract(15, 0, v1)), Concat(0, Extract(15, 0, v2)))))
    public static long emu_div_gpr_one_16__reg_rax__csharp(long a, long b, long c) {
        return m(cat(ext(b, 63, 16), ext(udiv(cat(a, ext(b, 15, 0), 16), cat(0L, c, 16), 32), 15, 0), 16), 64);
    }
    ...
    public static void Main() {
        ...
        while (line != null) {
            ...
            if (text.Length != 0) {
                ...
                try {
                    answers.Append(((ulong) emu_div_gpr_one_16__reg_rax__csharp(values[0], values[1], values[2])).ToString());
                    answers.Append("\n");
