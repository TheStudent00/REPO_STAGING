...
// one named local per gate, over the term of add_gpr_same_8__reg_rdi__dart.
// The term's layer-5 text, LITERAL:
//   Concat(Extract(63, 8, v0), Extract(7, 0, v0)*2)
int emu_add_gpr_same_8__reg_rdi__dart(int a) {
  int x0_0 = ext(a, 0, 0);
  int x0_1 = ext(a, 1, 1);
  int x0_2 = ext(a, 2, 2);
  int x0_3 = ext(a, 3, 3);
  ...
  int w56 = cat(w55, x0_6, 1);
  int w57 = cat(w56, x0_5, 1);
  int w58 = cat(w57, x0_4, 1);
  int w59 = cat(w58, x0_3, 1);
  int w60 = cat(w59, x0_2, 1);
  int w61 = cat(w60, x0_1, 1);
  int w62 = cat(w61, x0_0, 1);
  ...
  while (line != null) {
    ...
    if (text.isNotEmpty) {
      final List<int> values = text
          ...
          .toList();
      try {
        final int answer = emu_add_gpr_same_8__reg_rdi__dart(values[0]);
