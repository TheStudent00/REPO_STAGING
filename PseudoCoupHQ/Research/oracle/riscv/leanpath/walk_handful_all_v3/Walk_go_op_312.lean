...
theorem meaning_go_op_312 (s : St) (a : BitVec 64) (b : BitVec 64) (h10 : s.regs.get? .x10 = some a) (h11 : s.regs.get? .x11 = some b) (hk0 : ∀ {α} (x0 : _) (s : St), (plat_term_write x0) s = EStateM.Result.ok () s) (hk1 : ∀ (x0 : _) (x1 : _) (s : St), (load_reservation x0 x1) s = EStateM.Result.ok () s) (hk2 : ∀ (x0 : _) (s : St), (cancel_reservation x0) s = EStateM.Result.ok () s) :
    runsTo (walk [LeanIM.instruction.C_ADD (LeanIM.regidx.Regidx 0x0a#5, LeanIM.regidx.Regidx 0x0b#5)]) s
      ((pure_RTYPE (a) (b) (ADD))) := by
  simp (config := {decide := true}) [runsTo, walk, step, execute, strip_C_ADD, strip_RTYPE,
