...
// one named local per gate, over the term of shr_imm_gpr_64__reg_rdi__php.
// The term's layer-5 text, LITERAL:
//   Concat(0, Extract(63, 3, v0))
function emu_shr_imm_gpr_64__reg_rdi__php($a) {
    $x0_3 = ex_ext($a, 3, 3);
    $x0_4 = ex_ext($a, 4, 4);
    $x0_5 = ex_ext($a, 5, 5);
    $x0_6 = ex_ext($a, 6, 6);
...
while (($line = fgets($handle)) !== false) {
    ...
    try {
        $answer = emu_shr_imm_gpr_64__reg_rdi__php(...$ints);
        if ($answer < 0) {
            echo sprintf("%s\n", sprintf("%u", $answer));
        } else {
            echo $answer . "\n";
