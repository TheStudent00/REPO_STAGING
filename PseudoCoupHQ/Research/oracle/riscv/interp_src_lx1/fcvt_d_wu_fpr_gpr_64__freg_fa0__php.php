...
// InterpRenderer from the layer-4 term of fcvt_d_wu_fpr_gpr_64__freg_fa0__php.
// The term's layer-5 text, LITERAL:
//   fp.to_ieee_bv(fpToFPUnsigned(RNE(), Extract(31, 0, v0)))
function emu_fcvt_d_wu_fpr_gpr_64__freg_fa0__php($a) {
    return ex_m(ex_u2f($a, 32, 64), 64);
}
...
while (($line = fgets($handle)) !== false) {
    ...
    try {
        $answer = emu_fcvt_d_wu_fpr_gpr_64__freg_fa0__php(...$ints);
        if ($answer < 0) {
            echo sprintf("%s\n", sprintf("%u", $answer));
        } else {
            echo $answer . "\n";
