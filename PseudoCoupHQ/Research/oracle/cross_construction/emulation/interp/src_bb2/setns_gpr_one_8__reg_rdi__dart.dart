...
// one named local per gate, over the term of setns_gpr_one_8__reg_rdi__dart.
// The term's layer-5 text, LITERAL:
//   Concat(Extract(63, 8, v2), If(Or(Extract(7, 7, v0) == 0, Extract(7, 7, v1) == 0), 1, 0))
int emu_setns_gpr_one_8__reg_rdi__dart(int a, int b, int c) {
  int x1_7 = ext(a, 7, 7);
  int x0_7 = ext(b, 7, 7);
  int x2_8 = ext(c, 8, 8);
  int x2_9 = ext(c, 9, 9);
  ...
  while (line != null) {
    ...
    if (text.isNotEmpty) {
      final List<int> values = text
          ...
          .toList();
      try {
        final int answer = emu_setns_gpr_one_8__reg_rdi__dart(values[0], values[1], values[2]);
