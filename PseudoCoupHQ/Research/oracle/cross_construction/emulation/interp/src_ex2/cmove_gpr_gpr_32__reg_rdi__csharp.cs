...
public static class Emu {
    ...
    // InterpRenderer from the layer-4 term of cmove_gpr_gpr_32__reg_rdi__csharp.
    // The term's layer-5 text, LITERAL:
    //   Concat(0, If(~(~Extract(31, 0, v0) | ~Extract(31, 0, v1)) == 0, Extract(31, 0, v2), Extract(31, 0, v3)))
    public static long emu_cmove_gpr_gpr_32__reg_rdi__csharp(long a, long b, long c, long d) {
        return m(cat(0L, ((eq(bnot(bor(bnot(b, 32), bnot(a, 32), 32), 32), 0L, 32)) ? (c) : (d)), 32), 64);
    }
    ...
    public static void Main() {
        ...
        while (line != null) {
            ...
            if (text.Length != 0) {
                ...
                try {
                    answers.Append(((ulong) emu_cmove_gpr_gpr_32__reg_rdi__csharp(values[0], values[1], values[2], values[3])).ToString());
                    answers.Append("\n");
