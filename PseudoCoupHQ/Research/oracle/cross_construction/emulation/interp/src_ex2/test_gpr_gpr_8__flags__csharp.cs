...
public static class Emu {
    ...
    // InterpRenderer from the layer-4 term of test_gpr_gpr_8__flags__csharp.
    // The term's layer-5 text, LITERAL:
    //   Concat(~(~Extract(7, 0, v0) | ~Extract(7, 0, v1)), 0)
    public static long emu_test_gpr_gpr_8__flags__csharp(long a, long b) {
        return m(cat(bnot(bor(bnot(a, 8), bnot(b, 8), 8), 8), 0L, 8), 16);
    }
    ...
    public static void Main() {
        ...
        while (line != null) {
            ...
            if (text.Length != 0) {
                ...
                try {
                    answers.Append(((ulong) emu_test_gpr_gpr_8__flags__csharp(values[0], values[1])).ToString());
                    answers.Append("\n");
