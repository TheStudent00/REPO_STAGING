...
// one named local per gate, over the term of sar_cl_gpr_8__reg_rdi__php.
// The term's layer-5 text, LITERAL:
//   Concat(Extract(63, 8, v0), Extract(7, 0, v0) >> Concat(0, Extract(4, 0, v1)))
function emu_sar_cl_gpr_8__reg_rdi__php($a, $b) {
    $x0_0 = ex_ext($a, 0, 0);
    $x0_1 = ex_ext($a, 1, 1);
    $x1_0 = ex_ext($b, 0, 0);
    $x0_2 = ex_ext($a, 2, 2);
    ...
    $w57 = ex_cat($w56, $g102, 1);
    $w58 = ex_cat($w57, $g96, 1);
    $w59 = ex_cat($w58, $g90, 1);
    $w60 = ex_cat($w59, $g86, 1);
    $w61 = ex_cat($w60, $g77, 1);
    $w62 = ex_cat($w61, $g60, 1);
    $w63 = ex_cat($w62, $g32, 1);
    return ex_m($w63, 64);
}
...
while (($line = fgets($handle)) !== false) {
    ...
    try {
        $answer = emu_sar_cl_gpr_8__reg_rdi__php(...$ints);
        if ($answer < 0) {
            echo sprintf("%s\n", sprintf("%u", $answer));
        } else {
            echo $answer . "\n";
