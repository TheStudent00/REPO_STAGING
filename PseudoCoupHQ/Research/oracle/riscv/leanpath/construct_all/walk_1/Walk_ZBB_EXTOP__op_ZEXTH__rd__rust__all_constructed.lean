...
theorem meaning_ZBB_EXTOP__op_ZEXTH__rd__rust__all_constructed (s : St) (a : BitVec 64) (h10 : s.regs.get? .x10 = some a) (hk0 : ∀ {α : Type} (x0 : α) (s : St), (plat_term_write x0) s = EStateM.Result.ok () s) (hk1 : ∀ (x0 : Arch.pa) (x1 : Nat) (s : St), (load_reservation x0 x1) s = EStateM.Result.ok () s) (hk2 : ∀ (x0 : Unit) (s : St), (cancel_reservation x0) s = EStateM.Result.ok () s) :
    runsTo (walk []) s
      (a) := by
  simp (config := {decide := true}) [runsTo, walk, step, execute, RETIRE_SUCCESS,
    ...
    csr_full_read_callback, csr_full_write_callback, csr_id_read_callback, csr_id_write_callback, csr_name_read_callback, csr_name_write_callback, fetch_callback, freg_write_callback, instret_callback, long_csr_write_callback, mem_exception_callback, mem_read_callback, mem_write_callback, pc_write_callback, ptw_fail_callback, ptw_start_callback, ptw_step_callback, ptw_success_callback, redirect_callback, tlb_add_callback, tlb_flush_begin_callback, tlb_flush_callback, tlb_flush_end_callback, trap_callback, vreg_write_callback, xreg_full_write_callback, xreg_write_callback, xret_callback, creg2reg_idx, fregidx_to_regidx, ra, regidx_offset_range, sp, t0, zreg, zero_extend, trunc, String, Sail.BitVec.extractLsb, Nat, Sail.Vector, Sail.BitVec.addInt, Sail.BitVec.zeroExtend, Sail.BitVec.truncate,
