...
// one named local per gate, over the term of setb_gpr_one_8__reg_rdi__dart.
// The term's layer-5 text, LITERAL:
//   Concat(Extract(63, 8, v0), If(ULE(Extract(7, 0, v1), Extract(7, 0, v2)), 0, 1))
int emu_setb_gpr_one_8__reg_rdi__dart(int a, int b, int c) {
  int x1_0 = ext(c, 0, 0);
  int x2_0 = ext(b, 0, 0);
  int x2_1 = ext(b, 1, 1);
  int x1_1 = ext(c, 1, 1);
  ...
  while (line != null) {
    ...
    if (text.isNotEmpty) {
      final List<int> values = text
          ...
          .toList();
      try {
        final int answer = emu_setb_gpr_one_8__reg_rdi__dart(values[0], values[1], values[2]);
