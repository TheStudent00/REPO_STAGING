...
import Sl1Lean.TypesExt
import Sl1Lean.Types
import Sl1Lean.VmemTypes
import Sl1Lean.SysRegs
import Sl1Lean.SyncException
import Sl1Lean.Mem
import Sl1Lean.VmemPte
import Sl1Lean.VmemPtw
import Sl1Lean.Callbacks0
import Sl1Lean.VmemTlb
...
def hgatp_mode (_ : Unit) : SailM HGATPMode := do
  ...
  let mbits ← (( do
    ...
    | .RV64 =>
      (do
        assert (xlen ≥b 64) "sys/vmem.sail:381.31-381.32"
    ...
    | .RV128 => (internal_error "sys/vmem.sail" 383 "RV128 not supported") ) : SailM hgatp_mode )
  match (← (hgatpMode_of_bits arch mbits)) with
  | .some m => (pure m)
  | none => (internal_error "sys/vmem.sail" 388 "invalid translation mode in hgatp")
...
mutual
/-- Type quantifiers: k_ex693734_ : Bool, level : Nat, k_ex693732_ : Bool, k_ex693731_ : Bool, sv_width
...
level).toNat
/-- Type quantifiers: k_ex693725_ : Bool, k_ex693724_ : Bool, sv_width : Nat, is_sv_or_svx4_mode(sv_width) -/
...
1000).toNat
/-- Type quantifiers: tlb_index : Nat, k_ex693718_ : Bool, k_ex693717_ : Bool, sv_width : Nat, is_sv_or_svx4_mode(sv_width), 0
...
1000).toNat
/-- Type quantifiers: k_ex693712_ : Bool, k_ex693711_ : Bool, sv_width : Nat, is_sv_or_svx4_mode(sv_width) -/
...
def translate_g_stage (gpa : (BitVec (if ( 64 = 32  : Bool) then 34 else 64))) (access : (MemoryAccessType mem_payload)) : SailM (Result ((BitVec (if ( 64
  ...
  then (pure (Ok (gpa, PBMT_PMA, init_ext_ptw)))
    ...
    (do
      ...
      assert (sv_width ≤b gpalen) "sys/vmem.sail:600.27-600.28"
...
def translate_stage (sv_width : Nat) (addr : (BitVec k_n)) (access : (MemoryAccessType mem_payload)) (priv : Privilege) (stage : TranslationStage) : SailM (Result ((BitVec (if ( 64
  ...
  | .Ok valid_addr =>
    (do
      assert (((xlen == 32) && ((sv_width == 32) || (sv_width == 34))) || ((xlen == 64) && ((sv_width != 32) && (sv_width != 34)))) "sys/vmem.sail:552.60-552.61"
...
def satp_mode (priv : Privilege) : SailM SATPMode := do
  if (((priv == Machine) || (not (← (currentlyEnabled Ext_S)))) : Bool)
  then (pure Bare)
  else
    (do
      let arch ← do (architecture Supervisor)
      let mbits ← (( do
        ...
        | .RV64 =>
          (do
            assert (xlen ≥b 64) "sys/vmem.sail:346.33-346.34"
        ...
        | .RV128 => (internal_error "sys/vmem.sail" 348 "RV128 not supported") ) : SailM satp_mode )
      match (← (satpMode_of_bits arch mbits)) with
      | .some m => (pure m)
      | none => (internal_error "sys/vmem.sail" 353 "invalid translation mode in satp"))
...
def vsatp_mode (_ : Unit) : SailM SATPMode := do
  ...
  let mbits ← (( do
    ...
    | .RV64 =>
      (do
        assert (xlen ≥b 64) "sys/vmem.sail:364.31-364.32"
        (pure (_get_Satp64_Mode (Mk_Satp64 (← readReg vsatp)))))
    | .RV32 =>
      (pure (0b000#3 +++ (_get_Satp32_Mode
            (Mk_Satp32 (Sail.BitVec.extractLsb (← readReg vsatp) 31 0)))))
    | .RV128 => (internal_error "sys/vmem.sail" 366 "RV128 not supported") ) : SailM satp_mode )
  match (← (satpMode_of_bits arch mbits)) with
  | .some m => (pure m)
  | none => (internal_error "sys/vmem.sail" 371 "invalid translation mode in vsatp")
...
def translate_vs_stage (vaddr : virtaddr) (access : (MemoryAccessType mem_payload)) (p : Privilege) : SailM (Result ((BitVec (if ( 64
  ...
  then (pure (Ok ((zero_extend (m := 64) (bits_of_virtaddr vaddr)), PBMT_PMA, init_ext_ptw)))
    ...
    (do
      ...
      assert (sv_width ≤b xlen) "sys/vmem.sail:587.25-587.26"
...
def build_exception_context (gva : virtaddr) (gpa : (BitVec (if ( 64 = 32  : Bool) then 34 else 64))) (access : (MemoryAccessType mem_payload)) (ptw_error : PTW_Error) (stage : TranslationStage) : SailM ExceptionContext := do
      ...
      | .PTW_Implicit_Error (e, pte_addr, access) =>
        (do
          let is_write ← (( do
            ...
            | .Store _ => (pure true)
            | .Load _ => (pure false)
            | _ => (internal_error "sys/vmem.sail" 643 "invalid memory access type") ) : SailM Bool
            )
          let tval2 :=
            if (sys_guest_page_fault_writes_htval : Bool)
            then (Sail.BitVec.truncate (pte_addr >>> 2) xlen)
            else (zeros (n := 64))
  ...
  | .G_Stage =>
    (do
      let tval2 :=
        if (sys_guest_page_fault_writes_htval : Bool)
        then (Sail.BitVec.truncate (gpa >>> 2) xlen)
        else (zeros (n := 64))
      (pure { trap := ← (pure (convertToGuestException
                    (← (translationException access ptw_error))))
              excinfo_is_gva := true
              excinfo := (zero_extend (m := 64) (bits_of_virtaddr gva))
              excinfo2 := (some tval2)
              excinst := none
...
def is_svukte_qualified (access : (MemoryAccessType mem_payload)) (eff_priv : Privilege) (mode : SATPMode) : SailM Bool := do
  let qualified_access_type :=
    ((access == (InstructionFetch ())) || (not (is_implicit_access access)))
  let ukte_enable ← do
    if (((eff_priv == VirtualUser) && ((← readReg cur_privilege) == User)) : Bool)
    then (pure (_get_Hstatus_HUKTE (← readReg hstatus)))
    else (pure (_get_SEnvcfg_UKTE (← (read_senvcfg ()))))
  (pure ((bne mode Bare) && (((eff_priv == User) || (eff_priv == VirtualUser)) && (qualified_access_type && (ukte_enable == 1#1)))))

def translateAddr (gva : virtaddr) (access : (MemoryAccessType mem_payload)) (p : Privilege) : SailM (Result (physaddr × page_based_mem_type × Unit) (ExceptionContext × Unit)) := SailME.run do
  ...
  if (((← (is_svukte_qualified access p mode)) && ((BitVec.access (bits_of_virtaddr gva)
           (xlen -i 1)) == 1#1)) : Bool)
  then
    (pure (Err
        ((make_exception_context (← (translationException access (PTW_Invalid_Addr ())))
          (zero_extend (m := 64) (bits_of_virtaddr gva)) is_virtual_access none), init_ext_ptw)))
  else
    (do
          ...
          then (pure (Ok ((Physaddr gpa), vs_pbmt, ext_ptw)))
            ...
            (do
              ...
              | .Err (ptw_error, ext_ptw) =>
                (pure (Err
                    ((← (build_exception_context gva gpa access ptw_error G_Stage)), ext_ptw)))))
      | .Err (ptw_error, ext_ptw) =>
        (pure (Err
