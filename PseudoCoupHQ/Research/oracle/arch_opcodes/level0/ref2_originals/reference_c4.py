...
(`PRIVATE/PseudoCoupHQ/Planning/node_0_3_research/node_0_3_5_compiler_graph/node_0_3_5_4_reference/CORE_0_3_5_4_reference.md`).
...
KEEPS_THE_UPPER_BITS = (8, 16)
"""the two write widths at which a general register KEEPS the bits
above the width written.  Intel SDM Vol. 1 section 3.4.1.1: a 32-bit
operand zero-extends into the 64-bit register, and an 8- or 16-bit
operand leaves the upper 56 or 48 bits UNMODIFIED."""


def full64(value, previous=None):
    """THE MACHINE'S OWN WRITE RULE into a general register.

    CORRECTED 2026-09-10 (task ref2, on the owner's word, brief
    `Research/briefs/task_ref2_brief.md` correction 1).  Until today
    this function zero-extended EVERY write below 64 bits, which is
    right at 32 and wrong at 8 and 16: `mov $1,%al` does not clear
    `%rax`.  `place_bits` below already carried the correct rule in
    its own docstring and applied it in the narrow division and
    widening-multiply forms alone; the general write path did not.
    Task ref1 measured the cost against the K-framework x86-64
    semantics (`PRIVATE/PseudoCoupHQ/DevComms/log_256...` section
    7.1): 116 disagreeing places over 27 mnemonics, every one of them
    this line.

    `previous` is the 64 bits the register held BEFORE the write, and
    it is what a write at 8 or 16 keeps.  A caller with no previous
    value is not writing a register -- it is SEEDING one from a row
    that resides there -- and gets the zero-extension that seeding
    means; `Operands.write_text` is the write path and it passes the
    previous value.
    """
    if value.size() == 64:
        return value
...
class FlagState(tuple):
    """THE FLAG STATE the last flag-setting arch opcode left.

    It IS the `(setter, left, right)` triple this file has always
    written -- every reader that unpacks three names still unpacks
    three names, and `isinstance(flags, tuple)` is still true -- and it
    carries ONE thing beside the three: the incoming carry, for the two
    opcodes whose own flags depend on it.

    WHY IT EXISTS (task ref2, 2026-09-10, on the owner's word, correction 2).
    `adc` and `sbb` compute `L + R + c` and `L - R - c`.  Until today
    `build_carry_binary` put `c` into the destination and left the
    triple `(mnemonic, L, R)` behind, so every flag a later opcode read
    after an `adc` or an `sbb` was computed WITHOUT the carry.  Task
    ref1 measured it against the K-framework x86-64 semantics
    (`PRIVATE/PseudoCoupHQ/DevComms/log_256...` section 7.2): 204
    disagreeing places over five flags.  Three w-bit slots cannot hold
    (L, R, c): folding the carry into either side loses the carry-out
    at `R = all ones` and the signed overflow at `R = 0x7f`, both of
    which z3 finds at once.  So the carry rides BESIDE the three rather
    than inside them, and the tuple's own shape does not change.

    `carry_in` is a z3 Bool, or nothing where the setter's flags do not
    depend on one.
    """

    def __new__(cls, setter, left, right, carry_in=None):
        return tuple.__new__(cls, (setter, left, right))

    def __init__(self, setter, left, right, carry_in=None):
        tuple.__init__(self)
        self.carry_in = carry_in


def carry_in_of(flags):
    """the incoming carry the flag state carries, or nothing.

    A plain three-tuple carries none: `model_translate.preseeded_state`
    builds one when it seeds a flag CONSUMER's model, and that is the
    one place in this pipeline where a flag state arrives without
    having been written by a builder here."""
    return getattr(flags, "carry_in", None)


UNRECORDED_CARRY_IN = (
    "the flag state names %r as its setter and carries no incoming "
    "carry, so the flags that opcode wrote cannot be rebuilt: `adc` "
    "and `sbb` compute L %s R %s c and every one of their flags is a "
    "function of c")

CARRY_SIGN = {"adc": ("+", "+"), "sbb": ("-", "-")}


def carry_value_of(flags, width):
    """the incoming carry as a `width`-bit 0 or 1, for the two setters
    whose own flags are a function of it.

    A flag state that names one of them and carries no carry is
    REFUSED BY CAUSE rather than answered with the carry left out --
    which is exactly the defect this correction closes."""
    carry_in = carry_in_of(flags)
    if carry_in is None:
        signs = CARRY_SIGN.get(flags[0], ("?", "?"))
        raise NotModeled(UNRECORDED_CARRY_IN
                         % (flags[0], signs[0], signs[1]))
    return z3.If(carry_in, z3.BitVecVal(1, width),
...
class Operands(object):
    ...
    def write_text(self, text, term):
        ...
        if self.is_vector(text):
            ...
            return
        if self.is_register(text):
            # THE PREVIOUS VALUE IS PART OF THE WRITE RULE (task ref2,
            # 2026-09-10): a write at 8 or 16 bits keeps the register's
            # other bits, so `full64` is handed what the register held.
            family = self.family_of(text)
...
#
# `sub` AND `sbb` WERE MISSING (task ref2, correction 4, 2026-09-10).
# Both end in `b`, both were read as byte-suffixed, and so
# `sub %esi,(%rax)` -- a 32-bit subtraction into memory -- was modelled
# at 8 bits.  Task ref1 measured it against the K-framework x86-64
# semantics (`PRIVATE/PseudoCoupHQ/DevComms/log_256...` section
# 7.4).  The audit of every other mnemonic this rule reaches is
# `level0/ref2_suffix_audit.py` and its json, which runs each builder
# with the mnemonic inside and outside this list and says whether the
# letter decides anything at all.
...
def build_carry_binary(ops):
    ...
    left, and set the flags from THE SAME SUM that carry entered.

    CORRECTED 2026-09-10 (task ref2, correction 2).  The line the
    correction replaces was `ops.state.flags = (ops.mnemonic, left,
    right)`: the carry went into the destination and not into the flag
    state, so every flag read after this opcode was computed from
    `L + R` where the machine computed `L + R + c`.  The flag state is
    now a `FlagState` carrying the same `carry_in` this line added, and
    `carry_bit`, `overflow_bit` and `flag_bits` all read it."""
    width = ops.destination_width()
    left = ops.read(1, width)
    right = ops.read(0, width)
...
def place_bits(old, value, offset):
    ...
    THE MACHINE'S OWN DESTINATION-WIDTH RULE: a 64-bit write replaces
    and a 32-bit write zero-extends, but an 8- or 16-bit write LEAVES
    THE UPPER BITS ALONE (`mov $1,%al` does not clear `%rax`).  Rule:
    machine_state CORE, 2026-09-03 (task 64).

    UNTIL 2026-09-10 only the narrow division and widening-multiply
    forms below used this, and the sentence here read "`full64` is
    untouched for every other opcode" -- which is exactly the defect
    task ref1 measured.  Task ref2 made `full64` the one write rule and
    it calls this function at 8 and 16, so the rule now has ONE
    statement and the narrow forms are its other callers rather than
    its only ones.
    """
    width = value.size()
    pieces = []
...
def carry_bit(flags):
    ...
    if setter in ("adc", "sbb"):
        # THE CARRY IS PART OF THE SUM (task ref2, correction 2).  The
        # borrow out of `L - R - c` is not `ULT(L, R)`: at `L = R` and
        # `c = 1` the machine borrows and `ULT(L, R)` is false.  Both
        # answers come out of the one-bit-wider arithmetic, which is
        # the same shape `add` already used.
        width = left.size()
...
def overflow_bit(flags):
    ...
    if setter in ("adc", "sbb"):
        # THE CARRY IS PART OF THE SUM (task ref2, correction 2).
        carry_value = z3.SignExt(1, carry_value_of(flags, width))
        if setter == "adc":
...
def flag_result(flags):
    """THE VALUE THE SETTER PRODUCED -- the one whose zero, sign and
    parity the machine left in `ZF`, `SF` and `PF`.

    Added 2026-09-10 (task ref2, correction 3).  Every builder in this
    file already states this value; until today nothing read it back.
    `condition_table.cond_to_z3` computed every condition on `L - R`
    instead, which is the result for the SUBTRACT family and for the
    families whose builders record `(mnemonic, result, 0)`, and is not
    the result for `add`, `adc`, `sbb` or `neg`.  Task ref1 measured it
    against the K-framework x86-64 semantics
    (`PRIVATE/PseudoCoupHQ/DevComms/log_256...` section 7.3).

    THE TABLE IS THE BUILDERS' OWN, one row per flag-setting opcode:
      ...
      `sbb`                 L - R - c
      `and` `or` `xor`
      `test` `inc` `bsr`    L, because those builders already record
                            the result in the left slot with a zero
                            right slot
      `neg`                 -L
      `imul` `mul`          L * R, the low half `build_binary` wrote.
                            Intel leaves `ZF`, `SF`, `PF` and `AF`
                            UNDEFINED after a multiply (ref1's own
                            undefined-region table), so this is our
                            reading of an undefined region and it is
                            not made better or worse by this task.

    A setter with no row here -- `bt`, whose zero and sign flags Intel
    leaves undefined, and the merged state -- is REFUSED BY CAUSE."""
    setter, left, right = flags
    if setter == MERGED_SETTER:
        raise NotModeled(MERGED_FLAG_REFUSAL)
    if setter == "add":
    ...
    if setter in ("sub", "cmp"):
        return left - right
    if setter == "adc":
        return left + right + carry_value_of(flags, left.size())
    if setter == "sbb":
        return left - right - carry_value_of(flags, left.size())
    if setter in ("and", "or", "xor", "test", "inc", "bsr"):
        return left
    if setter == "neg":
        return -left
    if setter in ("imul", "mul"):
        return left * right
    if setter == "bt":
        raise NotModeled(
            "the zero, sign and parity flags are read after `bt`, "
            "which writes only the carry flag and leaves those three "
            "undefined")
    raise NotModeled(
        "the flag-setting arch opcode %r states no result in this "
        "file, so the zero, sign and parity flags it left cannot be "
        "rebuilt" % setter)


class FlagReading(object):
    """the five flags of one flag state, each computed ONLY when it is
    asked for.

    Added 2026-09-10 (task ref2, correction 3).  Laziness is the point,
    not an optimisation: `bt` writes the carry and leaves the zero and
    sign flags undefined, so a reading that computed all five at once
    would refuse the carry too.  `AF` is absent because this file has
    never written a route to it, which task ref1 recorded as a refusal
    cause rather than a gap to be filled here."""
    ...
    def bit(self, name):
        if name == "CF":
            return carry_bit(self.flags)
        if name == "OF":
            return overflow_bit(self.flags)
        result = flag_result(self.flags)
        if name == "ZF":
            return result == z3.BitVecVal(0, result.size())
        if name == "SF":
            top = result.size() - 1
            return z3.Extract(top, top, result) == z3.BitVecVal(1, 1)
        if name == "PF":
            return even_parity(result)
        raise NotModeled(
            "this file writes no route to the flag %r: it holds the "
            "flags as (setter, left, right) and rebuilds a bit on "
            "demand, and %r is not one of the five it rebuilds" %
            (name, name))


def even_parity(result):
    """`PF` is the parity of the LOW BYTE of the result, and it is 1
    when the count of set bits there is EVEN.  The fold is the one
    `condition_table.cond_to_z3` already wrote for `CondPAR`, moved
    here so it reads the setter's result rather than a difference."""
    low = z3.Extract(7, 0, result)
    parity = z3.BitVecVal(0, 1)
    for index in range(8):
        parity = parity ^ z3.Extract(index, index, low)
    return parity == z3.BitVecVal(0, 1)


def predicate_of(state, suffix):
    """THE CONDITION ROUTE: the suffix names a synthetic condition, and
    the condition is a reading of THE FLAGS the last flag-setting
    opcode wrote.

    CORRECTED 2026-09-10 (task ref2, correction 3).  The line this
    replaces ended `return CT.cond_to_z3(condition, left, right, z3)`
    -- every condition computed on `L - R`, whatever the setter had
    done.  It is now `CT.cond_from_flags`, Intel's own condition table
    over `CF`, `ZF`, `SF`, `OF` and `PF`, and each of those five is
    computed from the setter's own arithmetic by `FlagReading.bit`.
    `cond_to_z3` keeps its meaning and its callers: it IS the reading
    for a setter whose result is `L - R`, which is what the render-back
    rule emits a `cmp` for.

    THREE SPECIAL CASES DISAPPEAR into the general rule and are named
    here so their absence is not read as a loss.  The carry route for a
    non-`cmp` setter is now `CondULT` reading `CF`.  The blanket
    refusal after `bt` is now `bt` answering `CF` and refusing `ZF`,
    `SF` and `PF` by cause, which is the same set of answers arrived at
    per flag instead of per setter.  And the `test` family's zero
    reading is `ZF` on the result its builder already recorded."""
    if suffix in ("o", "no"):
...
def merge_flags(condition, taken, otherwise):
    ...
    try:
        ...
        carry = merge_carry_in(condition, taken, otherwise)
    except NotModeled:
        ...
        return (MERGED_SETTER, None, None)
    return FlagState(taken[0], left, right, carry_in=carry)


def merge_carry_in(condition, taken, otherwise):
    """the incoming carry the merged flag state carries.

    Added 2026-09-10 with `FlagState` (task ref2, correction 2).  Two
    sides that both carry one merge it the way every other value
    merges; two sides that both carry none carry none; ONE side
    carrying one and the other not is two different flag states under
    one setter name, and the merge refuses by cause rather than
    picking a side."""
    left = carry_in_of(taken)
    right = carry_in_of(otherwise)
    if left is None and right is None:
        return None
    if left is None or right is None:
        raise NotModeled(
            "the two sides of a branch left the same flag-setting arch "
            "opcode %r and only one of them recorded an incoming carry"
            % (taken[0],))
    if same_term(left, right):
        return left
    return z3.If(condition, left, right)
