#!/bin/bash
# lp3_l119_every_external_the_corpus_calls_walked_at_rm0.sh
#
...
# ones time out.
#
# Fetches nothing. Writes $A/runs/float_rm0 only. Budget: four hours.
set -uo pipefail
...
SF = "PseudoCoupHQ/Research/oracle/riscv/softfloat_slices"
G  = "PseudoCoupHQ/Research/oracle/riscv/leanpath/runs/float_rm0"
...
for op in sorted(need):
    dis = os.path.join(SF, "flattened", "%s.rm0.value.flat.dis" % op)
    if not os.path.exists(dis):
        print("  MISSING slice: %s" % op); continue
    words = [m.group(1) for m in (WORD.match(l) for l in open(dis)) if m]
    if not words:
