...
import LeanIMZ.ZicfilpRegs
import LeanIMZ.StateenAccessChecks
...
def is_CSR_accessible (arg0 : (BitVec 12)) (arg1 : Privilege) (arg2 : CSRAccessType) : SailM Bool := do
  ...
  | (v__1124, g__94, g__95) =>
    (do
      if (((Sail.BitVec.extractLsb v__1124 11 4) == (0x3A#8 : (BitVec 8))) : Bool)
      then
        (let idx : (BitVec 4) := (Sail.BitVec.extractLsb v__1124 3 0)
        (pure ((sys_pmp_count >b (4 *i (BitVec.toNatInt idx))) && (((BitVec.access idx 0) == 0#1) || (xlen == 32)))))
      else
        (do
          if (((Sail.BitVec.extractLsb v__1124 11 4) == (0x3B#8 : (BitVec 8))) : Bool)
          then
            (let idx : (BitVec 4) := (Sail.BitVec.extractLsb v__1124 3 0)
            (pure (sys_pmp_count >b (BitVec.toNatInt (0b00#2 +++ idx)))))
          else
            (do
              if (((Sail.BitVec.extractLsb v__1124 11 4) == (0x3C#8 : (BitVec 8))) : Bool)
              then
                (let idx : (BitVec 4) := (Sail.BitVec.extractLsb v__1124 3 0)
                (pure (sys_pmp_count >b (BitVec.toNatInt (0b01#2 +++ idx)))))
              else
                (do
                  if (((Sail.BitVec.extractLsb v__1124 11 4) == (0x3D#8 : (BitVec 8))) : Bool)
                  then
                    (let idx : (BitVec 4) := (Sail.BitVec.extractLsb v__1124 3 0)
                    (pure (sys_pmp_count >b (BitVec.toNatInt (0b10#2 +++ idx)))))
                  else
                    (do
                      if (((Sail.BitVec.extractLsb v__1124 11 4) == (0x3E#8 : (BitVec 8))) : Bool)
                      then
                        (let idx : (BitVec 4) := (Sail.BitVec.extractLsb v__1124 3 0)
                        (pure (sys_pmp_count >b (BitVec.toNatInt (0b11#2 +++ idx)))))
                      else
                        (do
                          match (v__1124, g__94, g__95) with
                          | (0x001, g__96, g__97) =>
...
/-- Type quantifiers: k_ex512861_ : Bool -/
