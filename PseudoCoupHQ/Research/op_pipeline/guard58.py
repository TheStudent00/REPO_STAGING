#!/usr/bin/env python3
"""guard58.py -- THE GUARD for task 58: the UNMODIFIED
...
WHAT IS WALKED: `acceptance58.json`, `audit58.json` and every one of
the 332 verdict files in `gate58_store/` -- every JSON artifact task
58 writes, none skipped.  No artifact declares `role: generator
provenance`, nothing is added to any field set, and no exemption is
declared, so `grep -c exempt` over the transcript must be 0.
...
TRANSCRIPT = os.path.join(HERE, "guard58_transcript.txt")
SUMMARY = os.path.join(HERE, "guard58.json")


def task58_paths():
    out = []
    out.append(os.path.join(HERE, "acceptance58.json"))
    out.append(os.path.join(HERE, "audit58.json"))
    for name in sorted(os.listdir(os.path.join(HERE,
                                               "gate58_store"))):
        if not name.endswith(".json"):
            continue
        out.append(os.path.join(HERE, "gate58_store", name))
    live = []
    for path in out:
...
def main(argv):
    ...
    counts = run_guard(
        paths, TRANSCRIPT,
        "guard58.py -- every artifact task 58 writes, unmodified "
        "guard, ONE process.  Nothing was added to any field set, "
        "and no artifact was declared out of the walk.")
    print("TASK 58: %d paths  PASS %d  FAIL %d  exempt %d  exit %d"
