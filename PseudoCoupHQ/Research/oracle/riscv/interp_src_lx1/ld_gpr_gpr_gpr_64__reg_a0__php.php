...
// InterpRenderer from the layer-4 term of ld_gpr_gpr_gpr_64__reg_a0__php.
// The term's layer-5 text, LITERAL:
//   v0
function emu_ld_gpr_gpr_gpr_64__reg_a0__php($a) {
    return ex_m($a, 64);
}
...
while (($line = fgets($handle)) !== false) {
    ...
    try {
        $answer = emu_ld_gpr_gpr_gpr_64__reg_a0__php(...$ints);
        if ($answer < 0) {
            echo sprintf("%s\n", sprintf("%u", $answer));
        } else {
            echo $answer . "\n";
