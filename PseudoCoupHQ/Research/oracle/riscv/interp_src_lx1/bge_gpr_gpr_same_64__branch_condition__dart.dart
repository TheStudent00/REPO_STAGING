...
// InterpRenderer from the layer-4 term of bge_gpr_gpr_same_64__branch_condition__dart.
// The term's layer-5 text, LITERAL:
//   If(v0 <= v1, 1, 0)
int emu_bge_gpr_gpr_same_64__branch_condition__dart(int a, int b) {
  return m(((sle(b, a, 64)) ? (1) : (0)), 64);
}
  ...
  while (line != null) {
    ...
    if (text.isNotEmpty) {
      final List<int> values = text
          ...
          .toList();
      try {
        final int answer = emu_bge_gpr_gpr_same_64__branch_condition__dart(values[0], values[1]);
