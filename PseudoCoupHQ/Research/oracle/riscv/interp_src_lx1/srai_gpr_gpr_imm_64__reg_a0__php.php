...
// InterpRenderer from the layer-4 term of srai_gpr_gpr_imm_64__reg_a0__php.
// The term's layer-5 text, LITERAL:
//   v0 >> 3
function emu_srai_gpr_gpr_imm_64__reg_a0__php($a) {
    return ex_m(ex_ashr($a, 3, 64), 64);
}
...
while (($line = fgets($handle)) !== false) {
    ...
    try {
        $answer = emu_srai_gpr_gpr_imm_64__reg_a0__php(...$ints);
        if ($answer < 0) {
            echo sprintf("%s\n", sprintf("%u", $answer));
        } else {
            echo $answer . "\n";
