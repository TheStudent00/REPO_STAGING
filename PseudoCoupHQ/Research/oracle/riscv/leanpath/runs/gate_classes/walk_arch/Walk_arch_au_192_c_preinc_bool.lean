...
theorem meaning_arch_au_192_c_preinc_bool (s : St)  (hk0 : ∀ {α : Type} (x0 : α) (s : St), (plat_term_write x0) s = EStateM.Result.ok () s) (hk1 : ∀ (x0 : Arch.pa) (x1 : Nat) (s : St), (load_reservation x0 x1) s = EStateM.Result.ok () s) (hk2 : ∀ (x0 : Unit) (s : St), (cancel_reservation x0) s = EStateM.Result.ok () s) :
    runsTo (walk [LeanIM.instruction.C_LI (0x01#6, LeanIM.regidx.Regidx 0x0a#5)]) s
      ((pure_ITYPE (zero_reg) (((sign_extend (m := 12) (0x01#6)))) (ADDI))) := by
