...
Scope: `PRIVATE/PseudoCoupHQ/Research/op_pipeline/`, its upstream
`PRIVATE/PseudoCoupHQ/Research/kind_fuzz_clustering/`, and the
...
- **lane** — one shell script written into
  `PUBLIC/Airlock/agent/drop/`; the file write is the run.
...
|---|---|---|---|---|---|---|
| 0 | operator inventory | `PRIVATE/PseudoCoupHQ/Research/kind_fuzz_clustering/operator_arity.py` (1,464 lines) | the pinned grammar sources in `grammar_cache/`, plus the file's own hand-typed table | `operator_arity.json` (5,859 lines; 13 languages) | not re-run today (unverified) | YES — 136 `G(rule, file, ops)` rows, 1,561 operator spellings hand-typed; rust's row set carries 97 |
| 1 | probe generation | `PRIVATE/PseudoCoupHQ/Research/op_pipeline/probe_gen.py` (502 lines) | `operator_arity.json`; its own HOLDERS, result-type rules and five emitters | `probe_manifest_{c,cpp,go,rust,swift}.json` — 4,440 candidates (c 750, cpp 1,002, go 744, rust 858, swift 1,086) | 0.10 s, all five | YES — 30 holder rows, 58 lines of result-type rule, 5 source templates, 2 operator exclusions, 3 bucket exclusions |
| 2 | lane generation | `PRIVATE/PseudoCoupHQ/Research/op_pipeline/lane_gen.py` (587 lines) | the five manifests | `op_pipeline/lanes/op_<lang>.sh`, optionally copied into `PUBLIC/Airlock/agent/drop/` | 0.08 s, all five | YES, once per language — a 5-row tool check and a 5-language × 2-mode compile-flag table |
| 3 | extraction: compile-or-refuse, ANCHOR + SHIP, objdump, DWARF | the generated lane, run by the Airlock daemon | the lane's own embedded probe table | `PUBLIC/Airlock/agent/out/op_<lang>.txt` | rust 29.8 s today (24.1 s on record); c 42.4 s; go 48.5 s; cpp 74.1 s; swift 159.8 s | NO — one file write starts it; the compiler is the acceptance oracle and no acceptance table is consulted |
| 4 | fold | `PRIVATE/PseudoCoupHQ/Research/op_pipeline/fold.py` (184 lines) | the manifest and the lane output | `op_units_<lang>.json` | rust 0.05 s | NO |
...
| 9 | class table and dominant operators | `PRIVATE/PseudoCoupHQ/Research/op_pipeline/dominant_table12.py` | `canon{4,7,8,9,10,11,12,13}_units_<lang>.json`, `result_types_<lang>.json` | `dominant_table12.json` (926 classes, 1,641 members), `dom_ops10.json` (135 nodes, 26 dom_ops, 23 edgeless) | 0.20 s | NO |
...
| 12 | the spelling guard | `PRIVATE/PseudoCoupHQ/Research/op_pipeline/check_no_spelling_keys.py` | any grouping artifact plus the manifests | PASS/FAIL naming each offending path | 0.03 s | NO — its 78-token operator inventory is derived from the manifests, not declared |
...
`PRIVATE/PseudoCoupHQ/Research/kind_fuzz_clustering/acceptance_rust_A2.json`
...
`PRIVATE/PseudoCoupHQ/Research/compiler_graph/build_graph.py`
...
|---|---|---|
| operator spellings, all 13 languages | `PRIVATE/PseudoCoupHQ/Research/kind_fuzz_clustering/operator_arity.py` | 136 `G()` rows, 1,561 spellings |
| operator spellings, rust alone | same | 97 (33 of them reach this branch: 8 prefix, 3 postfix, 22 binary) |
| holder rows (which types are probed) | `PRIVATE/PseudoCoupHQ/Research/op_pipeline/probe_gen.py`, lines 76–119 | 30 rows (5 languages × 6 types), 44 lines |
...
| compile-flag table and tool checks | `PRIVATE/PseudoCoupHQ/Research/op_pipeline/lane_gen.py` | 5 tool checks, 10 compile commands (5 languages × ANCHOR/SHIP) |
| named refusal categories in the lifter | `PRIVATE/PseudoCoupHQ/Research/op_pipeline/expr_to_canon.py`, `REFUSAL_CATEGORY` | 37 |
...
| condition-suffix table | `PRIVATE/PseudoCoupHQ/Research/op_pipeline/condition_table.py`, `SUFFIX_TO_COND` | 28 |
| standard register names | `PRIVATE/PseudoCoupHQ/Research/op_pipeline/canon.py`, `GP_NAMES` | 8 families |
| temp register pool, ordered | `PRIVATE/PseudoCoupHQ/Research/op_pipeline/canon7_render.py`, `TEMP_POOL_ORDER` | 9 |
| reading-form templates | `PRIVATE/PseudoCoupHQ/Research/op_pipeline/reading_form.py` + `SPEC_reading_form.md` | ~35 dispatch arms; 55 spec template rows + 16 vocabulary rows; `FLAG_NAME` 15 |
| opcode glossary | `PRIVATE/PseudoCoupHQ/Research/op_pipeline/GLOSSARY_arch_opcodes.md` | 93 mnemonics, 227 lines (prose authored; the occurrence counts are derived from the corpus) |
...
|---|---|---|---|---|
| A | `PUBLIC/Airlock/agent/drop/d78_rust196.sh` | `rustc 1.96.1 (31fca3adb 2026-06-26)`, the image default | 29.8 s | `PUBLIC/Airlock/agent/out/d78_rust196.txt`, 104,927 bytes |
| B | `PUBLIC/Airlock/agent/drop/d78_rust198.sh` | `rustc 1.98.0 (88d9e12ae 2026-08-18)`, from `/persist/rustup076/` | 23.8 s | `PUBLIC/Airlock/agent/out/d78_rust198.txt`, 105,041 bytes |
...
assumed. The pin was not moved; `PUBLIC/Airlock/Containerfile`
line 82 still says `--default-toolchain 1.96.1`.

### the control: is the pipeline reproducible at all?

`PUBLIC/Airlock/agent/out/d78_rust196.txt` is **byte-identical**
to `PUBLIC/Airlock/agent/out/op_rust.txt`, the extraction
...
> `PUBLIC/Airlock/agent/out/d78_rust196.txt`, record
...
> `PUBLIC/Airlock/agent/out/d78_rust198.txt`, the same record:
>
> ```
> lea 0x0(%rip),%rdi !!reloc=R_X86_64_PC32:.data.rel.ro..Lanon.e999d8e928eeaaa2810ddee94f44a439.1-0x4
...
> `PUBLIC/Airlock/agent/out/d78_rust196.txt`,
...
`PRIVATE/PseudoCoupHQ/Research/op_pipeline/canon13_units_rust.json`:
...
`PRIVATE/PseudoCoupHQ/Research/op_pipeline/dominant_table12.json`
...
| measure | count |
|---|---|
| files under `PRIVATE/PseudoCoupHQ/Research/` naming the 1.96.1 `core` disambiguator | **41** |
...
> `PRIVATE/PseudoCoupHQ/Research/kind_fuzz_clustering/operator_arity.py`
...
and **0 times** in
`PRIVATE/PseudoCoupHQ/Research/kind_fuzz_clustering/grammar_cache/rust.js`.
...
|---|---|---|---|
| `PUBLIC/Airlock/agent/drop/d78_alg196.sh` | 1.96.1 | 0.4 s | 10 of 10 algebraic probes REFUSED; 2 of 2 controls extracted |
| `PUBLIC/Airlock/agent/drop/d78_alg198.sh` | 1.98.0 | 0.6 s | 10 of 10 algebraic probes accepted, SHIP and ANCHOR units extracted, DWARF read for all |
...
`PUBLIC/Airlock/agent/out/d78_alg198.txt`:
...
|---|---|
| language | `rust` |
| pinned version and where the pin lives | `1.96.1`, `PUBLIC/Airlock/Containerfile` line 82 |
...
| stored extraction to diff against | `PUBLIC/Airlock/agent/out/op_rust.txt`, 104,927 bytes |
...
- **The pin was not moved and the image was not rebuilt.** rust 1.98.0
  ...
  `PUBLIC/Airlock/Containerfile` line 82 would REPLACE 1.96.1
...
- **The five methods were extracted but nothing was installed.** No
  ...
  `PUBLIC/Airlock/agent/drop/d78_alg196.sh` and
...
- **`result_types_go.json` and `result_types_swift.json` do not
  ...
  flagged, not chased.
- **`bash PRIVATE/PseudoCoupHQ/hq.sh check` ends at 0 errors**
...
Lanes, in `PUBLIC/Airlock/agent/drop/` and archived by the
daemon into `.done/`:
...
Outputs, in `PUBLIC/Airlock/agent/out/`: `d78_rust196.txt`
(104,927 bytes), `d78_rust198.txt` (105,041 bytes),
`d78_alg196.txt`, `d78_alg198.txt`.

Logs, in `PUBLIC/Airlock/agent/logs/`, stamped `20260829T21*`
and `20260829T22*`.

Nothing under `PRIVATE/PseudoCoupHQ/Research/`,
`PRIVATE/PseudoCoupHQ/Planning/` or any census page was edited.
