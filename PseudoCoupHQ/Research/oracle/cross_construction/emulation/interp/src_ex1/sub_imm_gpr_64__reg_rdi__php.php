...
// InterpRenderer from the layer-4 term of sub_imm_gpr_64__reg_rdi__php.
// The term's layer-5 text, LITERAL:
//   v0 + 18446744073709551613
function emu_sub_imm_gpr_64__reg_rdi__php($a) {
...
while (($line = fgets($handle)) !== false) {
    ...
    try {
        $answer = emu_sub_imm_gpr_64__reg_rdi__php(...$ints);
        if ($answer < 0) {
            echo sprintf("%s\n", sprintf("%u", $answer));
        } else {
            echo $answer . "\n";
