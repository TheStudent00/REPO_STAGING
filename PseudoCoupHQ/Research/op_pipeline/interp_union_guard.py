#!/usr/bin/env python3
"""interp_union_guard.py -- the spelling guard on TASK 32's artifacts,
...
no top-level grouping field.  TASK 32's artifacts are grouping-shaped
and matching-shaped by construction -- `interp_table1.json` has rows
with members, `interp_join1.json` pairs an interpreter class with a
compiled class on every row, `union_table1.json` groups classes into
components, and `interp_canon34.json` carries the per-unit records
