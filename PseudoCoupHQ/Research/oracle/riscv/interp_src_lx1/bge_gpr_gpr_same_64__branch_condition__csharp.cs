...
public static class Emu {
    ...
    // InterpRenderer from the layer-4 term of bge_gpr_gpr_same_64__branch_condition__csharp.
    // The term's layer-5 text, LITERAL:
    //   If(v0 <= v1, 1, 0)
    public static long emu_bge_gpr_gpr_same_64__branch_condition__csharp(long a, long b) {
        return m(((sle(b, a, 64)) ? (1L) : (0L)), 64);
    }
    ...
    public static void Main() {
        ...
        while (line != null) {
            ...
            if (text.Length != 0) {
                ...
                try {
                    answers.Append(((ulong) emu_bge_gpr_gpr_same_64__branch_condition__csharp(values[0], values[1])).ToString());
                    answers.Append("\n");
