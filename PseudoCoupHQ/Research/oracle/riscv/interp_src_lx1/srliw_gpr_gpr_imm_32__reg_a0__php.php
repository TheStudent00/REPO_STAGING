...
// InterpRenderer from the layer-4 term of srliw_gpr_gpr_imm_32__reg_a0__php.
// The term's layer-5 text, LITERAL:
//   Concat(0, Extract(31, 3, v0))
function emu_srliw_gpr_gpr_imm_32__reg_a0__php($a) {
    return ex_m(ex_cat(0, ex_ext($a, 31, 3), 29), 64);
}
...
while (($line = fgets($handle)) !== false) {
    ...
    try {
        $answer = emu_srliw_gpr_gpr_imm_32__reg_a0__php(...$ints);
        if ($answer < 0) {
            echo sprintf("%s\n", sprintf("%u", $answer));
        } else {
            echo $answer . "\n";
