...
# InterpRenderer from the layer-4 term of shrd_cl_gpr_gpr_64__reg_rdi__cpython.
# The term's layer-5 text, LITERAL:
#   Extract(63, 0, LShR(Concat(v0, v1), Concat(0, Extract(5, 0, v2))))
def emu_shrd_cl_gpr_gpr_64__reg_rdi__cpython(a, b, c):
    return m(ext(lshr(cat(a, b, 64), cat(0, ext(c, 5, 0), 6), 128), 63, 0), 64)
...
def main():
    for line in sys.stdin:
        ...
        try:
            answer = emu_shrd_cl_gpr_gpr_64__reg_rdi__cpython(*values)
            sys.stdout.write("%d\n" % answer)
