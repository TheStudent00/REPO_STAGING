...
```
$ ls PRIVATE/PseudoCoupHQ/DevComms/ | grep -E "log_1(1[5-9]|2[0-6])"
...
rust   REFUSED: these lane files carry no '#verbatim-escape v1' marker, so they are legacy captures and must not be decoded: PUBLIC/Airlock/agent/out/op_rust.txt
rc=1
```
...
New, in `PRIVATE/PseudoCoupHQ/Research/op_pipeline/`:
...
New, in `PRIVATE/PseudoCoupHQ/DevComms/`:
