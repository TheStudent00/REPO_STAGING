...
// InterpRenderer from the layer-4 term of setns_gpr_one_8__reg_rdi__php.
// The term's layer-5 text, LITERAL:
//   Concat(0, If(0 <= ~(~Extract(7, 0, v0) | ~Extract(7, 0, v1)), 1, 0))
function emu_setns_gpr_one_8__reg_rdi__php($a, $b) {
    return ex_m(ex_cat(0, ((ex_sle(0, ex_bnot(ex_bor(ex_bnot($a, 8), ex_bnot($b, 8), 8), 8), 8)) ? (1) : (0)), 8), 64);
}
...
while (($line = fgets($handle)) !== false) {
    ...
    try {
        $answer = emu_setns_gpr_one_8__reg_rdi__php(...$ints);
        if ($answer < 0) {
            echo sprintf("%s\n", sprintf("%u", $answer));
        } else {
            echo $answer . "\n";
