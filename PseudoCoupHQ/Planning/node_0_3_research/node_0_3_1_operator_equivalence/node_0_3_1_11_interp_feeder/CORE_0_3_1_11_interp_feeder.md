...
- [target](node_0_3_1_11_6_target/CORE_0_3_1_11_6_target.md) — Which build of each interpreter is read, so a unit is attributed to a named binary and not to "php": the instrumented php builds (`~/Programming/Airlock/php-{7.4.33,8.2.13,8.3.0}.tar.gz`), the instrumented ruby 3.3.0 build, the image's cpython and openjdk 25.
- [output](node_0_3_1_11_7_output/CORE_0_3_1_11_7_output.md) — The records the feeder writes and where: per pilot, `canon_interp_units_{java,cpython}.json` and `canon_interp_units_ruby_php.json`; after the normalizer, `canon40_interp.json` (eleven units, ten with bodies), all under `~/Programming/PseudoCoupHQ/Research/op_pipeline/`.
...
- artifacts: `~/Programming/PseudoCoupHQ/Research/op_pipeline/`
