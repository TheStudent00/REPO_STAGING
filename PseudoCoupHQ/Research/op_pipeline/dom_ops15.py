#!/usr/bin/env python3
"""dom_ops15.py -- the dom_op (family) construction over dominant_
...
CONSUMING dominant_table17.json's own class rows rather than
...
present on each dominant_table17.json class-row member -- this does
...
dominant_table17.json with no operator token in it; this file adds a
`display_label` per node (the token's one-time-per-unit display use),
never as a key.

usage:
  dom_ops15.py [--out DIR] [--table PATH]
"""
...
def main(argv):
    outdir = HERE
    table_path = os.path.join(HERE, "dominant_table17.json")
    args = argv[1:]
    i = 0
    while i < len(args):
        if args[i] == "--out":
    ...
    dom_ops_path = os.path.join(outdir, "dom_ops15.json")
