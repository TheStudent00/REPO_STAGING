...
// InterpRenderer from the layer-4 term of fsw_fpr_mem_32__mem_MEM_0x0_a1___php.
// The term's layer-5 text, LITERAL:
//   Concat(Extract(63, 32, v0), Extract(31, 0, v1))
function emu_fsw_fpr_mem_32__mem_MEM_0x0_a1___php($a, $b) {
    return ex_m(ex_cat(ex_ext($a, 63, 32), $b, 32), 64);
}
...
while (($line = fgets($handle)) !== false) {
    ...
    try {
        $answer = emu_fsw_fpr_mem_32__mem_MEM_0x0_a1___php(...$ints);
        if ($answer < 0) {
            echo sprintf("%s\n", sprintf("%u", $answer));
        } else {
            echo $answer . "\n";
