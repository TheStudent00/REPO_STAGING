...
// InterpRenderer from the layer-4 term of cmovne_gpr_gpr_32__reg_rdi__javascript.
// The term's layer-5 text, LITERAL:
//   Concat(0, If(~(~Extract(31, 0, v0) | ~Extract(31, 0, v1)) == 0, Extract(31, 0, v2), Extract(31, 0, v3)))
function emu_cmovne_gpr_gpr_32__reg_rdi__javascript(a, b, c, d) {
    return m(cat(0n, ((eq(bnot(bor(bnot(b, 32), bnot(a, 32), 32), 32), 0n, 32)) ? (d) : (c)), 32), 64);
}
...
for (const line of lines) {
    ...
    try {
        out.push(emu_cmovne_gpr_gpr_32__reg_rdi__javascript(...values).toString());
    } catch (problem) {
