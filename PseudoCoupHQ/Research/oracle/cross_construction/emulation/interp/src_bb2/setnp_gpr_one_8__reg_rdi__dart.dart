...
// one named local per gate, over the term of setnp_gpr_one_8__reg_rdi__dart.
...
int emu_setnp_gpr_one_8__reg_rdi__dart(int a, int b, int c) {
  int x1_0 = ext(a, 0, 0);
  int x0_0 = ext(b, 0, 0);
  int x1_1 = ext(a, 1, 1);
  int x0_1 = ext(b, 1, 1);
  ...
  while (line != null) {
    ...
    if (text.isNotEmpty) {
      final List<int> values = text
          ...
          .toList();
      try {
        final int answer = emu_setnp_gpr_one_8__reg_rdi__dart(values[0], values[1], values[2]);
