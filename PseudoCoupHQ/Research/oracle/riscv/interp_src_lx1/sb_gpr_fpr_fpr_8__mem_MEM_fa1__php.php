...
// InterpRenderer from the layer-4 term of sb_gpr_fpr_fpr_8__mem_MEM_fa1__php.
// The term's layer-5 text, LITERAL:
//   Concat(Extract(63, 8, v0), Extract(7, 0, v1))
function emu_sb_gpr_fpr_fpr_8__mem_MEM_fa1__php($a, $b) {
    return ex_m(ex_cat(ex_ext($a, 63, 8), $b, 8), 64);
}
...
while (($line = fgets($handle)) !== false) {
    ...
    try {
        $answer = emu_sb_gpr_fpr_fpr_8__mem_MEM_fa1__php(...$ints);
        if ($answer < 0) {
            echo sprintf("%s\n", sprintf("%u", $answer));
        } else {
            echo $answer . "\n";
