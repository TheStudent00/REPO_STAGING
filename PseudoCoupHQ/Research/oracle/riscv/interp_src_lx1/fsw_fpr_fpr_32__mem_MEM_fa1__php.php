...
// InterpRenderer from the layer-4 term of fsw_fpr_fpr_32__mem_MEM_fa1__php.
// The term's layer-5 text, LITERAL:
//   Concat(Extract(63, 32, v0), Extract(31, 0, v1))
function emu_fsw_fpr_fpr_32__mem_MEM_fa1__php($a, $b) {
...
while (($line = fgets($handle)) !== false) {
    ...
    try {
        $answer = emu_fsw_fpr_fpr_32__mem_MEM_fa1__php(...$ints);
        if ($answer < 0) {
            echo sprintf("%s\n", sprintf("%u", $answer));
        } else {
            echo $answer . "\n";
