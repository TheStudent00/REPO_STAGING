#!/usr/bin/env bash
# t91_l5_regate_unattached_control.sh -- TASK 91, lane 5: the whole canon38
# population (30,436 units) put to the ONE reference in configuration
# 'no_attached_callees', run label 'control'.
#
...
#
# Product: t91_regate_store_no_attached_callees_control/ in the project tree.
set -uo pipefail
...
for i in 0 1 2 3; do
  python3 t91_regate_run.py --configuration=no_attached_callees \
      --label=control --shard=$i/4 > /tmp/t91_l5_shard_$i.log 2>&1 &
done
wait
...
for i in 0 1 2 3; do
  echo "=================== shard $i/4 ==================="
  tail -6 /tmp/t91_l5_shard_$i.log
  if ! grep -q "finished" /tmp/t91_l5_shard_$i.log; then
    echo "SHARD $i DID NOT FINISH -- full log follows"
    cat /tmp/t91_l5_shard_$i.log
    status=4
  fi
done
...
echo ""
echo "store contents:"
ls t91_regate_store_no_attached_callees_control | wc -l
exit $status
