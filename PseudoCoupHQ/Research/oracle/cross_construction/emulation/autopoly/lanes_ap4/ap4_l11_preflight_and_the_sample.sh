#!/usr/bin/env bash
# ap4_l11_preflight_and_the_sample.sh -- task ap4: the outer set as the
...
# collecting process, named abort ABORT_MEMORY_AP4, checked after every
# run by `autopoly4.check_memory`.  The sample is the first twenty runs
...
echo "[1/2] preflight"
python3 autopoly4.py preflight
echo ""
echo "[2/2] the twenty-run sample"
python3 autopoly4.py run 20
echo "done"
