...
// one named local per gate, over the term of test_gpr_same_32__flags__dart.
// The term's layer-5 text, LITERAL:
//   Concat(Extract(31, 0, v0), 0)
int emu_test_gpr_same_32__flags__dart(int a) {
  int x0_0 = ext(a, 0, 0);
  int x0_1 = ext(a, 1, 1);
  int x0_2 = ext(a, 2, 2);
  int x0_3 = ext(a, 3, 3);
  ...
  int w48 = cat(w47, k0, 1);
  int w49 = cat(w48, k0, 1);
  int w50 = cat(w49, k0, 1);
  int w51 = cat(w50, k0, 1);
  int w52 = cat(w51, k0, 1);
  int w53 = cat(w52, k0, 1);
  int w54 = cat(w53, k0, 1);
  int w55 = cat(w54, k0, 1);
  ...
  while (line != null) {
    ...
    if (text.isNotEmpty) {
      final List<int> values = text
          ...
          .toList();
      try {
        final int answer = emu_test_gpr_same_32__flags__dart(values[0]);
