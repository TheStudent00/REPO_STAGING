...
// InterpRenderer from the layer-4 term of fld_fpr_fpr_fpr_64__freg_fa0__php.
// The term's layer-5 text, LITERAL:
//   v0
function emu_fld_fpr_fpr_fpr_64__freg_fa0__php($a) {
    return ex_m($a, 64);
}
...
while (($line = fgets($handle)) !== false) {
    ...
    try {
        $answer = emu_fld_fpr_fpr_fpr_64__freg_fa0__php(...$ints);
        if ($answer < 0) {
            echo sprintf("%s\n", sprintf("%u", $answer));
        } else {
            echo $answer . "\n";
