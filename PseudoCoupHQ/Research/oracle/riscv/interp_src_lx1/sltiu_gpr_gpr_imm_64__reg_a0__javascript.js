...
// InterpRenderer from the layer-4 term of sltiu_gpr_gpr_imm_64__reg_a0__javascript.
// The term's layer-5 text, LITERAL:
//   If(Or(Extract(1, 0, v0) == 3, Not(Extract(63, 2, v0) == 0)), 0, 1)
function emu_sltiu_gpr_gpr_imm_64__reg_a0__javascript(a) {
    return m(((((eq(ext(a, 1, 0), 3n, 2)) || ((!(eq(ext(a, 63, 2), 0n, 62)))))) ? (0n) : (1n)), 64);
}
...
for (const line of lines) {
    ...
    try {
        out.push(emu_sltiu_gpr_gpr_imm_64__reg_a0__javascript(...values).toString());
    } catch (problem) {
