#!/usr/bin/env python3
"""autopoly5.py -- task ap5: AutoPoly's loop, FIFTH pass.

THE TWO MECHANICAL REMAINDERS OF THE FOUR LANGUAGES, each closed in
the layer that owns it, and then task ap4's loop run again.

  * THE x87 STACK, READ END TO END.  Task ap4 put the ledger's edges
    in (`fstpt OUT-0`, `fldt IN-k`) and then had to read the answer
    and align the arrivals IN THE DRIVER, because the two shared
    functions that do those things for every other place --
    `reference.answer_of` and
    `pool100_entry_equivalence.align_by_row` -- cannot read an x87
    value and were not files its brief named.  This task's brief names
    them: `answer_of` reads an `X87_<k>` home off the reference's own
    x87 stack, `align_by_row` aligns an x87 arrival by its IN row like
    any other, and the driver's two stand-ins are gone
    (`emulate.x87_answer_for_unit` removed; `handful.x87_aligned`
    keeps only the EMULATION's side, which is c's calling rule for a
    `long double` argument and is the driver's own business).
  * THE IMMEDIATE AS AN INPUT OF THE MAPPING.  A cell key is (`mnem`,
    operand shape, `key_width`) and carries no immediate, so the
    sweep's `imm_*` spelling bakes its own `$0x3` in while the corpus
    rows the cell is attested by spell `$0x1`, `$0x8` and the rest.
    `model_translate.shapes_for` now spells each `imm_*` shape a
    second time with a register of the operand's own width in the
    immediate's slot -- which is how the reference's own operand
    reader spells a value of that width that is not a literal -- the
    cells file carries those rows marked `imm_symbolic`, and the
    driver chooses them (`handful.the_symbolic_immediate_row`), takes
    the immediate as one more parameter and does not ask the primitive
    route, whose lookup key carries no immediate.

The change table reads task ap4's store as the before.

--- what follows is task ap4's own docstring, kept because the loop is
--- its loop and nothing about the four steps has moved.
...
RUNS = os.path.join(HERE, "autopoly5_runs.jsonl")
AGGREGATE = os.path.join(HERE, "autopoly5.json")
REPORT = os.path.join(HERE, "autopoly5.md")
SRC_DIR = os.path.join(HERE, "src5")
PRIMITIVE = os.path.join(HERE, "autopoly5_primitive.json")
SPELLINGS = os.path.join(HERE, "autopoly5_spellings.json")
...
OFF_RUNS = os.path.join(HERE, "autopoly5_off_runs.jsonl")
OFF_SRC_DIR = os.path.join(HERE, "src5_off")
...
BEFORE_RUNS = os.path.join(HERE, "autopoly4_runs.jsonl")
"""the store the change table and `measure` read as the BEFORE: task
ap4's, which is the pass this one follows."""
...
def use_task_ap5():
    ...
    memory bound, and at task ap4's route WITH TASK ap5'S TWO CHANGES.
    ...
    lookup).  `handful.use_task_ap5` answers all three the way `ap4`
    answers them, so the ROUTE is unchanged; what differs is the two
    remainders task ap4 left, answered in the layer that owns each --
    the reference and the shared aligner for the x87 stack, the sweep's
    own operand spellings and the driver's route choice for the
    immediate -- and neither gated on a task name
    (`handful.use_task_ap5`'s own docstring says why).

    Every path `handful.py` writes through is repointed into this
    folder, and every one of them is an `autopoly5_*` name, so no lane
    of this task can write task ap1's, ap2's, ap3's or ap4's
    products."""
    H.use_task_ap5()
    H.RESULTS = AGGREGATE
    H.REPORT = REPORT
...
def change_command():
    """THE CHANGE TABLE: task ap4's loop beside this one, cause by cause
    and cell by cell.

    Read off the two stores, run for run, joined on the (cell, target)
    pair.  Task ap4 ran all 1,012, so every pair is on both stores; a
    pair that were not would show as `not run in ap4`."""
    ...
    say("runs on task ap4's store: %d" % len(theirs))
    say("runs on task ap5's store: %d" % len(mine))
    ...
    say("Table C1 -- every cause of task ap4's loop, what it counted "
        "then, and what those same runs say now.")
    say("")
    say("| task ap4's cause | ap4 runs | ap5 runs, same cause | where "
        "the rest went |")
    say("|---|---|---|---|")
    ...
    # few there are.
    say("Table C2 -- runs task ap4 proved that task ap5 does not.")
    say("")
    say("| cell | lang | ap5's cause, which task ap4 did not have |")
    say("|---|---|---|")
    regressed = 0
    for key in sorted(theirs):
    ...
    say("")
    say("runs task ap4 proved and task ap5 does not: %d" % regressed)
    say("")
    ...
    say("")
    say("| cell | lang | ap4's cause | the region the counterexample "
        "names |")
    say("|---|---|---|---|")
    to_sat = 0
...
def measure_command(wanted):
    ...
    `wanted` is a substring of the cause task ap4 recorded.  This
    command reads task ap4's own store, takes EXACTLY the (cell,
    target) pairs whose cause contains it, runs each one again through
    the driver as task ap5 leaves it, and prints the cause before and
    ...
    for run in read_runs(BEFORE_RUNS):
        ...
        if cause is None or wanted not in cause:
            continue
        before.append(run)
    say("task ap4's own runs carrying a cause holding %r: %d"
        % (wanted, len(before)))
    say("")
    counted = {}
    say("| cell | lang | ap4's cause | ap5's cause |")
    say("|---|---|---|---|")
    index = 0
    for run in before:
        index = index + 1
...
def x87_command():
    """EVERY x87 PLACE OF THE LOOP, with the gate's verdict in z3's own
    words, and every `sat` with its counterexample.

    The brief's own deliverable for the first change.  A place is on
    this list exactly when the cell it belongs to is at `key_width`
    80, which is the width the model table gives an x87 register (task
    m1b's one width rule) -- so the list is the population and not a
    search, and nothing about it is chosen by a mnemonic."""
    runs = read_runs()
    rows = []
    for run in runs:
        if run.get("key_width") != 80:
            continue
        for place in (run.get("places") or []):
            rows.append((run, place, place.get("check") or {}))
        if not run.get("places"):
            rows.append((run, None, {}))
    say("x87 places, over %d run(s) at key_width 80: %d"
        % (len([r for r in runs if r.get("key_width") == 80]),
           len(rows)))
    say("")
    say("| cell | lang | place | route | the gate's verdict | z3's own "
        "words |")
    say("|---|---|---|---|---|---|")
    counted = {}
    for run, place, check in rows:
        ...
        if place is None:
            verdict = "REFUSED BEFORE THE GATE"
            words = first_line("%s: %s" % (run.get("refusal_cause"),
                                           run.get("refusal_detail")))
        elif place.get("rendered") is False:
            verdict = "NOT_RENDERED"
            words = first_line(str(place.get("refusal_cause")))
        else:
            verdict = verdict or "--"
            words = first_line(check.get("reason")
                               or check.get("counterexample")
                               or check.get("z3") or "--")
        counted[verdict] = counted.get(verdict, 0) + 1
        say("| `%s` %s %s | %s | %s | %s | %s | %s |"
            % (run["mnem"], run["shape"], run["key_width"], run["lang"],
               "--" if place is None else place.get("writes"),
               run.get("route") or "--", verdict, words[:150]))
    say("")
    say("the x87 places, by verdict:")
    for verdict in sorted(counted, key=lambda v: (-counted[v], v)):
        say("   %4d  %s" % (counted[verdict], verdict))
    say("")
    say("every x87 place the gate answered `sat` on, with its "
        "counterexample LITERAL:")
    found = 0
    for run, place, check in rows:
        if place is None:
            continue
        ...
        say("   `%s` %s %s / %s  place `%s`"
            % (run["mnem"], run["shape"], run["key_width"], run["lang"],
               place.get("writes")))
        say("      %s" % check.get("counterexample"))
    if not found:
        say("   none")
    say("")
    say("the aligned IN rows, where the x87 branch built them:")
    for run, place, check in rows:
        if place is None or not check.get("aligned_rows"):
            continue
        say("   `%s` %s %s / %s  place `%s`"
            % (run["mnem"], run["shape"], run["key_width"], run["lang"],
               place.get("writes")))
        for entry in check["aligned_rows"]:
            say("      %s: the cell reads %s; the body reads %s"
                % (entry.get("row"), entry.get("the cell reads"),
                   entry.get("the body reads")))
    say("peak resident: %d kB" % peak_kb())
    return 0


def immediate_command():
    """EVERY imm_* CELL, before and after, with the corpus's own
    immediates counted under the one cell.

    A cell is on this list exactly when the cells file gives it a row
    marked `imm_symbolic` OR its own asked shape begins `imm` -- both
    read off the file's own fields, neither off an operand text."""
    cells = read_json(CELLS)
    mine = read_runs()
    theirs = read_runs(BEFORE_RUNS)
    before = {}
    for run in theirs:
        before[(run["mnem"], run["shape"], run["key_width"],
                run["lang"])] = (route_of(run), outcome_of(run))
    after = {}
    for run in mine:
        after[(run["mnem"], run["shape"], run["key_width"],
               run["lang"])] = (route_of(run), outcome_of(run))
    rows = []
    for record in cells["asked"]:
        if not (record["asked"].get("shape") or "").startswith("imm"):
            continue
        rows.append(record)
    rows.sort(key=lambda r: -r["attested_ledger_rows"])
    symbolic = 0
    for record in rows:
        for row in record["rows"]:
            if row.get("imm_symbolic"):
                symbolic = symbolic + 1
                break
    say("imm_* cells on the outer set: %d" % len(rows))
    say("of those, cells the cells file gives a symbolic-immediate "
        "row: %d" % symbolic)
    say("cells on the outer set, before and after: %d and %d"
        % (len(read_json(CELLS)["asked"]), len(cells["asked"])))
    say("")
    say("Table I1 -- every imm_* cell, task ap4's route and verdict "
        "against task ap5's, per target.")
    say("")
    say("| cell | symbolic row | c | rust | go | swift |")
    say("|---|---|---|---|---|---|")
    for record in rows:
        asked = record["asked"]
        key = (asked["mnem"], asked["shape"], asked["key_width"])
        got = "yes"
        if not any(r.get("imm_symbolic") for r in record["rows"]):
            got = "no: %s" % (the_symbolic_refusal(record) or "--")
        said = []
        for lang in TARGETS:
            was = before.get(key + (lang,))
            now = after.get(key + (lang,))
            said.append("%s -> %s"
                        % ("--" if was is None else "%s / %s" % was,
                           "--" if now is None else "%s / %s" % now))
        say("| `%s` %s %s | %s | %s |"
            % (key[0], key[1], key[2], got, " | ".join(said)))
    say("")
    say("Table I2 -- the corpus's own immediates, counted under the "
        "one cell: every distinct constant the single-opcode rows that "
        "classify to the cell spell, over the four targets.")
    say("")
    say("| cell | rows | distinct immediates |")
    say("|---|---|---|")
    for record in rows:
        asked = record["asked"]
        seen = {}
        ...
        for lang in TARGETS:
            ...
            for row in (found.get("candidate_rows") or []):
                total = total + 1
                for hit in IMMEDIATE_IN_A_BODY.findall(
                        row.get("body_text") or ""):
                    seen[hit] = seen.get(hit, 0) + 1
        say("| `%s` %s %s | %d | %s |"
            % (asked["mnem"], asked["shape"], asked["key_width"],
               total, ", ".join("`$%s`" % k for k in sorted(seen))
               or "--"))
    say("peak resident: %d kB" % peak_kb())
    return 0


IMMEDIATE_IN_A_BODY = re.compile(r"\$(-?(?:0x[0-9a-fA-F]+|\d+))")
"""an immediate as a corpus body spells it -- `reference.IMMEDIATE_RE`
without its anchors, because a body text holds the operand inside a
line rather than alone."""


def the_symbolic_refusal(record):
    """the reference's own words for why this cell has no
    symbolic-immediate row, as the cells file recorded them."""
    for row in record["rows"]:
        if row.get("imm_symbolic_refusal"):
            return row["imm_symbolic_refusal"]
    return record.get("imm_symbolic_refusal")
...
def write_head(lines, document):
    meta = document["meta"]
    lines.append("# autopoly5.md -- task ap5: AutoPoly's loop, fifth pass")
    lines.append("")
