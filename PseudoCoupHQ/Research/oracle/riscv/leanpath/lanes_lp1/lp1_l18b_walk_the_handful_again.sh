#!/bin/bash
# lp1_l18b_walk_the_handful_again.sh -- lane 18 again: the executable library's own name and namespaces, and the decoder run on a blank state the model's init_model filled.
...
OUT=$A/walk_handful_6266b40c_8eb1fb6b_IMZ_b
...
f=$(python3 - <<'PY'
import json
d=json.load(open("PseudoCoupHQ/Research/oracle/riscv/leanpath/walk_handful_6266b40c_8eb1fb6b_IMZ_b/walk.json"))
