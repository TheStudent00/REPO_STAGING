# log_251 — task l3: the check's composer names both sides by the stored line's own rule (19 DISCREPANCY → 0), and the native-evaluation trust class measured
...
`<runs>/l3/agent/logs/20260910T024319Z__l3_l2_verify_the_nineteen_b.sh.log`.
...
`l3_l6_read_the_toolchain_on_native_evaluation.sh` §[3/5], host log
`<runs>/l3/agent/logs/20260910T025619Z__l3_l6_read_the_toolchain_on_native_evaluation.sh.log` (§9 [6/8] re-runs it):
...
**LITERAL**, lane `l3_l9_trust_classes_guard_and_build.sh` §[3/6] (host log
`<runs>/l3/agent/logs/20260910T031250Z__l3_l9_trust_classes_guard_and_build.sh.log`) — `lake build` of the whole project, exit 0, 318 jobs:
...
**LITERAL**, lane `l3_l9_trust_classes_guard_and_build.sh` §[5/6] (host log
`<runs>/l3/agent/logs/20260910T031250Z__l3_l9_trust_classes_guard_and_build.sh.log`) — the guard run unmodified over every json this task wrote, its output filtered to the
...
this log. Five passes; the first three ran while this log was numbered 250,
which task ex2 had also taken (`log_250_task_ex2_the_interpreted_loop.md`),
so it was renumbered to 251 and the checker was run again over the new path,
and once more after three attributions were given their host lane log paths.
The last two passes read the same numbers, which is the tally below. Pass 1 (`l3_l11_verify.sh`, host log
`<runs>/l3/agent/logs/20260910T032000Z__l3_l11_verify.sh.log`)
...
`<runs>/l3/agent/logs/20260910T032042Z__l3_l12_verify2.sh.log`)
reads, **LITERAL**:
...
Pass 3 (`l3_l13_verify3.sh`, host log
`<runs>/l3/agent/logs/20260910T032125Z__l3_l13_verify3.sh.log`)
ran with this section present and read 30 claims, 8 MATCHES, **0 DIFFERS**,
22 UNVERIFIABLE — the two extra claims being this section's own prose and its
own pasted tally. Pass 4 is the run of record, over the renumbered path
(`l3_l14_verify4_renumbered.sh`, host log
`<runs>/l3/agent/logs/20260910T032329Z__l3_l14_verify4_renumbered.sh.log`).

**TALLY LINE (passes 4 and 5, the runs of record): claims 29 | MATCHES 8 | DIFFERS 0 |
UNVERIFIABLE 21 | REFUSED 0 | NOT_RERUNNABLE 0. Zero DIFFERS.** Pass 5
(`l3_l15_verify5_final.sh`, host log
`<runs>/l3/agent/logs/20260910T032513Z__l3_l15_verify5_final.sh.log`)
reads the same four numbers over the file as this task finally leaves it. The 8 MATCHES
are §9's eight commands. The 21 UNVERIFIABLE are ten prose sentences and
eleven attributions, each of the latter carrying its host lane log path.
Passes 3, 4 and 5 are not pasted here as live transcripts, because a transcript
that compares its own output to itself is the self-reference log 232 §11
hit.
