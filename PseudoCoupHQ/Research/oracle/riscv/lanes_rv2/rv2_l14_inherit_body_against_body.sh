#!/usr/bin/env bash
# rv2 lane 14 -- THE INHERITANCE. Every preferred proved/agreed
...
# Lane 12 then bound EVERY source by argument position and proved fewer
# (27 against 87), which said the rule depends on WHICH ROUTE wrote the
# source: a RENDERED source declares one parameter per family in the
# term's own printed order, and a PRIMITIVE source is a corpus probe
# whose parameter k arrives in the target's k-th argument register. The
# certificate records its route, so this lane reads it. Lane 13 still
# disproved 17 rows whose two terms PRINT THE SAME TEXT -- the cell's
# symbols are named after the SWEEP'S operand registers (%rdi, %rsi)
# whatever the language, and which argument feeds which operand is the
# compiler's choice. So this lane stops aligning to the cell at all: it
# walks the certificate's OWN x86-64 body beside the riscv64 one, both as
# functions of the same arguments (argument k in rdi/rsi/... and in a0..),
# which is task rv1's own method. The bank already proved the x86-64 body
# computes the cell's term; the two together are the inheritance.
set -u
