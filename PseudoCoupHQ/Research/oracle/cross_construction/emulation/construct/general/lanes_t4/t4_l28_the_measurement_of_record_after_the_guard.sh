#!/bin/bash
# t4_l28_the_measurement_of_record_after_the_guard.sh -- task t4, lane
# 28: lane 27 again, with the three files the spelling guard refused
# corrected.
#
# THE GUARD WAS RIGHT ALL THREE TIMES, and nothing about it was changed.
# `vocabulary_x86.json` and `vocabulary_riscv64.json` carried the z3
# DECLARATION NAME on a field called `kind`, and four of those names --
# `=`, `and`, `not`, `or` -- are operator tokens on a structure field;
# the name now sits in the field `mnem`, which the ruling of 2026-09-08
# states is machine form and which the guard exempts, and which is the
# same remedy task ref2 applied to three of its own files (log_261 §0).
# `general_measurement.json` carried `--` as the landing of a place that
# has none, and `--` is a range in swift and in ruby; a place with no
# landing recorded now says so in words.
#
# (Everything below is lane 27's, unchanged.)  The bank
