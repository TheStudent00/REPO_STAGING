...
for f in $LP/leanpath/*.py $OUT $LP/lanes_lp1/lp1_l6_*.sh $LP/lanes_lp1/lp1_l7_*.sh $LP/lanes_lp1/lp1_l7b_*.sh $LP/lanes_lp1/lp1_l8_*.sh $LP/lanes_lp1/lp1_l9_*.sh $LP/lanes_lp1/lp1_l9b_*.sh; do
  echo "    $(grep -c exempt "$f") $(basename "$f")"
done
echo "lane lp1_l9b done at $(date -u +%FT%TZ)"
