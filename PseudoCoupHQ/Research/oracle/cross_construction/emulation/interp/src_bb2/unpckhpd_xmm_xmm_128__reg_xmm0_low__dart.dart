...
// one named local per gate, over the term of unpckhpd_xmm_xmm_128__reg_xmm0_low__dart.
// The term's layer-5 text, LITERAL:
//   v0
int emu_unpckhpd_xmm_xmm_128__reg_xmm0_low__dart(int a) {
  int x0_0 = ext(a, 0, 0);
  int x0_1 = ext(a, 1, 1);
  int x0_2 = ext(a, 2, 2);
  int x0_3 = ext(a, 3, 3);
  ...
  while (line != null) {
    ...
    if (text.isNotEmpty) {
      final List<int> values = text
          ...
          .toList();
      try {
        final int answer = emu_unpckhpd_xmm_xmm_128__reg_xmm0_low__dart(values[0]);
