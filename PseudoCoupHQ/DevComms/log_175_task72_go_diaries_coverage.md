...
```sh
cd PRIVATE/PseudoCoupHQ/Research/compiler_graph
...
```sh
bash PUBLIC/Airlock/up.sh --instance t72
cd PUBLIC/Airlock
...
`PUBLIC/Airlock/instances/t72.conf` — gitignored per-machine
...
| `PUBLIC/Airlock/instances/t72.conf` | the instance's settings; gitignored caller config, Airlock source untouched |

**Files edited, this task:**

| path | change |
|---|---|
| `PRIVATE/PseudoCoupHQ/.gitignore` | one rule for `Research/compiler_graph/diaries/`, with the reason and the regeneration command |
...
and anything under `PUBLIC/Airlock` except the gitignored
`instances/t72.conf`.
...
`PUBLIC/Airlock/agent/logs/` for lane 0.
