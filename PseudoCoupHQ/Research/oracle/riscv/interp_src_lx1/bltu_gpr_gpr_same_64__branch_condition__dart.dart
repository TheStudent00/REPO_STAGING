...
// InterpRenderer from the layer-4 term of bltu_gpr_gpr_same_64__branch_condition__dart.
// The term's layer-5 text, LITERAL:
//   If(ULE(v0, v1), 0, 1)
int emu_bltu_gpr_gpr_same_64__branch_condition__dart(int a, int b) {
  return m(((ule(b, a, 64)) ? (0) : (1)), 64);
}
  ...
  while (line != null) {
    ...
    if (text.isNotEmpty) {
      final List<int> values = text
          ...
          .toList();
      try {
        final int answer = emu_bltu_gpr_gpr_same_64__branch_condition__dart(values[0], values[1]);
