/* task o7 emulation -- rendered by emulate.py Renderer from the layer-4 term of E01555__go_regen_352.  The term's layer-5 text, LITERAL:
   Concat(0, ~(~LShR(Extract(15, 0, v0), Concat(0, Extract(4, 0, v1))) | ~(65535*If(Or(Not(Extract(15, 5, v1) == 0), ULE(16, Extract(4, 0, v1))), 0, 1)))) */
#include <stdint.h>

uint32_t
emu_E01555__go_regen_352(uint16_t a, uint16_t b)
{
