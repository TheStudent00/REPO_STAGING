...
theorem meaning_emul_au_196_c_predec_bool (s : St) (a : BitVec 64) (d : BitVec 64) (h10 : s.regs.get? .x10 = some a) (h13 : s.regs.get? .x13 = some d) (hk0 : ∀ {α : Type} (x0 : α) (s : St), (plat_term_write x0) s = EStateM.Result.ok () s) (hk1 : ∀ (x0 : Arch.pa) (x1 : Nat) (s : St), (load_reservation x0 x1) s = EStateM.Result.ok () s) (hk2 : ∀ (x0 : Unit) (s : St), (cancel_reservation x0) s = EStateM.Result.ok () s) :
    runsTo (walk [LeanIM.instruction.C_LI (0x01#6, LeanIM.regidx.Regidx 0x0b#5), LeanIM.instruction.ZBB_RTYPE (LeanIM.regidx.Regidx 0x0a#5, LeanIM.regidx.Regidx 0x0b#5, LeanIM.regidx.Regidx 0x0a#5, LeanIM.brop_zbb.ANDN)]) s
      ((pure_ZBB_RTYPE ((pure_ITYPE (zero_reg) (((sign_extend (m := 12) (0x01#6)))) (ADDI))) (a) (LeanIM.brop_zbb.ANDN))) := by
  simp (config := {decide := true}) [runsTo, walk, step, execute, strip_C_LI, strip_ITYPE, strip_ZBB_RTYPE, alias_C_LI,
