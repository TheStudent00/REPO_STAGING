import LeanIMZ.Flow
import LeanIMZ.Mapping
import LeanIMZ.Vector
import LeanIMZ.HexBits
import LeanIMZ.HexBitsSigned
import LeanIMZ.Prelude
import LeanIMZ.Errors
import LeanIMZ.PmTypes
import LeanIMZ.Xlen
import LeanIMZ.Flen
import LeanIMZ.Vlen
...
/-- Type quantifiers: k_ex512080_ : Bool -/
...
def currentlyEnabled (merge_var : extension) : SailM Bool := do
  ...
  | _ =>
    (do
      assert false "Pattern match failure at extensions/C/zcb_insts.sail:9.0-9.93"
      throw Error.Exit)
termination_by (let ext := merge_var
...
/-- Type quantifiers: k_ex512664_ : Bool, width : Nat, width ∈ {1, 2, 4, 8} -/
