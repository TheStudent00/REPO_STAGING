...
// InterpRenderer from the layer-4 term of beq_gpr_gpr_same_64__branch_condition__php.
// The term's layer-5 text, LITERAL:
//   If(v0 == v1, 1, 0)
function emu_beq_gpr_gpr_same_64__branch_condition__php($a, $b) {
    return ex_m(((ex_eq($a, $b, 64)) ? (1) : (0)), 64);
}
...
while (($line = fgets($handle)) !== false) {
    ...
    try {
        $answer = emu_beq_gpr_gpr_same_64__branch_condition__php(...$ints);
        if ($answer < 0) {
            echo sprintf("%s\n", sprintf("%u", $answer));
        } else {
            echo $answer . "\n";
