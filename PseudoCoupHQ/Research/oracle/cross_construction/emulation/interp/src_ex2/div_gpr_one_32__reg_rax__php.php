...
// InterpRenderer from the layer-4 term of div_gpr_one_32__reg_rax__php.
// The term's layer-5 text, LITERAL:
//   Concat(0, Extract(31, 0, bvudiv_i(Concat(Extract(31, 0, v0), Extract(31, 0, v1)), Concat(0, Extract(31, 0, v2)))))
function emu_div_gpr_one_32__reg_rax__php($a, $b, $c) {
    return ex_m(ex_cat(0, ex_ext(ex_udiv(ex_cat($a, $b, 32), ex_cat(0, $c, 32), 64), 31, 0), 32), 64);
}
...
while (($line = fgets($handle)) !== false) {
    ...
    try {
        $answer = emu_div_gpr_one_32__reg_rax__php(...$ints);
        if ($answer < 0) {
            echo sprintf("%s\n", sprintf("%u", $answer));
        } else {
            echo $answer . "\n";
