...
// InterpRenderer from the layer-4 term of test_gpr_same_32__flags__php.
// The term's layer-5 text, LITERAL:
//   Concat(Extract(31, 0, v0), 0)
function emu_test_gpr_same_32__flags__php($a) {
    return ex_m(ex_cat($a, 0, 32), 64);
}
...
while (($line = fgets($handle)) !== false) {
    ...
    try {
        $answer = emu_test_gpr_same_32__flags__php(...$ints);
        if ($answer < 0) {
            echo sprintf("%s\n", sprintf("%u", $answer));
        } else {
            echo $answer . "\n";
