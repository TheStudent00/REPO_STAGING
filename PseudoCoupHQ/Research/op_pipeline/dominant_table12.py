#!/usr/bin/env python3
"""dominant_table12.py -- re-run the class table and dom_op construction
on THIS LAP'S newest converged text per unit (Stages 1-4: canon11_render.
py/canon12_render.py/canon13_render.py's own accepted candidates, layered
on top of canon10.py's own trivial-rescue pass and everything before it).

Byte-for-byte the SAME construction dominant_table11.py already used
...
generations (canon13, canon12, canon11) ahead of the ones
dominant_table11.py already knew about. `final_text_of` already reads
"newest generation whose <gen>_text field is present", so a unit none
of Stage 1-4 touched keeps falling through to its canon10_text (or
further) exactly as before -- no other logic changes.

SCOPE: 0-BRANCH POPULATION ONLY, same population-integrity assumption
dominant_table11.py already asserts (Stages 1-4's own drivers -- canon11.
py/canon12.py/canon13.py -- each explicitly skip any unit whose
branch_kind isn't "straight_line", so this lap cannot have touched a
branching unit; the SAME "refuse silently-wrong scope" check is kept,
now widened to cover canon13/canon12/canon11 too).

BASELINE (dominant_table11.json/dom_ops9.json, already on disk, produced
before this lap's Stage 1-4 work, matching the task's own cited figures):
923 classes / 135 nodes / 26 families / 23 edgeless.

THE SPELLING BAN: unchanged, same as dominant_table11.py -- the class key
...
GENERATIONS = ["canon13", "canon12", "canon11", "canon10", "canon9",
               "canon8", "canon7"]

BRANCH_CHECK_GENERATIONS = ("canon13", "canon12", "canon11", "canon10",
                             "canon9", "canon8")
    ...
    for lang in LANGS:
        ...
        for n, u in canon4_doc["units"].items():
            ...
            if branching:
                ...
                # if Stage 1-4 (or any earlier job) DID touch a
...
def main(argv):
    ...
    class_doc = {
        "meta": {
            ...
            "role_note": "this file is a GROUPING/matching artifact "
                        ...
                        "exemption (unlike dom_ops10.json beside it), "
                        "matching dominant_table11.json's own "
            ...
            "population": "canon4_units_*.json, 0-branch (erasure == "
                         ...
                         "from canon13_text/canon12_text/canon11_text/"
                         "canon10_text/canon9_text/canon8_text/"
                         "canon7_text, newest generation that touched "
                         "each unit -- see this file's own header",
            "baseline": "923 classes / 135 nodes / 26 families / 23 "
                       "edgeless (dominant_table11.json/dom_ops9.json, "
                       "already on disk, produced before this lap's "
                       "Stage 1-4 work -- the task's own cited figures)",
        },
        "classes": len(table["rows"]),
        "rows": table["rows"],
    }
    class_path = os.path.join(outdir, "dominant_table12.json")
    ...
    dom_ops_doc = {
        "meta": {
            ...
            "note": "the dom_op (family) construction over the SAME "
                   "population/class table as dominant_table12.json "
                   "-- written to dom_ops10.json, the next number "
                   "after dom_ops9.json",
        },
    ...
    dom_ops_path = os.path.join(outdir, "dom_ops10.json")
