...
theorem meaning_c_op_174 (s : St) (a : BitVec 64) (b : BitVec 64) (h10 : s.regs.get? .x10 = some a) (h11 : s.regs.get? .x11 = some b) :
    runsTo (walk [LeanIM.instruction.MULW (LeanIM.regidx.Regidx 0x0a#5, LeanIM.regidx.Regidx 0x0b#5, LeanIM.regidx.Regidx 0x0a#5)]) s
      ((pure_MULW (b) (a))) := by
  simp (config := {decide := true}) only [runsTo, walk, step, execute, strip_MULW,
