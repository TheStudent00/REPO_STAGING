#!/usr/bin/env bash
# t81 lane 11 — `Graph.coverage`: the c and cpp diaries joined to
# graph_cpp.json.
#
# WHY THIS IS LANE 11. Lane 10 ran and its MEMORY GATE refused the full
# join at a projected 28,997 MB. The projection was wrong in a way worth
# recording: it multiplied a single 50-probe peak by 27.6, which treats
# the 1,049.3 MB the GRAPH ITSELF occupies as if it grew with the number
# of probes. The cost is `fixed + per-probe x probes`; one measurement
# cannot separate the two terms, so lane 11 takes TWO and fits them.
# Lane 10 stays on disk as the record of that refusal -- it refused
# safely, which is the behaviour the rule asks for.
#
# WHY THIS IS LANE 10 AND NOT LANE 6. Lane 6 ran against a t81-runner
# container that had been created FIFTEEN HOURS BEFORE the image was
# rebuilt, so `import tree_sitter` failed inside it while the rebuilt
# image carries tree-sitter 0.25.2. `airlock up` reuses an existing
# container rather than recreating it, so the instance kept the stale
# image. The container was removed and brought up again on the current
# image (the named volume `t81-persist`, and so the built clang, are
# untouched by that). Lane 6 stays on disk as the record of the stop.
# TWO THINGS ARE FIXED HERE, both defects lane 6 exposed:
#   1. the memory gate accepted a peak of -1 MB -- a MISSING measurement
#      read as a small one. It now refuses when no peak was measured.
#   2. every step that can fail now stops the lane instead of letting a
#      later step run on an absent file.
#
...
PY
if [ $? -ne 0 ]; then echo "   STOP: the graph did not load"; exit 6; fi

say "[3/6] TWO SAMPLE joins -- 50 and 400 cpp diaries"
# TWO POINTS, NOT ONE, and the reason is the shape of the cost. Lane 10
# measured a single 50-probe sample at 1,050.6 MB and projected it by
# multiplying, which read the 1,049.3 MB the GRAPH costs as if it grew
# with the probes and refused a pass that plainly fits. The cost is
# `fixed + per-probe x probes`, so it takes two measurements to say what
# the two terms are. Both are the kernel's own ru_maxrss.
for N in 50 400 ; do
...
print('   sampling %d of %d cpp diaries' % (min(count, len(names)), len(names)))
PY
  python3 t81/run_with_peak.py graph.py join \
      ...
      --out "/work/coverage_sample_$N.json" | tee "/work/sample_peak_$N.txt"
  echo "   sample artifact bytes: $(stat -c %s /work/coverage_sample_$N.json)"
done

say "[4/6] the MEMORY GATE: fixed cost plus per-probe cost, projected"
python3 - <<'PY'
import os, re, sys
def read(path):
    text = open(path).read()
    ...
    if not found:
        print('   MEMORY GATE: NO PEAK WAS MEASURED in %s. A missing '
              'measurement is not a small one, so the full join is '
              'REFUSED BY NAME.' % path)
        sys.exit(9)
    if 'exit 0' not in text:
        print('   MEMORY GATE: the sample in %s did not exit 0, so its peak '
              'is not a measurement of the pass. REFUSED BY NAME.' % path)
        sys.exit(9)
    return float(found.group(1))
low_n, high_n = 50, 400
low = read('/work/sample_peak_50.txt')
high = read('/work/sample_peak_400.txt')
per_probe = (high - low) / float(high_n - low_n)
fixed = low - per_probe * low_n
print('   peak at %d probes : %.1f MB' % (low_n, low))
...
for language in ('c', 'cpp', 'c_and_cpp'):
    ...
    counts[language] = len([n for n in os.listdir(path)
                            if n.endswith('.txt')]) if os.path.isdir(path) else 0
print('   diaries to join, c     : %d' % counts['c'])
print('   diaries to join, cpp   : %d' % counts['cpp'])
print('   diaries to join, joint : %d' % counts['c_and_cpp'])
projected = fixed + per_probe * counts['c_and_cpp']
print('   PROJECTION for the joint join : %.0f MB' % projected)
headroom = (6144 - fixed) / per_probe if per_probe > 0 else 0
print('   probes the 6,144 MB ceiling allows at this rate : %.0f' % headroom)
if projected > 6144:
