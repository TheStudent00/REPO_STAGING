...
public static class Emu {
    ...
    // InterpRenderer from the layer-4 term of divsd_mem_xmm_64__reg_xmm0__csharp.
    // The term's layer-5 text, LITERAL:
    //   fp.to_ieee_bv(fpToFP(Extract(63, 0, v0)) / fpToFP(v1))
    public static long emu_divsd_mem_xmm_64__reg_xmm0__csharp(long a, long b) {
        return m(fdiv(b, a, 64), 64);
    }
    ...
    public static void Main() {
        ...
        while (line != null) {
            ...
            if (text.Length != 0) {
                ...
                try {
                    answers.Append(((ulong) emu_divsd_mem_xmm_64__reg_xmm0__csharp(values[0], values[1])).ToString());
                    answers.Append("\n");
