#!/bin/bash
# lp1_l20_walk_the_handful_fourth.sh -- the walk again: the decoder state is the one the emulator's own main starts from (sail_model_init, then init_model). A probe of that state first, LITERAL.
...
OUT=$A/walk_handful_6266b40c_8eb1fb6b_IMZ_d
...
f=$(python3 - <<'PY'
import json
d=json.load(open("PseudoCoupHQ/Research/oracle/riscv/leanpath/walk_handful_6266b40c_8eb1fb6b_IMZ_d/walk.json"))
