...
// one named local per gate, over the term of shr_cl_gpr_16__reg_rdi__php.
// The term's layer-5 text, LITERAL:
//   Concat(Extract(63, 16, v0), LShR(Extract(15, 0, v0), Concat(0, Extract(4, 0, v1))))
function emu_shr_cl_gpr_16__reg_rdi__php($a, $b) {
    $x0_0 = ex_ext($a, 0, 0);
    $x0_1 = ex_ext($a, 1, 1);
    $x1_0 = ex_ext($b, 0, 0);
    $x0_2 = ex_ext($a, 2, 2);
    ...
    $w49 = ex_cat($w48, $g243, 1);
    $w50 = ex_cat($w49, $g238, 1);
    $w51 = ex_cat($w50, $g235, 1);
    $w52 = ex_cat($w51, $g231, 1);
    $w53 = ex_cat($w52, $g229, 1);
    $w54 = ex_cat($w53, $g227, 1);
    $w55 = ex_cat($w54, $g225, 1);
    $w56 = ex_cat($w55, $g222, 1);
    $w57 = ex_cat($w56, $g211, 1);
    $w58 = ex_cat($w57, $g200, 1);
    $w59 = ex_cat($w58, $g188, 1);
    $w60 = ex_cat($w59, $g176, 1);
    $w61 = ex_cat($w60, $g149, 1);
    $w62 = ex_cat($w61, $g121, 1);
    $w63 = ex_cat($w62, $g61, 1);
    return ex_m($w63, 64);
}
...
while (($line = fgets($handle)) !== false) {
    ...
    try {
        $answer = emu_shr_cl_gpr_16__reg_rdi__php(...$ints);
        if ($answer < 0) {
            echo sprintf("%s\n", sprintf("%u", $answer));
        } else {
            echo $answer . "\n";
