/* task t4 emulation -- rendered by render_general.py, one named
   intermediate per node of the term of remw_gpr_gpr_gpr_32__reg_a0__cpp__native_first.  The term's text, LITERAL:
   Concat(If(Extract(31, 0, v1) == 0, Extract(31, 31, v0), If(And(Extract(31, 0, v0) == 2147483648, Extract(31, 0, v1) == 4294967295), 0, Extract(31, 31, bvsrem_i(Extract(31, 0, v0), Extract(31, 0, v1))))), If(Extract(31, 0, v1) == 0, Extract(31, 31, v0), If(And(Extract(31, 0, v0) == 2147483648, Extract(31, 0, v1) == 4294967295), 0, Extract(31, 31, bvsrem_i(Extract(31, 0, v0), Extract(31, 0, v1))))), If(Extract(31, 0, v1) == 0, Extract(31, 31, v0), If(And(Extract(31, 0, v0) == 2147483648, Extract(31, 0, v1) == 4294967295), 0, Extract(31, 31, bvsrem_i(Extract(31, 0, v0), Extract(31, 0, v1))))), If(Extract(31, 0, v1) == 0, Extract(31, 31, v0), If(And(Extract(31, 0, v0) == 2147483648, Extract(31, 0, v1) == 4294967295), 0, Extract(31, 31, bvsrem_i(Extract(31, 0, v0), Extract(31, 0, v1))))), If(Extract(31, 0, v1) == 0, Extract(31, 31, v0), If(And(Extract(31, 0, v0) == 2147483648, Extract(31, 0,  */
#include <cstdint>

extern "C"
uint64_t
emu_remw_gpr_gpr_gpr_32__reg_a0__cpp__native_first(uint32_t a, uint32_t b)
{
    uint32_t v0 = (uint32_t)b;
    uint32_t v1 = (uint32_t)a;
    uint32_t v2 = (uint32_t)((int32_t)((int32_t)(v1)) % (int32_t)((int32_t)(v0)));
    int v3 = (((uint32_t)(v0) == (uint32_t)(UINT32_C(0xffffffff)))) ? 1 : 0;
    int v4 = (((uint32_t)(v1) == (uint32_t)(UINT32_C(0x80000000)))) ? 1 : 0;
    int v5 = (((v4) && (v3))) ? 1 : 0;
    uint32_t v6 = ((v5) ? (uint32_t)(UINT32_C(0x0)) : (uint32_t)(v2));
    int v7 = (((uint32_t)(v0) == (uint32_t)(UINT32_C(0x0)))) ? 1 : 0;
    uint32_t v8 = ((v7) ? (uint32_t)(v1) : (uint32_t)(v6));
    uint32_t v9 = ((uint32_t)((uint32_t)(v2) >> 31) & UINT32_C(0x1));
    uint32_t v10 = ((v5) ? (uint32_t)(UINT32_C(0x0)) : (uint32_t)(v9));
    uint32_t v11 = ((uint32_t)((uint32_t)a >> 31) & UINT32_C(0x1));
    uint32_t v12 = ((v7) ? (uint32_t)(v11) : (uint32_t)(v10));
    uint64_t v13 = (uint64_t)(((uint64_t)(v12) << 63) | ((uint64_t)(v12) << 62) | ((uint64_t)(v12) << 61) | ((uint64_t)(v12) << 60) | ((uint64_t)(v12) << 59) | ((uint64_t)(v12) << 58) | ((uint64_t)(v12) << 57) | ((uint64_t)(v12) << 56) | ((uint64_t)(v12) << 55) | ((uint64_t)(v12) << 54) | ((uint64_t)(v12) << 53) | ((uint64_t)(v12) << 52) | ((uint64_t)(v12) << 51) | ((uint64_t)(v12) << 50) | ((uint64_t)(v12) << 49) | ((uint64_t)(v12) << 48) | ((uint64_t)(v12) << 47) | ((uint64_t)(v12) << 46) | ((uint64_t)(v12) << 45) | ((uint64_t)(v12) << 44) | ((uint64_t)(v12) << 43) | ((uint64_t)(v12) << 42) | ((uint64_t)(v12) << 41) | ((uint64_t)(v12) << 40) | ((uint64_t)(v12) << 39) | ((uint64_t)(v12) << 38) | ((uint64_t)(v12) << 37) | ((uint64_t)(v12) << 36) | ((uint64_t)(v12) << 35) | ((uint64_t)(v12) << 34) | ((uint64_t)(v12) << 33) | ((uint64_t)(v12) << 32) | (uint64_t)(v8));
    return (uint64_t)(v13);
}
