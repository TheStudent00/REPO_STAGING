#!/usr/bin/env python3
"""canon37_wrapped.py -- TASK 47: the memory-wrapped form over the
ORIGINAL population (the 1,779 compiled units).

THE FORM is ledger47.py's and THE GATE is canon37_gate.py's; read
...
usage:
  canon37_wrapped.py --lang c [--limit N] [--fresh]
  canon37_wrapped.py --all
"""
...
def out_path(lang):
    return os.path.join(HERE, "canon37_wrapped_%s.json" % lang)
...
def write_checkpoint(lang, units, tally):
    document = {
        "meta": {
            ...
            "generated_by": "canon37_wrapped.py",
            "form": "ledger47: the memory-wrapped form -- prelude from "
    ...
    if prior_text:
        if prior_text != body_text:
            ...
            gate_prior = GATE.compare(
                L47.split_lines(prior_text), wrapped_lines, seed, home,
