#!/usr/bin/env python3
"""compare_pool2_pool3.py -- TASK 54, every entry that SPLIT or MERGED.
...
read; no entry is matched by name.  Two pools are read read-only:
  ...
  the_pool2.json   the 30,436 units TASK 47 proved over canon37,
                   merged on the layer-5 normalized text, the layer-3
                   wrapped text and proved edges
  the_pool3.json   the same 30,436 units as TASK 52 proved them over
                   canon38, merged on ruling 1's three grounds
...
folded into a split or a merge.
  ...
  SPLIT   -- one pool2 entry whose shared members land in MORE THAN ONE
             pool3 entry.
  MERGED  -- one pool3 entry whose shared members come from MORE THAN
             ONE pool2 entry.
...
forms directly:
  ...
  from different pool2 entries share a layer-5 normalized text, a
...
This file was derived from `compare_pool1_pool2.py` by name
substitution only; `compare_pool1_pool2.py` was not edited.

usage: compare_pool2_pool3.py
"""
...
def proved_edge_pairs():
    ...
    import build_the_pool3 as POOL
    import build_the_pool2 as POOL_EDGES
    records = POOL.take_all()
    present = {}
    for rec in records:
        present[rec["unit"]] = True
    out = set()
    for edge in POOL_EDGES.proved_edge_list(present):
...
def main():
    ...
    pool3 = json.load(open(os.path.join(HERE, "the_pool3.json")))
    where1, entries1 = index_pool(pool2, "wrapped_text")
    where2, entries2 = index_pool(pool3, "wrapped_text")

    units1 = set(where1)
    units2 = set(where2)
    shared = units1 & units2
    log("== populations")
    log("   pool2 member units                       %d" % len(units1))
    log("   pool3 member units                       %d" % len(units2))
    log("   units in both, the comparable population %d" % len(shared))
    log("   in pool2 only -- absent from the newer pool %d"
        % len(units1 - units2))
    log("   in pool3 only -- absent from the older pool %d"
        % len(units2 - units1))

    # the member facts pool3 recorded, per unit
    fact = {}
    ...
    for eid in sorted(by1):
        ...
        splits.append({
            ...
            "pool3_parts": sorted(parts),
            "pool3_part_count": len(parts),
    ...
    log("   pool2 entries holding a shared unit       %d" % len(by1))
    log("   pool3 entries holding a shared unit       %d" % len(by2))
    log("   pool2 entries that SPLIT                  %d" % len(splits))
    log("   pool3 entries that MERGED                 %d" % len(merges))
    ...
    doc = {
        "meta": {
            "generator": "compare_pool2_pool3.py",
            ...
            "task": "TASK 54 -- every entry that split or merged "
                    "between the_pool2.json and the_pool3.json",
        ...
        "summary": {
            "pool2_member_units": len(units1),
            "pool3_member_units": len(units2),
            "units_in_both": len(shared),
            "units_in_pool2_only": len(units1 - units2),
            "units_in_pool3_only": len(units2 - units1),
            "pool2_entries_holding_a_shared_unit": len(by1),
            "pool3_entries_holding_a_shared_unit": len(by2),
            "pool2_entries_that_split": len(splits),
            "pool3_entries_that_merged": len(merges),
        ...
        "splits": splits,
        "merges": merges,
        "units_in_pool2_only": sorted(units1 - units2),
        "units_in_pool3_only_count": len(units2 - units1),
    }
    path = os.path.join(HERE, "pool2_pool3_delta.json")
    with open(path, "w") as fh:
        json.dump(doc, fh, indent=1, sort_keys=True)
    log("")
    log("-- wrote pool2_pool3_delta.json")
    return 0
