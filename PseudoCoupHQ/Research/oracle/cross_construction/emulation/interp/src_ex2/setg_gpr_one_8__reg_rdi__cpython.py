...
# InterpRenderer from the layer-4 term of setg_gpr_one_8__reg_rdi__cpython.
# The term's layer-5 text, LITERAL:
#   Concat(0, If(Extract(7, 0, v0) <= Extract(7, 0, v1), 0, 1))
def emu_setg_gpr_one_8__reg_rdi__cpython(a, b):
    return m(cat(0, ((0) if (sle(a, b, 8)) else (1)), 8), 64)
...
def main():
    for line in sys.stdin:
        ...
        try:
            answer = emu_setg_gpr_one_8__reg_rdi__cpython(*values)
            sys.stdout.write("%d\n" % answer)
