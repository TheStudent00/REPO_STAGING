...
// InterpRenderer from the layer-4 term of test_imm_gpr_64__flags.low__php.
// The term's layer-5 text, LITERAL:
//   0
function emu_test_imm_gpr_64__flags.low__php() {
    return ex_m(0, 64);
}
...
while (($line = fgets($handle)) !== false) {
    ...
    try {
        $answer = emu_test_imm_gpr_64__flags.low__php(...$ints);
        if ($answer < 0) {
            echo sprintf("%s\n", sprintf("%u", $answer));
        } else {
            echo $answer . "\n";
