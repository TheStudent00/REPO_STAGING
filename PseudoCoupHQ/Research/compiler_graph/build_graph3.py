...
but kept the probe's original four-file region unchanged, so most of
the 37,733 unresolved-call frontier entries were either (a) local
lambdas/constructors the reader's node model does not descend into
(SDValue, SDLoc, APInt, a lambda like CanonicalizeShuffleInput --
these are reader-depth gaps, not file-boundary gaps, confirmed by
`git show`-reading X86ISelLowering.cpp directly: CanonicalizeShuffleInput
is a local `auto ... = [&](...)` at line 39956, never a candidate for
cross-file resolution no matter how wide the region gets) or (b) real
calls into functions that live in SIBLING files of the SelectionDAG
legalization family that lap two never read.

This lap keeps every reader/resolver/edge-builder in build_graph2.py
UNCHANGED (imported by function, not re-implemented) and widens exactly
one thing: REGION_FILES. The added files are the natural continuation
of the four-file region: LegalizeDAG.cpp already sits in
llvm/lib/CodeGen/SelectionDAG/; its direct siblings implementing the
other legalization passes (SelectionDAG.cpp, LegalizeTypes.cpp,
LegalizeFloatTypes.cpp, LegalizeIntegerTypes.cpp, LegalizeVectorOps.cpp,
LegalizeVectorTypes.cpp, LegalizeTypesGeneric.cpp) are exactly the
"functions those calls reach" the brief names -- ReplaceNode,
RemoveDeadNode, and a large share of the Expand/Promote/Split family
called from ExpandNode's own dispatch live there. This is a bounded,
principled widening (one directory, the SAME pass family already in
region), not an attempt to pull in the whole compiler; the STOP RULE
from lap two applies again here -- named, not silently narrowed.

Below (unchanged from build_graph2.py, lap two's own docstring, kept as
a comment block for provenance): build_graph2.py -- generalized,
language-dispatched compiler-graph builder.
...
REGION_FILES = [
    ...
    # lap-three widening: the rest of the SelectionDAG legalization
    # family LegalizeDAG.cpp already belongs to (same directory, same
    # pass group) -- named in build_graph3.py's own docstring as the
    # bounded, principled widening this lap performs.
