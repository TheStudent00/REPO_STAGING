#!/bin/bash
# lp3_l36_equals_staged_chunked_final_tally.sh -- equals over every
...
for k in 0 1 2 3; do for h in 0 1; do
  ( python3 -u -m leanpath equals $P $PLIB $S $A/walk_corpus_${k}_half$h.json $A/equals_corpus_f_${k}_$h 120 64 > $A/equals_corpus_f_${k}_$h.log 2>&1 ) &
done; done
wait
echo "  wall seconds=$(( $(date +%s) - start ))"
for k in 0 1 2 3; do for h in 0 1; do grep -E "equals:|Traceback" $A/equals_corpus_f_${k}_$h.log | tail -2; done; done
echo "[2/2] the final tallies over everything"
python3 -m leanpath tally $A/tally_corpus_final.json $A/walk_corpus_0 $A/walk_corpus_1 $A/walk_corpus_2 $A/walk_corpus_3 $A/walk_corpus_fix2 -- $A/equals_corpus_f_0_0 $A/equals_corpus_f_0_1 $A/equals_corpus_f_1_0 $A/equals_corpus_f_1_1 $A/equals_corpus_f_2_0 $A/equals_corpus_f_2_1 $A/equals_corpus_f_3_0 $A/equals_corpus_f_3_1 $A/equals_corpus_fix2 2>&1 | cut -c1-200
echo done
