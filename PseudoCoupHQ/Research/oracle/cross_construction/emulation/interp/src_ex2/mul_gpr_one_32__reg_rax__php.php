...
// InterpRenderer from the layer-4 term of mul_gpr_one_32__reg_rax__php.
// The term's layer-5 text, LITERAL:
//   Concat(0, Extract(31, 0, v0)*Extract(31, 0, v1))
function emu_mul_gpr_one_32__reg_rax__php($a, $b) {
...
while (($line = fgets($handle)) !== false) {
    ...
    try {
        $answer = emu_mul_gpr_one_32__reg_rax__php(...$ints);
        if ($answer < 0) {
            echo sprintf("%s\n", sprintf("%u", $answer));
        } else {
            echo $answer . "\n";
