#!/usr/bin/env python3
"""guard54.py -- THE GUARD for task 54: the UNMODIFIED
...
WHAT IS WALKED: `the_pool3.json`, `the_families3.json`,
`pool2_pool3_delta.json` and `the_pool3_bytes.json` -- every JSON
artifact task 54 writes.  No artifact this task writes
declares `role: generator provenance`, so every line of the transcript
must say the guard walked the file rather than skipped it, and
...
TRANSCRIPT = os.path.join(HERE, "guard54_transcript.txt")
SUMMARY = os.path.join(HERE, "guard54.json")


def task54_paths():
    out = []
    out.append(os.path.join(HERE, "the_pool3.json"))
    out.append(os.path.join(HERE, "the_families3.json"))
    out.append(os.path.join(HERE, "pool2_pool3_delta.json"))
    out.append(os.path.join(HERE, "the_pool3_bytes.json"))
    live = []
    for path in out:
...
def main(argv):
    ...
    counts = run_guard(
        paths, TRANSCRIPT,
        "guard54.py -- every artifact task 54 writes, unmodified "
        "guard, ONE process.  No file declares the provenance role "
        "and nothing was added to any field set.")
    print("TASK 54: %d paths  PASS %d  FAIL %d  exempt %d  exit %d"
