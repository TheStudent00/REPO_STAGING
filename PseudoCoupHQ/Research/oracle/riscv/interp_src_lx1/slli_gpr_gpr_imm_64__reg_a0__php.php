...
// InterpRenderer from the layer-4 term of slli_gpr_gpr_imm_64__reg_a0__php.
// The term's layer-5 text, LITERAL:
//   Concat(Extract(60, 0, v0), 0)
function emu_slli_gpr_gpr_imm_64__reg_a0__php($a) {
    return ex_m(ex_cat(ex_ext($a, 60, 0), 0, 3), 64);
}
...
while (($line = fgets($handle)) !== false) {
    ...
    try {
        $answer = emu_slli_gpr_gpr_imm_64__reg_a0__php(...$ints);
        if ($answer < 0) {
            echo sprintf("%s\n", sprintf("%u", $answer));
        } else {
            echo $answer . "\n";
