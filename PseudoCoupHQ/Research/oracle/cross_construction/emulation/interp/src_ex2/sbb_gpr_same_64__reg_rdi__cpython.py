...
# InterpRenderer from the layer-4 term of sbb_gpr_same_64__reg_rdi__cpython.
# The term's layer-5 text, LITERAL:
#   If(Extract(64, 64, Concat(0, v0) + Concat(0, v1)) == 1, 1, 0)*18446744073709551615
def emu_sbb_gpr_same_64__reg_rdi__cpython(a, b, c):
    return m(mul(((1) if (eq(ext(add(cat(0, b, 64), cat(0, a, 64), 65), 64, 64), 1, 1)) else (0)), 18446744073709551615, 64), 64)
...
def main():
    for line in sys.stdin:
        ...
        try:
            answer = emu_sbb_gpr_same_64__reg_rdi__cpython(*values)
            sys.stdout.write("%d\n" % answer)
