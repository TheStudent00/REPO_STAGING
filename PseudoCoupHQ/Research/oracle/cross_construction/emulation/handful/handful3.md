# handful3.md -- task g1: the same ten cells on FOUR targets, primitive-first

The same ten cells of the arch-opcode model table tasks h1 and h2 ran, on four targets (c, rust, go and swift) and by a route that is tried before the term route: does the target have an OPERATOR whose whole lowered body IS this cell? Where it does, that operator on holders of the operand types the corpus recorded is what is rendered, and nothing else; where it does not, the term route of tasks h1 and h2 runs unchanged, with task h2's two printing fixes still on. Written by `handful.py report3`; never hand-edited. The products of tasks h1 and h2 are not written by this task: they sit beside these as `handful.json` / `handful.md` and `handful2.json` / `handful2.md`.

**There is no swift toolchain in the image this task ran under**, so every swift row is refused at the compile step with the literal answer the machine gave. The swift renderer is written and its rendering is on the record; every swift spelling in it is UNMEASURED and says so.
...
| swiftc, as this machine answers it | /persist/swift/usr/bin/swiftc: No such file or directory |
| solver ceiling | 3000 ms |
| memory bound | 4194304 kB, named abort ABORT_MEMORY_G1 |
| peak resident | 466868 kB |
...
The source, LITERAL (`src3/add_gpr_gpr_32__reg_rdi__c.c`):
...
The source, LITERAL (`src3/add_gpr_gpr_32__flags__c.c`):
...
The source, LITERAL (`src3/add_gpr_gpr_32__reg_rdi__rust.rs`):
...
The source, LITERAL (`src3/add_gpr_gpr_32__flags__rust.rs`):
...
The source, LITERAL (`src3/add_gpr_gpr_32__primitive__go.go`):
...
The source, LITERAL (`src3/add_gpr_gpr_32__reg_rdi__swift.swift`):
...
**Step 3, the compile.** REFUSED: /persist/swift/usr/bin/swiftc: No such file or directory.
...
The source, LITERAL (`src3/add_gpr_gpr_32__flags__swift.swift`):
...
The source, LITERAL (`src3/sub_imm_gpr_64__reg_rdi__c.c`):
...
The source, LITERAL (`src3/sub_imm_gpr_64__flags__c.c`):
...
The source, LITERAL (`src3/sub_imm_gpr_64__reg_rdi__rust.rs`):
...
The source, LITERAL (`src3/sub_imm_gpr_64__flags__rust.rs`):
...
The source, LITERAL (`src3/sub_imm_gpr_64__reg_rdi__go.go`):
...
The source, LITERAL (`src3/sub_imm_gpr_64__reg_rdi__swift.swift`):
...
The source, LITERAL (`src3/imul_gpr_gpr_32__primitive__c.c`):
...
The source, LITERAL (`src3/imul_gpr_gpr_32__primitive__rust.rs`):
...
The source, LITERAL (`src3/imul_gpr_gpr_32__primitive__go.go`):
...
The source, LITERAL (`src3/imul_gpr_gpr_32__reg_rdi__swift.swift`):
...
The source, LITERAL (`src3/imul_gpr_gpr_32__flags__swift.swift`):
...
The source, LITERAL (`src3/sar_cl_gpr_32__primitive__c.c`):
...
The source, LITERAL (`src3/sar_cl_gpr_32__primitive__rust.rs`):
...
The source, LITERAL (`src3/sar_cl_gpr_32__reg_rdi__go.go`):
...
The source, LITERAL (`src3/sar_cl_gpr_32__reg_rdi__swift.swift`):
...
The source, LITERAL (`src3/shr_cl_gpr_64__primitive__c.c`):
...
The source, LITERAL (`src3/shr_cl_gpr_64__primitive__rust.rs`):
...
The source, LITERAL (`src3/shr_cl_gpr_64__reg_rdi__go.go`):
...
The source, LITERAL (`src3/shr_cl_gpr_64__reg_rdi__swift.swift`):
...
The source, LITERAL (`src3/idiv_gpr_one_32__reg_rax__c.c`):
...
The source, LITERAL (`src3/idiv_gpr_one_32__reg_rdx__c.c`):
...
The source, LITERAL (`src3/idiv_gpr_one_32__reg_rax__rust.rs`):
...
The source, LITERAL (`src3/idiv_gpr_one_32__reg_rdx__rust.rs`):
...
The source, LITERAL (`src3/idiv_gpr_one_32__reg_rax__go.go`):
...
The source, LITERAL (`src3/idiv_gpr_one_32__reg_rdx__go.go`):
...
The source, LITERAL (`src3/idiv_gpr_one_32__reg_rax__swift.swift`):
...
The source, LITERAL (`src3/idiv_gpr_one_32__reg_rdx__swift.swift`):
...
The source, LITERAL (`src3/cmovne_gpr_gpr_32__reg_rdi__c.c`):
...
The source, LITERAL (`src3/cmovne_gpr_gpr_32__reg_rdi__rust.rs`):
...
The source, LITERAL (`src3/cmovne_gpr_gpr_32__reg_rdi__go.go`):
...
The source, LITERAL (`src3/cmovne_gpr_gpr_32__reg_rdi__swift.swift`):
...
The source, LITERAL (`src3/setne_gpr_one_8__reg_rdi__c.c`):
...
The source, LITERAL (`src3/setne_gpr_one_8__reg_rdi__rust.rs`):
...
The source, LITERAL (`src3/setne_gpr_one_8__reg_rdi__go.go`):
...
The source, LITERAL (`src3/setne_gpr_one_8__reg_rdi__swift.swift`):
...
The source, LITERAL (`src3/addss_xmm_xmm_32__reg_xmm0__c.c`):
...
The source, LITERAL (`src3/addss_xmm_xmm_32__reg_xmm0__rust.rs`):
...
The source, LITERAL (`src3/addss_xmm_xmm_32__reg_xmm0__go.go`):
...
```
push %rbp; mov %rsp,%rbp; sub $0x8,%rsp; addss %xmm1,%xmm0; movss %xmm0,0x4(%rsp); movss 0x4(%rsp),%xmm0; add $0x8,%rsp; pop %rbp; ret
```

Chaff-stripped by task o2's own narrow rule, LITERAL:

```
sub $0x8,%rsp; addss %xmm1,%xmm0; add $0x8,%rsp
```
...
The source, LITERAL (`src3/addss_xmm_xmm_32__reg_xmm0__swift.swift`):
...
{
    return Float(bitPattern: UInt32(truncatingIfNeeded: (UInt32((((a) + (b))).bitPattern))))
}
```
...
The source, LITERAL (`src3/cvtsi2sd_gpr_xmm_64__reg_xmm0__c.c`):
...
The source, LITERAL (`src3/cvtsi2sd_gpr_xmm_64__reg_xmm0__rust.rs`):
...
The source, LITERAL (`src3/cvtsi2sd_gpr_xmm_64__reg_xmm0__go.go`):
...
The source, LITERAL (`src3/cvtsi2sd_gpr_xmm_64__reg_xmm0__swift.swift`):
...
## 2a. c and rust: task h2's verdict beside this task's

Table 2 -- the twenty runs task h2 also ran (`handful2.json`), so the primitive route's effect on the two targets that already had a term-route verdict is one comparison and not two documents.
