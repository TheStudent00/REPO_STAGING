...
Rendered: 3. Refused at render: 72. Lowered and read back: 3.
...
| ZBS_RTYPE BINV | (rs1_val ^^^ (shift_bits_left (zero_extend (m := 64) 1#1) (Sail.BitVec.extractLsb rs2_val 5 0))) | REFUSED: no key of operator_for matches: (rs1_val ^^^ (shift_bits_left (zero_extend (m := 64) 1#1) (Sail.BitV | | | | | |
...
| ZBS_IOP BINVI | (rs1_val ^^^ (shift_bits_left (zero_extend (m := 64) 1#1) shamt)) | REFUSED: no key of operator_for matches: (rs1_val ^^^ (shift_bits_left (zero_extend (m := 64) 1#1) shamt)) | | | | | |
...
| ZBB_RTYPE XNOR | (Complement.complement (rs1_val ^^^ rs2_val)) | REFUSED: no key of operator_for matches: (rs1_val ^^^ rs2_val) | | | | | |
...
| RTYPE SLT | (zero_extend (m := 64) (bool_to_bit (zopz0zI_s v_rs1 v_rs2))) | uint64(op_535(int64(a), int64(b))) | (zero_extend (m := 64) (bool_to_bit (zop -> go__op_535 |  |  | REFUSED: compile rc=1: ./main.go:9:16: cannot convert op_53 |  |
| RTYPE SLTU | (zero_extend (m := 64) (bool_to_bit (zopz0zI_u v_rs1 v_rs2))) | uint64(op_542(uint64(a), uint64(b))) | (zero_extend (m := 64) (bool_to_bit (zop -> go__op_542 |  |  | REFUSED: compile rc=1: ./main.go:9:16: cannot convert op_54 |  |
...
| RTYPE XOR | (v_rs1 ^^^ v_rs2) | REFUSED: no key of operator_for matches: (v_rs1 ^^^ v_rs2) | | | | | |
...
| RTYPE SUB | (v_rs1 - v_rs2) | REFUSED: no key of operator_for matches: (v_rs1 - v_rs2) | | | | | |
...
| ITYPE XORI | (v_rs1 ^^^ (sign_extend (m := 64) imm)) | REFUSED: no key of operator_for matches: (v_rs1 ^^^ (sign_extend (m := 64) imm)) | | | | | |
...
| C_NOT one | (Complement.complement v_r) | uint64(op_19(int64(a))) | (Complement.complement a) -> go__op_19 | c.nop; xori a0, a0, -0x1; jalr zero, 0x0(ra); c.unimp; c.unimp; c.unimp |  | REFUSED: no certified pure form for C_NOP (a pure clause th |  |
