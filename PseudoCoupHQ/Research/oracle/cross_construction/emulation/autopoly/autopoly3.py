#!/usr/bin/env python3
"""autopoly3.py -- task ap3: AutoPoly's loop, THIRD pass.

The loop is task ap2's, unchanged, over the same 253 cells; what
moved is two more mechanical causes, each fixed in the layer that
owns it and each measured on its own cells before the loop:
  * FIX 1, the ARRIVAL side of task ap2's fix 3 -- a 128-bit
    arriving vector register is two 64-bit arriving values
    (`handful.vector_arrivals_in_halves`);
  * FIX 2, the 32 x87 cells at `key_width` 80 -- a preseeded row
    that reads no flag state is not a flag consumer
    (`handful.a_place_reads_the_arriving_flags`), and c's own
    80-bit holder `long double` is what an x87 place is answered
    in (`handful.the_x87_value`, `emulate.FLOAT[79]`).
Two more things this file adds:
  * FIX 4's measurement: the whole loop run TWICE, once with task
    h2's normalise-before-render OFF and once ON, all else as
    task ap2 left it -- `off_run` and `fix4`.
...
CELLS = os.path.join(HERE, "autopoly3_cells.json")
RUNS = os.path.join(HERE, "autopoly3_runs.jsonl")
AGGREGATE = os.path.join(HERE, "autopoly3.json")
REPORT = os.path.join(HERE, "autopoly3.md")
SRC_DIR = os.path.join(HERE, "src3")
PRIMITIVE = os.path.join(HERE, "autopoly3_primitive.json")
SPELLINGS = os.path.join(HERE, "autopoly3_spellings.json")
...
OFF_RUNS = os.path.join(HERE, "autopoly3_off_runs.jsonl")
OFF_SRC_DIR = os.path.join(HERE, "src3_off")
...
BEFORE_RUNS = os.path.join(HERE, "autopoly2_runs.jsonl")
"""the store the change table and `measure` read as the BEFORE: task
ap2's, which is the pass this one follows."""
...
def use_task_ap3():
    ...
    memory bound, and at task ap2's route WITH TASK ap3'S TWO FIXES.
    ...
    lookup).  `handful.use_task_ap3` answers all three the way `ap2`
    answers them, so the ROUTE is unchanged; what differs is the two
    mechanical causes task ap2 left, each fixed in the layer that owns
    it and neither gated on a task name (`handful.use_task_ap3`'s own
    docstring says why).

    Every path `handful.py` writes through is repointed into this
    folder, and every one of them is an `autopoly3_*` name, so no lane
    of this task can write task ap1's or task ap2's products."""
    H.use_task_ap3()
    H.RESULTS = AGGREGATE
    H.REPORT = REPORT
    H.SRC_DIR = SRC_DIR
...
def change_command():
    """THE CHANGE TABLE: task ap2's loop beside this one, cause by cause
    and cell by cell.

    Read off the two stores, run for run, joined on the (cell, target)
    pair.  Task ap2 ran all 1,012, so every pair is on both stores; a
    ...
    say("runs on task ap2's store: %d" % len(theirs))
    say("runs on task ap3's store: %d" % len(mine))
    ...
    say("Table C1 -- every cause of task ap2's loop, what it counted "
        "then, and what those same runs say now.")
    say("")
    say("| task ap2's cause | ap2 runs | ap3 runs, same cause | where "
        "the rest went |")
    say("|---|---|---|---|")
    ...
    # few there are.
    say("Table C2 -- runs task ap2 proved that task ap3 does not.")
    say("")
    ...
    say("")
    say("runs task ap2 proved and task ap3 does not: %d" % regressed)
    say("")
    ...
    say("")
    say("| cell | lang | ap2's cause | the region the counterexample "
        "names |")
    say("|---|---|---|---|")
    to_sat = 0
...
def measure_command(wanted):
    ...
    `wanted` is a substring of the cause task ap2 recorded.  This
    command reads task ap2's own store, takes EXACTLY the (cell,
    target) pairs whose cause contains it, runs each one again through
    the driver as task ap3 leaves it, and prints the cause before and
    the cause after, pair by pair and summed.

    NOTHING IS WRITTEN.  The runs go nowhere near
    `autopoly3_runs.jsonl`; this is a measurement of a fix and not a
    ...
    for run in read_runs(BEFORE_RUNS):
        ...
        if cause is None or wanted not in cause:
            continue
        before.append(run)
    say("task ap2's own runs carrying a cause holding %r: %d"
        % (wanted, len(before)))
    say("")
    counted = {}
    say("| cell | lang | ap2's cause | ap3's cause |")
    say("|---|---|---|---|")
    index = 0
    for run in before:
        index = index + 1
...
def write_head(lines, document):
    meta = document["meta"]
    lines.append("# autopoly3.md -- task ap3: AutoPoly's loop, third pass")
    lines.append("")
