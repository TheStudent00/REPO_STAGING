...
# InterpRenderer from the layer-4 term of cmove_gpr_gpr_64__reg_rdi__cpython.
# The term's layer-5 text, LITERAL:
#   If(~(~v0 | ~v1) == 0, v2, v3)
def emu_cmove_gpr_gpr_64__reg_rdi__cpython(a, b, c, d):
    return m(((c) if (eq(bnot(bor(bnot(b, 64), bnot(a, 64), 64), 64), 0, 64)) else (d)), 64)
...
def main():
    for line in sys.stdin:
        ...
        try:
            answer = emu_cmove_gpr_gpr_64__reg_rdi__cpython(*values)
            sys.stdout.write("%d\n" % answer)
