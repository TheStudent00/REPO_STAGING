...
// InterpRenderer from the layer-4 term of fsd_fpr_mem_64__mem_MEM_0x0_a1___php.
// The term's layer-5 text, LITERAL:
//   v0
function emu_fsd_fpr_mem_64__mem_MEM_0x0_a1___php($a) {
    return ex_m($a, 64);
}
...
while (($line = fgets($handle)) !== false) {
    ...
    try {
        $answer = emu_fsd_fpr_mem_64__mem_MEM_0x0_a1___php(...$ints);
        if ($answer < 0) {
            echo sprintf("%s\n", sprintf("%u", $answer));
        } else {
            echo $answer . "\n";
