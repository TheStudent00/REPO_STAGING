#!/bin/bash
# rv9 lane 6 -- SECTION 2, THE WHOLE POPULATION at setting 'one':
# every optimization level moved to 1 and go's inlining off.
...
mkdir -p "$GOCACHE" "$GOPATH" /work/rv9_set_one
total=3
i=1
...
echo "[$i/$total] the whole population at setting 'one'"; i=$((i+1))
timeout 20000 python3 "$G/rv9_settings.py" run one "$OP" "$OP" \
  "$EMU" "$RV/twins.json" "$RV/model_table_rv.json" \
  "$G/rv9_set_one" "$G/src_rv9_set_one" "/work/rv9_set_one" \
...
python3 "$OP/check_no_spelling_keys.py" "$G/rv9_set_one.json"
echo "  guard rc=$?"
ls -la "$G"/rv9_set_one.jsonl "$G"/rv9_set_one.json
echo "lane rv9_l6 done"
