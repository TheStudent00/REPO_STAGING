#!/usr/bin/env python3
"""exception_families3.py -- STEP 2/3 of THE JOB (EXCEPTION-FAMILY TABLE).

GUARD IDENTITY (AgentMemory, SEEDED GROUPING UNDER CONDITIONS): two
guard rows from guards5.json are the SAME exception-family component
when
...
is not clustered into an exception family. It stays in guards5.json
...
for byte in its clustering LOGIC, at guards5.json (the record of
...
def main():
    g2 = json.load(open(os.path.join(HERE, "guards5.json")))
    rows = g2["rows"]
    ...
    out = dict(
        meta=dict(
            generator="exception_families3.py",
    ...
    json.dump(out, open(os.path.join(HERE, "exception_families3.json"), "w"),
               indent=1)
