#!/usr/bin/env python3
"""build_table18.py -- dominant_table18.json: dominant_table17.json's
same construction, REKEYED so every member of a proved representatives.
json group shares one text (the group's representative text) instead
of its own newest-converged text. Class key: (type_pair, machine-fact
result_type, REPRESENTATIVE TEXT). Additive: does not touch
...
def main():
    reps = json.load(open(os.path.join(HERE, "representatives.json")))
    rep_text_of = {}
    ...
    print("classes    %d  (dominant_table17 baseline: 926)" % len(rows))
    ...
    class_doc = {
        "meta": {
            ...
            "construction": "dominant_table17.py's rule, unchanged, "
                           ...
                           "representatives.json (THE REPRESENTATIVE "
                           "RULE, AgentMemory 2026-08-29) rather than "
                           "each unit's own newest-converged text",
            "population": DT17.__doc__ and "1,641 0-branch units, same "
                         "population as dominant_table17.json",
    ...
    out_path = os.path.join(HERE, "dominant_table18.json")
