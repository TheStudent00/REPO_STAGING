...
// InterpRenderer from the layer-4 term of lea_lea_mem_32__reg_rdi__php.
// The term's layer-5 text, LITERAL:
//   v0 + v1
function emu_lea_lea_mem_32__reg_rdi__php($a, $b) {
    return ex_m(ex_add($b, $a, 64), 64);
}
...
while (($line = fgets($handle)) !== false) {
    ...
    try {
        $answer = emu_lea_lea_mem_32__reg_rdi__php(...$ints);
        if ($answer < 0) {
            echo sprintf("%s\n", sprintf("%u", $answer));
        } else {
            echo $answer . "\n";
