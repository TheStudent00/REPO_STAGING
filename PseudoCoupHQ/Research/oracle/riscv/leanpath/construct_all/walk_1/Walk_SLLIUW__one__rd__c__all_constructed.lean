...
def alias_C_ZEXT_W (rsdc : cregidx) : instruction :=
  (ZBA_RTYPEUW (zreg, ((creg2reg_idx rsdc)), ((creg2reg_idx rsdc)), 0b00#2))

theorem strip_C_ZEXT_W (rsdc : cregidx) :
    execute_C_ZEXT_W rsdc = (ExecuteAs (alias_C_ZEXT_W rsdc)) := by
  rfl
