#!/usr/bin/env bash
# ap5_l6_probe_the_two_changes_end_to_end_b.sh -- task ap5: the two
...
#
# THE NAME: lane `ap5_l5` was this lane without the one line that makes
# `src5/`, the folder the renderer writes into, so every run refused
# with `FileNotFoundError`.  A lane name is used ONCE (the standing
# rule of 2026-09-07), so this is a new one and ap5_l5 stays on the
# record with its defect.
#
