...
// InterpRenderer from the layer-4 term of div_gpr_one_16__reg_rax__php.
// The term's layer-5 text, LITERAL:
//   Concat(Extract(63, 16, v1), Extract(15, 0, bvudiv_i(Concat(Extract(15, 0, v0), Extract(15, 0, v1)), Concat(0, Extract(15, 0, v2)))))
function emu_div_gpr_one_16__reg_rax__php($a, $b, $c) {
    return ex_m(ex_cat(ex_ext($b, 63, 16), ex_ext(ex_udiv(ex_cat($a, ex_ext($b, 15, 0), 16), ex_cat(0, $c, 16), 32), 15, 0), 16), 64);
}
...
while (($line = fgets($handle)) !== false) {
    ...
    try {
        $answer = emu_div_gpr_one_16__reg_rax__php(...$ints);
        if ($answer < 0) {
            echo sprintf("%s\n", sprintf("%u", $answer));
        } else {
            echo $answer . "\n";
