#!/usr/bin/env python3
"""dom_ops17.py -- the dom_op (family) construction over dominant_
table19.json (the corrected representative-rule class table), the
next number after dom_ops16.json's own (defective-input) pairing.
CONSUMES dominant_table19.json's own class rows rather than
rebuilding the class table itself -- SAME construction dom_ops16.py
already used (dom_ops7.py's rule via dom_ops_0branch.py, imported not
restated), unchanged; only the source table changed.

Per-unit provenance (display_label, arity_bucket) is re-derived from
canon4_units_<lang>.json (the same source dom_ops16.py itself reads)
keyed by the "lang/op_N" label already present on each dominant_
table19.json class-row member -- this does NOT re-run the class-
...
dominant_table19.json with no operator token in it; this file adds a
...
usage:
  dom_ops17.py [--out DIR] [--table PATH]
"""
...
def main(argv):
    outdir = HERE
    table_path = os.path.join(HERE, "dominant_table19.json")
    args = argv[1:]
    i = 0
    while i < len(args):
        if args[i] == "--out":
    ...
    dom_ops_doc = {
        "meta": {
            ...
            "generator": "dom_ops17.py",
            "note": "the dom_op (family) construction over dominant_"
                   "table19.json's own class rows (the CORRECTED "
                   "representative-rule table) -- written to dom_ops17"
                   ".json, the next number after dom_ops16.json's own "
                   "(defective-input) pairing",
            ...
            "supersedes": "dom_ops16.json, built over the defective "
                         "dominant_table18.json -- kept on disk as "
                         "the defect record, not deleted.",
        },
    ...
    dom_ops_path = os.path.join(outdir, "dom_ops17.json")
    fh = open(dom_ops_path, "w")
    json.dump(dom_ops_doc, fh, indent=1)
    fh.close()

    print("wrote %s" % dom_ops_path)
    print("classes    %d  (pre-defect baseline: 926)" % result["class_count"])
