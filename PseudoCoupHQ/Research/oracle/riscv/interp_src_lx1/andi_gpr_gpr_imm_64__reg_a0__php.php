...
// InterpRenderer from the layer-4 term of andi_gpr_gpr_imm_64__reg_a0__php.
// The term's layer-5 text, LITERAL:
//   Concat(0, Extract(1, 0, v0))
function emu_andi_gpr_gpr_imm_64__reg_a0__php($a) {
    return ex_m(ex_cat(0, ex_ext($a, 1, 0), 2), 64);
}
...
while (($line = fgets($handle)) !== false) {
    ...
    try {
        $answer = emu_andi_gpr_gpr_imm_64__reg_a0__php(...$ints);
        if ($answer < 0) {
            echo sprintf("%s\n", sprintf("%u", $answer));
        } else {
            echo $answer . "\n";
