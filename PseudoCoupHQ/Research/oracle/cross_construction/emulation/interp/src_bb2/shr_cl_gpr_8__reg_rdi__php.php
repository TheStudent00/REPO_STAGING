...
// one named local per gate, over the term of shr_cl_gpr_8__reg_rdi__php.
// The term's layer-5 text, LITERAL:
//   Concat(Extract(63, 8, v0), LShR(Extract(7, 0, v0), Concat(0, Extract(4, 0, v1))))
function emu_shr_cl_gpr_8__reg_rdi__php($a, $b) {
    $x1_4 = ex_ext($b, 4, 4);
    $x1_3 = ex_ext($b, 3, 3);
    $x0_0 = ex_ext($a, 0, 0);
    $x0_1 = ex_ext($a, 1, 1);
    ...
    $w57 = ex_cat($w56, $g97, 1);
    $w58 = ex_cat($w57, $g92, 1);
    $w59 = ex_cat($w58, $g89, 1);
    $w60 = ex_cat($w59, $g85, 1);
    $w61 = ex_cat($w60, $g73, 1);
    $w62 = ex_cat($w61, $g60, 1);
    $w63 = ex_cat($w62, $g31, 1);
    return ex_m($w63, 64);
}
...
while (($line = fgets($handle)) !== false) {
    ...
    try {
        $answer = emu_shr_cl_gpr_8__reg_rdi__php(...$ints);
        if ($answer < 0) {
            echo sprintf("%s\n", sprintf("%u", $answer));
        } else {
            echo $answer . "\n";
