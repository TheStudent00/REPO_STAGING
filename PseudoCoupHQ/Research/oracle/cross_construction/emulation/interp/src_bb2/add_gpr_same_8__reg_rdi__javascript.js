...
// one named local per gate, over the term of add_gpr_same_8__reg_rdi__javascript.
// The term's layer-5 text, LITERAL:
//   Concat(Extract(63, 8, v0), Extract(7, 0, v0)*2)
function emu_add_gpr_same_8__reg_rdi__javascript(a) {
    ...
    const w56 = cat(w55, x0_6, 1);
    const w57 = cat(w56, x0_5, 1);
    const w58 = cat(w57, x0_4, 1);
    const w59 = cat(w58, x0_3, 1);
    const w60 = cat(w59, x0_2, 1);
    const w61 = cat(w60, x0_1, 1);
    const w62 = cat(w61, x0_0, 1);
...
for (const line of lines) {
    ...
    try {
        out.push(emu_add_gpr_same_8__reg_rdi__javascript(...values).toString());
    } catch (problem) {
