...
sed -n '185,196p' PseudoCoupHQ/Research/oracle/arch_opcodes/model/model_table.py

echo "[2/6] reference.build_binary's one-operand branch, after the change"
sed -n '785,801p' PseudoCoupHQ/Research/op_pipeline/reference.py

echo "[3/6] the three operand shapes added to model_translate.shapes_for"
sed -n '253,255p' PseudoCoupHQ/Research/op_pipeline/lean/model_translate.py

echo "[4/6] the two readings model_table.py added to the attestation"
sed -n '838,843p;861,866p' PseudoCoupHQ/Research/oracle/arch_opcodes/model/model_table.py
