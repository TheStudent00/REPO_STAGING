// arch-unit 114  --  c  `a && b`  lhs=uint64_t rhs=float
// symbol op_333   outcome LIFTED
//
...
function au_114_c_land_u64_f32(p0, p1) {
  // a0: operand `a` (uint64_t) arrives in a0
  const v1 = p0;
  // fa0: operand `b` (float) arrives in fa0
  const v2 = ((p1) & 0xffffffffn);
  const v3 = (((0x0n) < (v1)) ? 1n : 0n);
  const v4 = ((0x0n) & 0xffffffffn);
  const v5 = AF.f32_eq_rm0(v2, v4);
  const v6 = ((v5) ^ 0x1n);
  const v7 = ((v3) & (v6));
  return v7;
}

module.exports = { au_114_c_land_u64_f32 };
