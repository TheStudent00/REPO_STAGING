#!/usr/bin/env bash
# ap4_l10_measure_the_three_changes_b.sh -- the same lane after two defects of its own: `measure` read this task.s own store as the BEFORE, and the lane.s own echo line held backticks -- task ap4: each change measured
