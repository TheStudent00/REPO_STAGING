...
// one named local per gate, over the term of sar_imm_gpr_8__reg_rdi__dart.
// The term's layer-5 text, LITERAL:
//   Concat(Extract(63, 8, v0), Extract(7, 0, v0) >> 3)
int emu_sar_imm_gpr_8__reg_rdi__dart(int a) {
  int x0_3 = ext(a, 3, 3);
  int x0_4 = ext(a, 4, 4);
  int x0_5 = ext(a, 5, 5);
  int x0_6 = ext(a, 6, 6);
  ...
  int w57 = cat(w56, x0_7, 1);
  int w58 = cat(w57, x0_7, 1);
  ...
  while (line != null) {
    ...
    if (text.isNotEmpty) {
      final List<int> values = text
          ...
          .toList();
      try {
        final int answer = emu_sar_imm_gpr_8__reg_rdi__dart(values[0]);
