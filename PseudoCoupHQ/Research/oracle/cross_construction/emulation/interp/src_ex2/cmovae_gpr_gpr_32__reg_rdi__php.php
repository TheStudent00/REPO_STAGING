...
// InterpRenderer from the layer-4 term of cmovae_gpr_gpr_32__reg_rdi__php.
// The term's layer-5 text, LITERAL:
//   Concat(0, If(ULE(Extract(31, 0, v0), Extract(31, 0, v1)), Extract(31, 0, v2), Extract(31, 0, v3)))
function emu_cmovae_gpr_gpr_32__reg_rdi__php($a, $b, $c, $d) {
    return ex_m(ex_cat(0, ((ex_ule($b, $a, 32)) ? ($c) : ($d)), 32), 64);
}
...
while (($line = fgets($handle)) !== false) {
    ...
    try {
        $answer = emu_cmovae_gpr_gpr_32__reg_rdi__php(...$ints);
        if ($answer < 0) {
            echo sprintf("%s\n", sprintf("%u", $answer));
        } else {
            echo $answer . "\n";
