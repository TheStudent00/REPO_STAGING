...
// InterpRenderer from the layer-4 term of div_gpr_one_16__reg_rax__dart.
// The term's layer-5 text, LITERAL:
//   Concat(Extract(63, 16, v1), Extract(15, 0, bvudiv_i(Concat(Extract(15, 0, v0), Extract(15, 0, v1)), Concat(0, Extract(15, 0, v2)))))
int emu_div_gpr_one_16__reg_rax__dart(int a, int b, int c) {
  return m(cat(ext(b, 63, 16), ext(udiv(cat(a, ext(b, 15, 0), 16), cat(0, c, 16), 32), 15, 0), 16), 64);
}
  ...
  while (line != null) {
    ...
    if (text.isNotEmpty) {
      final List<int> values = text
          ...
          .toList();
      try {
        final int answer = emu_div_gpr_one_16__reg_rax__dart(values[0], values[1], values[2]);
