...
// InterpRenderer from the layer-4 term of fcvt_d_s_fpr_fpr_fpr_64__freg_fa0__php.
// The term's layer-5 text, LITERAL:
//   fp.to_ieee_bv(fpToFP(RNE(), fpToFP(Extract(31, 0, v0))))
function emu_fcvt_d_s_fpr_fpr_fpr_64__freg_fa0__php($a) {
    return ex_m(ex_fwiden($a, 32, 64), 64);
}
...
while (($line = fgets($handle)) !== false) {
    ...
    try {
        $answer = emu_fcvt_d_s_fpr_fpr_fpr_64__freg_fa0__php(...$ints);
        if ($answer < 0) {
            echo sprintf("%s\n", sprintf("%u", $answer));
        } else {
            echo $answer . "\n";
