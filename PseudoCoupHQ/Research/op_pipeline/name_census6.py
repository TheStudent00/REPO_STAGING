#!/usr/bin/env python3
"""name_census6.py -- the census over canon39, written by
...
ROUND 13.  `name_census5.py` is round 12's record and is not edited;
this file is round 13's census.  The one difference is the store it
filters: `term65_store`, walked with the branch-following
`reference.py` of task 64, instead of `term61_store`.  The delta is
taken against `name_census5.json` and matched on the REASON SENTENCE,
never on a producer's spelling.

WRITES:
  name_census6.json          the census over canon39, round 13
  name_census6_printed.txt   the census printed, and the delta vs
                             census5 by cause
...
def census5_entries():
    """census5's entries, read in the shape `Term.census_delta` takes.

    census5 carries a `reasons` LIST on each entry -- the same shape
    ...
    producer's spelling.  (census4, one round earlier, carried a single
    `cannot_model_because` string; that shape is not this one's, and
    reading census5 through it would report every cause as new.)"""
    path = os.path.join(HERE, "name_census5.json")
    document = json.load(open(path))
    before = []
...
def main():
    ...
    before, census5 = census5_entries()
    delta = maker.census_delta(before, entries)
    log("")
    log("-- the delta against census5, matched on the REASON SENTENCE")
    log("   census5: %d producers, %d rows"
        % (len(census5["entries"]),
           sum(one["rows_blocked"] for one in census5["entries"])))
    log("   census6: %d producers, %d rows" % (len(entries), rows))
    ...
    document = {
        "meta": {
            "generated_by": "name_census6.py, running "
                            "term.Term.census",
            ...
            "keyed_on": "the TYPED producer object -- {kind, mnem} for "
                        ...
                        "an operator token.",
            "supersedes": "name_census5.json, which was filtered over "
                          "the round-12 term61_store.  It stays on "
        ...
        "delta_against_census5": delta,
        "entries": entries,
    }
    handle = open(os.path.join(HERE, "name_census6.json"), "w")
    json.dump(document, handle, indent=1, sort_keys=True)
    handle.close()
    log("")
    log("-- wrote name_census6.json")
    handle = open(os.path.join(HERE, "name_census6_printed.txt"), "w")
    handle.write("\n".join(LINES) + "\n")
    handle.close()
    return 0
