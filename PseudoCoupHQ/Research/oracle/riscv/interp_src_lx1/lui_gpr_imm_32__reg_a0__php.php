...
// InterpRenderer from the layer-4 term of lui_gpr_imm_32__reg_a0__php.
// The term's layer-5 text, LITERAL:
//   12288
function emu_lui_gpr_imm_32__reg_a0__php() {
    return ex_m(12288, 64);
}
...
while (($line = fgets($handle)) !== false) {
    ...
    try {
        $answer = emu_lui_gpr_imm_32__reg_a0__php(...$ints);
        if ($answer < 0) {
            echo sprintf("%s\n", sprintf("%u", $answer));
        } else {
            echo $answer . "\n";
