...
// InterpRenderer from the layer-4 term of fadd_d_fpr_fpr_fpr_64__freg_fa0__php.
// The term's layer-5 text, LITERAL:
//   fp.to_ieee_bv(fpToFP(v0) + fpToFP(v1))
function emu_fadd_d_fpr_fpr_fpr_64__freg_fa0__php($a, $b) {
    return ex_m(ex_fadd($a, $b, 64), 64);
}
...
while (($line = fgets($handle)) !== false) {
    ...
    try {
        $answer = emu_fadd_d_fpr_fpr_fpr_64__freg_fa0__php(...$ints);
        if ($answer < 0) {
            echo sprintf("%s\n", sprintf("%u", $answer));
        } else {
            echo $answer . "\n";
