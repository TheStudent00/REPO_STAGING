...
// InterpRenderer from the layer-4 term of sets_gpr_one_8__reg_rdi__dart.
// The term's layer-5 text, LITERAL:
//   Concat(0, If(0 <= ~(~Extract(7, 0, v0) | ~Extract(7, 0, v1)), 0, 1))
int emu_sets_gpr_one_8__reg_rdi__dart(int a, int b) {
  return m(cat(0, ((sle(0, bnot(bor(bnot(a, 8), bnot(b, 8), 8), 8), 8)) ? (0) : (1)), 8), 64);
}
  ...
  while (line != null) {
    ...
    if (text.isNotEmpty) {
      final List<int> values = text
          ...
          .toList();
      try {
        final int answer = emu_sets_gpr_one_8__reg_rdi__dart(values[0], values[1]);
