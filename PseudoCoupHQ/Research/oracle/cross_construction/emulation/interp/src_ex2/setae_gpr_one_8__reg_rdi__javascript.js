...
// InterpRenderer from the layer-4 term of setae_gpr_one_8__reg_rdi__javascript.
// The term's layer-5 text, LITERAL:
//   Concat(0, If(Extract(8, 8, Concat(0, Extract(7, 0, v0)) + Concat(0, Extract(7, 0, v1))) == 1, 0, 1))
function emu_setae_gpr_one_8__reg_rdi__javascript(a, b) {
    return m(cat(0n, ((eq(ext(add(cat(0n, a, 8), cat(0n, b, 8), 9), 8, 8), 1n, 1)) ? (0n) : (1n)), 8), 64);
}
...
for (const line of lines) {
    ...
    try {
        out.push(emu_setae_gpr_one_8__reg_rdi__javascript(...values).toString());
    } catch (problem) {
