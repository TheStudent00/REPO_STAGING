#!/usr/bin/env bash
# task o6 lane 6b (resume, over the second build's sites file; the join now splits go's untyped spellings by kind, adds the second reading of the completed row, and names the printed-differently disagreements): go_types_join.py -- o4's walk re-run per site
