#!/bin/bash
# lp3_l111_the_gate_on_the_float_classes.sh
#
# THE BIGGEST UNMEASURED THING IN THE LINE. l105 gated the INTEGER layer and
# the bridge lanes took it to 131 classes and 542 arch-units. The FLOAT layer
# -- 144 classes covering 329 arch-units -- has never been put to Lean at all,
# not once, at any width.
#
# WHY IT MIGHT NOT WORK, said before the run rather than after. A float
# arch-unit's emulation does not compute with a float instruction; it CALLS
# the SoftFloat slice for that operation, and those slices are two to five
# hundred straight-line integer instructions each. So the walk will produce a
# very large term and `bv_decide` will be handed a very large circuit. Whether
# it takes them is exactly what is unknown, and there is no way to find out
# but to ask.
#
# Nothing is new here except the selection: same carve, same walk, same
# `equals` through `simp_set_with_bridge`, and only the `abi` statement.
# A measurement, not a capability.
#
...
picks, skipped = [], {"no emulation": 0, "float layer": 0, "blocked": 0,
                      "no probe": 0, "integer layer": 0}
for c in classes:
    have = [m for m in c["members"] if m in recs]
    if not have:
        skipped["no emulation"] += 1; continue
    if not any(recs[m][0] == "float" for m in have):
        skipped["integer layer"] += 1; continue
    # the representative is the class's own, when it has an emulation
    floats = [m for m in have if recs[m][0] == "float"]
    rep = c["representative"] if c["representative"] in floats else sorted(floats)[0]
    tag, u = recs[rep]
...
from leanpath.equals import equals, simp_set_with_bridge
...
for p in pairs:
    ...
    tag = re.sub(r"\W", "_", u)
    defs, emit = simp_set_with_bridge(defs_index, heads, tag, bodies, F, Gx,
                                      P, PLIB)
    bodies = emit + bodies
    args = []
    for v in unknowns:
