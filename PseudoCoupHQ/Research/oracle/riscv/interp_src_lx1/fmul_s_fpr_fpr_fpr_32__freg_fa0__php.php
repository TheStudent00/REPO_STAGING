...
// InterpRenderer from the layer-4 term of fmul_s_fpr_fpr_fpr_32__freg_fa0__php.
// The term's layer-5 text, LITERAL:
//   Concat(4294967295, fp.to_ieee_bv(fpToFP(Extract(31, 0, v0)) * fpToFP(Extract(31, 0, v1))))
function emu_fmul_s_fpr_fpr_fpr_32__freg_fa0__php($a, $b) {
    return ex_m(ex_cat(4294967295, ex_fmul($a, $b, 32), 32), 64);
}
...
while (($line = fgets($handle)) !== false) {
    ...
    try {
        $answer = emu_fmul_s_fpr_fpr_fpr_32__freg_fa0__php(...$ints);
        if ($answer < 0) {
            echo sprintf("%s\n", sprintf("%u", $answer));
        } else {
            echo $answer . "\n";
