...
// InterpRenderer from the layer-4 term of div_gpr_gpr_same_64__reg_a0__php.
...
function emu_div_gpr_gpr_same_64__reg_a0__php($a) {
    return ex_m(((ex_eq($a, 0, 64)) ? (-1) : (((((ex_eq($a, -1, 64)) && (ex_eq($a, -9223372036854775808, 64)))) ? (-9223372036854775808) : (ex_sdiv($a, $a, 64))))), 64);
}
...
while (($line = fgets($handle)) !== false) {
    ...
    try {
        $answer = emu_div_gpr_gpr_same_64__reg_a0__php(...$ints);
        if ($answer < 0) {
            echo sprintf("%s\n", sprintf("%u", $answer));
        } else {
            echo $answer . "\n";
