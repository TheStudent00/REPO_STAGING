...
public static class Emu {
    ...
    // one named local per gate, over the term of mov_imm_gpr_8__reg_rdi__csharp.
    // The term's layer-5 text, LITERAL:
    //   Concat(Extract(63, 8, v0), Extract(7, 0, v1))
    public static long emu_mov_imm_gpr_8__reg_rdi__csharp(long a, long b) {
        long x1_0 = ext(b, 0, 0);
        long x1_1 = ext(b, 1, 1);
        long x1_2 = ext(b, 2, 2);
        long x1_3 = ext(b, 3, 3);
    ...
    public static void Main() {
        ...
        while (line != null) {
            ...
            if (text.Length != 0) {
                ...
                try {
                    answers.Append(((ulong) emu_mov_imm_gpr_8__reg_rdi__csharp(values[0], values[1])).ToString());
                    answers.Append("\n");
