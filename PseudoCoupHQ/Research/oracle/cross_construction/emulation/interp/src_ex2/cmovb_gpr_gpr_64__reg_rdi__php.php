...
// InterpRenderer from the layer-4 term of cmovb_gpr_gpr_64__reg_rdi__php.
// The term's layer-5 text, LITERAL:
//   If(ULE(v0, v1), v2, v3)
function emu_cmovb_gpr_gpr_64__reg_rdi__php($a, $b, $c, $d) {
    return ex_m(((ex_ule($b, $a, 64)) ? ($d) : ($c)), 64);
}
...
while (($line = fgets($handle)) !== false) {
    ...
    try {
        $answer = emu_cmovb_gpr_gpr_64__reg_rdi__php(...$ints);
        if ($answer < 0) {
            echo sprintf("%s\n", sprintf("%u", $answer));
        } else {
            echo $answer . "\n";
