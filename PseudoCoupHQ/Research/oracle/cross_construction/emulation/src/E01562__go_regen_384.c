/* task o7 emulation -- rendered by emulate.py Renderer from the layer-4 term of E01562__go_regen_384.  The term's layer-5 text, LITERAL:
   Concat(0, ~(~LShR(Extract(7, 0, v0), Concat(0, Extract(4, 0, v1))) | ~(255*If(Or(Not(Extract(63, 4, v1) == 0), ULE(8, Extract(3, 0, v1))), 0, 1)))) */
#include <stdint.h>

uint32_t
emu_E01562__go_regen_384(uint8_t a, uint64_t b)
{
