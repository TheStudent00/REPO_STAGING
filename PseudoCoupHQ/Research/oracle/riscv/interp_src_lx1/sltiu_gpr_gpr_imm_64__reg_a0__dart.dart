...
// InterpRenderer from the layer-4 term of sltiu_gpr_gpr_imm_64__reg_a0__dart.
// The term's layer-5 text, LITERAL:
//   If(Or(Extract(1, 0, v0) == 3, Not(Extract(63, 2, v0) == 0)), 0, 1)
int emu_sltiu_gpr_gpr_imm_64__reg_a0__dart(int a) {
  return m(((((eq(ext(a, 1, 0), 3, 2)) || ((!(eq(ext(a, 63, 2), 0, 62)))))) ? (0) : (1)), 64);
}
  ...
  while (line != null) {
    ...
    if (text.isNotEmpty) {
      final List<int> values = text
          ...
          .toList();
      try {
        final int answer = emu_sltiu_gpr_gpr_imm_64__reg_a0__dart(values[0]);
