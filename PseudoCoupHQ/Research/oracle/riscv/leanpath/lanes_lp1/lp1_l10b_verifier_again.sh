#!/usr/bin/env bash
# lp1 lane 10b -- lane 10 again over the log after its §6 grep became a
# one-line command the verifier can run (the first pass scored the
# 72-column-wrapped pattern DIFFERS: "grep: Unmatched ( or \("); the
# verifier itself is untouched.
