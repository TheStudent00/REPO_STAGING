...
set -uo pipefail
CU=/projects/PseudoCoupHQ/Research/oracle/compiler_units
mkdir -p /work/o6
...
for f in /work/o6/go_types_sites.json "$CU/go_types_join.json" "$CU/go_types_package_shape_cost.json"; do
  python3 /projects/PseudoCoupHQ/Research/op_pipeline/check_no_spelling_keys.py "$f"; echo "guard exit for $(basename $f): $?"
done
