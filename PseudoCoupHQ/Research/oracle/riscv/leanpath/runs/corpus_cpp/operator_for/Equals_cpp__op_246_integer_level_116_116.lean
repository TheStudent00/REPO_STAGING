...
theorem cpp__op_246_116_integer_level (a : BitVec 64) (b : BitVec 64) :
    ((pure_REMW (a) (b) (false))) = ((sign_extend (m := 64) (to_bits_truncate (l := 32) (if (((BitVec.toInt (Sail.BitVec.extractLsb b 31 0)) == 0) : Bool) then (BitVec.toInt (Sail.BitVec.extractLsb a 31 0)) else (Int.tmod (BitVec.toInt (Sail.BitVec.extractLsb a 31 0)) (BitVec.toInt (Sail.BitVec.extractLsb b 31 0))))))) := by
  try simp only [pure_REMW, sign_extend, to_bits_truncate, Sail.BitVec.signExtend, Sail.get_slice_int, Sail.BitVec.extractLsb]
  all_goals grind
