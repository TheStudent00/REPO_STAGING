...
theorem meaning_czero_eqz_gpr_gpr_gpr_64__reg_a0__rust__native_first (s : St) (a : BitVec 64) (b : BitVec 64) (d : BitVec 64) (h10 : s.regs.get? .x10 = some a) (h11 : s.regs.get? .x11 = some b) (h13 : s.regs.get? .x13 = some d) (hk0 : ∀ {α : Type} (x0 : α) (s : St), (plat_term_write x0) s = EStateM.Result.ok () s) (hk1 : ∀ (x0 : Arch.pa) (x1 : Nat) (s : St), (load_reservation x0 x1) s = EStateM.Result.ok () s) (hk2 : ∀ (x0 : Unit) (s : St), (cancel_reservation x0) s = EStateM.Result.ok () s) :
    runsTo (walk [LeanIM.instruction.ITYPE (0x001#12, LeanIM.regidx.Regidx 0x0a#5, LeanIM.regidx.Regidx 0x0a#5, LeanIM.iop.SLTIU), LeanIM.instruction.C_ADDI (0x3f#6, LeanIM.regidx.Regidx 0x0a#5), LeanIM.instruction.C_AND (LeanIM.cregidx.Cregidx 0x2#3, LeanIM.cregidx.Cregidx 0x3#3)]) s
      ((pure_RTYPE ((pure_ITYPE ((pure_ITYPE (a) (0x001#12) (LeanIM.iop.SLTIU))) (((sign_extend (m := 12) (0x3f#6)))) (ADDI))) (b) (AND))) := by
