...
public static class Emu {
    ...
    // InterpRenderer from the layer-4 term of cmovne_gpr_gpr_64__reg_rdi__csharp.
    // The term's layer-5 text, LITERAL:
    //   If(~(~v0 | ~v1) == 0, v2, v3)
    public static long emu_cmovne_gpr_gpr_64__reg_rdi__csharp(long a, long b, long c, long d) {
        return m(((eq(bnot(bor(bnot(b, 64), bnot(a, 64), 64), 64), 0L, 64)) ? (d) : (c)), 64);
    }
    ...
    public static void Main() {
        ...
        while (line != null) {
            ...
            if (text.Length != 0) {
                ...
                try {
                    answers.Append(((ulong) emu_cmovne_gpr_gpr_64__reg_rdi__csharp(values[0], values[1], values[2], values[3])).ToString());
                    answers.Append("\n");
