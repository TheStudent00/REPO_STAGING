...
// one named local per gate, over the term of setbe_gpr_one_8__reg_rdi__dart.
// The term's layer-5 text, LITERAL:
//   Concat(Extract(63, 8, v2), If(Or(Extract(7, 0, v0) == Extract(7, 0, v1), Not(ULE(Extract(7, 0, v0), Extract(7, 0, v1)))), 1, 0))
int emu_setbe_gpr_one_8__reg_rdi__dart(int a, int b, int c) {
  int x1_7 = ext(a, 7, 7);
  int x0_7 = ext(b, 7, 7);
  int x1_6 = ext(a, 6, 6);
  int x0_6 = ext(b, 6, 6);
  ...
  while (line != null) {
    ...
    if (text.isNotEmpty) {
      final List<int> values = text
          ...
          .toList();
      try {
        final int answer = emu_setbe_gpr_one_8__reg_rdi__dart(values[0], values[1], values[2]);
