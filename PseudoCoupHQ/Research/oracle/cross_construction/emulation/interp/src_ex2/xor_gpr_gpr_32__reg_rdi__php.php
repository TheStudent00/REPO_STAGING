...
// InterpRenderer from the layer-4 term of xor_gpr_gpr_32__reg_rdi__php.
// The term's layer-5 text, LITERAL:
//   Concat(0, Extract(31, 0, v0) ^ Extract(31, 0, v1))
function emu_xor_gpr_gpr_32__reg_rdi__php($a, $b) {
    return ex_m(ex_cat(0, ex_bxor($a, $b, 32), 32), 64);
}
...
while (($line = fgets($handle)) !== false) {
    ...
    try {
        $answer = emu_xor_gpr_gpr_32__reg_rdi__php(...$ints);
        if ($answer < 0) {
            echo sprintf("%s\n", sprintf("%u", $answer));
        } else {
            echo $answer . "\n";
