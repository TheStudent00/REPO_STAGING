#!/bin/bash
# t4_l5_the_float_constructions_after_the_correction.sh -- task t4, lane 5: the constructions
...
i=1
echo "[$i/$total] THE FLOAT CONSTRUCTIONS AT A TINY FORMAT, after the correction"
python3 "$G/check_constructions.py" float tiny
echo "  exit: $?"

i=2
echo ""
echo "[$i/$total] THE FLOAT CONSTRUCTIONS AT THE HALF FORMAT (5 exponent, 11 significand)"
python3 "$G/check_constructions.py" float half
echo "  exit: $?"
