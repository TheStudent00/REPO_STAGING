#!/usr/bin/env python3
"""build_union_table1.py -- TASK 32 part (d): THE UNION VIEW.
...
three views:
  ...
  * `views.compiled`    -- an index of `dominant_table24.json`'s own
                           ...
                           changed in any way.
  * `views.interpreter` -- the same index over `interp_table1.json`'s
                           classes, which carry the extra
  ...
  * `views.union_by_relations` -- the connected components of the graph
                           ...
                           from `interp_join1.json`.  A component is a
...
the comparison, php's three specialised handlers -- is carried in its
...
OUT = os.path.join(HERE, "union_table1.json")
...
def main():
    compiled = load("dominant_table24.json")
    interp = load("interp_table1.json")
    join = load("interp_join1.json")
    ...
    for edge in join.get("interpreter_to_interpreter_edges", []):
        ...
        edges.append({
            ...
            "source_artifact": "interp_join1.json",
        })
    ...
    out = {
        "meta": {
            ...
            "writes_to_the_compiled_table": "NONE -- "
                "dominant_table24.json and dom_ops22.json are not "
                "written by this file or by any file in TASK 32",
        },
        "views": {
            "compiled": {
                "source": "dominant_table24.json (read-only)",
                "class_count": len(compiled_view),
                "classes": compiled_view,
            },
            "interpreter": {
                "source": "interp_table1.json (read-only)",
                "class_count": len(interp_view),
                "classes": interp_view,
            },
            "union_by_relations": {
                "source": "interp_join1.json's PROVED_EQUAL rows",
