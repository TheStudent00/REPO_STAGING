#!/usr/bin/env python3
"""name_census2.py -- THE CENSUS, built as a FILTER over the layer-4
...
def shape_of(entry):
    ...
    if producer in REGULAR_FAMILIES:
        return "regular"
    return "irregular"
...
def documents():
    ...
    for lang in ("c", "cpp", "go", "rust", "swift"):
        path = os.path.join(HERE, "layer4_terms_%s.json" % lang)
        if os.path.exists(path):
            out.append(("original", path))
    path = os.path.join(HERE, "layer4_interp.json")
    if os.path.exists(path):
...
def build():
    ...
    for population, path in documents():
        ...
        for name, record in document["units"].items():
            ...
            for entry in record.get("cascades", []):
                cascades[entry["producer"]] += 1
                cascade_units[entry["producer"]].add(name)
    ...
    for key in sorted(rows, key=lambda one: (-rows[one], one)):
        entry = {
            "producer": key,
            "producer_shape": "pair" if " + " in key else "single",
    ...
    return {
        "meta": {
            ...
            "keyed_on": "producer opcodes (machine form): an arch "
                        ...
                        "flag-reading opcode)",
            "supersedes": "name_census.json, which was keyed to "
                          "canon24 and to lifter names",
        },
        "tally": dict(tally),
        "entries": entries,
...
def main():
    census = build()
    path = os.path.join(HERE, "name_census2.json")
