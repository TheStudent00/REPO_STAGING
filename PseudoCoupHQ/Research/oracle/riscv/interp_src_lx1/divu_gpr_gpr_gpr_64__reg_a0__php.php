...
// InterpRenderer from the layer-4 term of divu_gpr_gpr_gpr_64__reg_a0__php.
// The term's layer-5 text, LITERAL:
//   If(v0 == 0, 18446744073709551615, bvudiv_i(v1, v0))
function emu_divu_gpr_gpr_gpr_64__reg_a0__php($a, $b) {
    return ex_m(((ex_eq($a, 0, 64)) ? (-1) : (ex_udiv($b, $a, 64))), 64);
}
...
while (($line = fgets($handle)) !== false) {
    ...
    try {
        $answer = emu_divu_gpr_gpr_gpr_64__reg_a0__php(...$ints);
        if ($answer < 0) {
            echo sprintf("%s\n", sprintf("%u", $answer));
        } else {
            echo $answer . "\n";
