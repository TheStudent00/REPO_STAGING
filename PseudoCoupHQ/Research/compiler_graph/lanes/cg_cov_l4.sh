#!/usr/bin/env bash
# cg_cov_l4 — l3 showed `go build -cover` instruments only the main package
...
say "build -cover -coverpkg=cmd/compile/..."
cd "$SRC/src/cmd" || exit 93
set -x
"$GO" build -cover -coverpkg=cmd/compile/... -o /persist/compile_cov cmd/compile
A=$?
set +x
