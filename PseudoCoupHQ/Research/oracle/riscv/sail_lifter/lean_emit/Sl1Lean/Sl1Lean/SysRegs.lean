...
def sys_hgatp_sv32x4_supported : Bool := false

def sys_hgatp_sv39x4_supported : Bool := true

def sys_hgatp_sv48x4_supported : Bool := true

def sys_hgatp_sv57x4_supported : Bool := true

def sys_vsatp_sv32_supported : Bool := false

def sys_vsatp_sv39_supported : Bool := true

def sys_vsatp_sv48_supported : Bool := true

def sys_vsatp_sv57_supported : Bool := true

def sys_guest_page_fault_writes_htval : Bool := true
...
def _get_Hstatus_HUKTE (v : (BitVec 64)) : (BitVec 1) :=
  (Sail.BitVec.extractLsb v 24 24)

def _update_Hstatus_HUKTE (v : (BitVec 64)) (x : (BitVec 1)) : (BitVec 64) :=
  (Sail.BitVec.updateSubrange v 24 24 x)

def _set_Hstatus_HUKTE (r_ref : (RegisterRef (BitVec 64))) (v : (BitVec 1)) : SailM Unit := do
  let r ← do (reg_deref r_ref)
  writeRegRef r_ref (_update_Hstatus_HUKTE r v)
                    ...
                    (_update_Hstatus_HUKTE
                      (_update_Hstatus_HUPMM h
                        (← do
                          ...
                          else (pure (_get_Hstatus_HUPMM h))))
                      (← do
                        if ((← (currentlyEnabled Ext_Svukte)) : Bool)
                        then (pure (_get_Hstatus_HUKTE v))
                        else (pure 0#1))) (_get_Hstatus_VTSR v)) (_get_Hstatus_VTW v))
                (_get_Hstatus_VTVM v))
              (if (((BitVec.toNatInt (_get_Hstatus_VGEIN v)) ≤b geilen) : Bool)
              then (_get_Hstatus_VGEIN v)
              else (_get_Hstatus_VGEIN h))) (_get_Hstatus_HU v)) (_get_Hstatus_SPVP v))
        (_get_Hstatus_SPV v)) (_get_Hstatus_GVA v)))
...
def _set_SEnvcfg_UKTE (r_ref : (RegisterRef (BitVec 64))) (v : (BitVec 1)) : SailM Unit := do
  let r ← do (reg_deref r_ref)
  writeRegRef r_ref (_update_SEnvcfg_UKTE r v)
...
/-- Type quantifiers: vectored_alignment_exp : Nat, k_ex665421_ : Bool, direct_alignment_exp : Nat, k_ex665419_
...
def legalize_tvec (o : (BitVec 64)) (v : (BitVec 64)) (direct_supported : Bool) (direct_alignment_exp : Nat) (vectored_supported : Bool) (vectored_alignment_exp : Nat) : SailM (BitVec 64) := do
  ...
  let base_alignment ← (( do
    ...
    | .TV_Reserved => (internal_error "core/sys_regs.sail" 647 "Reserved mode in xtvec.") ) : SailM
    tvec_alignment )
...
/-- Type quantifiers: k_ex665433_ : Bool -/
def legalize_cause (o : (BitVec 64)) (v : (BitVec 64)) (is_scause : Bool) : SailM (BitVec 64) := SailME.run do
  let v := (Mk_Mcause v)
  let code := (_get_Mcause_Cause v)
  if ((is_scause && (((BitVec.toNatInt code) ≤b 31) : Bool)) : Bool)
  then (pure v)
  else
    (do
      if (((_get_Mcause_IsInterrupt v) == 1#1) : Bool)
      then
        (do
          let is_platform_interrupt :=
            ((code &&& (sign_extend (m := (64 -i 1)) 0xF0#8)) != (zeros (n := (64 -i 1))))
          if (is_platform_interrupt : Bool)
          then SailME.throw (o : (BitVec 64))
          else
            (do
              let intr ← do (interruptType_bits_backwards (Sail.BitVec.extractLsb code 5 0))
              if ((is_reserved_interrupt intr) : Bool)
              then SailME.throw (o : (BitVec 64))
              else
                (do
                  if (((intr == I_COF) && (not (hartSupports Ext_Sscofpmf))) : Bool)
                  then SailME.throw (o : (BitVec 64))
                  else
                    (do
                      if (((is_hypervisor_interrupt intr) && (not (hartSupports Ext_H))) : Bool)
                      then SailME.throw (o : (BitVec 64))
                      else (pure ())))))
      else
        (do
          if (((code &&& (sign_extend (m := (64 -i 1)) 0xC0#8)) != (zeros (n := (64 -i 1)))) : Bool)
          then SailME.throw (o : (BitVec 64))
          else
            (do
              let code := (Sail.BitVec.extractLsb code 5 0)
              if ((not (← (exceptionType_bits_backwards_matches code))) : Bool)
              then SailME.throw (o : (BitVec 64))
              else
                (do
                  let exc ← do (exceptionType_bits_backwards code)
                  if ((is_reserved_or_custom_exception exc) : Bool)
                  then SailME.throw (o : (BitVec 64))
                  else
                    (do
                      if (((exc == (E_Software_Check ())) && (not
                             ((hartSupports Ext_Zicfilp) || (hartSupports Ext_Zicfiss)))) : Bool)
                      then SailME.throw (o : (BitVec 64))
                      else
                        (do
                          if (((exc == (E_Hardware_Error ())) && (privileged_isa_version_lt
                                 priv_isa_version Privileged_ISA_1_13)) : Bool)
                          then SailME.throw (o : (BitVec 64))
                          else
                            (do
                              if (((is_hypervisor_exception exc) && (not (hartSupports Ext_H))) : Bool)
                              then SailME.throw (o : (BitVec 64))
                              else (pure ())))))))
      (pure v))
...
def legalize_hgatp (arch : Architecture) (prev_value : (BitVec 64)) (written_value : (BitVec 64)) : SailM (BitVec 64) := do
  ...
  let h :=
    (_update_Hgatp64_PPN
      (_update_Hgatp64_VMID h
        (zero_extend (m := 14) (Sail.BitVec.extractLsb (_get_Hgatp64_VMID h) (vmidlen -i 1) 0)))
      (zero_extend (m := 44)
  ...
  | none => (pure (_update_Hgatp64_Mode h (_get_Hgatp64_Mode (Mk_Hgatp64 prev_value))))
  | .some mode =>
    (match mode with
    | .HBare => (pure h)
    | .Sv39x4 =>
      (if (sys_hgatp_sv39x4_supported : Bool)
      then (pure h)
      else (pure (_update_Hgatp64_Mode h (_get_Hgatp64_Mode (Mk_Hgatp64 prev_value)))))
    | .Sv48x4 =>
      (if (sys_hgatp_sv48x4_supported : Bool)
      then (pure h)
      else (pure (_update_Hgatp64_Mode h (_get_Hgatp64_Mode (Mk_Hgatp64 prev_value)))))
    | .Sv57x4 =>
      (if (sys_hgatp_sv57x4_supported : Bool)
      then (pure h)
      else (pure (_update_Hgatp64_Mode h (_get_Hgatp64_Mode (Mk_Hgatp64 prev_value)))))
    | _ => (pure (_update_Hgatp64_Mode h (_get_Hgatp64_Mode (Mk_Hgatp64 prev_value)))))
...
/-- Type quantifiers: k_ex665577_ : Bool -/
def legalize_satp (arch : Architecture) (is_vsatp : Bool) (prev_value : (BitVec 64)) (written_value : (BitVec 64)) : SailM (BitVec 64) := do
  ...
  let s :=
    (_update_Satp64_Asid s
      (zero_extend (m := 16) (Sail.BitVec.extractLsb (_get_Satp64_Asid s) (asidlen -i 1) 0)))
  let s :=
    if (is_vsatp : Bool)
    then s
    else
      (_update_Satp64_PPN s
        (zero_extend (m := 44)
          (Sail.BitVec.extractLsb (_get_Satp64_PPN s)
            ((Min.min (physaddr_bits -i pagesize_bits) 44) -i 1) 0)))
  let sv39_supported ← do
    if (is_vsatp : Bool)
    then (pure sys_vsatp_sv39_supported)
    else (currentlyEnabled Ext_Sv39)
  let sv48_supported ← do
    if (is_vsatp : Bool)
    then (pure sys_vsatp_sv48_supported)
    else (currentlyEnabled Ext_Sv48)
  let sv57_supported ← do
    if (is_vsatp : Bool)
    then (pure sys_vsatp_sv57_supported)
  ...
  | .some Sv_mode =>
    (do
      ...
      | .Bare =>
        (do
          if ((is_vsatp || (← (currentlyEnabled Ext_Svbare))) : Bool)
          then (pure s)
          else (pure prev_value))
      | .Sv39 =>
        (if (sv39_supported : Bool)
        then (pure s)
        else (pure prev_value))
      | .Sv48 =>
        (if (sv48_supported : Bool)
        then (pure s)
        else (pure prev_value))
      | .Sv57 =>
        (if (sv57_supported : Bool)
        then (pure s)
        else (pure prev_value))
      | _ => (pure prev_value))
