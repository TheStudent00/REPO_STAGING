...
theorem meaning_emul_au_394_c_eq_i32_bool (s : St) (a : BitVec 64) (b : BitVec 64) (d : BitVec 64) (h10 : s.regs.get? .x10 = some a) (h11 : s.regs.get? .x11 = some b) (h13 : s.regs.get? .x13 = some d) (hk0 : ∀ {α : Type} (x0 : α) (s : St), (plat_term_write x0) s = EStateM.Result.ok () s) (hk1 : ∀ (x0 : Arch.pa) (x1 : Nat) (s : St), (load_reservation x0 x1) s = EStateM.Result.ok () s) (hk2 : ∀ (x0 : Unit) (s : St), (cancel_reservation x0) s = EStateM.Result.ok () s) :
    runsTo (walk [LeanIM.instruction.C_ADDIW (0x00#6, LeanIM.regidx.Regidx 0x0a#5), LeanIM.instruction.C_ANDI (0x01#6, LeanIM.cregidx.Cregidx 0x3#3), LeanIM.instruction.C_XOR (LeanIM.cregidx.Cregidx 0x2#3, LeanIM.cregidx.Cregidx 0x3#3), LeanIM.instruction.ITYPE (0x001#12, LeanIM.regidx.Regidx 0x0a#5, LeanIM.regidx.Regidx 0x0a#5, LeanIM.iop.SLTIU)]) s
      ((pure_ITYPE ((pure_RTYPE ((pure_ADDIW (a) ((sign_extend (m := 12) (0x00#6))))) ((pure_ITYPE (b) ((sign_extend (m := 12) (0x01#6))) (ANDI))) (XOR))) (0x001#12) (LeanIM.iop.SLTIU))) := by
  simp (config := {decide := true}) [runsTo, walk, step, execute, strip_C_ADDIW, strip_C_ANDI, strip_C_XOR, strip_ADDIW, strip_ITYPE, strip_RTYPE, alias_C_ADDIW, alias_C_ANDI, alias_C_XOR,
